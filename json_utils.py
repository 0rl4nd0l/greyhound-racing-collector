...
def safe_correlation(x: List[Union[int, float]], y: List[Union[int, float]]) -> float:
    ...
    try:
        ...
        for i in range(len(x)):
            if (x[i] is not None and y[i] is not None and 
                not pd.isna(x[i]) and not pd.isna(y[i])):
                try:
                    x_val = float(x[i])
                    y_val = float(y[i])
...
def clean_data_dict(data: dict) -> dict:
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            ...
            cleaned[key] = [clean_data_dict(item) if isinstance(item, dict) else safe_float(item) 
                           for item in value]
        else:
            cleaned[key] = safe_float(value) if isinstance(value, (int, float)) else value
    return cleaned
