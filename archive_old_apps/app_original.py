...
def api_recent_races():
    ...
    return jsonify(
        {"races": races, "count": len(races), "timestamp": datetime.now().isoformat()}
    )
...
def perform_prediction_background():
    ...
    try:
        safe_log_to_processing(
            "🎯 Starting comprehensive race predictions...", "INFO", 0
        )
        ...
        if os.path.exists(UPCOMING_DIR):
            ...
            upcoming_files_with_time = [
                (f, os.path.getmtime(os.path.join(UPCOMING_DIR, f)))
                for f in upcoming_files
            ]
            upcoming_files_with_time.sort(
        ...
        try:
            safe_log_to_processing(
                "🚀 Using Unified Predictor (Most Advanced System)", "INFO", 60
            )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"🔍 Running integrated prediction analysis for historical race {race_id}..."
                )
                prediction_result = (
                    integrated_system.unified_predictor.predict_race_file(
                        temp_race_file, enhancement_level="weather"
                    )
                )

                if prediction_result and prediction_result.get("success"):
                    print(f"✅ Real prediction completed successfully!")
    ...
    try:
        ...
        try:
            ...
            if force_rerun:
                ...
                print(
                    f"   🔄 Force rerun requested - will overwrite existing predictions"
                )
...
def api_check_performance_drift():
    ...
    try:
        ...
        if previous_accuracy > 0:
            drift_percentage = (
                abs(latest_accuracy - previous_accuracy) / previous_accuracy * 100
            )
