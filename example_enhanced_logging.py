...
def demonstrate_enhanced_logging():
    ...
    races = [
        ...
        ("2025-07-25", "AP_K", "2", "FETCH", "CSV downloaded successfully", 200, "INFO"),
        ("2025-07-25", "AP_K", "3", "FETCH", "CSV download failed - no CSV link found", 404, "WARNING"),
        ("2025-07-25", "AP_K", "4", "CACHE", "Race already collected", None, "INFO"),
        ("2025-07-25", "AP_K", "5", "FETCH", "CSV downloaded successfully", 200, "INFO"),
        ("2025-07-25", "AP_K", "6", "SKIP", "Race is upcoming, not historical", None, "INFO"),
    ]
    ...
    race_logs = [log for log in logger.web_logs["process"] if log.get("component") == "race_operation"]
...
def demonstrate_fetch_statistics_integration():
    ...
    scenarios = [
        ...
        },
        {
            ...
            "stats_updates": ["races_requested", "fetches_attempted", "successful_saves"],
            "log_params": {
                "race_date": "2025-07-25",
                "venue": "CANN",
                "race_number": "9", 
    ...
    for scenario in scenarios:
        ...
        print(f"   Log Entry: [{scenario['log_params']['operation']}] "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Demonstrate enhanced race operation logging')
    parser.add_argument('--verbose-fetch', action='store_true', 
                       help='Enable verbose fetch logging (shows all operations)')
    parser.add_argument('--stats-integration', action='store_true',
                       help='Show integration with fetch statistics')
    
    args = parser.parse_args()
    
    print(f"🚀 Starting Enhanced Logging Demo")
    print(f"   Verbose Fetch: {'ENABLED' if args.verbose_fetch else 'DISABLED (only warnings/errors)'}")
    ...
    print(f"💡 Try running with --stats-integration to see fetch statistics integration")
