    ...
    try:
        ...
        with app.test_client() as client:
            ...
            for i, params in enumerate(test_cases):
                print(f"   Testing case {i+1}: page={params['page']}, per_page={params['per_page']}")
                
                response = client.get(f"/api/races/paginated?page={params['page']}&per_page={params['per_page']}")
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success'):
                        ...
                        print(f"     📄 Pagination: page {pagination.get('page', 0)}, total {pagination.get('total_count', 0)}")
                        ...
                        if races and len(races) > 0:
                            ...
                            if runners:
                                runner = runners[0]
                                print(f"     👤 Sample runner: {runner.get('dog_name', 'N/A')} (Box {runner.get('box_number', 'N/A')})")
                                print(f"     🎯 Win probability: {runner.get('win_probability', 0)}")
                    else:
...
def test_database_connection():
    ...
    try:
        ...
        print(f"   📊 Database stats: {stats.get('total_races', 0)} races, {stats.get('unique_dogs', 0)} dogs")
        ...
        if recent_races:
            ...
            print(f"   🏁 Sample race: {sample_race.get('race_name', 'N/A')} at {sample_race.get('venue', 'N/A')}")
