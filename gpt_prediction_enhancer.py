...
class GPTPredictionEnhancer:
    ...
    def __init__(self, database_path: str = "greyhound_racing_data.db", 
        ...
        if not key_loaded:
            logger.error("❌ OpenAI API key verification failed. GPT features will be unavailable.")
            logger.error("   Please ensure OPENAI_API_KEY is properly set in your .env file.")
        ...
        if not verification_result.get('success'):
            if not verification_result.get('mock_mode'):
                logger.error(f"❌ OpenAI API authentication failed: {verification_result.get('error')}")
                self.gpt_available = False
                self.gpt_analyzer = None
                return
            else:
                logger.info("🔧 Mock mode active - GPT features available with simulated responses")
        ...
        try:
            ...
            self.gpt_analyzer = OpenAIEnhancedAnalyzer(
                api_key=self.connectivity_verifier.api_key, 
            ...
            if verification_result.get('mock_mode'):
                logger.info("🔧 GPT analyzer initialized in mock mode for local development")
            else:
                logger.info("✅ GPT analyzer initialized successfully with verified OpenAI connection")
                logger.info(f"   Connection test used {verification_result.get('tokens_used', 0)} tokens")
