...
class FeatureMapper:
    ...
    def transform_features(self, temporal_features: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Transforming {len(temporal_features)} rows from temporal builder format to model format"
        )
        ...
        if "venue" in transformed.columns:
            transformed["venue_encoded"] = transformed["venue"].apply(
                self._encode_venue
            )

        if "track_condition" in transformed.columns:
            transformed["track_condition_encoded"] = transformed[
                "track_condition"
            ].apply(self._encode_track_condition)

        if "grade" in transformed.columns:
            transformed["grade_encoded"] = transformed["grade"].apply(
                self._encode_grade
            )
        ...
        logger.info(
            f"Feature transformation complete: {len(transformed.columns)} features"
        )
        return transformed
    ...
    def _add_weather_features(self, df: pd.DataFrame):
        ...
        if "weather" in df.columns:
            df["weather_clear"] = df["weather"].apply(
                lambda x: (
                    1.0
                    if pd.notna(x)
                    and ("clear" in str(x).lower() or "fine" in str(x).lower())
                    else 0.0
                )
            )
            df["weather_cloudy"] = df["weather"].apply(
                lambda x: (
                    1.0
                    if pd.notna(x)
                    and ("cloud" in str(x).lower() or "overcast" in str(x).lower())
                    else 0.0
                )
            )
            df["weather_rain"] = df["weather"].apply(
                lambda x: (
                    1.0
                    if pd.notna(x)
                    and ("rain" in str(x).lower() or "shower" in str(x).lower())
                    else 0.0
                )
            )
            df["weather_fog"] = df["weather"].apply(
                lambda x: (
                    1.0
                    if pd.notna(x)
                    and ("fog" in str(x).lower() or "mist" in str(x).lower())
                    else 0.0
                )
            )
        else:
        ...
        if "temperature" in df.columns:
            ...
            df["temp_cool"] = (
                (df["temperature"] >= 10) & (df["temperature"] < 18)
            ).astype(float)
            df["temp_optimal"] = (
        ...
        if "wind_speed" in df.columns:
            ...
            df["wind_moderate"] = (
                (df["wind_speed"] >= 15) & (df["wind_speed"] < 25)
            ).astype(float)
    ...
    def _add_traditional_features(self, df: pd.DataFrame):
        ...
        if (
            "historical_win_rate" in df.columns
            and "historical_place_rate" in df.columns
        ):
            df["traditional_overall_score"] = (
                df["historical_win_rate"] + df["historical_place_rate"]
            ) / 2
        ...
        if "historical_time_consistency" in df.columns:
            df["traditional_consistency_score"] = (
                df["historical_time_consistency"] / 10.0
            )  # Normalize
        else:
        ...
        if "venue_experience" in df.columns:
            df["traditional_experience_score"] = np.minimum(
                df["venue_experience"] / 10.0, 1.0
            )
        else:
            df["traditional_experience_score"] = 0.2

        if "grade_experience" in df.columns:
            df["traditional_class_score"] = np.minimum(
                df["grade_experience"] / 5.0, 1.0
            )
        else:
...
def apply_feature_mapping_to_migrated_model(
