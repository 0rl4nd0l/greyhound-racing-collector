...
def load_and_process_csv_data():
    ...
    for csv_file in csv_files:
        try:
            ...
            if not df.empty:
                unique_dates = (
                    df["DATE"].dropna().unique() if "DATE" in df.columns else []
                )
                unique_tracks = (
                    df["TRACK"].dropna().unique() if "TRACK" in df.columns else []
                )
                unique_distances = (
                    df["DIST"].dropna().unique() if "DIST" in df.columns else []
                )
                dog_names = (
                    df["Dog Name"].dropna().unique() if "Dog Name" in df.columns else []
                )
                ...
                race_record = {
                    ...
                    "csv_dog_names": list(
                        dog_names[:5]
                    ),  # First 5 dog names for verification
                    "data_source": "csv",
                }
...
def load_and_process_log_data():
    ...
    try:
        ...
        race_logs = logs_df[
            ...
            | (logs_df["event"].notna())
            | (
                logs_df["details.file"].notna()
                & logs_df["details.file"].str.contains(".csv", na=False)
            )
        ].copy()
...
def detect_discrepancies(csv_data, log_data):
    ...
    if not log_data.empty and "log_file" in log_data.columns:
        log_files = set(
            log_data["log_file"].dropna().apply(lambda x: os.path.basename(x)).unique()
        )
    ...
    for log_file in log_only_files:
        if log_file and log_file != "nan":
            log_records = log_data[
                log_data["log_file"].str.contains(log_file, na=False)
            ]
    ...
    for csv_file in csv_files.intersection(log_files):
        ...
        matching_logs = (
            log_data[log_data["log_file"].str.contains(csv_file, na=False)]
                ...
                if log_record["log_timestamp"] and csv_record["date_from_filename"]:
                    ...
                    if log_date != csv_date:
                        ...
                        conflict_details.append(
                            f"Date mismatch: CSV={csv_date}, Log={log_date}"
                        )
...
def save_discrepancies_report(discrepancies):
    ...
    if all_discrepancies:
        ...
        print(
            f"Saved {len(all_discrepancies)} discrepancies to race_data_discrepancies.csv"
        )
    else:
        # Create empty DataFrame with expected columns
        discrepancies_df = pd.DataFrame(
            columns=["discrepancy_category", "description", "type"]
        )
        discrepancies_df.to_csv("race_data_discrepancies.csv", index=False)
        print(
            "No discrepancies found - saved empty report to race_data_discrepancies.csv"
        )
