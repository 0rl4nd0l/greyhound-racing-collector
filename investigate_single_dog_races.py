...
def check_database_completeness():
    ...
    try:
        ...
        db_total = pd.read_sql_query("SELECT COUNT(*) as total FROM race_metadata", conn)
        ...
        single_field_races = field_sizes[
            (field_sizes['field_size'] == 1) | 
            (field_sizes['actual_field_size'] == 1)
        ]
        logger.info(f"Races with field_size=1: {single_field_races['race_count'].sum() if not single_field_races.empty else 0}")
        ...
        logger.info(f"Races with only 1 actual runner in database: {len(single_runners_db)}")
        ...
        if not single_runners_db.empty:
            ...
            for _, race in single_runners_db.head(5).iterrows():
                logger.info(f"  {race['venue']} Race {race['race_id']} on {race['race_date']}: {race['distance']}m Grade {race['grade']}")
...
def investigate_sample_files():
    ...
    for file_path in sample_files[:5]:  # Limit to first 5 files
        try:
            ...
            dog_columns = [col for col in df.columns if 'dog' in col.lower() or 'name' in col.lower()]
            if dog_columns:
...
def generate_investigation_report():
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("## Summary\n")
        f.write("Investigation into the 30.39% single-dog races identified in the data quality analysis.\n\n")
        ...
        f.write("\n## Recommendations\n")
        f.write("1. **Data Collection Review**: Verify data collection processes at venues with high single-dog rates\n")
        f.write("2. **Source Validation**: Cross-reference with official racing records\n")
        f.write("3. **Parsing Logic**: Review CSV parsing logic for potential issues\n")
        f.write("4. **Automated Alerts**: Implement monitoring for unusual runner counts\n")
