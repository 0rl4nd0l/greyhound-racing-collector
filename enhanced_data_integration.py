...
class EnhancedDataIntegrator:
    ...
    def __init__(self, db_path="greyhound_racing_data.db"):
        ...
        if self.enhanced_data_available:
            print(f"📁 Enhanced CSV files: {len(list(self.enhanced_csv_dir.glob('*.csv')))}")
            print(f"📄 Enhanced JSON files: {len(list(self.enhanced_json_dir.glob('*.json')))}")
    
    def _check_enhanced_data_availability(self):
        """Check if enhanced expert form data is available"""
        try:
            return (
                self.enhanced_data_dir.exists() and
                self.enhanced_csv_dir.exists() and
                self.enhanced_json_dir.exists() and
                len(list(self.enhanced_csv_dir.glob('*.csv'))) > 0
            )
        except Exception:
            return False
    ...
    def get_enhanced_dog_data(self, dog_name, venue=None, max_races=10):
        ...
        try:
            ...
            for csv_file in csv_files:
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        ...
                        if current_dog and self._normalize_dog_name(current_dog) == self._normalize_dog_name(dog_name):
            ...
            enhanced_data['enhanced_features'] = self._calculate_enhanced_features(enhanced_data)
    ...
    def _calculate_enhanced_features(self, enhanced_data):
        ...
        try:
            ...
            if sectional_times:
                first_sections = [st['first_section'] for st in sectional_times if st['first_section']]
                if first_sections:
            ...
            if weight_history:
                ...
                if len(weights) > 1:
                    ...
                    features['current_weight_vs_avg'] = (weights[0] - np.mean(weights)) / np.mean(weights) if weights else 0
            
            # Margin analysis
            margins = enhanced_data.get('margins', [])
            if margins:
                margin_values = [m['margin'] for m in margins if m['margin'] is not None]
                if margin_values:
                    features['avg_winning_margin'] = np.mean([m for m in margin_values if m > 0])
                    features['avg_losing_margin'] = np.mean([abs(m) for m in margin_values if m < 0])
                    features['margin_consistency'] = 1 / (np.std(margin_values) + 1)
            
            # Performance indicators
            features['enhanced_data_quality'] = len(sectional_times) + len(pir_ratings) + len(weight_history)
            features['enhanced_data_available'] = 1 if features['enhanced_data_quality'] > 0 else 0
    ...
    def get_enhanced_race_context(self, venue, race_date, race_number):
        ...
        try:
            ...
            target_filename_pattern = f"{venue}_Race{race_number}_{race_date.replace('-', '_')}"
                ...
                if target_filename_pattern in str(json_file):
                    with open(json_file, 'r') as f:
                        ...
                        return {
                            ...
                            'data_quality': data.get('csv_data', {}).get('row_count', 0)
                        }
        ...
        try:
            ...
            if enhanced_data and enhanced_data.get('enhanced_features'):
                ...
                if 'avg_first_section' in enhanced_features:
                    dog_stats['sectional_speed'] = enhanced_features['avg_first_section']
                ...
                if 'sectional_consistency' in enhanced_features:
                    dog_stats['speed_consistency'] = enhanced_features['sectional_consistency']
                
                if 'weight_consistency' in enhanced_features:
                    dog_stats['weight_reliability'] = enhanced_features['weight_consistency']
                
                # Add new enhanced-specific features
                dog_stats.update({
                    'enhanced_features': enhanced_features,
                    'enhanced_data_quality': enhanced_features.get('enhanced_data_quality', 0),
                    'has_enhanced_data': enhanced_features.get('enhanced_data_available', 0) == 1
                })
                
                print(f"✅ Enhanced data integrated for {dog_name}: {enhanced_features.get('enhanced_data_quality', 0)} data points")
    ...
    def create_enhanced_database_table(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_enhanced_dog_name ON enhanced_expert_data(dog_clean_name)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_enhanced_race_date ON enhanced_expert_data(race_date)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_enhanced_venue ON enhanced_expert_data(venue)')
    ...
    def sync_enhanced_data_to_database(self):
        ...
        try:
                        ...
                        for idx, row in df.iterrows():
                            ...
                            if current_dog:
                                ...
                                if race_data:
                                    ...
                                    ''', (
                                        race_id, current_dog, self._normalize_dog_name(current_dog),
                                        venue, race_date, race_number, race_data.get('position'),
                                        race_data.get('box'), race_data.get('weight'), race_data.get('distance'),
                                        race_data.get('grade'), race_data.get('time'), race_data.get('win_time'),
                                        race_data.get('bonus_time'), race_data.get('sectional_1st'), 
                                        race_data.get('margin'), race_data.get('pir'), race_data.get('starting_price'),
                                        datetime.now().isoformat(), 'enhanced_expert_form'
                                    ))
            ...
            print(f"✅ Enhanced data sync completed: {records_added} records added/updated")
            return True
    ...
    def get_enhanced_statistics(self):
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(DISTINCT dog_clean_name) FROM enhanced_expert_data")
