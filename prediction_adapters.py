...
class V3Adapter:
    ...
    def _initialize_pipeline(self):
        ...
        try:
            ...
            logger.error(
                f"❌ V3Adapter: Failed to initialize PredictionPipelineV3: {e}"
            )
            self.pipeline = None
        ...
        try:
            ...
            for pred in pipeline_result.get("predictions", []):
                standardized_pred = {
                    ...
                    "confidence": (
                        pred.get("confidence", 0.0) if "confidence" in pred else None
                    ),
                }
                standardized_predictions.append(standardized_pred)

            # Apply group normalization
            standardized_predictions = StandardizedResult.normalize_probabilities(
                standardized_predictions
            )
    ...
    def _initialize_predictors(self):
        ...
        if SIMPLE_PREDICTOR_AVAILABLE:
            try:
                ...
                logger.info(
                    "✅ V3SAdapter: Simple predictor (UnifiedPredictor) initialized"
                )
            except Exception as e:
                logger.warning(
                    f"⚠️ V3SAdapter: Simple predictor initialization failed: {e}"
                )
                self.simple_predictor = None
        ...
        if self.simple_predictor:
            try:
                ...
                if simple_result.get("success", False):
                    ...
                    # Apply group normalization
                    standardized_predictions = (
                        StandardizedResult.normalize_probabilities(
                            standardized_predictions
                        )
                    )
                    ...
                    logger.info(
                        f"✅ V3SAdapter: Simple predictor succeeded for {race_id}"
                    )
                    return StandardizedResult.create_result(
                        race_id=race_id,
                        predictions=standardized_predictions,
                    ...
                    logger.warning(
                        f"⚠️ V3SAdapter: Simple predictor failed for {race_id}, falling back to V3Adapter"
                    )

            except Exception as e:
                logger.warning(
                    f"⚠️ V3SAdapter: Simple predictor exception for {race_id}: {e}, falling back to V3Adapter"
                )
        ...
        if self.fallback_adapter:
            ...
            if "metadata" in fallback_result:
                ...
                fallback_result["metadata"]["fallback_reason"] = (
                    "simple_predictor_unavailable"
                    if not self.simple_predictor
                    else "simple_predictor_failed"
                )
        ...
        try:
            ...
            for pred in v4_result.get("predictions", []):
                standardized_pred = {
                    ...
                    "raw_prob": pred.get(
                        "raw_win_probability", pred.get("win_probability", 0.0)
                    ),
                    "win_prob_norm": pred.get(
                        "normalized_win_probability", pred.get("win_probability", 0.0)
                    ),
                    "box_number": pred.get("box_number", 0),
                    "method": "ml_system_v4",
            ...
            if standardized_predictions and not any(
                pred.get("win_prob_norm", 0) > 0 for pred in standardized_predictions
            ):
            ...
            return StandardizedResult.create_result(
                race_id=race_id, predictions=standardized_predictions, metadata=metadata
            )
    ...
    def _preprocess_csv(self, race_file_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            processed_data = self.ml_system.preprocess_upcoming_race_csv(
                race_data, filename.replace(".csv", "")
            )

            logger.info(
                f"✅ V4Adapter: Preprocessed {len(processed_data)} dogs from {filename}"
            )
...
def predict_with_v3(
    race_file_path: str, db_path: str = "greyhound_racing_data.db"
) -> Dict[str, Any]:
