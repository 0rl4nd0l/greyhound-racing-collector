...
def validate_step2_requirements():
    ...
    print("-" * 40)
    print(
        f"   Performance Predictability: {features['performance_predictability']:.3f}"
    )
    ...
    print(
        f"   Distance Specialization: {features['distance_specialization']:.0f} distances"
    )
...
def demonstrate_all_dogs():
    ...
    for dog_name in summary_df.index:
        print(f"\n{dog_name.upper()}:")
        print(
            f"  Race Performance: {summary_df.loc[dog_name, 'mean_race_time']:.2f}s avg, {summary_df.loc[dog_name, 'best_race_time']:.2f}s best"
        )
        print(
            f"  Success Rates: {summary_df.loc[dog_name, 'win_rate']:.1%} wins, {summary_df.loc[dog_name, 'place_rate_top3']:.1%} top-3"
        )
        print(
            f"  Form Trend: {summary_df.loc[dog_name, 'recent_position_trend']:.3f} (>0 = improving)"
        )
        print(
            f"  Early Speed: {summary_df.loc[dog_name, 'early_speed_score']:.3f}/1.000"
        )
        print(
            f"  Ballarat Exp: {summary_df.loc[dog_name, 'ballarat_experience']:.0f} races"
        )
    ...
    print(
        f"Best Single Time: {all_features['best_race_time'].idxmin()} ({all_features['best_race_time'].min():.2f}s)"
    )
    print(
        f"Highest Win Rate: {all_features['win_rate'].idxmax()} ({all_features['win_rate'].max():.1%})"
    )
    print(
        f"Best Place Rate: {all_features['place_rate_top3'].idxmax()} ({all_features['place_rate_top3'].max():.1%})"
    )
    print(
        f"Best Recent Form: {all_features['recent_position_trend'].idxmax()} ({all_features['recent_position_trend'].max():.3f})"
    )
    print(
        f"Best Early Speed: {all_features['early_speed_score'].idxmax()} ({all_features['early_speed_score'].max():.3f})"
    )
    ...
    print("\nKey Correlations:")
    print(
        f"Win Rate vs Mean Time: {correlations.loc['win_rate', 'mean_race_time']:.3f}"
    )
    print(
        f"Win Rate vs Early Speed: {correlations.loc['win_rate', 'early_speed_score']:.3f}"
    )
    print(
        f"Win Rate vs Recent Form: {correlations.loc['win_rate', 'recent_position_trend']:.3f}"
    )
    print(
        f"Time vs Early Speed: {correlations.loc['mean_race_time', 'early_speed_score']:.3f}"
    )
...
def feature_quality_check():
    ...
    expected_features = [
        "mean_race_time",
        "median_race_time",
        "best_race_time",  # Time stats
        "win_rate",
        "place_rate_top3",  # Place percentages
        "avg_beaten_margin",
        "avg_winning_margin",  # Margin analysis
        "mean_first_section",
        "early_speed_rank",  # Early speed
        "recent_position_trend",
        "recent_time_trend",  # Form trends
    ]
...
def main():
    ...
    try:
        ...
        print(
            "✓ Race time statistics (mean, median, best) - overall and venue-specific"
        )
