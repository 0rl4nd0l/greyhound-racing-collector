...
from temporal_feature_builder_optimized import (
    OptimizedTemporalFeatureBuilder as OptimizedBuilder,
)
...
class PerformanceBenchmark:
    ...
    def create_test_data(
        self, num_races: int = 5, dogs_per_race: int = 6
    ) -> List[pd.DataFrame]:
        """Create test race data for benchmarking."""
        logger.info(
            f"Creating test data: {num_races} races with {dogs_per_race} dogs each"
        )
        ...
        try:
            ...
            for _, row in race_ids.iterrows():
                ...
                race_data = pd.read_sql_query(
                    race_query, conn, params=[race_id, dogs_per_race]
                )
            ...
            if len(test_races) < num_races:
                logger.warning(
                    f"Only found {len(test_races)} suitable races (requested {num_races})"
                )
    ...
    def benchmark_builder(
        self, builder, test_races: List[pd.DataFrame], builder_name: str
    ) -> Dict[str, Any]:
        ...
        for i, race_data in enumerate(test_races):
            ...
            try:
                ...
                results["features_generated"] += len(features_df.columns) * len(
                    features_df
                )
                ...
                logger.debug(
                    f"Race {i+1}: {len(race_data)} dogs, {len(features_df.columns)} features, {race_time:.2f}s"
                )
        ...
        results.update(
            {
                ...
                "avg_time_per_race": (
                    np.mean(results["individual_race_times"])
                    if results["individual_race_times"]
                    else 0
                ),
                "avg_time_per_dog": (
                    total_time / results["dogs_processed"]
                    if results["dogs_processed"] > 0
                    else 0
                ),
                "races_per_second": (
                    results["races_processed"] / total_time if total_time > 0 else 0
                ),
                "dogs_per_second": (
                    results["dogs_processed"] / total_time if total_time > 0 else 0
                ),
                "avg_features_per_dog": (
                    np.mean(results["feature_counts"])
    ...
    def run_benchmark(
        ...
        logger.info(
            f"Test parameters: {num_races} races, {dogs_per_race} dogs per race"
        )
    ...
    def print_comparison_report(self):
        ...
        if (
            not self.results
            or "original" not in self.results
            or "optimized" not in self.results
        ):
        ...
        time_improvement = (
            ((orig_time - opt_time) / orig_time * 100) if orig_time > 0 else 0
        )
        print(
            f"{'Total Time (seconds)':<30} {orig_time:<15.2f} {opt_time:<15.2f} {time_improvement:<15.1f}%"
        )
        ...
        race_improvement = (
            ((orig_race_time - opt_race_time) / orig_race_time * 100)
            if orig_race_time > 0
            else 0
        )
        print(
            f"{'Time per Race (seconds)':<30} {orig_race_time:<15.2f} {opt_race_time:<15.2f} {race_improvement:<15.1f}%"
        )
        ...
        dog_improvement = (
            ((orig_dog_time - opt_dog_time) / orig_dog_time * 100)
            if orig_dog_time > 0
            else 0
        )
        print(
            f"{'Time per Dog (seconds)':<30} {orig_dog_time:<15.2f} {opt_dog_time:<15.2f} {dog_improvement:<15.1f}%"
        )
        ...
        print(
            f"{'Races per Second':<30} {orig_rps:<15.2f} {opt_rps:<15.2f} {rps_improvement:<15.1f}%"
        )
        ...
        print(
            f"{'Dogs per Second':<30} {orig_dps:<15.2f} {opt_dps:<15.2f} {dps_improvement:<15.1f}%"
        )
        ...
        print(
            f"{'Memory Used (MB)':<30} {orig_mem:<15.2f} {opt_mem:<15.2f} {mem_change:<15.1f}%"
        )
        ...
        print(
            f"{'Avg Features per Dog':<30} {orig_features:<15.1f} {opt_features:<15.1f} {features_match:<15}"
        )

        print(
            f"{'Races Processed':<30} {original['races_processed']:<15} {optimized['races_processed']:<15} {'✅ Match' if original['races_processed'] == optimized['races_processed'] else '❌ Mismatch':<15}"
        )
        print(
            f"{'Dogs Processed':<30} {original['dogs_processed']:<15} {optimized['dogs_processed']:<15} {'✅ Match' if original['dogs_processed'] == optimized['dogs_processed'] else '❌ Mismatch':<15}"
        )
        print(
            f"{'Errors':<30} {original['errors']:<15} {optimized['errors']:<15} {'✅ Match' if original['errors'] == optimized['errors'] else '❌ Mismatch':<15}"
        )
        ...
        if rps_improvement > 0:
            print(
                f"✅ Optimized version processes {rps_improvement:.1f}% more races per second"
            )
        else:
            print(
                f"❌ Optimized version processes {abs(rps_improvement):.1f}% fewer races per second"
            )
...
def main():
    ...
    parser.add_argument(
        "--db", type=str, default="greyhound_racing_data.db", help="Database path"
    )
