...
class AdvancedSystemAnalyzer:
    ...
    def analyze_data_quality(self):
        ...
        try:
            ...
            # Basic statistics
            race_count = pd.read_sql_query("SELECT COUNT(*) as count FROM race_metadata", conn).iloc[0]['count']
            dog_count = pd.read_sql_query("SELECT COUNT(*) as count FROM dog_race_data", conn).iloc[0]['count']
            unique_dogs = pd.read_sql_query("SELECT COUNT(DISTINCT dog_clean_name) as count FROM dog_race_data", conn).iloc[0]['count']
            unique_venues = pd.read_sql_query("SELECT COUNT(DISTINCT venue) as count FROM race_metadata", conn).iloc[0]['count']
            ...
            race_completeness = {
                'winners': race_data.iloc[0]['races_with_winners'] / race_data.iloc[0]['total_races'],
                'track_condition': race_data.iloc[0]['races_with_track_condition'] / race_data.iloc[0]['total_races'],
                'weather': race_data.iloc[0]['races_with_weather'] / race_data.iloc[0]['total_races'],
                'temperature': race_data.iloc[0]['races_with_temperature'] / race_data.iloc[0]['total_races']
            }
            
            dog_completeness = {
                'finish_position': dog_data.iloc[0]['entries_with_position'] / dog_data.iloc[0]['total_entries'],
                'individual_time': dog_data.iloc[0]['entries_with_time'] / dog_data.iloc[0]['total_entries'],
                'starting_price': dog_data.iloc[0]['entries_with_odds'] / dog_data.iloc[0]['total_entries'],
                'weight': dog_data.iloc[0]['entries_with_weight'] / dog_data.iloc[0]['total_entries']
            }
            
            # Overall data quality score
            all_completeness = list(race_completeness.values()) + list(dog_completeness.values())
    ...
    def analyze_model_performance(self):
        ...
        try:
            ...
            model_analysis = {
                ...
                'performance_assessment': self._assess_model_performance(model_data.get('accuracy', 0))
            }
    ...
    def analyze_prediction_accuracy(self):
        ...
        try:
            ...
            recent_predictions = sorted(prediction_files, key=lambda x: x.stat().st_mtime, reverse=True)[:50]
            ...
            confidence_scores = []
            for pred_file in recent_predictions[:10]:  # Sample first 10 for detailed analysis
                try:
            ...
            if confidence_scores:
                accuracy_stats['average_confidence'] = round(np.mean(confidence_scores), 3)
    ...
    def analyze_system_performance(self):
        ...
        try:
            ...
            logger.info("📂 Step 1: Analyzing directory structure and file organization")
            ...
            for name, path in directories.items():
                ...
                if path.exists():
                    ...
                    other_files = [f for f in files if f.suffix not in ['.csv', '.json']]
                    ...
                    logger.info(f"    ✅ {name}: {len(files)} files ({len(csv_files)} CSV, {len(json_files)} JSON, {len(recent_files)} recent)")
            ...
            analysis_steps.append({'step': 'Directory Analysis', 'time_seconds': step1_time, 'files_processed': total_files_processed})
            logger.info(f"  ✅ Step 1 completed in {step1_time:.2f}s - Processed {total_files_processed} files")
            ...
            if os.path.exists(self.db_path):
                    ...
                    with sqlite3.connect(self.db_path) as conn:
                        ...
                        for (table_name,) in tables:
                            try:
                                ...
                                logger.warning(f"    ⚠️ Could not count {table_name}: {e}")
                ...
                logger.info(f"  📊 Database: {db_size_mb:.2f} MB with {len(db_analysis.get('tables', {}))} tables")
            else:
                db_analysis = {'error': 'Database file not found'}
                logger.error(f"  ❌ Database not found: {self.db_path}")
            
            step2_time = time.time() - step_start
            analysis_steps.append({'step': 'Database Analysis', 'time_seconds': step2_time})
            ...
            analysis_steps.append({'step': 'Health Assessment', 'time_seconds': step3_time})
            logger.info(f"  ✅ Step 3 completed in {step3_time:.2f}s - Health: {system_health['overall_status']}")
            ...
            performance_metrics = {
                ...
                'files_per_second': round(total_files_processed / total_analysis_time, 2) if total_analysis_time > 0 else 0,
                'analysis_steps': analysis_steps,
                'timestamp': datetime.now().isoformat()
            }
            
            step4_time = time.time() - step_start
            analysis_steps.append({'step': 'Metrics Calculation', 'time_seconds': step4_time})
            ...
            performance_analysis = {
                ...
                'analysis_summary': {
                    ...
                    'directories_healthy': sum(1 for stats in file_stats.values() if 'error' not in stats),
                    'analysis_success': True
                }
            }
            
            logger.info(f"🎉 System performance analysis completed successfully in {total_analysis_time:.2f}s")
            logger.info(f"📊 Summary: {len(directories)} directories, {total_files_processed} files, Health: {system_health['overall_status']}")
    ...
    def generate_recommendations(self, analysis_results):
        ...
        if 'data_quality' in analysis_results:
            quality_score = analysis_results['data_quality']['data_completeness']['overall_quality_score']
        ...
        if 'model_performance' in analysis_results and 'model_info' in analysis_results['model_performance']:
    ...
    def _assess_system_health(self, file_stats):
        ...
        if predictions_path.exists():
            recent_files = [f for f in predictions_path.glob('*.json') 
            ...
            if hasattr(model, 'feature_importances_') and feature_columns:
                ...
                return {
                    ...
                    'importance_distribution': {
                        ...
                        'medium_importance': len([f for f in importances if 0.01 < f <= 0.05]),
    ...
    def run_complete_analysis(self):
        ...
        # Save results
        output_file = self.analysis_dir / f"system_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _print_analysis_summary(self, results):
        ...
        if results['data_quality']:
            ...
            print(f"  • Overall Quality: {dq['data_completeness']['overall_quality_score']:.1%} ({dq['quality_assessment']})")
        
        # Model Performance Summary
        if results['model_performance'] and 'model_info' in results['model_performance']:
        ...
        if results['recommendations']:
            ...
            print(f"  • Advanced Enhancements: {len(recs['advanced_enhancements'])} items")
