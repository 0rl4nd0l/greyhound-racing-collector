...
def test_tgr_integrator_basic():
    ...
    expected_features = ['tgr_total_races', 'tgr_win_rate', 'tgr_form_trend', 'tgr_sentiment_score']
    for feature in expected_features:
        assert feature in feature_names, f"Expected feature {feature} not found in TGR features"
...
def test_tgr_historical_features():
    ...
    for feature in feature_names:
        assert feature in tgr_features, f"TGR feature {feature} missing from generated features"
        
        # Verify feature values are reasonable
        value = tgr_features[feature]
        assert isinstance(value, (int, float)), f"TGR feature {feature} has invalid type: {type(value)}"
...
def test_temporal_builder_tgr_integration():
    ...
    if builder.tgr_integrator:
        logger.info("✅ TGR integrator successfully initialized in TemporalFeatureBuilder")
    else:
        logger.warning("⚠️ TGR integrator not initialized - features may not be available")
    ...
    if tgr_feature_columns:
        logger.info(f"✅ Found {len(tgr_feature_columns)} TGR features in temporal builder output")
        ...
        for feature in tgr_feature_columns:
            ...
            assert not np.isnan(values).any(), f"TGR feature {feature} contains NaN values"
...
def test_ml_system_v4_tgr_integration():
    ...
    try:
        ...
        if result['success']:
            logger.info("✅ ML System V4 successfully generated predictions with TGR integration")
            logger.info(f"Predictions for {len(result['predictions'])} dogs")
            
            # Check prediction structure
            for i, pred in enumerate(result['predictions']):
                logger.info(f"Dog {i+1}: {pred['dog_name']} - Win Prob: {pred['win_prob_norm']:.3f}")
                
                # Verify required prediction fields
                required_fields = ['dog_name', 'win_prob_norm', 'confidence', 'predicted_rank']
                for field in required_fields:
                    assert field in pred, f"Missing required prediction field: {field}"
            
            logger.info("✅ ML System V4 TGR Integration test passed")
        else:
            logger.error(f"❌ ML System V4 prediction failed: {result.get('error', 'Unknown error')}")
...
def test_tgr_feature_caching():
    ...
    if second_call_time < first_call_time:
        ...
        logger.info("ℹ️ Cache timing inconclusive (both calls very fast with default features)")
...
def test_tgr_integration_end_to_end():
    ...
    race_data = pd.DataFrame({
        'dog_clean_name': ['BALLARAT STAR', 'SPEED DEMON', 'TRACK MASTER', 'FAST FINISH'],
        'box_number': [1, 2, 3, 4],
    ...
    if hasattr(ml_system.temporal_builder, 'tgr_integrator') and ml_system.temporal_builder.tgr_integrator:
    ...
    if prediction_result['success']:
        ...
        for pred in predictions:
            logger.info(f"  {pred['dog_name']}: Win {pred['win_prob_norm']:.3f}, Rank {pred['predicted_rank']}")
        ...
        assert 0.95 <= total_prob <= 1.05, f"Win probabilities don't sum to 1.0: {total_prob}"
...
def main():
    ...
    try:
        ...
        logger.info("")
        logger.info("✅ The Greyhound Recorder data is now successfully integrated into the prediction pipeline")
