...
class ProbabilityConverter:
    ...
    def load_strength_scores(self) -> pd.DataFrame:
        ...
        if self.strength_scores_file is None:
            ...
            files = [f for f in os.listdir('.') if f.startswith('step4_strength_scores_')]
            if not files:
                raise FileNotFoundError("No strength scores file found. Run Step 4 first.")
            
            # Get most recent file
            self.strength_scores_file = max(files, key=os.path.getctime)
            logger.info(f"Using most recent strength scores file: {self.strength_scores_file}")
    ...
    def apply_bayesian_smoothing(self, probabilities: np.ndarray, alpha: float = None) -> np.ndarray:
        """
    ...
    def create_synthetic_race_outcomes(self, scores_df: pd.DataFrame, n_races: int = 1000) -> List[Dict]:
        """
        ...
        for race_id in range(n_races):
            ...
            selected_dogs = scores_df.sample(n=race_size, replace=False if race_size <= len(scores_df) else True)
    ...
    def calibrate_temperature(self, scores_df: pd.DataFrame, use_synthetic: bool = True) -> float:
        """
        ...
        if use_synthetic:
            ...
            def temperature_loss(temperature):
                ...
                for race in races:
                    ...
                    outcome = np.array([1 if dog == winner_name else 0 for dog in race['dogs']])
            ...
            logger.info(f"Temperature calibration complete: optimal τ = {optimal_temp:.3f}")
            logger.info(f"Final log-loss: {result.fun:.4f}")
            
        else:
            # TODO: Load real historical race data for calibration
            logger.warning("Real historical data calibration not implemented yet. Using default temperature.")
            optimal_temp = 2.0
        
        self.optimal_temperature = optimal_temp
        return optimal_temp
    
    def convert_to_probabilities(self, scores_df: pd.DataFrame = None, 
                               calibrate_temperature: bool = True) -> pd.DataFrame:
        """
        ...
        logger.info(f"Applying softmax with temperature τ = {self.optimal_temperature:.3f}")
        ...
        logger.info(f"Probability range: {final_probabilities.min():.4f} - {final_probabilities.max():.4f}")
        logger.info(f"Percentage range: {final_probabilities.min()*100:.2f}% - {final_probabilities.max()*100:.2f}%")
    ...
    def simulate_race_outcomes(self, probabilities_df: pd.DataFrame, 
        ...
        for dog_name in dog_names:
            predicted_prob = probabilities_df[probabilities_df['dog_name'] == dog_name]['final_probability'].iloc[0]
            ...
            comparison_data.append({
                ...
                'relative_error': abs(predicted_prob - empirical_rate) / predicted_prob if predicted_prob > 0 else 0
            })
    ...
    def plot_probability_distribution(self, probabilities_df: pd.DataFrame, 
                                    save_path: str = None):
        """
        ...
        try:
            ...
            ax2.barh(range(len(top_20)), top_20['win_percentage'], color='green', alpha=0.7)
            ...
            ax3.plot(probabilities_df['probability_rank'], probabilities_df['win_percentage'], 
...
def main():
    ...
    try:
        ...
        print(f"Mean Probability: {analysis['mean_probability']:.4f} ({analysis['mean_probability']*100:.2f}%)")
        print(f"Min Probability: {analysis['min_probability']:.4f} ({analysis['min_probability']*100:.2f}%)")
        print(f"Max Probability: {analysis['max_probability']:.4f} ({analysis['max_probability']*100:.2f}%)")
        ...
        simulation_results = converter.simulate_race_outcomes(probabilities_df, n_simulations=10000)
        ...
        print(f"Minimum Probability Floor: {converter.min_probability:.3f} ({converter.min_probability*100:.1f}%)")
        print(f"Sum of all probabilities: {probabilities_df['final_probability'].sum():.6f}")
