...
class OptimizedTGRProcessor:
    ...
    def process_dogs_bulk(
        self, dog_names: List[str], batch_size: int = 10
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Bulk processing complete: {results['processed']} processed, {results['errors']} errors"
        )
        return results
    ...
    def get_tgr_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "real_tgr_coverage": (
                    (real_tgr_dogs / total_dogs * 100) if total_dogs > 0 else 0
                ),
            }
            ...
            logger.info(
                f"  Dogs with TGR data: {stats['tgr_dogs']} ({stats['tgr_coverage']:.1f}%)"
            )
            logger.info(
                f"  Real TGR data: {stats['real_tgr_dogs']} ({stats['real_tgr_coverage']:.1f}%)"
            )
