...
class OptimizedTemporalFeatureBuilder:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db"):
        ...
        if TGRPredictionIntegrator and tgr_flag:
            try:
                self.tgr_integrator = TGRPredictionIntegrator(db_path=self.db_path, enable_tgr_lookup=True)
                logger.info("✅ TGR integration initialized for enhanced historical features (TGR_ENABLED=1)")
            ...
            if not batch_data.empty:
                ...
                for dog_name in dog_names:
                    ...
                    if not dog_data.empty:
                        dog_data = dog_data.sort_values('race_timestamp', ascending=False).head(100)
                    result[dog_name] = dog_data
            
            logger.debug(f"Batch query for {len(dog_names)} dogs: {len(batch_data)} rows")
    ...
    def create_historical_features(self, historical_data: pd.DataFrame,
        ...
        positions = pd.to_numeric(historical_data['finish_position'], errors='coerce').dropna()
        ...
        times = pd.to_numeric(historical_data['individual_time'], errors='coerce').dropna()
        if len(times) > 0:
