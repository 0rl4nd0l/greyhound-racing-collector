...
class TraditionalRaceAnalyzer:
    ...
    def analyze_race(
        self, race_data: Dict, historical_data: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for dog_data in dogs:
                ...
                traditional_factors = self.analyze_dog(
                    dog_name, race_data, historical_data
                )
                ...
                prediction = {
                    ...
                    "ml_enrichment_features": self._extract_ml_features(
                        traditional_factors, dog_data
                    ),
                }
            ...
            # Add race insights
            results["race_insights"] = self._generate_race_insights(
    ...
    def analyze_dog(
        self, dog_name: str, race_context: Dict, historical_data: Optional[Dict] = None
    ) -> TraditionalFactors:
        """
        ...
        try:
            ...
            track_condition_score = self._calculate_track_condition_score(
    ...
    def _process_race_results(self, results: List[Tuple]) -> Dict[str, Any]:
        ...
        for i, result in enumerate(results):
            ...
            # Count wins, places, shows - convert position to int safely
            if position:
                try:
                    ...
                    stats["positions"].append(position_int)
    ...
    def _calculate_derived_stats(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if races_count > 0:
            ...
            if positions:
                ...
                derived["consistency"] = (
                    1 / (1 + derived["position_std"])
                    if derived["position_std"] > 0
                    else 1.0
                )
            ...
            if times:
                ...
                derived["time_consistency"] = (
                    1 / (1 + np.std(times)) if np.std(times) > 0 else 1.0
                )
            ...
            if stats["trainers"]:
                ...
                derived["trainer_stability"] = (
                    max(trainer_counts.values()) / races_count
                )

            # Venue diversity
            derived["venue_count"] = len(set(stats["venues"])) if stats["venues"] else 0
            derived["venue_diversity"] = (
                derived["venue_count"] / races_count if races_count > 0 else 0
            )
            ...
            if stats["distances"]:
                ...
                derived["preferred_distance"] = self._find_preferred_distance(
                    distance_performance
                )
            ...
            for condition, cond_positions in stats["track_conditions"].items():
                if cond_positions:
                    track_condition_stats[condition] = {
                        ...
                        "win_rate": sum(1 for p in cond_positions if p == 1)
                        / len(cond_positions),
                    }
            derived["track_condition_performance"] = track_condition_stats

            # Recent activity
            derived["recent_activity"] = self._calculate_recent_activity(
    ...
    def _calculate_form_score(self, dog_stats: Dict[str, Any]) -> float:
        ...
        recent_avg_position = dog_stats.get(
            "recent_avg_position", dog_stats.get("avg_position", 5)
        )
        ...
        recent_score = (
            max(0, (8 - recent_avg_position) / 8) if recent_avg_position > 0 else 0
        )
    ...
    def _calculate_class_score(
        self, dog_stats: Dict[str, Any], race_context: Dict
    ) -> float:
    ...
    def _calculate_track_condition_score(
        self, dog_stats: Dict[str, Any], race_context: Dict
    ) -> float:
    ...
    def _extract_ml_features(
        self, factors: TraditionalFactors, dog_data: Dict
    ) -> Dict[str, float]:
    ...
    def _generate_race_insights(self, predictions: List[Dict]) -> Dict[str, Any]:
        ...
        high_confidence_dogs = len(
            [p for p in predictions if p["confidence_level"] >= 0.7]
        )
        strong_contenders = len(
            [p for p in predictions if p["traditional_score"] >= 0.6]
        )

        top_dog = predictions[0]
        score_gap = (
            top_dog["traditional_score"] - predictions[1]["traditional_score"]
        ...
        return {
            ...
            "data_quality": (
                "Good" if high_confidence_dogs >= total_dogs * 0.6 else "Limited"
            ),
        }
    ...
    def _find_preferred_distance(
        self, distance_performance: Dict[str, List[int]]
    ) -> Optional[str]:
    ...
    def _calculate_recent_activity(self, recent_results: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            last_race_date = datetime.strptime(
                recent_results[0]["race_date"], "%Y-%m-%d"
            )
...
def calculate_traditional_score(
    dog_name: str, race_context: Dict, db_path: str = "greyhound_data.db"
) -> float:
    """
...
def get_traditional_ml_features(
    dog_name: str, race_context: Dict, db_path: str = "greyhound_data.db"
) -> Dict[str, float]:
    """
...
if __name__ == "__main__":
    ...
    if results.get("traditional_predictions"):
        ...
        print(
            f"Top pick: {top_pick['dog_name']} - Score: {top_pick['traditional_score']:.3f}"
        )
