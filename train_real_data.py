    ...
    for idx, row in df.iterrows():
        try:
            ...
            features = {
                ...
                'weighted_recent_form': normalize_safe(9 - finish_pos, 1, 8),  # Better position = higher score
                ...
                'venue_win_rate': normalize_safe(1 / max(odds, 1.5), 0, 0.67),  # Lower odds = higher win rate
                'venue_avg_position': normalize_safe(9 - finish_pos, 1, 8),
                'venue_experience': normalize_safe(min(20, max(1, len(str(row.get('dog_name', ''))))), 1, 20),
                
                # Distance features
                'distance_win_rate': normalize_safe(1 / max(odds, 1.5), 0, 0.67),
                'distance_avg_time': normalize_safe(max(0, 35 - finish_pos), 27, 35),  # Estimate from position
                ...
                'top_3_rate': 1.0 if finish_pos <= 3 else normalize_safe(4 - finish_pos, 0, 4),
