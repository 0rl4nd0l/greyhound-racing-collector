...
class SHAPExplainer:
    ...
    def __init__(
        ...
        logger.info(
            f"🔍 Initializing SHAP explainer with models from: {self.model_path}"
        )
        ...
        try:
            ...
            if cache_path.exists():
                try:
                    ...
                    logger.debug(
                        f"Loaded cached background data with shape: {background.shape}"
                    )
                    return background
                except Exception as e:
    ...
    def get_shap_values(
        ...
        try:
            ...
            feature_names = (
                ...
                else [f"feature_{i}" for i in range(len(shap_vals))]
            )
    ...
    def add_explainability_to_prediction(
        self,
