...
from openai_enhanced_analyzer import (DogProfile, OpenAIEnhancedAnalyzer,
                                      RaceContext)
...
class GPTPredictionEnhancer:
    ...
    def __init__(
        ...
        # Initialize OpenAI connectivity verification
        self.connectivity_verifier = OpenAIConnectivityVerifier()

        # Step 1: Read OPENAI_API_KEY from .env; abort gracefully if absent
        key_loaded = self.connectivity_verifier.load_api_key()
        if not key_loaded:
            logger.error(
                "❌ OpenAI API key verification failed. GPT features will be unavailable."
            )
            logger.error(
                "   Please ensure OPENAI_API_KEY is properly set in your .env file."
            )
            self.gpt_available = False
            self.gpt_analyzer = None
            return

        # Step 2: Initialize client (with mock fallback for local dev)
        client_initialized = self.connectivity_verifier.initialize_client()
        if not client_initialized:
            logger.error("❌ OpenAI client initialization failed. Using mock mode.")
            self.gpt_available = False
            self.gpt_analyzer = None
            return

        # Step 3: Execute minimal chat completion to confirm authentication
        verification_result = self.connectivity_verifier.verify_connection()
        if not verification_result.get("success"):
            if not verification_result.get("mock_mode"):
                logger.error(
                    f"❌ OpenAI API authentication failed: {verification_result.get('error')}"
                )
                self.gpt_available = False
                self.gpt_analyzer = None
                return
                ...
                logger.info(
                    "🔧 Mock mode active - GPT features available with simulated responses"
                )

        # Initialize OpenAI analyzer with verified connectivity
        try:
            enhanced_client = self.connectivity_verifier.get_enhanced_client()
            # Create custom analyzer that uses our enhanced client
            self.gpt_analyzer = OpenAIEnhancedAnalyzer(
                api_key=self.connectivity_verifier.api_key, database_path=database_path
            )
            # Replace the client with our enhanced version that has retry logic
            self.gpt_analyzer.client = enhanced_client
            self.gpt_available = True

            if verification_result.get("mock_mode"):
                logger.info(
                    "🔧 GPT analyzer initialized in mock mode for local development"
                )
            else:
                logger.info(
                    "✅ GPT analyzer initialized successfully with verified OpenAI connection"
                )
                logger.info(
                    f"   Connection test used {verification_result.get('tokens_used', 0)} tokens"
                )

        except Exception as e:
            logger.error(f"❌ GPT analyzer initialization failed: {e}")
            self.gpt_available = False
            self.gpt_analyzer = None
    ...
    def enhance_race_prediction(
        ...
        try:
            ...
            # Run GPT analysis
            logger.info(
                f"Running GPT analysis for race: {race_context.venue} Race {race_context.race_number}"
            )
            ...
            if existing_predictions:
                ...
                enhanced_predictions["base_predictions"] = existing_predictions[
                    "predictions"
                ]
            ...
            final_enhanced_predictions = self._merge_gpt_with_ml_predictions(
                (
                    existing_predictions.get("predictions", [])
                    if existing_predictions
                    else []
                ),
                race_analysis,
    ...
    def enhance_multiple_races(
        ...
        for i, race_file in enumerate(race_files[:max_races]):
            logger.info(
                f"Enhancing race {i+1}/{min(len(race_files), max_races)}: {race_file}"
            )
            ...
            results.append({"race_file": race_file, "enhancement": enhancement})
        ...
        batch_summary = {
            ...
            "average_tokens_per_race": (
                total_tokens / successful_enhancements
        ...
        return {"batch_summary": batch_summary, "race_enhancements": results}
    ...
    def generate_daily_insights(self, date_str: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            wrapper = OpenAIWrapper(self.gpt_analyzer.client, get_openai_config())
            from src.ai.prompts import system_prompt
            resp = wrapper.chat(
                messages=[
                    {
                        "role": "system",
                        "content": system_prompt("daily"),
                    },
            ...
            daily_insights = resp.text

            return {
                "date": date_str,
                "total_races_analyzed": len(daily_race_data),
                "daily_insights": daily_insights,
                "structured_recommendations": self._parse_daily_recommendations(
                    daily_insights
                ),
                "timestamp": datetime.now().isoformat(),
    ...
    def create_comprehensive_report(self, race_ids: List[str]) -> str:
        ...
        report_sections.append("Enhanced with OpenAI GPT-4 Analysis")
        report_sections.append("=" * 80)
        report_sections.append("")
        ...
        for race_id in race_ids:
            ...
            if enhanced_file.exists():
                try:
                    ...
                    report_sections.append(
                        f"RACE: {race_info.get('venue')} Race {race_info.get('race_number')}"
                    )
                    report_sections.append(f"Date: {race_info.get('race_date')}")
                    report_sections.append(
                        f"Distance: {race_info.get('distance')} | Grade: {race_info.get('grade')}"
                    )
                    ...
                    if "raw_analysis" in gpt_analysis:
                        ...
                        report_sections.append(
                            gpt_analysis["raw_analysis"][:1000] + "..."
                        )
                        report_sections.append("")
        ...
        # Footer
        report_sections.append(
            "Report generated by GPT-Enhanced Greyhound Racing Analysis System"
        )
    ...
    def _load_race_data(self, race_file_path: str) -> Optional[Dict]:
        ...
        try:
            ...
            race_data = {
                ...
                "venue": venue,
                "race_number": (
                    int(race_number_part) if race_number_part.isdigit() else 1
                ),
                "distance": distance,
            ...
            for _, row in df.iterrows():
                ...
                if dog_name and dog_name != "nan" and not dog_name.startswith('"'):
                    ...
                    current_dog = {
                        "name": actual_name,
                        "box_number": (
                            int(row.get("BOX", 0)) if pd.notna(row.get("BOX")) else 0
                        ),
                        "trainer": "Unknown",  # Not in this CSV format
                        "weight": (
                            float(row.get("WGT", 0)) if pd.notna(row.get("WGT")) else 0
                        ),
                        "recent_form": (
                            [int(row.get("PLC", 0))] if pd.notna(row.get("PLC")) else []
                        ),
                        "best_time": (
                    ...
                    if (
                        ...
                        and float(row.get("TIME", 0)) < current_dog["best_time"]
                    ):
    ...
    def _create_analysis_summary(
        self, race_analysis: Dict, enhanced_predictions: Dict, betting_strategy: Dict
    ) -> Dict:
    ...
    def _parse_daily_recommendations(self, insights_text: str) -> Dict:
        ...
        for line in lines:
            line = line.strip()
            if "value" in line.lower() and (
                "bet" in line.lower() or "opportunity" in line.lower()
            ):
    ...
    def _merge_gpt_with_ml_predictions(
        self,
        ml_predictions: List[Dict],
        gpt_race_analysis: Dict,
        gpt_enhanced_predictions: Dict,
    ) -> List[Dict]:
        ...
        try:
            if "raw_analysis" in gpt_race_analysis:
                ...
                order_match = re.search(
                    r'"order":\s*\[(.*?)\]', analysis_text, re.DOTALL
                )
                if order_match:
        ...
        for ml_pred in ml_predictions:
            ...
            original_final_score = ml_pred.get(
                "final_score", ml_pred.get("prediction_score", 0.5)
            )
    ...
    if upcoming_dir.exists():
        ...
        if csv_files:
            ...
            if "error" not in result:
                ...
                print(
                    f"Analysis confidence: {result.get('gpt_race_analysis', {}).get('analysis_confidence', 0):.2f}"
                )
            else:
