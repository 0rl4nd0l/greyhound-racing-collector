...
import numpy as np
import pandas as pd
from prediction_assistant import (calculate_metrics, clean_dog_name,
                                  get_weight_bracket,
                                  get_weight_bracket_performance)
...
if __name__ == "__main__":
    ...
    print("Key Insights:")
    print(
        f"• Top pick: {predictions[0]['greyhound_name']} with {predictions[0]['confidence']*100:.1f}% confidence"
    )
    print(
        f"• Fastest early speed: {max(p['early_speed'] for p in predictions):.1f} m/s"
    )
    print(
        f"• Weight range: {min(p['weight'] for p in predictions):.1f}kg - {max(p['weight'] for p in predictions):.1f}kg"
    )
