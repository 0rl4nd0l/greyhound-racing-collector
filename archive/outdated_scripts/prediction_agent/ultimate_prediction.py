...
def calculate_ultimate_score(metrics, ultimate_insights):
    ...
    estimated_rank = (
        1
        if metrics["starting_price"] <= 3
        else (
            2
            if metrics["starting_price"] <= 6
            else (
                3
                if metrics["starting_price"] <= 10
                else 4 if metrics["starting_price"] <= 15 else 5
            )
        )
    )
...
def run_ultimate_prediction(race_file_path):
    ...
    print(
        f"Total Analysis Records: {ultimate_insights['track_bias_analysis']['total_bias_records']}"
    )
    print(
        f"Market Records: {ultimate_insights['market_dynamics']['total_market_records']}"
    )
    print()
...
def generate_ultimate_ai_insights(predictions, ultimate_insights):
    ...
    race_summary = {
        ...
        "track_insights": {
            ...
            "total_bias_records": ultimate_insights["track_bias_analysis"][
                "total_bias_records"
            ],
        },
    }
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a professional greyhound racing analyst specializing in track bias and market analysis. Use the provided data to give specific, actionable insights.",
                },
...
def print_ultimate_analysis(predictions, ultimate_insights):
    ...
    for i, pred in enumerate(predictions, 1):
        ...
        print(
            f"   📈 Market Rank: {pred['estimated_rank']} (Win Rate: {pred['market_win_rate']:.1%})"
        )
    ...
    print(
        f"• TOP PICK: {top_pick['greyhound_name']} ({top_pick['confidence']*100:.1f}% confidence)"
    )
    print(f"• HEA TRACK BIAS: Box {hea_bias['best_box']} is statistically best")
    print(
        f"• MARKET LEADER: {min(predictions, key=lambda x: x['starting_price'])['greyhound_name']} (${min(p['starting_price'] for p in predictions):.1f})"
    )
    ...
    if best_box_runners:
        print(
            f"• BEST BOX RUNNERS: {', '.join([p['greyhound_name'] for p in best_box_runners])}"
        )

    print(
        f"• MARKET EFFICIENCY: Favorites win {54.1:.1f}% of races, 2nd favorites win {14.3:.1f}%"
    )
...
def run_complete_ultimate_analysis():
    ...
    try:
        ...
        if best_box_runners:
            print(
                f"  • TRACK BIAS SPECIAL: {best_box_runners[0]['greyhound_name']} (Box {hea_bias['best_box']} advantage)"
            )
