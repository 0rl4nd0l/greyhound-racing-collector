...
class DataConfidenceScorer:
    ...
    def generate_confidence_report(self):
        ...
        print(
            f"   Average Record Confidence: {record_confidence['confidence_score'].mean():.1f}%"
        )
        print(
            f"   Average Race Confidence: {race_confidence['confidence_score'].mean():.1f}%"
        )
        ...
        print(
            f"   Usable Records: {len(usable_records):,}/{len(record_confidence):,} ({(len(usable_records)/len(record_confidence))*100:.1f}%)"
        )
        print(
            f"   Usable Races: {len(usable_races):,}/{len(race_confidence):,} ({(len(usable_races)/len(race_confidence))*100:.1f}%)"
        )

        # High confidence data
        high_confidence = record_confidence[record_confidence["confidence_score"] >= 70]
        print(
            f"   High Confidence (B+ or better): {len(high_confidence):,} ({(len(high_confidence)/len(record_confidence))*100:.1f}%)"
        )
        ...
        if avg_critical < 0.8:
            print(
                f"   ⚠️  Critical field completeness is low ({avg_critical:.1%}) - focus on basic race data"
            )
        if avg_performance < 0.5:
            print(
                f"   ⚠️  Performance field completeness is low ({avg_performance:.1%}) - enhance scraping for times/weights"
            )
        if len(high_confidence) < len(record_confidence) * 0.3:
            print(
                f"   ⚠️  Low proportion of high-confidence data - consider data enrichment"
            )
        ...
        report_data = {
            "summary": {
                ...
                "avg_record_confidence": float(
                    record_confidence["confidence_score"].mean()
                ),
                "avg_race_confidence": float(
                    race_confidence["confidence_score"].mean()
                ),
    ...
    try:
        ...
        if results:
            ...
            if weighted_data is not None:
                ...
                print(
                    f"   Average confidence weight: {weighted_data['confidence_weight'].mean():.3f}"
                )
        else:
