...
def basic_prediction_fallback(race_file_path):
    ...
    try:
        ...
        prediction_file = predictions_dir / build_prediction_filename(race_id, datetime.now(), "basic_fallback")
...
def save_prediction_results(race_file_path, results):
    ...
    try:
        ...
        filepath = predictions_dir / build_prediction_filename(race_id, datetime.now(), "unified")
        
        # Check if file already exists and is recent (within last hour)
        if filepath.exists():
            file_age = time.time() - filepath.stat().st_mtime
            if file_age < 3600:  # Less than 1 hour old
                print(f"⚠️ Recent prediction already exists: {filepath.name} (age: {file_age/60:.1f} min)")
                print(f"💡 Skipping save to prevent duplicate")
                return
            else:
                # Create backup of old prediction
                backup_filename = f"{filepath.name}.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
