...
class ComprehensivePredictionPipeline:
    ...
    def predict_race_file(self, race_file_path, force_rerun=False):
        ...
        if self.gpt_enhancer and len(final_predictions) > 0:
            try:
                enhanced_predictions = self._apply_gpt_enhancement(final_predictions, race_info, dogs_data)
    ...
    def predict_race_file_with_all_enhancements(self, race_file_path, enhancement_level="full"):
        """Enhanced prediction method with all available enhancements"""
        print(f"🚀 Starting comprehensive prediction with {enhancement_level} enhancements...")
    ...
    def _calculate_weighted_final_score(self, prediction_scores, data_quality):
        ...
        weights = {
            'ml_system': 0.4 * data_quality,
            'weather_enhanced': 0.2 * data_quality,
            'enhanced_data': 0.15 * data_quality,
            'traditional': 0.25  # Always available baseline
        }
        ...
        try:
            ...
            for pred in predictions:
                ...
                if dog_name in dogs_data:
                    dog_info = {
                        'name': dog_name,
                        'box': pred['box_number'],
                        'historical_data': dogs_data[dog_name].get('database_data', [])[:10],
                        'form_data': dogs_data[dog_name].get('form_guide_data', [])[:10],
            ...
            for pred in predictions:
                ...
                if gpt_insights and pred['dog_name'] in gpt_insights:
                    ...
                    enhanced_pred['final_score'] = min(max(
                        pred['final_score'] + gpt_score_adjustment, 0.1), 0.9)
                    
                    # Add GPT reasoning
                    if gpt_data.get('reasoning'):
                        enhanced_pred['reasoning'].append(f"GPT: {gpt_data['reasoning']}")
                    ...
                    elif gpt_confidence_boost > 0.2 and pred['confidence_level'] == 'MEDIUM':
        ...
        try:
            ...
            for dog in race_data['dogs']:
                ...
                if dog['historical_data']:
                    recent_positions = [r.get('finish_position', 5) for r in dog['historical_data'][:5] 
                ...
                if recent_positions:
                    avg_recent = np.mean([float(p) for p in recent_positions if str(p).replace('.', '').isdigit()])
                ...
                venue_experience = len([r for r in dog['historical_data'] 
    ...
    def predict_all_upcoming_races(self, upcoming_dir='upcoming_races', force_rerun=False):
        """
        ...
        """
        print(f"🚀 Starting batch prediction for all upcoming races in '{upcoming_dir}'")
        ...
        summary_filename = f"batch_prediction_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python comprehensive_prediction_pipeline.py <race_file_path_or_all> [--force-rerun]")
        print("  <race_file_path_or_all>: Path to a single race file or 'all' to run all upcoming races.")
    ...
    if target.lower() == 'all':
        ...
        if batch_results['success']:
            ...
            print(f"❌ Batch prediction failed: {batch_results.get('message', 'Unknown error')}")
            sys.exit(1)
