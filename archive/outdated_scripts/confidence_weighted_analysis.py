...
class ConfidenceWeightedAnalysis:
    ...
    def __init__(
        self, db_path="comprehensive_greyhound_data.db", min_confidence_grade="C"
    ):
    ...
    def load_confidence_weighted_data(self):
        ...
        try:
            ...
            print(
                f"   Average confidence weight: {self.weighted_data['confidence_weight'].mean():.3f}"
            )
            ...
            analysis = {
                ...
                "confidence_statistics": {
                    "mean_confidence": float(
                        self.weighted_data["confidence_score"].mean()
                    ),
                    "median_confidence": float(
                        self.weighted_data["confidence_score"].median()
                    ),
                    "std_confidence": float(
                        self.weighted_data["confidence_score"].std()
                    ),
                    "min_confidence": float(
                        self.weighted_data["confidence_score"].min()
                    ),
                    "max_confidence": float(
                        self.weighted_data["confidence_score"].max()
                    ),
                },
                "data_quality_metrics": {
                    "total_records": len(self.weighted_data),
                    "high_confidence_records": len(
                        self.weighted_data[self.weighted_data["confidence_score"] >= 70]
                    ),
                    "low_confidence_records": len(
                        self.weighted_data[self.weighted_data["confidence_score"] < 50]
                    ),
                    "avg_confidence_weight": float(
                        self.weighted_data["confidence_weight"].mean()
                    ),
                },
            }
    ...
    def weighted_track_bias_analysis(self):
        ...
        try:
            ...
            bias_data = (
                self.weighted_data.groupby(["venue", "box_number"])
                .apply(
                    lambda x: pd.Series(
                        {
                            "weighted_win_rate": (
                                (
                                    (x["finish_position"] == "1st")
                                    * x["confidence_weight"]
                                ).sum()
            ...
            significant_bias = bias_data[
                (bias_data["total_weight"] >= 5) & (bias_data["sample_size"] >= 10)
            ]
            ...
            return {
                ...
                "bias_reliability": {
                    ...
                    "avg_confidence_of_significant": (
                        float(significant_bias["avg_confidence"].mean())
    ...
    def weighted_trainer_performance(self):
        ...
        try:
            ...
            trainer_stats = (
                self.weighted_data.groupby("trainer_name")
                .apply(
                    lambda x: pd.Series(
                        {
                            "weighted_win_rate": (
                            ...
                            "sample_size": len(x),
                            "avg_confidence": x["confidence_score"].mean(),
                            "confidence_range": x["confidence_score"].max()
            ...
            return {
                ...
                "trainer_reliability": {
                    ...
                    "avg_confidence_of_qualified": (
                        float(qualified_trainers["avg_confidence"].mean())
                        if len(qualified_trainers) > 0
                        else 0
                    ),
                    "most_reliable_trainer": (
                        qualified_trainers.loc[
                            qualified_trainers["avg_confidence"].idxmax(),
                            "trainer_name",
                        ]
                        if len(qualified_trainers) > 0
                        else None
    ...
    def weighted_market_efficiency(self):
        ...
        try:
            ...
            odds_data = self.weighted_data[
                self.weighted_data["odds_decimal"].notna()
            ].copy()
            ...
            bracket_performance = (
                odds_data.groupby("odds_bracket", observed=True)
                .apply(
                    lambda x: pd.Series(
                        {
                            "weighted_win_rate": (
                            ...
                            "implied_probability": (
                                1 / x["odds_decimal"] * x["confidence_weight"]
                            ).sum()
                            / x["confidence_weight"].sum(),
                            "total_weight": x["confidence_weight"].sum(),
                            "sample_size": len(x),
            ...
            bracket_performance["weighted_value_ratio"] = (
                bracket_performance["weighted_win_rate"]
                / bracket_performance["implied_probability"]
            )
            bracket_performance["unweighted_value_ratio"] = (
                bracket_performance["unweighted_win_rate"]
                / bracket_performance["implied_probability"]
            )

            return {
                "confidence_weighted_market_efficiency": bracket_performance.to_dict(
                    "records"
                ),
                "market_reliability": {
                    "total_odds_records": len(odds_data),
                    "avg_confidence_with_odds": float(
                        odds_data["confidence_score"].mean()
                    ),
                    "weighted_correlation": (
                        float(
                            np.corrcoef(
                                odds_data["implied_probability"]
                                * odds_data["confidence_weight"],
                                (odds_data["finish_position"] == "1st").astype(int)
                                * odds_data["confidence_weight"],
                            )[0, 1]
                        )
                        if len(odds_data) > 1
    ...
    def build_confidence_weighted_model(self):
        ...
        try:
            ...
            feature_data = model_data[
                existing_numeric + existing_categorical + ["finish_position"]
            ].copy()
            ...
            feature_data["is_winner"] = (
                feature_data["finish_position"] == "1st"
            ).astype(int)
            ...
            X_train, X_test, y_train, y_test, weights_train, weights_test = (
                train_test_split(X, y, weights, test_size=0.3, random_state=42)
            )
            ...
            weighted_accuracy = accuracy_score(
                y_test, y_pred, sample_weight=weights_test
            )
    ...
    def print_confidence_weighted_summary(self, insights):
        ...
        if "confidence_distribution" in insights:
            ...
            print(
                f"   Mean Confidence: {dist['confidence_statistics']['mean_confidence']:.1f}%"
            )
            print(
                f"   Average Weight: {dist['data_quality_metrics']['avg_confidence_weight']:.3f}"
            )
            print(
                f"   High Confidence Records: {dist['data_quality_metrics']['high_confidence_records']:,}"
            )
        ...
        if "weighted_track_bias" in insights:
            ...
            if "confidence_weighted_biases" in bias:
                ...
                print(
                    f"   Significant Combinations: {bias['bias_reliability']['statistically_significant']}"
                )
                if bias["confidence_weighted_biases"]:
                    top_bias = bias["confidence_weighted_biases"][0]
                    print(
                        f"   Top Bias: Box {top_bias['box_number']} at {top_bias['venue']}"
                    )
                    print(
                        f"             Weighted: {top_bias['weighted_win_rate']:.1%} | Unweighted: {top_bias['unweighted_win_rate']:.1%}"
                    )
        ...
        if "weighted_trainer_performance" in insights:
            ...
            if "confidence_weighted_trainers" in trainers:
                ...
                print(
                    f"   Qualified Trainers: {trainers['trainer_reliability']['qualified_trainers']}"
                )
                if trainers["confidence_weighted_trainers"]:
                    ...
                    print(
                        f"               Weighted: {top_trainer['weighted_win_rate']:.1%} | Unweighted: {top_trainer['unweighted_win_rate']:.1%}"
                    )
        ...
        if "weighted_market_efficiency" in insights:
            ...
            if "confidence_weighted_market_efficiency" in market:
                ...
                print(
                    f"   Records with Odds: {market['market_reliability']['total_odds_records']:,}"
                )
                print(
                    f"   Weighted Correlation: {market['market_reliability']['weighted_correlation']:.3f}"
                )
