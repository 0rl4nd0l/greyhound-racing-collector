...
class AutomatedMonitor:
    ...
    def process_results(self, results: Dict):
        ...
        if results["value_opportunities"] > 0:
            self.logger.info(
                f"🎯 Found {results['value_opportunities']} value betting opportunities!"
            )
    ...
    def generate_daily_report(self):
        ...
        try:
            ...
            report_file = Path(
                f"daily_reports/report_{datetime.now().strftime('%Y%m%d')}.json"
            )
    ...
    def start(self):
        ...
        # Log configuration
        self.logger.info(
            f"   Monitoring interval: {self.config['monitoring_interval_minutes']} minutes"
        )
        self.logger.info(
            f"   Value threshold: {self.config['value_threshold_percentage']}%"
        )
        ...
        try:
            while self.running:
                ...
                self.logger.info(
                    f"⏱️  Waiting {self.config['monitoring_interval_minutes']} minutes until next cycle..."
                )
...
def create_sample_config():
    ...
    config = {
        ...
        "confidence_threshold": 0.6,
        "max_stake_percentage": 5.0,
        "alert_thresholds": {"high_value": 10.0, "medium_value": 7.5, "low_value": 5.0},
        "notifications": {
            "console": True,
            "file": True,
            "email": False,
...
def main():
    parser = argparse.ArgumentParser(
        description="Automated Greyhound Racing Value Bet Monitor"
    )
