...
class TGRPredictionIntegrator:
    ...
    def __init__(
        ...
        try:
            ...
            if self._tgr_db_available is False:
                logger.info(
                    "TGR DB tables not present; will prefer scraper-only mode when enabled"
                )
        except Exception:
        ...
        try:
            self._allow_on_demand_scrape = str(
                os.getenv("ALLOW_TGR_ON_DEMAND_SCRAPE", "0")
    ...
    def enhance_prediction_features(
        self, dog_data: pd.DataFrame, race_timestamp: datetime
    ) -> pd.DataFrame:
        ...
        if not self.enable_tgr_lookup:
            ...
            return dog_data
        logger.info(
            f"🔍 Enhancing prediction features with TGR data for {len(dog_data)} dogs"
        )
        ...
        for _, dog in dog_data.iterrows():
            ...
            tgr_features = self._get_tgr_historical_features(
                dog_dict["dog_clean_name"], race_timestamp
            )
    ...
    def _get_tgr_historical_features(
        self, dog_name: str, race_timestamp: datetime
    ) -> Dict[str, Any]:
    ...
    def _search_tgr_dog_form(
        ...
        if self._tgr_db_available is False:
            logger.debug(
                "Skipping TGR DB queries (tables not present); will use scraper if enabled"
            )
        else:
            ...
            try:
                ...
                if form_data:
                    logger.debug(
                        f"Found {len(form_data)} TGR legacy form entries for {dog_name}"
                    )
                    return form_data
            except Exception as e:
                if "no such table" in str(e).lower():
                    logger.info(
                        f"Legacy TGR tables not found while querying for {dog_name}; will try enhanced or scraper"
                    )
                    self._tgr_db_available = (
                        False  # mark as unavailable to avoid future attempts
                    )
                else:
                    logger.warning(
                        f"Error querying legacy TGR form data for {dog_name}: {e}"
                    )
            ...
            try:
                ...
                if form_data:
                    logger.debug(
                        f"Enhanced fallback: found {len(form_data)} entries for {dog_name}"
                    )
            except Exception as e:
                if "no such table" in str(e).lower():
                    logger.info(
                        f"Enhanced TGR table not found while querying for {dog_name}; will use scraper if enabled"
                    )
                    self._tgr_db_available = False
                else:
                    logger.warning(
                        f"Error querying enhanced TGR form data for {dog_name}: {e}"
                    )

        # Attempt 3: Targeted scrape-and-persist (only when scraper is enabled and allowed)
        if (
            not form_data
            and getattr(self, "tgr_scraper", None)
            and getattr(self, "_allow_on_demand_scrape", False)
        ):
            try:
                ...
                if self._tgr_db_available is not False:
                    try:
                        ...
                        if form_data:
                            logger.debug(
                                f"Scrape fallback: found {len(form_data)} entries for {dog_name}"
                            )
                    except Exception as e:
                        if "no such table" in str(e).lower():
                            logger.info(
                                "Enhanced TGR table still not present after scrape; skipping re-query"
                            )
                            self._tgr_db_available = False
                        else:
                            logger.debug(
                                f"Re-query after scrape failed for {dog_name}: {e}"
                            )
            except Exception as e:
            ...
            not form_data
            and getattr(self, "tgr_scraper", None)
            and not getattr(self, "_allow_on_demand_scrape", False)
        ):
            logger.debug(
                "On-demand TGR scrape fallback disabled by ALLOW_TGR_ON_DEMAND_SCRAPE=0; returning defaults"
            )

        return form_data

    def _calculate_tgr_features(
        self, form_data: List[Dict[str, Any]], race_timestamp: datetime
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if all_positions:
                features["tgr_avg_finish_position"] = sum(all_positions) / len(
                    all_positions
                )
                features["tgr_best_finish_position"] = min(all_positions)
                features["tgr_win_rate"] = len(
                    [p for p in all_positions if p == 1]
                ) / len(all_positions)
                features["tgr_place_rate"] = len(
                    [p for p in all_positions if p <= 3]
                ) / len(all_positions)
                features["tgr_consistency"] = 1.0 - (
                    max(all_positions) - min(all_positions)
                ) / max(all_positions, 1)
            else:
            ...
            if recent_positions and len(recent_positions) >= 3:
                ...
                early_3 = (
                    ...
                    else recent_positions[3:6]
                )

                if early_3:
                    recent_avg = sum(recent_3) / len(recent_3)
                    early_avg = sum(early_3) / len(early_3)
                    features["tgr_form_trend"] = (
                        early_avg - recent_avg
                    ) / early_avg  # Positive = improving
                else:
                    features["tgr_form_trend"] = 0.0

                features["tgr_recent_avg_position"] = sum(recent_positions) / len(
                    recent_positions
                )
                features["tgr_recent_best_position"] = min(recent_positions)
            else:
                features["tgr_form_trend"] = 0.0
                features["tgr_recent_avg_position"] = features[
                    "tgr_avg_finish_position"
                ]
                features["tgr_recent_best_position"] = features[
            ...
            if form_data:
                ...
                most_recent_date = datetime.strptime(
                    most_recent["race_date"], "%Y-%m-%d"
                )
                days_since_last_race = (
                    race_timestamp.date() - most_recent_date.date()
                ).days
            ...
            comments = [
                entry["comments"] for entry in form_data if entry.get("comments")
            ]
            ...
            logger.debug(
                f"Calculated TGR features: win_rate={features['tgr_win_rate']:.3f}, "
    ...
    def _cache_tgr_features(
        self, dog_name: str, race_timestamp: datetime, features: Dict[str, Any]
    ):
...
def integrate_tgr_with_temporal_builder(
    temporal_builder, db_path: str = "greyhound_racing_data.db"
):
    ...
    def enhanced_create_historical_features(
        historical_data, target_venue=None, target_grade=None, target_distance=None
    ):
        ...
        if (
            hasattr(temporal_builder, "current_dog_name")
            and temporal_builder.current_dog_name
        ):
            if (
                hasattr(temporal_builder, "current_race_timestamp")
                and temporal_builder.current_race_timestamp
            ):
                tgr_features = (
                    temporal_builder.tgr_integrator._get_tgr_historical_features(
                ...
                prefixed_tgr_features = {
                    f"tgr_{k}" if not k.startswith("tgr_") else k: v
                    for k, v in tgr_features.items()
                }
                original_features.update(prefixed_tgr_features)

                logger.debug(
                    f"Added {len(tgr_features)} TGR features for {temporal_builder.current_dog_name}"
                )
