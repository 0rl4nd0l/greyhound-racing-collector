...
class DatabaseSchemaSynchronizer:
    ...
    def validate_race_metadata_table(self):
        ...
        if missing_columns:
            logger.warning(
                f"Missing columns in race_metadata: {[col[0] for col in missing_columns]}"
            )
            for column, column_type in missing_columns:
                try:
                    self.cursor.execute(
                        f"ALTER TABLE race_metadata ADD COLUMN {column} {column_type}"
                    )
    ...
    def validate_dog_race_data_table(self):
        ...
        if missing_columns:
            logger.warning(
                f"Missing columns in dog_race_data: {[col[0] for col in missing_columns]}"
            )
            for column, column_type in missing_columns:
                try:
                    self.cursor.execute(
                        f"ALTER TABLE dog_race_data ADD COLUMN {column} {column_type}"
                    )
    ...
    def validate_foreign_keys(self):
        ...
        if self.table_exists("gpt_analysis"):
            ...
            if fk_violations:
                logger.warning(
                    f"Foreign key violations in gpt_analysis: {len(fk_violations)}"
                )
            else:
    ...
    def run_schema_validation_tests(self):
        ...
        race_columns_exist = all(
            self.column_exists("race_metadata", col) for col in critical_race_columns
        )
        ...
        dog_columns_exist = all(
            self.column_exists("dog_race_data", col) for col in critical_dog_columns
        )
        ...
        try:
            ...
            if race_count > 0 and dog_count > 0:
                tests_passed += 1
                logger.info(
                    f"✓ Test 4 passed: Data exists (races: {race_count}, dogs: {dog_count})"
                )
            else:
                logger.warning(
                    f"⚠ Test 4 warning: Limited data (races: {race_count}, dogs: {dog_count})"
                )
        except Exception as e:
