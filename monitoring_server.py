...
def dashboard():
    ...
    try:
        ...
        logger.log_error(
            f"Error serving dashboard: {str(e)}", context={"component": "web_server"}
        )
...
def get_health():
    ...
    try:
        ...
        logger.log_error(
            f"Health endpoint error: {str(e)}", context={"component": "web_api"}
        )
        return (
            jsonify(
                {
...
def get_metrics():
    ...
    try:
        ...
        logger.log_error(
            f"Metrics endpoint error: {str(e)}", context={"component": "web_api"}
        )
        return (
            jsonify(
                {
...
def get_predictions():
    ...
    try:
        ...
        logger.log_error(
            f"Predictions endpoint error: {str(e)}", context={"component": "web_api"}
        )
...
def get_trends():
    ...
    try:
        ...
        logger.log_error(
            f"Trends endpoint error: {str(e)}", context={"component": "web_api"}
        )
...
def get_alerts():
    ...
    try:
        ...
        logger.log_error(
            f"Alerts endpoint error: {str(e)}", context={"component": "web_api"}
        )
    ...
    try:
        ...
        combined_status = {
            ...
            "system": {
                ...
                "message": health.get("system_status", {}).get(
            ...
            "predictions": {
                ...
                "predictions_today": metrics.get("metrics", {}).get(
                    "predictions_today", 0
                ),
        ...
        logger.log_error(
            f"Status endpoint error: {str(e)}", context={"component": "web_api"}
        )
        return (
            jsonify(
                {
                    "success": False,
                    "error": str(e),
                    "system": {"status": "error", "message": "Status check failed"},
                }
            ),
            500,
        )
...
def internal_error(error):
    """Handle 500 errors"""
    logger.log_error(
        f"Internal server error: {str(error)}", context={"component": "web_server"}
    )
    return (
        jsonify(
            {
...
def log_request():
    ...
    if request.endpoint and not request.endpoint.startswith("static"):
        logger.log_system(
            f"API Request: {request.method} {request.path}", "INFO", "WEB_API"
        )
...
def run_server(host="localhost", port=int(os.environ.get('DEFAULT_PORT', '5002')), debug=False):
    ...
    if not dashboard_file.exists():
        ...
        print(
            "   Make sure 'monitoring_dashboard.html' exists in the current directory"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Greyhound Racing Monitoring Web Server"
    )
    parser.add_argument(
