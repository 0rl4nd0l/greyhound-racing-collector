...
try:
    from config.logging_config import get_component_logger, log_config
except ImportError:
...
class ConfigLoader:
    ...
    def __init__(
        ...
        log_config(
            "Configuration loaded successfully", details=self._get_config_summary()
        )
    ...
    def _load_from_yaml(self) -> None:
        ...
        try:
            ...
            if not self.config_file.exists():
                log_config(
                    f"YAML config file not found: {self.config_file}, using defaults"
                )
                self.config = self._get_default_config()
                return
            ...
            log_config(
                f"Loaded YAML configuration from {self.config_file}",
            ...
            if (
                ...
                raise ConfigurationError(
                    f"Error loading YAML config file {self.config_file}: {e}"
                )
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        """
        return {
            param: config["default"] for param, config in self.CONFIG_PARAMS.items()
        }
        ...
        for param_name, param_config in self.CONFIG_PARAMS.items():
            ...
            if env_key in os.environ:
                try:
                    ...
                    log_config(
                        f"Invalid environment variable {env_key}: {e}", level="WARNING"
                    )
    ...
    def _override_with_cli_args(self) -> None:
        ...
        try:
            ...
            if overridden:
                log_config(
                    f"CLI argument overrides applied",
                    details={"overridden": overridden},
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Form Guide Required Fields: {config_loader.get_form_guide_required_fields()}"
        )
