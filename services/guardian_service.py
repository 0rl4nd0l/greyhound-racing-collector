...
class GuardianService:
    ...
    def __init__(self, scan_interval_minutes: int = 240):
        ...
        self.monitored_directories = ["./upcoming_races", "./processed"]
    ...
    def _perform_scan(self):
        ...
        for directory in self.monitored_directories:
            ...
            print(
                f"  📂 Scanning: {directory} ({dir_file_count} files, {dir_size/1024/1024:.1f}MB)"
            )
            results = self.guardian.scan_directory(
                directory, max_age_hours=self.max_file_age_hours
            )
        ...
        test_files_removed = self.guardian.cleanup_test_files(
            self.monitored_directories, max_age_hours=self.max_file_age_hours
        )
        ...
        print(
            f"   Data rate: {total_directory_size/1024/1024/scan_duration:.1f} MB/sec"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scan-now", action="store_true", help="Perform immediate scan"
    )
    parser.add_argument(
        "--interval", type=int, default=240, help="Scan interval in minutes"
    )
