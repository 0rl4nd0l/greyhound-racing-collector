...
class LightweightGuardianService:
    ...
    def _set_ionice_priority(self):
        ...
        try:
            ...
            subprocess.run(['which', 'ionice'], check=True, 
                         capture_output=True, text=True)
            
            # Set ionice class to idle (class 3)
            ionice_class = self.config.get("ionice_class", 3)
            subprocess.run(['ionice', '-c', str(ionice_class), '-p', str(os.getpid())], 
        ...
        try:
            ...
            for directory in self.config["directories"]:
                ...
                if elapsed > self.config["max_execution_time"]:
                    self._log(f"Execution time limit reached ({elapsed:.1f}s), stopping scan", "WARNING")
                    break
            ...
            summary = {
                ...
                "directories_scanned": len([d for d in self.config["directories"] if os.path.exists(d)])
            }
            
            self._log(f"Scan completed in {execution_time:.2f}s: "
