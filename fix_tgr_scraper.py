    ...
    class FixedTGRScraper(TheGreyhoundRecorderScraper):
        ...
        def _fetch_race_details_fixed(self, race_url: str) -> Dict[str, Any]:
            ...
            try:
                    ...
                    if 'Form Guide' in heading_text and 'Race' in heading_text:
                        ...
                        if len(parts) >= 2:
                            ...
                            if 'Race' in race_part and ')' in race_part:
                                # Extract race number
                                race_num_text = race_part.split('Race')[1].split(')')[0].strip()
                                try:
                ...
                for table in tables:
                    ...
                    if len(rows) >= 2:  # At least header + 1 data row
                        ...
                        if headers:
                            ...
                            if any(term in ' '.join(header_text) for term in ['greyhound', 'rug', 'form', 'comment']):
                                main_table = table
                                logger.debug(f"Found main table with headers: {[h.get_text(strip=True) for h in headers]}")
                                break
                ...
                comment_elements = soup.find_all(['div', 'p'], 
                    class_=lambda x: x and any(word in str(x).lower() for word in ['comment', 'analysis', 'preview']))
                ...
                logger.info(f"Extracted {race_details['field_size']} dogs and {len(race_details['expert_comments'])} comments from {race_url}")
        ...
        def fetch_enhanced_dog_data_fixed(self, dog_name: str, limit_days: int = 365) -> Dict[str, Any]:
            ...
            try:
                ...
                for meeting in form_guides.get('meetings', [])[:10]:  # Limit to first 10 meetings
                    if meeting.get('long_form_url'):
                        try:
                            race_data = self._fetch_race_details_fixed(meeting['long_form_url'])
                            dog_entries = self._extract_dog_entries_fixed(race_data, dog_name)
                ...
                logger.info(f"Enhanced data collected for {dog_name}: {len(enhanced_data['form_entries'])} races")
        ...
        def _analyze_venue_performance(self, form_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            for venue, stats in venue_stats.items():
                ...
                if stats['early_speeds']:
                    stats['average_early_speed'] = sum(stats['early_speeds']) / len(stats['early_speeds'])
        ...
        def _analyze_distance_performance(self, form_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            for entry in form_entries:
                ...
                if entry.get('early_speed'):
                    distance_stats[distance]['early_speeds'].append(entry['early_speed'])
        ...
        def _extract_dog_entries_fixed(self, race_data: Dict[str, Any], target_dog_name: str) -> List[Dict[str, Any]]:
...
def test_fixed_tgr_scraper():
    ...
    if race_data.get('dogs'):
        ...
        if first_dog_name:
            ...
            logger.info(f"  Performance summary: {bool(enhanced_data.get('performance_summary'))}")
