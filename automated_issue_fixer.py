        ...
        with self.connect_db() as conn:
            ...
            for _, row in missing_races.iterrows():
                ...
                if not race_info.empty:
                    ...
                    logger.info(
                        f"Created metadata for race {race_id}: {venue}, {field_size} runners"
                    )
    ...
    def optimize_single_runner_races(self):
        ...
        with self.connect_db() as conn:
            ...
            for _, row in stats_df.iterrows():
                logger.info(
                    f"  {row['venue']} {row['grade']}: {row['count']} races ({row['percentage']:.2f}%)"
                )
            ...
            logger.info(
                f"Marked {marked_races} suspicious single runner races for review"
            )
