...
def scrape_all_races():
    ...
    try:
        ...
        venue_name_map = {
            ...
            'DARW': 'Darwin', 'BROKEN-HILL': 'Broken Hill', 'GRAFTON': 'Grafton',
            'DUBBO': 'Dubbo'
        }
        ...
        for link, href in race_links:
            try:
                ...
                race_url = href if href.startswith('http') else f"https://www.thedogs.com.au{href}"
        ...
        for venue, venue_races in sorted(venues.items()):
            ...
            print(f'  {venue}: {venue_name} - {len(venue_races)} races across {len(dates)} days')
