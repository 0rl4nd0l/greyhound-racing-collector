...
class FeatureQualityAssessment:
    ...
    def catalogue_all_features(self, data: pd.DataFrame) -> Dict[str, Dict]:
        ...
        derived_features = [
            ...
            "box_advantage",
            "is_favorite",
            "weight_to_field_ratio",
            "price_to_field_ratio",
        ]

        # Process each feature category
        for feature in numerical_features:
            if feature in data.columns:
                feature_catalog[feature] = self.analyze_feature(
                    data, feature, "numerical"
                )

        for feature in categorical_features:
            if feature in data.columns:
                feature_catalog[feature] = self.analyze_feature(
                    data, feature, "categorical"
                )

        for feature in time_features:
            if feature in data.columns:
                feature_catalog[feature] = self.analyze_feature(
                    data, feature, "time_based"
                )

        for feature in outcome_features:
            if feature in data.columns:
                feature_catalog[feature] = self.analyze_feature(
                    data, feature, "outcome"
                )
        ...
        logger.info(
            f"✅ Catalogued {len(feature_catalog)} features across all categories"
        )
    ...
    def _analyze_numerical_feature(self, feature_data: pd.Series) -> Dict:
        ...
        analysis = {
            "cardinality": (
                "high"
                if len(valid_data.unique()) > 100
                else "medium" if len(valid_data.unique()) > 10 else "low"
            ),
        ...
        IQR = Q3 - Q1
        outlier_count = len(
            valid_data[(valid_data < Q1 - 1.5 * IQR) | (valid_data > Q3 + 1.5 * IQR)]
        )
    ...
    def _analyze_categorical_feature(self, feature_data: pd.Series) -> Dict:
        ...
        analysis = {
            "cardinality": (
                ...
                else "medium" if len(value_counts) > 10 else "low"
            ),
    ...
    def calculate_predictive_utility(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        logger.info(
            "📊 Calculating predictive utility (mutual information with target)..."
        )
        ...
        if "finish_position" in data.columns:
            ...
            logger.warning(
                "No finish_position column found, using winner_name as target"
            )
        ...
        for feature in data.columns:
            if (
                feature not in self.potential_leakage_fields
                and feature not in ["dog_name", "dog_clean_name", "race_id"]
                and not feature.startswith("scraped_")
            ):
        ...
        sorted_scores = dict(
            sorted(mutual_info_scores.items(), key=lambda x: x[1], reverse=True)
        )

        logger.info(
            f"✅ Calculated predictive utility for {len(sorted_scores)} features"
        )
    ...
    def perform_leakage_scan(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "race_date" in data.columns and "extraction_timestamp" in data.columns:
            data["race_date_parsed"] = pd.to_datetime(
                data["race_date"], errors="coerce"
            )
            data["extraction_parsed"] = pd.to_datetime(
                data["extraction_timestamp"], errors="coerce"
            )

            # Check if extraction happened after race
            future_extractions = data[
                data["extraction_parsed"] > data["race_date_parsed"]
            ]
        ...
        logger.info(
            f"   High-risk features: {len(leakage_results['high_risk_features'])}"
        )
        logger.info(
            f"   Medium-risk features: {len(leakage_results['medium_risk_features'])}"
        )
    ...
    def recommend_feature_transformations(
        self, data: pd.DataFrame, mutual_info_scores: Dict
    ) -> Dict[str, List[str]]:
        ...
        for feature in data.columns:
            if feature in mutual_info_scores and pd.api.types.is_numeric_dtype(
                data[feature]
            ):
        ...
        for feature in categorical_features:
            if feature in data.columns:
                ...
                if unique_count > 50:
                    high_cardinality_cats.append(
                        f"{feature} ({unique_count} categories)"
                    )
        ...
        logger.info(
            f"✅ Generated {sum(len(v) for v in recommendations.values())} transformation recommendations"
        )
        return recommendations

    def generate_comprehensive_report(
        self, output_dir: str = "feature_assessment_report"
    ):
        ...
        transformation_recommendations = self.recommend_feature_transformations(
            data, mutual_info_scores
        )
        ...
        summary_stats = {
            ...
            "features_with_high_missing_rate": len(
                [
                    f
                    for f, info in feature_catalog.items()
        ...
        self._create_assessment_visualizations(
            output_path, data, feature_catalog, mutual_info_scores
        )

        # Generate executive summary
        self._generate_executive_summary(
            output_path, summary_stats, leakage_results, transformation_recommendations
        )
    ...
    def _create_assessment_visualizations(
        self,
        output_path: Path,
        data: pd.DataFrame,
        ...
        try:
            ...
            if len(missing_data) > 0:
                ...
                plt.tight_layout()
                plt.savefig(
                    output_path / "missing_data_analysis.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / "predictive_utility_ranking.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
            ...
            plt.pie(
                cardinality_counts.values(),
                labels=cardinality_counts.keys(),
                autopct="%1.1f%%",
            )
            plt.title("Feature Cardinality Distribution")
            plt.savefig(
                output_path / "cardinality_distribution.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def _generate_executive_summary(
        ...
        transformation_recommendations: Dict,
    ):
...
"""
        ...
        for category, recommendations in transformation_recommendations.items():
            if recommendations and category in [
                "normalization",
                "log_transform",
                "interaction",
            ]:
