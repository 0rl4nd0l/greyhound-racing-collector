...
def test_endpoint(
    method, endpoint, expected_status=200, data=None, headers=None, params=None
):
    ...
    try:
        if method.upper() == "GET":
            ...
            response = requests.post(
                url, json=data, timeout=TIMEOUT, headers=headers, params=params
            )
...
def test_frontend_endpoints():
    ...
    test_endpoint(
        "GET", "/", expected_status=[200, 500]
    )  # May fail on template issues but route exists
...
def test_post_endpoints():
    ...
    test_endpoint(
        "POST", "/api/diagnostics/run", expected_status=[400, 422, 500]
    )  # Missing required data
    test_endpoint("POST", "/api/rescan_upcoming", expected_status=[200, 405, 500])
    test_endpoint(
        "POST", "/api/batch/predict", expected_status=[400, 422, 500]
    )  # Missing data
    test_endpoint(
        "POST", "/api/ingest_csv", expected_status=[400, 422, 500]
    )  # Missing file
