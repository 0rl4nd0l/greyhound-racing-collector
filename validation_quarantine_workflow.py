...
from enhanced_form_guide_parser import (
    EnhancedFormGuideParser,
    ParsingResult,
    ValidationIssue,
    ValidationSeverity,
)
...
class ValidationQuarantineWorkflow:
    ...
    def __init__(self, config: QuarantineConfig = None):
        ...
        self.parser = EnhancedFormGuideParser(
            quarantine_dir=self.config.base_quarantine_dir
        )
    ...
    def _determine_validation_decision(
        self, parsing_result: ParsingResult, file_path: Path
        ...
        """
        critical_errors, warnings, info_messages = self._count_critical_errors(
            parsing_result.issues
        )
        ...
        if parsing_result.quarantined:
            return (
                ValidationDecision.QUARANTINE_CRITICAL,
                "Parser determined file should be quarantined",
            )
    ...
    def _quarantine_file(
        self, file_path: Path, parsing_result: ParsingResult, reason: str
    ) -> str:
        """
        ...
        self.logger.warning(
            f"File quarantined: {file_path} -> {quarantine_file_path} (Reason: {reason})"
        )
    ...
    def _emit_validation_log(self, validation_report: ValidationReport):
        ...
        log_entry = {
            ...
            "quarantine_info": (
                {
                    "quarantined": validation_report.processing_decision
                    != ValidationDecision.CONTINUE_TO_DB,
                    "quarantine_reason": validation_report.quarantine_reason,
                    "quarantine_path": validation_report.quarantine_path,
                }
                if validation_report.quarantine_reason
                else None
            ),
    ...
    def process_file(self, file_path: Union[str, Path]) -> ValidationReport:
        ...
        try:
            ...
            decision, quarantine_reason = self._determine_validation_decision(
                parsing_result, file_path
            )
            ...
            if decision != ValidationDecision.CONTINUE_TO_DB:
                quarantine_path = self._quarantine_file(
                    file_path, parsing_result, quarantine_reason
                )
            ...
            if decision == ValidationDecision.CONTINUE_TO_DB:
                self.logger.info(
                    f"✅ File validation passed - continuing to DB: {file_path}"
                )
                self.logger.info(
                    f"   📊 Records: {len(parsing_result.data)}, Issues: {len(parsing_result.issues)} ({critical_errors} critical)"
                )
            else:
    ...
    def batch_process_files(
        ...
        self.logger.info(
            f"🚀 Starting batch validation workflow for {len(file_paths)} files"
        )
        ...
        quarantined_files = sum(
            1
            for r in results
            if r.processing_decision != ValidationDecision.CONTINUE_TO_DB
        )
        ...
        batch_summary = {
            ...
            "quarantine_directories_used": list(
                set(
                    Path(r.quarantine_path).parent.name
                    for r in results
        ...
        self.logger.info(
            f"✅ Batch processing complete: {success_files}/{total_files} files passed validation"
        )
...
def main():
    ...
    parser.add_argument(
        "--quarantine-dir", default="./quarantine", help="Base quarantine directory"
    )
    parser.add_argument(
        "--critical-threshold", type=int, default=5, help="Critical error threshold"
    )
    parser.add_argument(
        "--critical-percentage",
        type=float,
        default=0.3,
        help="Critical error percentage threshold",
    )
    parser.add_argument(
        "--no-date-dirs", action="store_true", help="Don't use date subdirectories"
    )
    parser.add_argument(
        "--summary", action="store_true", help="Show quarantine summary"
    )
    ...
    if args.summary:
        ...
        for date, info in summary["date_directories"].items():
            print(
                f"  {date}: {info['csv_files']} files ({info['report_files']} reports)"
            )
        return
    ...
    for result in results:
        status = (
            "✅ PASS"
            if result.processing_decision == ValidationDecision.CONTINUE_TO_DB
            else "🚨 QUARANTINE"
        )
    ...
    success_count = sum(
        1 for r in results if r.processing_decision == ValidationDecision.CONTINUE_TO_DB
    )
