...
class AdvancedDeduplicator:
    ...
    def analyze_duplicates(self):
        ...
        print(
            f"Data Corruption Level: {(total_duplicate_records/total_records)*100:.1f}%"
        )
        print()
    ...
    def deduplicate_form_guide(self):
        ...
        logger.info(
            f"📊 Will keep {records_to_keep:,} records, remove {records_to_remove:,} duplicates"
        )
    ...
    def verify_deduplication(self):
        ...
        print(
            f"Data reduction: {(self.stats['duplicates_removed']/self.stats['records_before'])*100:.1f}%"
        )
        ...
        for i, (race_date, venue, position, weight) in enumerate(kiwi_races, 1):
            print(
                f"  {i:2d}. {race_date} | {venue} | Place {position} | Weight {weight}"
            )
    ...
    def update_ml_data_quality(self):
        ...
        improvement = (
            self.stats["duplicates_removed"] / self.stats["records_before"]
        ) * 100
        logger.info(
            f"📈 Data quality improved by {improvement:.1f}% through deduplication"
        )
    ...
    def generate_deduplication_report(self):
        ...
        report_path = os.path.join(
            self.base_path,
            f"deduplication_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        )
    ...
    def run_full_deduplication(self):
        ...
        try:
            ...
            if success:
                ...
                print(
                    f"  • Fixed {self.stats['duplicates_found']:,} impossible dog-day combinations"
                )
                print(
                    f"  • Improved data quality by {(self.stats['duplicates_removed']/self.stats['records_before'])*100:.1f}%"
                )
