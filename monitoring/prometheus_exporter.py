...
class SystemMetricsCollector:
    ...
    def __init__(self, registry: CollectorRegistry = None):
        ...
        self.load_average_1m = Gauge(
            "system_load_average_1m", "1-minute load average", registry=self.registry
        )

        self.load_average_5m = Gauge(
            "system_load_average_5m", "5-minute load average", registry=self.registry
        )

        self.load_average_15m = Gauge(
            "system_load_average_15m", "15-minute load average", registry=self.registry
        )
...
class ApplicationMetricsCollector:
    ...
    def record_prediction(
        self, duration: float, accuracy: float = None, prediction_type: str = "win"
    ):
        ...
        if accuracy is not None:
            self.prediction_accuracy.labels(prediction_type=prediction_type).set(
                accuracy
            )
    ...
    def start(self):
        ...
        try:
            ...
            self.update_thread = threading.Thread(
                target=self._update_metrics_loop, daemon=True
            )
...
def main():
    ...
    parser.add_argument(
        "--interval", type=int, default=15, help="Metrics update interval"
    )
