...
def start_memory_watchdog(
    rss_soft_mb=900, rss_hard_mb=1200, check_sec=5, on_soft=None, on_hard=None
):
    """
    ...
    def _run():
        while True:
            try:
                ...
                if rss > rss_soft:
                    log.warning(
                        f"[WATCHDOG] Soft limit exceeded: RSS={rss/1024/1024:.1f}MB"
                    )
                    gc.collect()
                    if on_soft:
                        try:
                ...
                if rss > rss_hard:
                    log.error(
                        f"[WATCHDOG] Hard limit exceeded: RSS={rss/1024/1024:.1f}MB – terminating worker"
                    )
                    try:
                        if on_hard:
                            on_hard(rss)
