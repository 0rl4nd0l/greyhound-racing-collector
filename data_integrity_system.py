...
class DataIntegrityManager:
    ...
    def load_validation_rules(self) -> Dict:
        ...
        return {
            ...
            "enhanced_expert_data": {
                ...
                "business_rules": {"one_race_per_dog_per_day": True},
            },
        }
    ...
    def validate_record_before_insert(
        ...
        for field, expected_type in rules.get("data_types", {}).items():
            if field in record and record[field] is not None:
                if not isinstance(record[field], expected_type):
                    errors.append(
                        f"Invalid data type for {field}: expected {expected_type}, got {type(record[field])}"
                    )
        ...
        if "box_number_range" in business_rules:
            ...
            if "box_number" in record and record["box_number"] is not None:
                if not (min_box <= record["box_number"] <= max_box):
                    errors.append(
                        f"Box number {record['box_number']} outside valid range {min_box}-{max_box}"
                    )
        ...
        if (
            business_rules.get("one_race_per_dog_per_day")
            and "dog_clean_name" in record
            and "race_date" in record
        ):
            dog_day_errors = self.check_one_race_per_dog_per_day(
                record["dog_clean_name"], record["race_date"], record.get("race_id")
            )
            errors.extend(dog_day_errors)

        return len(errors) == 0, errors

    def check_for_duplicates_before_insert(
        self, table_name: str, record: Dict
    ) -> List[str]:
        ...
        unique_constraints = self.validation_rules[table_name].get(
            "unique_constraints", []
        )
            ...
            if where_conditions:
                ...
                if count > 0:
                    constraint_desc = " + ".join(
                        [f"{field}={record.get(field)}" for field in unique_constraints]
                    )
                    errors.append(
                        f"Duplicate record found in {table_name}: {constraint_desc}"
                    )
    ...
    def check_one_race_per_dog_per_day(
        self, dog_name: str, race_date: str, exclude_race_id: str = None
    ) -> List[str]:
        ...
        try:
            ...
            if result and result[1] > 0:
                errors.append(
                    f"Dog {dog_name} already has a race on {race_date} (race_id: {result[0]})"
                )
    ...
    def safe_insert_record(
        ...
        is_valid, validation_errors = self.validate_record_before_insert(
            table_name, record
        )

        if not is_valid:
            self.integrity_logger.warning(
                f"Record validation failed for {table_name}: {validation_errors}"
            )
    ...
    def run_comprehensive_integrity_check(self) -> Dict:
        ...
        try:
            ...
            for table in tables_to_check:
                if table in self.validation_rules:
                    unique_fields = self.validation_rules[table].get(
                        "unique_constraints", []
                    )
                    if unique_fields:
                        ...
                        if duplicates:
                            report["issues_found"].append(
                                f"Found {len(duplicates)} duplicate groups in {table}"
                            )
                            report["statistics"][f"{table}_duplicates"] = len(
                                duplicates
                            )
                            # Also get detailed records for reporting
                            detailed_duplicates = self.find_duplicate_records_detail(
                                table, unique_fields
                            )
                            if detailed_duplicates:
                                report.setdefault("detailed_duplicates", {}).setdefault(
                                    table, []
                                ).extend(detailed_duplicates)

            # Check 3: One race per dog per day rule
            dog_day_violations = self.find_dog_day_violations()
            if dog_day_violations:
                report["issues_found"].append(
                    f"Found {len(dog_day_violations)} dog-day rule violations"
                )
            ...
            if orphaned_records:
                report["issues_found"].append(
                    f"Found orphaned records: {orphaned_records}"
                )
            ...
            if report["issues_found"]:
                ...
                report["recommendations"].append(
                    "Consider implementing stricter validation"
                )
            else:
                report["recommendations"].append(
                    "Data integrity is good - maintain current practices"
                )
        ...
        self.logger.info(
            f"Integrity check completed. Found {len(report['issues_found'])} issues"
        )
        return report

    def find_duplicate_records(
        self, table_name: str, unique_fields: List[str]
    ) -> List[Dict]:
    ...
    def find_duplicate_records_detail(
        self, table_name: str, unique_fields: List[str]
    ) -> List[Dict]:
    ...
    def check_data_quality(self) -> List[str]:
        ...
        try:
            ...
            if missing_critical > 0:
                issues.append(
                    f"Found {missing_critical} race records with missing critical data"
                )
