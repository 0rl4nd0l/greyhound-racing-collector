...
class BallaratCalibrationValidator:
    ...
    def load_ballarat_holdout_data(self, days_back: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"✅ Loaded {len(ballarat_data)} dogs from {len(ballarat_data['race_id'].unique())} Ballarat races")
    ...
    def _clean_ballarat_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        required_fields = ['dog_clean_name', 'finish_position', 'race_id', 'venue', 'distance']
    ...
    def create_synthetic_probabilities(self, ballarat_data: pd.DataFrame, 
                                     temperature: float = 2.0) -> pd.DataFrame:
        """
        ...
        logger.info(f"✅ Created probabilities for {len(combined_results['race_id'].unique())} races")
        logger.info(f"   Probability range: {combined_results['predicted_probability'].min():.4f} - {combined_results['predicted_probability'].max():.4f}")
    ...
    def _calculate_synthetic_strength(self, race_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'box_number' in race_data.columns:
            box_nums = pd.to_numeric(race_data['box_number'], errors='coerce').fillna(4)  # Default to middle box
        ...
        if 'weight' in race_data.columns:
            ...
            if len(valid_weights) > 1:  # Need at least 2 valid weights for normalization
                weights_filled = weights.fillna(weights.median())
                weight_effect = (weights_filled - weights_filled.mean()) / weights_filled.std()
                # Fill any remaining NaN with 0
                weight_effect = weight_effect.fillna(0)
                race_data['synthetic_strength'] -= weight_effect * 0.2  # Heavier = slightly slower
    ...
    def compute_calibration_metrics(self, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            metrics = {
                ...
                'calibration_slope': np.corrcoef(prob_pred, prob_true)[0, 1] if len(prob_true) > 1 else 0.0,
    ...
    def _compute_mce(self, y_true: np.ndarray, y_prob: np.ndarray, n_bins: int = 10) -> float:
    ...
    def optimize_temperature(self, ballarat_data: pd.DataFrame, 
        ...
        for temp in temperatures:
            ...
            temp_data = self.create_synthetic_probabilities(ballarat_data, temperature=temp)
            ...
            logger.info(f"   τ={temp:.2f}: Log-Loss={log_loss_val:.4f}, Brier={metrics.get('brier_score', 0):.4f}")
        
        logger.info(f"✅ Optimal temperature: τ={best_temp:.2f} (Log-Loss={best_log_loss:.4f})")
    ...
    def apply_isotonic_regression(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"   Isotonic Log Loss: {calibrated_metrics.get('log_loss', 0):.4f}")
        logger.info(f"   Isotonic Brier Score: {calibrated_metrics.get('brier_score', 0):.4f}")
    ...
    def plot_calibration_curve(self, data: pd.DataFrame, save_path: str = None):
        ...
        try:
            ...
            ax1.plot(prob_pred, prob_true, marker='o', linewidth=2, label='Ballarat Hold-out')
            ...
            bin_accuracy = np.divide(bin_correct, bin_total, out=np.zeros_like(bin_correct, dtype=float), where=bin_total!=0)
    ...
    def run_full_calibration_validation(self, days_back: int = 30) -> Dict:
        ...
        calibrated_data = self.create_synthetic_probabilities(ballarat_data, optimal_temperature)
        ...
        final_metrics = self.compute_calibration_metrics(
            isotonic_data.assign(predicted_probability=isotonic_data['isotonic_calibrated_probability'])
        )
        ...
        results = {
            ...
            'improvement': {
                'log_loss_reduction': baseline_metrics.get('log_loss', 0) - final_metrics.get('log_loss', 0),
                'brier_score_reduction': baseline_metrics.get('brier_score', 0) - final_metrics.get('brier_score', 0),
                'ece_reduction': baseline_metrics.get('expected_calibration_error', 0) - final_metrics.get('expected_calibration_error', 0)
            },
        ...
        logger.info(f"   Final ECE: {final_metrics.get('expected_calibration_error', 0):.4f}")
...
def main():
    ...
    try:
        ...
        results = validator.run_full_calibration_validation(days_back=60)  # Look back 60 days
        ...
        print(f"  Expected Calibration Error: {baseline.get('expected_calibration_error', 0):.4f}")
        print(f"  Max Calibration Error: {baseline.get('max_calibration_error', 0):.4f}")
        ...
        print(f"  Expected Calibration Error: {final.get('expected_calibration_error', 0):.4f}")
