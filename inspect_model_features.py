...
def inspect_model(model_path: str, output_path: Path | None = None):
    ...
    try:
        ...
        if hasattr(model, 'named_steps'):
            print(f"🔍 Model is a pipeline with steps: {list(model.named_steps.keys())}")
            ...
            if 'preprocessor' in model.named_steps:
                ...
                if hasattr(preprocessor, 'get_feature_names_out'):
                    try:
                        ...
                        print("ℹ️ Preprocessor provides get_feature_names_out; names require fitted context")
                    except Exception:
                        pass
            ...
            for step_name, step in model.named_steps.items():
                if hasattr(step, 'feature_names_'):
                    ...
                    print(f"✅ Found feature_names_ in step '{step_name}': {len(feature_names)} features")
                    break
                elif hasattr(step, 'feature_names_in_'):
                    feature_names = step.feature_names_in_
                    print(f"✅ Found feature_names_in_ in step '{step_name}': {len(feature_names)} features")
                    break
        ...
        if feature_names is None:
            if hasattr(model, 'feature_names_'):
                ...
                print(f"✅ Found feature_names_ on model: {len(feature_names)} features")
            elif hasattr(model, 'feature_names_in_'):
                feature_names = model.feature_names_in_
                print(f"✅ Found feature_names_in_ on model: {len(feature_names)} features")
        ...
        if feature_count is not None:
            print(f"⚠️ Model expects {feature_count} features, but feature names not found")
            print("💡 You'll need to inspect the training code or data to get the exact feature names")
            return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract feature contract from a serialized sklearn model/pipeline")
    parser.add_argument("--model-path", required=True, help="Path to model artifact (.joblib/.pkl)")
    parser.add_argument("--output", required=False, help="Optional output path for contract JSON")
