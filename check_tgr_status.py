...
def check_database_health():
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM tgr_enrichment_jobs WHERE created_at >= datetime('now', '-24 hours')"
        )
        recent_jobs = cursor.fetchone()[0]

        cursor.execute(
            "SELECT COUNT(*) FROM tgr_enrichment_jobs WHERE status = 'completed' AND completed_at >= datetime('now', '-24 hours')"
        )
        ...
        # Check cache entries
        cursor.execute(
            "SELECT COUNT(*) FROM tgr_enhanced_feature_cache WHERE expires_at > datetime('now')"
        )
        ...
        return {
            ...
            "success_rate": (
                (completed_jobs / recent_jobs * 100) if recent_jobs > 0 else 0
            ),
        }
...
def print_status_report():
    ...
    print(
        f"🎯 Overall Status: {health_icons.get(overall_health, '❓')} {overall_health}"
    )
