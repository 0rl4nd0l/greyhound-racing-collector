...
class PerformanceProfiler:
    ...
    def profile_database_operations(self):
        ...
        with self.profile_context("database_operations"):
            ...
            for query in operations:
                ...
                end_time = time.time()
                print(f"   📊 Query executed in {end_time - start_time:.4f}s, returned {len(results)} rows")
            ...
            try:
                ...
                if os.path.exists(upcoming_dir):
                    ...
                    if csv_files:
                        ...
                        result = pipeline.predict_race_file(test_file, enhancement_level="basic")
            ...
            try:
                ...
                with app.test_client() as client:
                    ...
                    for endpoint in endpoints_to_test:
                        ...
                        end_time = time.time()
                        print(f"   🌐 {endpoint}: {response.status_code} in {end_time - start_time:.4f}s")
    ...
    def run_py_spy_analysis(self, duration=30):
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    if proc.info['cmdline'] and any('app.py' in cmd or 'gunicorn' in cmd for cmd in proc.info['cmdline']):
    ...
    def _calculate_improvements(self):
        ...
        for metric in self.baseline_metrics:
            if metric in self.tuned_metrics and self.baseline_metrics[metric] and self.tuned_metrics[metric]:
                try:
                    ...
                    if baseline_val > 0:
                        improvement_pct = ((baseline_val - tuned_val) / baseline_val) * 100
...
def main():
    ...
    run_py_spy = input("\n🔍 Run py-spy analysis? (requires running Flask app) [y/N]: ").lower() == 'y'
    if run_py_spy:
    ...
    input("Press Enter after implementing optimizations to measure tuned performance...")
