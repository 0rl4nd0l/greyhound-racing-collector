...
from config.venue_mapping import VENUE_MAPPING, normalize_venue  # type: ignore
...
class RaceInfoPatcher:
    ...
    def extract_from_race_url(self, race_url: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            response = requests.get(race_url, timeout=10, headers={
                'User-Agent': 'Mozilla/5.0 (compatible; RaceInfoPatcher/1.0)'
            })
    ...
    def find_colocated_race_data(self, identifiers: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    ...
    def extract_from_filename(self, file_path: Path) -> Tuple[Optional[str], Optional[str]]:
        ...
        for pattern in venue_patterns:
            ...
            if match:
                ...
                log.info(
                    ...
                    details={"file": str(file_path), "venue_raw": venue_raw, "venue_normalized": venue_normalized},
                    component="race_info_patcher"
                )
    ...
    def extract_from_json_content(self, data: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
        """Extract distance and grade from JSON content fields."""
        # Fields to search in
        search_fields = [
            'race_name', 'event_name', 'meeting_name', 'title',
            'description', 'race_title'
        ]
    ...
    def find_csv_file_fuzzy(self, filename: str, search_dirs: List[Path]) -> Optional[Path]:
            ...
            try:
                ...
                for csv_file in csv_files:
                    if csv_file.is_file():
                        ...
                        if len(target_parts - file_parts) <= 1 and len(target_parts) > 2:
                            log.info(
                                f"Found CSV file via partial matching: {filename} -> {csv_file.name}",
                                action="csv_partial_match_success", 
                                details={"target": filename, "found": str(csv_file), "target_parts": list(target_parts), "file_parts": list(file_parts)},
                                component="race_info_patcher"
                            )
    ...
    def extract_from_csv_headers(self, identifiers: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _extract_from_csv_file(self, csv_path: Path) -> Tuple[Optional[str], Optional[str]]:
        """Extract distance and grade from a CSV file."""
        # Read first few lines to get headers and any metadata
        with open(csv_path, 'r', encoding='utf-8') as f:
            lines = [f.readline().strip() for _ in range(10)]  # Read more lines to get data rows
        ...
        if re.search(r'\b(dog|greyhound|runner)\b', full_text, re.IGNORECASE):
            ...
            for line in lines:
                if not line or line.startswith('""'):  # Skip empty lines or continuation lines
                    continue
                ...
                if len(parts) > 5:  # Ensure we have enough columns
                    ...
                    for i, part in enumerate(parts):
                        ...
                        if re.match(r'^\d{3,4}$', part):  # 3-4 digit numbers (likely distance)
    ...
    def query_database_fallback(self, identifiers: Dict[str, Any], file_path: Path) -> Tuple[Optional[str], Optional[str]]:
        ...
        if venue == 'UNKNOWN' or not venue:
            ...
            if extracted_venue:
                ...
                log.info(
                    ...
                    details={"file": str(file_path), "extracted_venue": extracted_venue},
                    component="race_info_patcher"
                )
        ...
        # Normalize venue
        venue_normalized = VENUE_MAPPING.get(venue.upper(), venue.upper()) if venue else None
            ...
            try:
                with sqlite3.connect(db_path) as conn:
                    ...
                    if result:
                        ...
                        distance = self.normalize_distance(str(distance_raw)) if distance_raw else None
                        grade = self.normalize_grade(str(grade_raw)) if grade_raw else None
    ...
    def derive_race_info(self, file_info: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
        ...
        print(f"[DEBUG] Starting derive_race_info for: {file_path.name}", file=sys.stderr)
        ...
        if race_url and (not final_distance or not final_grade):
            ...
            print(f"[DEBUG] Strategy 1 result: distance={distance}, grade={grade}", file=sys.stderr)
            if distance and needs_distance and not final_distance:
                ...
                log.info(
                    ...
                    details={"source": "race_url", "field": "distance", "file": str(file_path)},
                    component="race_info_patcher"
                )
            if grade and needs_grade and not final_grade:
                ...
                log.info(
                    ...
                    action="extraction_success",
                    details={"source": "race_url", "field": "grade", "file": str(file_path)},
                    component="race_info_patcher"
                )
        ...
        if (needs_distance and not final_distance) or (needs_grade and not final_grade):
            ...
            print(f"[DEBUG] Strategy 3 result: distance={distance}, grade={grade}", file=sys.stderr)
            if distance and needs_distance and not final_distance:
                final_distance = distance
                log.info(
                    f"Extracted distance from filename: {distance}",
                    action="extraction_success",
                    details={"source": "filename", "field": "distance", "file": str(file_path)},
                    component="race_info_patcher"
                )
            if grade and needs_grade and not final_grade:
                final_grade = grade
                log.info(
                    ...
                    action="extraction_success",
                    details={"source": "filename", "field": "grade", "file": str(file_path)},
                    component="race_info_patcher"
                )
        ...
        if (needs_distance and not final_distance) or (needs_grade and not final_grade):
            try:
                ...
                print(f"[DEBUG] Strategy 4 result: distance={distance}, grade={grade}", file=sys.stderr)
                if distance and needs_distance and not final_distance:
                    final_distance = distance
                    log.info(
                        f"Extracted distance from JSON content: {distance}",
                        action="extraction_success",
                        details={"source": "json_content", "field": "distance", "file": str(file_path)},
                        component="race_info_patcher"
                    )
                if grade and needs_grade and not final_grade:
                    final_grade = grade
                    log.info(
                        ...
                        action="extraction_success",
                        details={"source": "json_content", "field": "grade", "file": str(file_path)},
                        component="race_info_patcher"
                    )
            except Exception:
                pass
        ...
        if (needs_distance and not final_distance) or (needs_grade and not final_grade):
            ...
            print(f"[DEBUG] Strategy 5 result: distance={distance}, grade={grade}", file=sys.stderr)
            if distance and needs_distance and not final_distance:
                final_distance = distance
                log.info(
                    f"Extracted distance from CSV headers: {distance}",
                    action="extraction_success",
                    details={"source": "csv_headers", "field": "distance", "file": str(file_path)},
                    component="race_info_patcher"
                )
            if grade and needs_grade and not final_grade:
                final_grade = grade
                log.info(
                    ...
                    action="extraction_success",
                    details={"source": "csv_headers", "field": "grade", "file": str(file_path)},
                    component="race_info_patcher"
                )
        ...
        if (needs_distance and not final_distance) or (needs_grade and not final_grade):
            ...
            print(f"[DEBUG] Strategy 6 result: distance={distance}, grade={grade}", file=sys.stderr)
            if distance and needs_distance and not final_distance:
                final_distance = distance
                log.info(
                    f"Extracted distance from database: {distance}",
                    action="extraction_success",
                    details={"source": "database", "field": "distance", "file": str(file_path)},
                    component="race_info_patcher"
                )
            if grade and needs_grade and not final_grade:
                final_grade = grade
                log.info(
                    ...
                    action="extraction_success",
                    details={"source": "database", "field": "grade", "file": str(file_path)},
                    component="race_info_patcher"
                )
        
        print(f"[DEBUG] Final results: distance={final_distance}, grade={final_grade}", file=sys.stderr)
    ...
    def patch_file(self, file_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.patched_files.append({
                ...
                "patched_fields": patched_fields,
                "backup_path": str(self.backup_dir / file_path.relative_to(Path.cwd())) if not self.dry_run else None
            })
    ...
    def patch_from_manifest(self, manifest_path: Path) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "success_rate": f"{(self.patched_count / len(files_to_patch) * 100):.1f}%" if files_to_patch else "0.0%",
