...
def archive_old_canary_files(
    base_dir: str = DIAG_BASE, retention_days: int = 7, max_files: int = 500
) -> dict:
...
def main(argv: list[str]) -> int:
    ...
    while i < len(argv):
        if argv[i] == "--interval" and i + 1 < len(argv):
            ...
            i += 1
    print(
        f"[canary] Starting periodic GPT canary; interval={interval}s, retention_days={retention_days}, max_files={max_files}"
    )
    try:
        while True:
            ...
            try:
                archive_info = archive_old_canary_files(
                    base_dir=DIAG_BASE,
                ...
                if archive_info.get("archived_files"):
                    print(
                        f"[canary] Archived {len(archive_info['archived_files'])} old files -> {archive_info['archives_dir']}"
                    )
