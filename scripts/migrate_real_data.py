...
def migrate_race_data(db_path: str, limit: Optional[int] = None, clean_first: bool = False) -> Dict[str, int]:
    ...
    race_metadata_df = pd.read_sql_query(
        "SELECT * FROM csv_race_metadata_staging ORDER BY race_date ASC", 
        conn
    )
    ...
    for race_id, race_group in race_groups:
        ...
        if len(winners) == 1:
            ...
            race_metadata_record['winner_odds'] = clean_numeric_field(winner.get('starting_price'))
            
            # Calculate winner margin if available
            if 'margin' in winner and pd.notna(winner['margin']):
                race_metadata_record['winner_margin'] = clean_numeric_field(winner['margin'])
        ...
        for _, dog_row in race_group.iterrows():
            ...
            try:
                ...
                expert_df.to_sql('enhanced_expert_data', conn, if_exists='append', index=False)
        ...
        # Progress reporting
        if migrated_counts["migrated_races"] % 100 == 0 and migrated_counts["migrated_races"] > 0:
...
def validate_migration(db_path: str) -> None:
    ...
    try:
        ...
        """
        winners_count = pd.read_sql_query(winners_query, conn).iloc[0]['races_with_winners']
