...
def latest_predictions_file(base_dir: str) -> str:
    files = sorted(
        glob.glob(os.path.join(base_dir, "walk_forward_predictions_*.jsonl"))
    )
    if not files:
...
def summarize(records, high_conf_thresholds=(0.6, 0.5, 0.4)):
    ...
    top1_acc = (
        sum(1 for r in scorable if r.get("correct")) / len(scorable)
        if scorable
        else 0.0
    )
...
def main():
    ...
    for thr, stats in result["high_conf"].items():
        print(
            f"  prob >= {thr:.2f}: count={stats['count']}, wrong={stats['wrong']}, wrong_rate={stats['wrong_rate']:.3f}"
        )

    print("\nCalibration (bin, n, avg_pred, emp_acc):")
    for row in result["calibration"]:
        print(
            f"  {row['bin']:>15}  n={row['n']:4d}  avg_pred={row['avg_pred']:.3f}  emp_acc={row['emp_acc']:.3f}"
        )

    print("\nField size buckets (n, top1_acc):")
    for row in result["field_size"]:
        print(
            f"  {row['field_size_bucket']:>4}: n={row['n']:4d}  acc={row['top1_acc']:.3f}"
        )
