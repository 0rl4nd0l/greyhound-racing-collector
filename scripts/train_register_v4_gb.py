...
def main() -> int:
    ...
    # Use only columns that exist in both train and test to avoid ColumnTransformer missing-column errors
    ...
    # Coerce numeric columns to real numbers to avoid imputer errors on strings
    for c in list(numerical_columns):
        try:
    ...
    with db_guard(db_path=registry_db_path, label="train_register_v4_gb") as guard:
