#!/usr/bin/env python3
"""
Train and register a V4-compatible HistGradientBoosting model using leakage-safe features.

- Uses MLSystemV4 to prepare time-ordered data and build leakage-safe features
- Builds a sklearn pipeline with ColumnTransformer + OneHotEncoder + HistGradientBoostingClassifier
- Wraps in CalibratedClassifierCV (isotonic)
- Evaluates metrics and registers the calibrated pipeline in the Model Registry as V4_HistGradientBoosting (STAGING)

Usage:
  python scripts/train_register_v4_hgb.py
...
def main() -> int:
    ...
    exclude_columns = [
        "race_date",
        "race_time",
    ]  # exclude non-numeric date/time strings
    numerical_columns = [
        c
        for c in X_train.columns
        if c in shared_cols
        and c not in categorical_columns + exclude_columns
    ...
    try:
        if "race_id" in test_features.columns:
            ...
            if races_eval > 0:
                hits_series = grouped.apply(
                    lambda g: int(g.loc[g["p"].idxmax(), "y"] == 1)
                ).astype(int)
    ...
    with db_guard(db_path=registry_db_path, label="train_register_v4_hgb") as guard:
        ...
        model_id = registry.register_model(
            ...
            notes="V4_STAGING: HistGradientBoosting trained on leakage-safe features",
        )
