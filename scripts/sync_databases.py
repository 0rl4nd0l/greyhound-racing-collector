...
def validate_databases(staging_path: str, analytics_path: str) -> Dict[str, any]:
    ...
    if not Path(analytics_path).exists():
        validation["warnings"].append(f"Analytics database not found: {analytics_path} (will be created)")
    ...
    try:
        ...
        for table in required_tables:
            if table not in staging_counts:
                validation["valid"] = False
                validation["errors"].append(f"Required table missing in staging DB: {table}")
    except Exception as e:
        validation["valid"] = False
...
def full_sync(staging_path: str, analytics_path: str, backup_dir: Path) -> Dict[str, any]:
    """Perform full database synchronization (complete copy)."""
    result = {"success": False, "method": "full_sync", "timestamp": datetime.now(timezone.utc).isoformat()}
...
def incremental_sync(staging_path: str, analytics_path: str, backup_dir: Path) -> Dict[str, any]:
    """Perform incremental database synchronization."""
    result = {"success": False, "method": "incremental_sync", "timestamp": datetime.now(timezone.utc).isoformat()}
    ...
    try:
        ...
        result["note"] = "Incremental sync not yet implemented, performed full sync instead"
...
def main():
    parser = argparse.ArgumentParser(description="Synchronize staging database to analytics database")
    ...
    parser.add_argument("--optimize", action="store_true",
                       help="Optimize analytics database after sync")
    parser.add_argument("--backup-dir", default="database_backups/sync",
    ...
    # Save sync report
    report_path = backup_dir / f"sync_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    backup_dir.mkdir(parents=True, exist_ok=True)
