...
def parse_args():
    ...
    p.add_argument(
        "--model",
        required=True,
        choices=["et", "xgb", "extratrees", "xgboost"],
        help="Model to calibrate/promote",
    )
    p.add_argument(
        "--calibration",
        required=True,
    ...
    p.add_argument(
        "--promote", action="store_true", help="Promote the calibrated/best model"
    )
    p.add_argument(
        "--max-races",
        type=int,
        default=0,
        help="Optional cap for upstream processes (ignored here)",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    try:
        ...
        artifact_path = (
            out_dir / f"ml_model_v4_{_dt.now().strftime('%Y%m%d_%H%M%S')}.joblib"
        )
        payload = {
    ...
    try:
        ...
        calib_proc = subprocess.run(
            [sys.executable, "run_calibration.py", "--model_path", saved_path],
            capture_output=True,
            text=True,
            env=env2,
        )
    ...
    if proc.stdout:
        try:
            ...
            model_id = reg_payload.get("registered_model_id") or reg_payload.get(
                "model_id"
            )
        except Exception:
            pass
