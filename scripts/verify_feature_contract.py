...
def _compute_diff(
    cur_sig: str, cur_cats: set, cur_nums: set, exp: Dict[str, Any]
...
def check_via_python(strict: bool = False, refresh: bool = False) -> Dict[str, Any]:
    ...
    # Current model view
    cur_sig = system._compute_feature_signature(
        getattr(system, "feature_columns", []) or []
    )
...
def check_via_api(
    base_url: str, strict: bool = False, refresh: bool = False
) -> Dict[str, Any]:
    if refresh:
        ...
        if status != 200 or not body.get("success"):
            return {
                "success": False,
                "error": f"refresh_failed: status={status}, body={body}",
            }
...
def main():
    ap = argparse.ArgumentParser(
        description="Validate V4 feature contract against the current model"
    )
    ap.add_argument(
        "--mode",
        choices=["python", "api"],
        default="python",
        help="Validation mode (python: no server; api: use running app)",
    )
    ap.add_argument(
        "--url", default="http://localhost:5000", help="Base URL for API mode"
    )
    ap.add_argument(
        "--refresh", action="store_true", help="Regenerate the contract before checking"
    )
    ap.add_argument(
        "--strict", action="store_true", help="Exit non-zero on mismatch (strict mode)"
    )
    ap.add_argument("--json", action="store_true", help="Emit JSON to stdout")
    ap.add_argument(
        "-v", "--verbose", action="store_true", help="Verbose logging to stdout"
    )
    ...
    if args.json:
        ...
        if not result.get("success") and result.get("error"):
            ...
            print(
                f"Contract validation: {'MATCHED' if matched else 'MISMATCH'} (strict={result.get('strict')})"
            )
            if "path" in result:
