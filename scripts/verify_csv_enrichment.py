...
if not RACE_FILENAME:
    ...
    upcoming_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "upcoming_races")
    )
...
print(
    f"Latest prediction file: {os.path.basename(latest_path)} (mtime={datetime.fromtimestamp(latest_mtime).isoformat()})"
)
