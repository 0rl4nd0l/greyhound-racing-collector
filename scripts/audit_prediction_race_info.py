...
def extract_race_identifiers(data: Dict[str, Any], file_path: Path) -> Dict[str, Optional[str]]:
...
def audit_race_info(data: Dict[str, Any]) -> Tuple[bool, bool, Any, Any, str, str]:
    ...
    has_distance = distance is not None and str(distance).strip().lower() not in ('', 'unknown', 'null')
    has_grade = grade is not None and str(grade).strip().lower() not in ('', 'unknown', 'null')
...
def audit_predictions_directory(predictions_dir: Path, dry_run: bool = False) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    ...
    for i, file_path in enumerate(prediction_files):
        try:
            ...
            # Audit race_info
            has_race_info, has_distance, has_grade, distance_value, grade_value, status, reason = audit_race_info(data)
            
            # Detect anomalies in distance/grade types
            distance_type = type(distance_value).__name__ if distance_value is not None else "None"
            grade_type = type(grade_value).__name__ if grade_value is not None else "None"
...
def write_audit_csv(audit_results: List[Dict[str, Any]], output_path: Path) -> None:
    ...
    fieldnames = [
        'file_path', 'has_race_info', 'has_distance', 'has_grade',
        'distance_value', 'grade_value', 'distance_type', 'grade_type', 'distance_anomaly',
        'race_url', 'track', 'date', 'race_number', 'status', 'reason'
    ]
...
def write_missing_manifest(missing_files: List[Dict[str, Any]], output_path: Path) -> None:
    """Write missing files manifest to JSON file."""
    manifest = {
        'timestamp': datetime.now().isoformat(),
        'total_missing_files': len(missing_files),
        'summary': {
            'missing_both': len([f for f in missing_files if f['missing_fields']['distance'] and f['missing_fields']['grade']]),
            'missing_distance_only': len([f for f in missing_files if f['missing_fields']['distance'] and not f['missing_fields']['grade']]),
            'missing_grade_only': len([f for f in missing_files if not f['missing_fields']['distance'] and f['missing_fields']['grade']])
        },
        'files': missing_files
    }
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        '--dry-run',
        help='Perform dry run without writing reports',
        action='store_true'
    )
    ...
    # Perform audit
    audit_results, missing_files = audit_predictions_directory(predictions_dir, args.dry_run)
    ...
    if not args.dry_run and audit_results:
        ...
        if missing_files:
            ...
            log.info(
                ...
                details={"path": str(manifest_path), "missing_files": len(missing_files)},
                component="data_quality"
            )
    ...
    if not args.dry_run and audit_results:
        ...
        if missing_files:
            print(f"- Missing files manifest: {reports_dir}/missing_race_info_manifest_{timestamp}.json")
