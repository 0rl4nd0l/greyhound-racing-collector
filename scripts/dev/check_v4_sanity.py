...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="V4 sanity check: prepare_time_ordered_data on a small slice"
    )
    parser.add_argument(
        "--max-races",
        type=int,
        default=200,
        help="Limit number of races for quick check",
    )
    ...
    try:
        ...
        train_races = (
            ...
            if not train_df.empty and "race_id" in train_df.columns
            else 0
        )
        test_races = (
            len(test_df["race_id"].unique())
            if not test_df.empty and "race_id" in test_df.columns
            else 0
        )
        ...
        if train_df.empty or test_df.empty:
            results["ok"] = False
            results["message"] = (
                "prepare_time_ordered_data returned empty splits (check data presence and filters)"
            )
            print(json.dumps(results, indent=2))
            return 1
