...
def run_preflight_method_check(db_path: str, expect_raise: bool, expect_missing: bool):
    ...
    obj.db_path = db_path
    try:
        res = V4._preflight_check_required_tables(
            obj, required=REQUIRED_TABLES, raise_on_fail=expect_raise
        )
        if expect_raise:
...
def main():
    ...
    if cands:
        ...
        try:
            ...
            constructor_test["message"] = (
                "Constructor preflight passed with detected DB"
            )
        except Exception as e:
        ...
        constructor_test["message"] = (
            "No DB candidates found. Set GREYHOUND_DB_PATH to a valid DB and re-run."
        )
    ...
    if Path(missing_path).exists():
        try:
            ...
            pass
    ok, msg, payload = run_preflight_method_check(
        missing_path, expect_raise=True, expect_missing=False
    )
    results["tests"]["missing_db_raises"] = {
        "ok": ok,
        "message": msg,
        "path": missing_path,
    }
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ok2, msg2, payload2 = run_preflight_method_check(
            empty_db, expect_raise=False, expect_missing=True
        )
        results["tests"]["empty_db_missing_tables"] = {
            "ok": ok2,
            "message": msg2,
            "path": empty_db,
            "missing": (payload2 or {}).get("missing"),
        }
