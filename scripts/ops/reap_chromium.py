    ...
    for p in psutil.process_iter(["name", "ppid", "create_time", "cmdline"]):
        try:
            ...
            if is_orphaned and is_old_enough:
                if dry_run:
                    print(
                        f"  [DRY RUN] Would kill PID {p.pid}: {name} (parent: {ppid}, age: {age_seconds:.1f}s)"
                    )
                    ...
                    try:
                        p.kill()
                        print(
                            f"  ☠️  Killed PID {p.pid}: {name} (parent: {ppid}, age: {age_seconds:.1f}s)"
                        )
                        killed += 1
...
def main():
    ...
    killed = reap_chromium_processes(
        dry_run=args.dry_run, grace_seconds=args.grace_seconds
    )
