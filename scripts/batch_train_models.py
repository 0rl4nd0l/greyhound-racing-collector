...
def run_batch_training(
    model_types: List[str], optimize_hyperparams: bool = False, cv_folds: int = 3
) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                if "top1_accuracy" in result["metrics"]:
                    print(
                        f"   🏆 Top-1 Race Accuracy: {result['metrics']['top1_accuracy']:.3f}"
                    )
            else:
    ...
    if successful_models:
        ...
        model_aucs = [
            (k, v["metrics"]["auc"]) for k, v in results.items() if v.get("success")
        ]
...
def main():
    ...
    parser.add_argument(
        "--cv-folds", type=int, default=3, help="Cross-validation folds"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Include comparison with existing models"
    )
