...
def find_fa_icons_in_file(file_path):
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            if icon_name and not icon_name.startswith('fa-') and is_valid_icon(icon_name):
                icons.add(icon_name)
...
def is_valid_icon(icon_name):
    ...
    invalid_patterns = [
        'champions-league', 'europa-league', 'competitions', 'cup',
        'womens-euro-2025', 'women-s-euro-2025-outright-9107609',
        'icon-name', 'subset', 'test'
    ]
...
def generate_import_statements(icons):
    ...
    brand_list = {
        ...
        'apple', 'microsoft', 'amazon', 'youtube', 'whatsapp', 'telegram'
    }
    
    # Common regular icons (this is a simplified list)
    regular_list = {
        'heart', 'star', 'bookmark', 'calendar', 'clock', 'envelope',
        'file', 'folder', 'image', 'user', 'thumbs-up', 'thumbs-down'
    }
    ...
    if solid_icons:
        imports.append(f"import {{ {', '.join(solid_icons)} }} from '@fortawesome/free-solid-svg-icons';")
    
    if regular_icons:
        imports.append(f"import {{ {', '.join(regular_icons)} }} from '@fortawesome/free-regular-svg-icons';")
    
    if brand_icons:
        imports.append(f"import {{ {', '.join(brand_icons)} }} from '@fortawesome/free-brands-svg-icons';")
