...
def check_database_health(db_path: str, db_type: str) -> Dict[str, any]:
    ...
    try:
        with open_sqlite_readonly(db_path) as conn:
            ...
            key_tables = ["race_metadata", "dog_race_data", "ml_model_registry", "tgr_enhanced_dog_form"]
                ...
                if table in tables:
                    try:
                        ...
                        if table == "race_metadata":
                            cursor.execute("SELECT MAX(race_date) FROM race_metadata WHERE race_date IS NOT NULL")
                            latest_date = cursor.fetchone()[0]
                            health["tables"][table]["latest_date"] = latest_date
                            
                        elif table == "dog_race_data":
                            cursor.execute("SELECT MAX(extraction_timestamp) FROM dog_race_data WHERE extraction_timestamp IS NOT NULL")
                            latest_extraction = cursor.fetchone()[0]
                            health["tables"][table]["latest_extraction"] = latest_extraction
                            
                        elif table == "ml_model_registry":
                            cursor.execute("SELECT COUNT(*) FROM ml_model_registry WHERE is_active = 1")
            ...
            if integrity_results and integrity_results[0][0] != "ok":
                health["issues"].extend([f"Integrity: {row[0]}" for row in integrity_results])
...
def print_detailed_report(report: Dict[str, any]):
    ...
    print(f"Issues: {report['summary']['total_issues']} total ({report['summary']['critical_issues']} critical, {report['summary']['warnings']} warnings)")
