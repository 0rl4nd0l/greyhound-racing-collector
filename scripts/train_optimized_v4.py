...
class OptimizedTrainingPipeline:
    ...
    def _add_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "avg_finish_position" in df.columns and "field_size" in df.columns:
            df["finish_position_ratio"] = df["avg_finish_position"] / df[
                "field_size"
            ].clip(lower=1)
    ...
    def create_model(
        self, model_type: str, optimize_hyperparams: bool = False
    ) -> BaseEstimator:
    ...
    def _optimize_hyperparameters(
        self, model: BaseEstimator, model_type: str
    ) -> BaseEstimator:
    ...
    def evaluate_model(
        ...
        if "race_id" in test_features.columns:
            race_metrics = self._calculate_race_level_metrics(
                test_features, y_test, y_prob
            )
    ...
    def _calculate_race_level_metrics(
        self, test_features: pd.DataFrame, y_true: pd.Series, y_prob: np.ndarray
    ) -> Dict[str, Any]:
    ...
    def register_model(
        ...
        with db_guard(
            db_path=self.staging_db_path, label=f"train_optimized_v4_{model_type}"
        ) as guard:
    ...
    def train_model(
        ...
        try:
            ...
            train_features, test_features = self.prepare_enhanced_features(
            ...
            feature_cols = [
                ...
                if col
                not in ["race_id", "dog_clean_name", "target", "target_timestamp"]
            ]
            ...
            calibrated_model = CalibratedClassifierCV(
                pipeline, method="isotonic", cv=cv_folds
            )
            ...
            metrics = self.evaluate_model(
                calibrated_model, X_test, y_test, test_features
            )
            ...
            model_id = self.register_model(
                calibrated_model, model_type, metrics, feature_cols
            )
            ...
            result = {
                ...
                "model_type": model_type,
                "metrics": metrics,
                "training_duration_minutes": (time.time() - self.training_start_time)
                / 60,
            }
...
def main():
    ...
    parser.add_argument(
        "--model-type",
        choices=["auto", "extratrees", "lgbm", "hgb", "gb", "ensemble"],
        default=None,
    ...
    parser.add_argument(
        "--cv-folds", type=int, default=3, help="Cross-validation folds for calibration"
    )
    ...
    optimize_hyperparams = (
        args.optimize_hyperparams or os.getenv("V4_OPTIMIZE", "0") == "1"
    )
