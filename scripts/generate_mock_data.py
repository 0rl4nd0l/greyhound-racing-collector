...
def generate_mock_race_data(
    ...
    for race_idx in range(num_races):
        ...
        condition_modifier = {"Fast": 1.0, "Good": 1.02, "Slow": 1.05, "Heavy": 1.08}[
            track_condition
        ]
        ...
        for dog_idx, dog_name in enumerate(race_dogs):
            ...
            win_rate = (recent_wins / recent_races) + (
                performance_boost * random.uniform(0, 0.2)
            )
            ...
            best_time = round(
                distance * 0.075 - (performance_boost * 2) + random.uniform(-1, 1), 2
            )
            avg_time = best_time + random.uniform(0.5, 2.5)

            starting_price = round(
                random.uniform(1.8, 50.0) - (performance_boost * 10), 2
            )
            ...
            first_sectional = round(
                distance * 0.02 - (performance_boost * 0.5) + random.uniform(-0.3, 0.3),
                2,
            )
            win_time = (
                race_time
                + random.uniform(-1.5, 3.0)
                + (0 if is_winner else random.uniform(0, 2))
            )
...
def main():
    ...
    parser.add_argument(
        "--races", type=int, default=100, help="Number of races to generate"
    )
    parser.add_argument(
        "--dogs-per-race", type=int, default=6, help="Average number of dogs per race"
    )
    parser.add_argument(
        "--clean-first",
        action="store_true",
    ...
    generate_mock_race_data(
        num_races=args.races, dogs_per_race=args.dogs_per_race, db_path=db_path
    )
