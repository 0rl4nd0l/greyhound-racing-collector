...
DEFAULT_OLD_COVERAGE = (
    ...
    / "full_evidence_orchestration_20260525"
    / "clean_history_feature_packet_20260602"
    / "feature_coverage_after_reconstruction.json"
)
...
def _dict_from_old_dictionary(old_dictionary: dict[str, Any], field_coverage: dict[str, Any]) -> dict[str, Any]:
    ...
    result["schema_version"] = "repaired_same_distance_feature_dictionary_v1"
    result["feature_policy"] = {
        "accepted_form_guide_policy": "Report-only repair consumes the frozen clean history packet and canonical DB history; it does not treat embedded historical DIST/G as target metadata.",
...
def _schema_parity(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    compatibility = {}
    for field in same_distance_fields:
        compatibility[field] = {
    ...
    return {
        ...
        "historical_present_fields": sorted(same_distance_fields & hist_fields),
        "rolling_present_fields": sorted(same_distance_fields & eval_fields),
...
def _leakage_audit(
    ...
    for row in new_rows:
        ...
        if row.get("same_distance_target_status") == "UNIQUE_DATE_VENUE":
            ...
            break
    return {
        "status": "PASS" if historical_safe and no_future_rows_used else "FAIL",
        "checks": {
            "historical_rows_use_canonical_db_history": historical_safe,
...
def _report_text(
    *,
    ...
    lines = [
        ...
        "## Target Resolution",
        "",
        f"- Resolution counts: `{audit.get('target_resolution_counts')}`",
        f"- Historical rows with resolved safe target metadata: `{audit.get('target_resolution_counts', {}).get('UNIQUE_DATE_VENUE', 0)}`",
        "",
        "## Coverage Delta",
        "",
        "```json",
        json.dumps(coverage_delta, indent=2, sort_keys=True, default=_json_default),
        "```",
        "",
        "## Leakage Audit",
        "",
...
def repair_packet(
    *,
    ...
    repaired_rows, audit = repair_packet_rows(rows, db_path=db_path)
    ...
    repaired_cov = {
        "schema_version": "repaired_same_distance_feature_coverage_v1",
        ...
        "coverage_by_packet": _packet_coverage(repaired_rows, _same_distance_fields()),
        "same_distance_repair_audit": audit,
    }

    output_dir.mkdir(parents=True, exist_ok=True)
    repaired_csv = output_dir / "repaired_pre_race_history_feature_packet.csv"
    repaired_dictionary = output_dir / "repaired_pre_race_history_feature_dictionary.json"
    repaired_cov_path = output_dir / "repaired_feature_coverage_after_reconstruction.json"
    audit_path = output_dir / "same_distance_repair_audit.json"
    leakage_path = output_dir / "feature_leakage_audit.json"
    parity_path = output_dir / "train_eval_schema_parity.json"
    delta_path = output_dir / "historical_same_distance_coverage_delta.json"
    ...
    _write_json(delta_path, coverage_delta)
    ...
    return {
        ...
        "same_distance_repair_audit": str(audit_path),
        "feature_leakage_audit": str(leakage_path),
        "train_eval_schema_parity": str(parity_path),
        "historical_same_distance_coverage_delta": str(delta_path),
