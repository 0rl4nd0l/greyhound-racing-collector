...
class MonitoringAPI:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "error": str(e),
                "system_status": {"status": "error", "message": "Health check failed"},
            }
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        ) < self.cache_timeout and self.metrics_cache:
        ...
        try:
            ...
            logger.log_error(
                f"Performance metrics calculation failed: {str(e)}",
                context={"component": "monitoring"},
            )
            return {
                "success": False,
        ...
        try:
            ...
            for file_path, mtime in prediction_files[:limit]:
                try:
                        ...
                        predictions.append(
                            {
                                ...
                                "race_name": race_info.get(
                                    "filename",
                                    f"{race_info.get('venue', 'Unknown')} Race {race_info.get('race_number', 'N/A')}",
                                ),
                                "predicted_winner": top_prediction.get(
                                    "dog_name", "Unknown"
                                ),
                                "confidence": top_prediction.get(
                                    "final_score",
                                    top_prediction.get("prediction_score", 0),
                                ),
                    ...
                    logger.log_error(
                        f"Error processing prediction file {file_path}: {str(e)}",
                        context={"component": "monitoring"},
                    )
                    continue
            ...
            logger.log_error(
                f"Recent predictions retrieval failed: {str(e)}",
                context={"component": "monitoring"},
            )
    ...
    def get_accuracy_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log_error(
                f"Accuracy trends calculation failed: {str(e)}",
                context={"component": "monitoring"},
            )
    ...
    def get_system_alerts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log_error(
                f"System alerts check failed: {str(e)}",
                context={"component": "monitoring"},
            )
    ...
    def _check_model_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": (
                    "healthy" if model_age_hours < 168 else "warning"
                ),  # 1 week threshold
    ...
    def _determine_overall_status(
        ...
        if (
            ...
            or memory > 95
            or disk > 95
            or db_health["status"] == "error"
            or model_health["status"] == "error"
            or api_health["status"] == "error"
        ):
            return "error"
        ...
        if (
            ...
            or memory > 85
            or disk > 90
            or db_health["status"] == "warning"
            or model_health["status"] == "warning"
            or api_health["status"] == "warning"
        ):
            return "warning"
    ...
    def _calculate_live_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log_error(
                f"Live metrics calculation failed: {str(e)}",
                context={"component": "monitoring"},
            )
    ...
    def _load_latest_backtesting_results(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result_files = list(
                self.ml_results_dir.glob("ml_backtesting_results_*.json")
            )
            if not result_files:
                return {}
            ...
            # Extract key metrics
            return {
                "win_accuracy": results.get("best_models", {})
                .get("win_model", {})
                .get("accuracy", 0)
                * 100,
                "place_accuracy": results.get("best_models", {})
                .get("place_model", {})
                .get("accuracy", 0)
                * 100,
                "race_accuracy": results.get("prediction_analysis", {}).get(
                    "race_accuracy", 0
                )
                * 100,
            }
            ...
            logger.log_error(
                f"Error loading backtesting results: {str(e)}",
                context={"component": "monitoring"},
            )
            return {}
    ...
    def _count_predictions_today(self) -> int:
        ...
        try:
            ...
            logger.log_error(
                f"Error counting today's predictions: {str(e)}",
                context={"component": "monitoring"},
            )
            return 0
    ...
    def _calculate_roi_30d(self) -> float:
        ...
        try:
            ...
            logger.log_error(
                f"Error calculating ROI: {str(e)}", context={"component": "monitoring"}
            )
    ...
    def _get_average_response_time(self) -> float:
        ...
        try:
            ...
            logger.log_error(
                f"Error calculating response time: {str(e)}",
                context={"component": "monitoring"},
            )
            return 1.2  # Default value

    def _determine_prediction_status(
        self, prediction_data: Dict, race_info: Dict
    ) -> str:
        ...
        try:
            ...
            if race_date_str:
                try:
                    ...
                    if race_date < datetime.now().date():
                        return (
                            "incorrect"  # Past race with no results = likely incorrect
                        )
                except:
                    pass
            ...
            logger.log_error(
                f"Error determining prediction status: {str(e)}",
                context={"component": "monitoring"},
            )
            return "pending"
    ...
    def _load_accuracy_trends(self, days: int) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                trends.append(
                    {
                        ...
                        "place_accuracy": max(
                            50, min(70, base_place_accuracy + variation)
                        ),
            ...
            logger.log_error(
                f"Error loading accuracy trends: {str(e)}",
                context={"component": "monitoring"},
            )
            return []
        ...
        try:
            ...
            if latest_timestamp:
                try:
                    ...
                    age_hours = (
                        datetime.now() - latest_dt.replace(tzinfo=None)
                    ).total_seconds() / 3600
                    return age_hours
                except:
                    pass
