    ...
    try:
        ...
        for p in psutil.process_iter(["name", "ppid", "create_time", "cmdline"]):
            ...
            if any(k in name for k in ("chrome", "chromium", "chromedriver")):
                ...
                if (
                    p.info.get("ppid", 0) in (0, 1)
                    or (now - create_time) > grace_seconds
                ):
                    try:
                        p.kill()
                        killed += 1
        ...
        if killed > 0:
            logging.getLogger(__name__).info(
                f"Killed {killed} orphaned Chrome processes"
            )
    except Exception:
        pass
...
class HybridOddsScraper:
    ...
    def scrape_odds(
        ...
        if api_success and odds_df is not None:
            ...
            metadata.update(
                {
                    ...
                    "success": True,
                    "markets_count": (
                        odds_df["market_id"].nunique()
                        if "market_id" in odds_df.columns
                        else 0
                    ),
            ...
            self.logger.warning(
                "❌ API-based scraping failed, trying Selenium fallback"
            )
    ...
    def _try_api_scraper(
        self, url: str, max_retries: int
        ...
        for attempt in range(max_retries):
            try:
                ...
                if scraper.error_message:
                    self.logger.warning(
                        f"API scraper error (attempt {attempt + 1}): {scraper.error_message}"
                    )
                ...
                if scraper.odds_df is not None and len(scraper.odds_df) > 0:
                    self.logger.info(
                        f"API scraper got {len(scraper.odds_df)} selections"
                    )
                    return scraper.odds_df, True

            except Exception as e:
                self.logger.error(
                    f"API scraper exception (attempt {attempt + 1}): {str(e)}"
                )
                if attempt < max_retries - 1:
        ...
        for attempt in range(max_retries):
            with _selenium_slot():
                try:
                    ...
                    self.logger.info(
                        f"Loading page with Selenium (attempt {attempt + 1})"
                    )
                    self.driver.get(url)
                    ...
                    self.logger.error(
                        f"Selenium scraper exception (attempt {attempt + 1}): {str(e)}"
                    )
                    if attempt < max_retries - 1:
                        time.sleep(2**attempt)
                finally:
                    ...
                    kill_orphaned_chromium(
                        grace_seconds=0
                    )  # Immediate cleanup after each attempt
    ...
    def _extract_odds_with_selenium(self) -> list:
        ...
        try:
            ...
            runner_containers = self.driver.find_elements(
                By.CSS_SELECTOR,
                "[data-automation-id*='runner'], .runner-card, .outcome-button",
            )

            self.logger.info(
                f"Found {len(runner_containers)} potential runner elements"
            )
            ...
            for i, container in enumerate(runner_containers[:20]):  # Limit for testing
                try:
                    ...
                    # Extract odds
                    odds_element = container.find_element(
                        By.CSS_SELECTOR, "[data-automation-id*='price'], .price, .odds"
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        api_total = (
            self.success_stats["api_success"] + self.success_stats["api_failure"]
        )
        selenium_total = (
            self.success_stats["selenium_success"]
            + self.success_stats["selenium_failure"]
        )
        ...
        stats = {
            ...
            "api_success_rate": (
                (self.success_stats["api_success"] / api_total * 100)
                if api_total > 0
                else 0
            ),
            "selenium_attempts": selenium_total,
            "selenium_success_rate": (
                (self.success_stats["selenium_success"] / selenium_total * 100)
                if selenium_total > 0
                else 0
            ),
            "overall_success_rate": (
                (
                    self.success_stats["api_success"]
                    + self.success_stats["selenium_success"]
                )
                / total_attempts
                * 100
