...
class DatabaseOptimizer:
        ...
        try:
            ...
            for table in tables:
                ...
                for idx_row in table_indexes:
                    ...
                    # Get index SQL
                    cursor.execute("SELECT sql FROM sqlite_master WHERE name = ?", (idx_name,))
    ...
    def create_missing_indexes(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        if not missing:
            return {"status": "success", "message": "All recommended indexes exist", "created": []}
        ...
        try:
            for query in queries:
                try:
                    ...
                    uses_index = any("USING INDEX" in str(row).upper() for row in query_plan)
                    ...
                    suggested_indexes = self._suggest_indexes_for_query(query, query_plan)
    ...
    def _suggest_indexes_for_query(self, query: str, query_plan: List[Tuple]) -> List[str]:
        ...
        try:
            ...
            if duplicate_races:
                integrity_report["duplicate_issues"].append({
                    ...
                    "sample": [{"race_id": row[0], "duplicates": row[1]} for row in duplicate_races[:5]]
                })
    ...
    def optimize_database(self, vacuum: bool = True, analyze: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            results["space_saved_mb"] = results["before_size_mb"] - results["after_size_mb"]
    ...
    def generate_optimization_report(self) -> str:
        ...
        for table, table_stats in stats["tables"].items():
            report.append(f"   {table}: {table_stats['row_count']:,} rows, {len(table_stats['indexes'])} indexes")
        
        # Index analysis
        report.extend(["", "🔍 INDEX ANALYSIS:"])
        if missing_indexes:
            report.append(f"   ⚠️  {sum(len(indexes) for indexes in missing_indexes.values())} recommended indexes missing")
        ...
        if "common_queries" in stats["performance_metrics"]:
            slow_queries = [q for q in stats["performance_metrics"]["common_queries"] 
                          if q["execution_time_ms"] > 100]
            if slow_queries:
                report.append(f"   ⚠️  {len(slow_queries)} slow queries detected (>100ms)")
                for query in slow_queries[:3]:
                    report.append(f"      - {query['execution_time_ms']:.1f}ms: {query['query'][:50]}...")
            else:
        ...
        if integrity["total_issues"] > 0:
            ...
            for issue_type in ["foreign_key_violations", "data_consistency_issues", 
                             "missing_data_issues", "duplicate_issues"]:
...
def main():
    ...
    indexes_parser = subparsers.add_parser('create-indexes', help='Create missing indexes')
    indexes_parser.add_argument('--dry-run', action='store_true', help='Show what would be created')
    ...
    full_optimize_parser = subparsers.add_parser('full-optimize', help='Full optimization')
    ...
    if args.command == 'report':
        ...
        if result["status"] == "dry_run":
            ...
            if result["created"]:
                ...
                for idx in result["created"]:
                    print(f"   - {idx['name']} on {idx['table']} ({idx['creation_time_seconds']:.2f}s)")
        ...
        for op in result["operations"]:
            print(f"   ✅ {op['operation']}: {op['description']} ({op['time_seconds']:.2f}s)")
        ...
        print(f"   ✅ Optimization completed in {opt_result['time_taken_seconds']:.2f}s")
