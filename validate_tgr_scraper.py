    ...
    def validate_racing_history_data(racing_history: list, dog_name: str) -> bool:
        ...
        for i, race in enumerate(racing_history):
            ...
            if race.get("finish_position") and not (1 <= race["finish_position"] <= 8):
                logger.warning(
                    f"⚠️ {dog_name} race {i}: unusual finish position {race['finish_position']}"
                )
            ...
            if race.get("distance") and isinstance(race["distance"], str):
                if not any(char.isdigit() for char in race["distance"]):
                    logger.warning(
                        f"⚠️ {dog_name} race {i}: distance has no numbers: {race['distance']}"
                    )

        completeness = (valid_races / total_races) * 100 if total_races > 0 else 0
        logger.info(
            f"✅ {dog_name}: {valid_races}/{total_races} races valid ({completeness:.1f}%)"
        )
    ...
    def test_cached_files():
        ...
        for test_url in test_urls:
            ...
            try:
                ...
                logger.info(
                    f"✅ Race data: {len(race_data.get('dogs', []))} dogs, venue: {race_data.get('venue')}"
                )
        ...
        logger.info(f"\\n📊 Test Summary:")
        logger.info(
            f"   • URLs tested successfully: {successful_tests}/{len(test_urls)}"
        )
