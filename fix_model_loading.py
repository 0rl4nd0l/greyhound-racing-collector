        ...
        if ml_system.pipeline is not None:
            ...
            if hasattr(ml_system, 'model_info') and ml_system.model_info:
                ...
                if 'saved_at' in ml_system.model_info:
                    logger.info(f"⏰ Model timestamp: {ml_system.model_info['saved_at']}")
                
                if 'version' in ml_system.model_info:
                    logger.info(f"🏷️ Model version tag: {ml_system.model_info['version']}")
...
def main():
    ...
    if model_loaded:
        ...
        logger.info("📋 The logs above should contain 'Loaded model' and 'model_info' entries")
    else:
