    ...
    if os.path.exists(VALIDATION_SUMMARY):
        ...
        if directory_error_matrix:
            ...
            heatmap_df = pd.DataFrame(
                matrix_data, index=all_dirs, columns=all_error_types
            )
            ...
            if not heatmap_df.empty:
                ...
                ax2.text(
                    0.5,
                    0.5,
                    "No directory-specific data available",
                    ha="center",
                    va="center",
            ...
            recommendations = generate_fix_recommendations(
                error_type_counts, df_summary if "df_summary" in locals() else None
            )
        ...
        recommendations["overall_health"] = {
            ...
            "health_score": (
                max(
                    0,
                    100
                    - (failed_files / total_files * 100)
                    - (avg_quarantine_rate * 100),
                )
                if total_files > 0
                else 0
            ),
