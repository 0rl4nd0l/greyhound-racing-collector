...
def demo_monitoring_dashboard():
    ...
    if "completeness" in data_quality:
        ...
        print(
            f"Complete Profiles: {comp.get('complete_profiles', 0)}/{comp.get('total_dogs', 0)}"
        )
    ...
    if "collection_efficiency" in performance:
        ...
        print(
            f"Avg Collection Time: {collection.get('avg_collection_time_sec', 0):.1f}s"
        )
...
def demo_enrichment_service():
    ...
    if status["statistics"]["total_processing_time"] > 0:
        avg_time = status["statistics"]["total_processing_time"] / max(
            status["statistics"]["jobs_processed"], 1
        )
    ...
    print(
        f"  Success Rate: {final_status['statistics']['jobs_succeeded']}/{final_status['statistics']['jobs_processed']}"
    )
    print(
        f"  Total Processing Time: {final_status['statistics']['total_processing_time']:.1f}s"
    )
...
def demo_scheduler_integration():
    ...
    try:
        from tgr_service_scheduler import SchedulerConfig, TGRServiceScheduler
...
def demo_data_quality_improvements():
    ...
    try:
        ...
        if sample_data:
            ...
            print(
                f"{'Dog Name':<20} {'Consistency':<12} {'Form Trend':<12} {'Win %':<8}"
            )
            print("-" * 55)
