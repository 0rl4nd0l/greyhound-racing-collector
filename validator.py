...
def validate_output(file_path):
    required_fields = {"success": bool, "race_info": dict, "predictions": list}
    prediction_fields = {
        "dog_name": str,
        "box_number": int,
        "win_prob_raw": float,
        ...
        for field, field_type in required_fields.items():
            if field not in data:
                ...
                validation_result["errors"].append(
                    f"Invalid field type for {field}: expected {field_type.__name__}"
                )
            ...
            for i, prediction in enumerate(data["predictions"]):
                for field, field_type in prediction_fields.items():
                    if field not in prediction:
                        validation_result["errors"].append(
                            f"Missing field in prediction {i}: {field}"
                        )
                    elif not isinstance(prediction[field], field_type):
                        validation_result["errors"].append(
                            f"Invalid field type in prediction {i} for {field}: expected {field_type.__name__}"
                        )
                ...
                for outcome_field in outcome_fields:
                    if outcome_field in reasoning_text:
                        validation_result["errors"].append(
                            f"Prediction {i} reasoning contains outcome field: {outcome_field}"
                        )
                ...
                for feature in prediction:
                    if feature in outcome_fields:
                        validation_result["errors"].append(
                            f"Prediction {i} contains outcome feature: {feature}"
                        )
