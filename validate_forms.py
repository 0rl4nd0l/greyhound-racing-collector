...
def validate_csv(file_path):
    ...
    try:
        ...
        # Try reading the CSV
        df = pd.read_csv(
            file_path, encoding=encoding, delimiter=delimiter, on_bad_lines="skip"
        )
        ...
        if missing_columns:
            ...
            warnings += 1
            if (
                len(missing_columns) > len(EXPECTED_COLUMNS) // 2
        ...
        validation_result = {
            ...
            "pct_rows_quarantined": (
                quarantined_rows / row_count if row_count > 0 else 0
            ),
            "delimiter": delimiter,
            "encoding": encoding,
...
if __name__ == "__main__":
    ...
    for file in os.listdir("."):
        if file.endswith(".csv") and any(
            ...
            for keyword in ["race", "form", "guide", "greyhound", "track"]
        ):
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        results = list(
            tqdm(executor.map(validate_csv, csv_files), total=len(csv_files))
        )
    ...
    if failures:
        top_offenders = sorted(
            failures,
            key=lambda x: (x["pct_rows_quarantined"], x["warnings"]),
            reverse=True,
        )[:20]
        ...
        for i, offender in enumerate(top_offenders):
            ...
            try:
                if os.path.exists(offender["file_path"]):
                    ...
                    print(
                        f"  Copied {file_name} (warnings: {offender['warnings']}, quarantined: {offender['pct_rows_quarantined']:.1%})"
                    )
            except Exception as e:
    ...
    summary_stats = {
        ...
        "most_common_delimiter": (
            df_summary["delimiter"].mode().iloc[0]
    ...
    print(
        f"  Average warnings per file: {summary_stats['average_warnings_per_file']:.1f}"
    )
