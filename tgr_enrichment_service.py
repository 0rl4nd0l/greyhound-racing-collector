...
class TGREnrichmentService:
    ...
    def _load_pending_jobs(self):
        ...
        try:
            ...
            for job_data in pending_jobs:
                (job_id, dog_name, job_type, priority, attempts, max_attempts,
                 error_message, est_duration, created_at) = job_data
                ...
                job = EnrichmentJob(
                    ...
                    attempts=attempts,
                    max_attempts=max(max_attempts or self.default_max_attempts, self.default_max_attempts),
            ...
            logger.info(f"✅ Loaded {added} pending enrichment jobs (total in DB: {len(pending_jobs)})")
    ...
    def start_service(self):
        ...
        logger.info(f"🚀 Starting TGR enrichment service with {self.max_workers} workers")
        ...
        # Log service start
        self._log_service_action("service_started", details=f"Started with {self.max_workers} workers")
    ...
    def add_enrichment_job(self, dog_name: str, job_type: str = "comprehensive",
                          priority: int = 5) -> str:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, 'pending', ?)
            """, [job_id, dog_name, job_type, priority, self._estimate_job_duration(job_type)])
            ...
            if cursor.rowcount > 0:
                ...
                logger.warning(f"Job already exists for {dog_name} with type {job_type}")
    ...
    def _process_enrichment_job(self, job: EnrichmentJob) -> bool:
        ...
        try:
            ...
            self._update_job_status(job.job_id, EnrichmentStatus.PROCESSING, 
                                  started_at=datetime.now())
            
            # Log job start
            self._log_service_action("job_started", job.job_id, 
            ...
            if success:
                ...
                self._update_job_status(job.job_id, EnrichmentStatus.COMPLETED,
                                      completed_at=datetime.now(),
                                      actual_duration=duration)
                
                self._log_service_action("job_completed", job.job_id,
                                       f"Successfully processed in {duration:.1f}s")
                
                logger.info(f"✅ {thread_name} completed job {job.job_id} in {duration:.1f}s")
                ...
                job.max_attempts = max(job.max_attempts or self.default_max_attempts, self.default_max_attempts)
                
                if job.attempts >= job.max_attempts:
                    # Mark as failed
                    self._update_job_status(job.job_id, EnrichmentStatus.FAILED,
                                          error_message="Max attempts exceeded")
                    
                    self._log_service_action("job_failed", job.job_id,
                                           f"Failed after {job.attempts} attempts")
                    
                    logger.error(f"❌ {thread_name} failed job {job.job_id} after {job.attempts} attempts")
                    # Remove from in-memory queued set
                    try:
                        self._queued_job_ids.discard(job.job_id)
                    except Exception:
                    ...
                    self._update_job_status(job.job_id, EnrichmentStatus.PENDING,
                    ...
                    jitter = random.randint(0, int(self.retry_jitter_seconds)) if self.retry_jitter_seconds > 0 else 0
                    ...
                    logger.warning(f"⚠️ {thread_name} retrying job {job.job_id} (attempt {job.attempts + 1}) in ~{delay + jitter}s")
    ...
    def _update_dog_performance_summary(self, dog_name: str):
        ...
        performance_data['win_percentage'] = (performance_data['wins'] / performance_data['total_starts']) * 100
        performance_data['place_percentage'] = (performance_data['places'] / performance_data['total_starts']) * 100
    ...
    def _update_job_status(self, job_id: str, status: EnrichmentStatus,
                          started_at: datetime = None, completed_at: datetime = None,
                          actual_duration: float = None, attempts: int = None,
                          error_message: str = None):
    ...
    def _load_settings(self):
        ...
        try:
            ...
            if kv.get('backoff_max_seconds'):
                try:
                    self.backoff_max_seconds = max(self.backoff_base_seconds, int(kv['backoff_max_seconds']))
                except Exception:
                    pass
