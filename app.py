...
def perform_prediction_background():
    ...
    try:
        ...
        # Use Unified Predictor for predictions (HIGHEST PRIORITY SYSTEM)
        from unified_predictor import UnifiedPredictor
        predictor = UnifiedPredictor()

        # Run prediction using unified predictor with full enhancement
        race_file_path = os.path.join(source_dir, latest_race_file)
        
        try:
            safe_log_to_processing("🚀 Using Unified Predictor (Most Advanced System)", "INFO", 60)
            ...
            if result.get('success'):
                processing_status['log'].append({
                    'timestamp': datetime.now().isoformat(),
                    'message': "✅ Unified race predictions completed successfully!"
                })
                ...
                processing_status['log'].append({
                    'timestamp': datetime.now().isoformat(),
                    'message': f"❌ Race predictions failed: {result.get('error', 'Unknown error')}"
                })
                success = False
...
def api_test_historical_prediction():
    ...
    try:
        ...
        if not data:
            test_prediction_status['error'] = 'No data provided'
            log_test_prediction("❌ No data provided in request", "ERROR", 0)
            return jsonify({
                'success': False,
        ...
        log_test_prediction(f"📋 Testing race: {venue} Race {race_number} on {race_date}", "INFO", 10)
        
        if not all([race_id, venue, race_number, race_date]):
            test_prediction_status['error'] = 'Missing required race information'
            log_test_prediction("❌ Missing required race information", "ERROR", 10)
            return jsonify({
                'success': False,
                'error': 'Missing required race information'
            }), 400
        
        test_prediction_status['current_step'] = 'Loading race data from database'
        ...
        # Try to run prediction using the unified predictor
        try:
            ...
            # Create a temporary CSV file for this race
            import tempfile
            import csv
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as temp_file:
                ...
                if dogs and len(dogs) > 0:
                    # Use the first dog's keys as the header, filtering out None values
                    header = [key for key in dogs[0].keys() if key is not None]
            ...
            prediction_result = predictor.predict_race_file(temp_csv_path)
            ...
            if prediction_result and 'predictions' in prediction_result:
                ...
                prediction_method = prediction_result.get('prediction_method', 'Unified Predictor')
                ...
                if accuracy_metrics['actual_winner_rank'] is None:
                    accuracy_metrics['actual_winner_rank'] = len(predictions) + 1
                ...
                return jsonify({
                    ...
                    'race_info': {
                        ...
                        'race_date': race_date,
                        'race_name': race_info.get('race_name', 'Unknown Race')
                    }
                })
            else:
                return jsonify({
                    'success': False,
                    'error': 'Prediction system returned no results'
                }), 500
                
        except ImportError:
            return jsonify({
                'success': False,
                'error': 'Unified predictor not available'
            }), 500
...
def api_upcoming_races():
    """API endpoint to fetch upcoming races from the website"""
    try:
