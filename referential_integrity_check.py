...
class ReferentialIntegrityChecker:
    ...
    def check_table_counts(self):
        ...
        tables = ['races', 'dog_performances', 'dogs', 'venues', 'predictions', 'form_guide']
        counts = {}
    ...
    def check_fk_orphans_dog_performances_to_races(self):
        ...
        total_performances = self.execute_query("SELECT COUNT(*) as count FROM dog_performances")['count'].iloc[0]
        orphan_count = len(orphans)
        orphan_percentage = (orphan_count / total_performances * 100) if total_performances > 0 else 0
    ...
    def check_fk_orphans_predictions_to_races(self):
        ...
        total_predictions = self.execute_query("SELECT COUNT(*) as count FROM predictions")['count'].iloc[0]
        orphan_count = len(orphans)
        orphan_percentage = (orphan_count / total_predictions * 100) if total_predictions > 0 else 0
    ...
    def check_dog_name_consistency(self):
        ...
        missing_from_dogs = self.execute_query(query1, "Dogs in performances but not in dogs table")
        ...
        missing_from_performances = self.execute_query(query2, "Dogs in dogs table but not in performances")
        ...
        missing_from_dogs_form = self.execute_query(query3, "Dogs in form_guide but not in dogs table")
        
        print(f"Dogs in dog_performances but not in dogs table: {len(missing_from_dogs):,}")
    ...
    def check_venue_consistency(self):
        ...
        missing_venues = self.execute_query(query1, "Venues in races but not in venues table")
        ...
        missing_venues_form = self.execute_query(query2, "Venues in form_guide but not in venues table")
    ...
    def check_data_uniqueness_constraints(self):
        ...
        duplicate_performances = self.execute_query(query2, "Duplicate dog performances")
    ...
    def check_cross_table_data_consistency(self):
        ...
        inconsistent_stats = self.execute_query(query, "Dogs with inconsistent statistics")
    ...
    def generate_summary_report(self):
        ...
        print(f"  dog_performances orphans: {dp_orphans.get('count', 0):,} ({dp_orphans.get('percentage', 0):.2f}%)")
        print(f"  predictions orphans     : {pred_orphans.get('count', 0):,} ({pred_orphans.get('percentage', 0):.2f}%)")
        
        print("\n🐕 DOG NAME CONSISTENCY:")
        dog_consistency = self.results.get('dog_name_consistency', {})
        print(f"  Missing from dogs table (performances): {dog_consistency.get('missing_from_dogs_table', 0):,}")
        print(f"  Missing from performances: {dog_consistency.get('missing_from_performances', 0):,}")
        print(f"  Missing from dogs table (form_guide): {dog_consistency.get('missing_from_dogs_form', 0):,}")
        
        print("\n🏟️ VENUE CONSISTENCY:")
        venue_consistency = self.results.get('venue_consistency', {})
        print(f"  Missing from venues table (races): {venue_consistency.get('missing_from_venues_table_races', 0):,}")
        print(f"  Missing from venues table (form): {venue_consistency.get('missing_from_venues_table_form', 0):,}")
        
        print("\n🔄 DUPLICATE DATA:")
        duplicates = self.results.get('duplicates', {})
        print(f"  Duplicate races: {duplicates.get('duplicate_races', 0):,}")
        print(f"  Duplicate performances: {duplicates.get('duplicate_performances', 0):,}")
        print(f"  Duplicate dogs: {duplicates.get('duplicate_dogs', 0):,}")
        
        print("\n📈 STATISTICAL CONSISTENCY:")
        stats_consistency = self.results.get('statistical_consistency', {})
        print(f"  Dogs with inconsistent stats: {stats_consistency.get('inconsistent_dogs', 0):,}")
