...
def get_database_schema(db_path):
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE '%backup%' AND name NOT LIKE 'sqlite_%' ORDER BY name")
...
def create_er_diagram(schema_info, output_path):
    ...
    def categorize_table(table_name):
        if table_name in ['race_metadata', 'dog_race_data', 'dogs']:
            ...
            return 'weather'
        elif 'odds' in table_name or 'value_bets' in table_name or 'predictions' in table_name:
            return 'odds'
        elif 'analytics' in table_name or 'gpt_analysis' in table_name:
            return 'analytics'
        elif 'ft_extra' in table_name or 'gr_' in table_name or 'enhanced' in table_name or 'comprehensive' in table_name:
            return 'enhanced'
        else:
            return 'core'
    ...
    analytics_tables = [t for t in schema_info.keys() if categorize_table(t) == 'analytics']
    ...
    # Enhanced tables (top)
    enhanced_tables = [t for t in schema_info.keys() if categorize_table(t) == 'enhanced']
    ...
    for table_name, table_info in schema_info.items():
        ...
        # Add row count
        ax.text(x, y + box_height/2 - 0.3, f"({table_info['row_count']:,} rows)", 
        ...
        for i, col_text in enumerate(key_columns):
            ax.text(x, y + box_height/2 - 0.5 - i * 0.12, col_text,
    ...
    for table_name, table_info in schema_info.items():
        ...
        for fk in table_info['foreign_keys']:
            ...
            if ref_table in table_boxes:
                ...
                mid_x = (x1 + x2) / 2
                mid_y = (y1 + y2) / 2
                ax.text(mid_x, mid_y, f"{fk['column']}", 
                       ha='center', va='center', fontsize=6, 
                       bbox=dict(boxstyle="round,pad=0.2", facecolor='white', alpha=0.8))
    
    # Add title and legend
    ax.text(12, 19.5, 'Greyhound Racing Database - Entity Relationship Diagram', 
            ha='center', va='center', fontsize=16, fontweight='bold')
    
    ax.text(12, 19, f'Total Tables: {len(schema_info)} | Generated from Live Database', 
    ...
    for i, (category, color) in enumerate(categories):
        ...
        legend_box = patches.Rectangle((legend_x, y_pos - 0.1), 0.3, 0.2, 
