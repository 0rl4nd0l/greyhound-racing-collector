...
class DataQualityImprover:
    """Data Quality Improver - wrapper for compatibility"""

    def __init__(self):
        pass

    def strip_box_prefixes(self, data):
        """Strip prefixes from box numbers"""
        data["box_number"] = data["box_number"].astype(str).str.replace(r"Box ", "")
        return data

    def harmonize_venue_codes(self, data):
        """Harmonize venue codes using GreyhoundVenueMapper"""
        mapper = GreyhoundVenueMapper()
        data["venue"] = data["venue"].apply(mapper.resolve_venue)
        return data

    def calculate_fingerprints(self, data):
        """Create fingerprints based on dog name and race date"""
        data["fingerprint"] = data.apply(
            lambda row: hash((row["dog_name"], row["race_date"])), axis=1
        )
        return data

    def compute_quality_score(self, data):
        """Compute the quality score for each record"""
...
def clean_problematic_analysis_files():
    ...
    for file_path in Path(".").rglob("Analysis_*.csv"):
        ...
        if re.search(r"_\d{2}_\d{2}", file_path.name):
            ...
            match = re.match(
                r"(Analysis_\w+_[A-Z_]+_\d{4}-\d{2}-\d{2}_\d{6}).*\.csv", file_path.name
            )
            if match:
...
def generate_quality_report():
    ...
    valid_files = sum(
        1 for r in race_validation.values() if r["readable"] and r["has_data"]
    )
    ...
    report["summary"] = {
        ...
        "validity_rate": (
            (valid_files / total_race_files * 100) if total_race_files > 0 else 100
        ),
        "compliance_rate": (
            (compliant_names / total_race_files * 100) if total_race_files > 0 else 100
        ),
    }
