...
class ProbabilityCalibrator:
    def validate_current_model(self) -> Dict[str, float]:
        ...
        slope_intercept_model = LinearRegression().fit(
            calibrated_probs.reshape(-1, 1), actuals
        )
        ...
        hits_per_decile = [
            np.mean(actuals[bin_indices == i]) for i in range(len(bins) - 1)
        ]
        ...
        if not slope_pass:
            logger.error(
                f"FAIL: Calibration slope deviation {slope_deviation:.4f} exceeds threshold 0.2"
            )
        else:
            logger.info(
                f"PASS: Calibration slope deviation {slope_deviation:.4f} within threshold 0.2"
            )
        ...
        monotonic_pass = len(valid_deciles) == 0 or valid_deciles == sorted(
            valid_deciles
        )
        test_results.append(
            (
                "No monotonic inversion in deciles",
                monotonic_pass,
                f"{len(valid_deciles)} valid deciles",
            )
        )
        if not monotonic_pass:
            logger.warning(
                f"WARNING: Monotonic inversion detected in confidence deciles: {valid_deciles}"
            )
        else:
        ...
        overall_pass = all(
            result[1] for result in test_results[:2]
        )  # Only Brier and slope are critical
        ...
        timestamp = os.environ.get(
            "AUDIT_TS", datetime.now().strftime("%Y%m%dT%H%M%SZ")
        )
        ...
        plt.savefig(
            output_path / "reliability_diagram.png", dpi=150, bbox_inches="tight"
        )
        plt.close()
        ...
        pd.DataFrame(df_data).to_csv(
            output_path / "reliability_diagram.csv", index=False
        )
    ...
    def train_calibrators(self, calibration_ratio: float = 0.1) -> bool:
        ...
        try:
            train_data, cal_data = train_test_split(
                ...
                random_state=42,
                stratify=(
                    cal_data["actual_win"] if "actual_win" in cal_data.columns else None
                ),
            )
        except ValueError:
            # Fallback if stratification fails
            train_data, cal_data = train_test_split(
                cal_data, test_size=calibration_ratio, random_state=42
            )
        ...
        if "raw_win_prob" in cal_data.columns and "actual_win" in cal_data.columns:
            ...
            win_mask = ~(
                np.isnan(cal_data["raw_win_prob"]) | np.isnan(cal_data["actual_win"])
            )
            ...
            if len(win_probs) > 10:  # Minimum samples for calibration
                ...
                logger.info(
                    f"✅ Win probability calibrator trained on {len(win_probs)} samples"
                )
            else:
        ...
        if success:
            logger.info(
                "✅ Isotonic regression calibrators trained and saved successfully!"
            )

        return success

    def calibrate_probs(
        self, raw_win_prob: float, raw_place_prob: float
    ) -> Dict[str, float]:
        """
        ...
        result = {
            ...
            "calibration_available": self.win_calibrator is not None
            or self.place_calibrator is not None,
        }
        ...
        if self.place_calibrator is not None and not np.isnan(raw_place_prob):
            try:
                ...
                calibrated_place = self.place_calibrator.predict([clipped_place_prob])[
                    0
                ]
                result["calibrated_place_prob"] = float(
                    np.clip(calibrated_place, 0.0, 1.0)
                )
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                try:
                    ...
                    logger.debug(
                        "Predictions table not found, trying alternative approach"
                    )
                ...
                np.random.seed(42)
                results_df["raw_win_prob"] = np.random.beta(
                    2, 10, len(results_df)
                )  # Skewed towards lower probabilities
                results_df["raw_place_prob"] = np.random.beta(
                    3, 7, len(results_df)
                )  # Slightly higher than win prob
                ...
                results_df["raw_win_prob"] = np.clip(
                    results_df["raw_win_prob"] + win_boost, 0, 1
                )
                results_df["raw_place_prob"] = np.clip(
                    results_df["raw_place_prob"] + place_boost, 0, 1
                )

                logger.info(
                    f"Generated synthetic calibration data for {len(results_df)} predictions"
                )
                return results_df
    ...
    def _evaluate_calibration(self, cal_data: pd.DataFrame):
        ...
        if (
            self.win_calibrator is not None
            and "raw_win_prob" in cal_data.columns
            and "actual_win" in cal_data.columns
        ):
            ...
            if win_mask.sum() > 0:
                ...
                logger.info(
                    f"  Improvement: {((raw_brier - cal_brier) / raw_brier * 100):.2f}%"
                )

        # Evaluate place probability calibration
        if (
            self.place_calibrator is not None
            and "raw_place_prob" in cal_data.columns
            and "actual_place" in cal_data.columns
        ):
    ...
    def _load_calibrators(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"   Place calibrator: {'✅' if self.place_calibrator else '❌'}"
            )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(
            f"Calibrated place probability: {calibrated['calibrated_place_prob']:.3f}"
        )
