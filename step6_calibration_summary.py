...
def load_calibration_results(
    results_file: str = "step6_calibration_results_20250804_140736.csv",
) -> pd.DataFrame:
    """Load the calibration results CSV file."""
    try:
        results_df = pd.read_csv(results_file)
        logger.info(
            f"Loaded calibration results: {len(results_df)} samples from {len(results_df['race_id'].unique())} races"
        )
        return results_df
    except FileNotFoundError:
        logger.error(
            f"Results file {results_file} not found. Please run step6_calibrate_validate.py first."
        )
        return pd.DataFrame()
...
def analyze_probability_distribution(results_df: pd.DataFrame):
    ...
    print(f"\nWinners vs Losers:")
    print(
        f"  Winners - Mean prob: {winners['predicted_probability'].mean():.4f} ({winners['predicted_probability'].mean()*100:.2f}%)"
    )
    print(
        f"  Losers  - Mean prob: {losers['predicted_probability'].mean():.4f} ({losers['predicted_probability'].mean()*100:.2f}%)"
    )
    print(
        f"  Separation: {winners['predicted_probability'].mean() - losers['predicted_probability'].mean():.4f}"
    )
    ...
    print(
        f"  All races sum to ~1.0: {all(abs(s - 1.0) < 0.001 for s in prob_sum_by_race)}"
    )
...
def create_simple_calibration_plot(results_df: pd.DataFrame):
    ...
    try:
        ...
        print("-" * 60)
        print(
            f"{'Bin Range':<15} {'Count':<8} {'Avg Pred':<10} {'Avg Actual':<12} {'Calibration'}"
        )
        print("-" * 60)
        ...
        for i in range(n_bins):
            ...
            if in_bin.sum() > 0:
                ...
                calibration_status = (
                    "✅ Good"
                    if calibration_diff < 0.05
                    else "⚠️  Fair" if calibration_diff < 0.1 else "❌ Poor"
                )

                print(
                    f"{bin_lower:.2f}-{bin_upper:.2f}      {bin_count:<8} {avg_pred:<10.4f} {avg_actual:<12.4f} {calibration_status}"
                )
