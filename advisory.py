...
class AdvisoryGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None):
            ...
            if openai_live:
                try:
                    from services.gpt_service import (
                        GPTService,  # lazy import to avoid heavy deps when unused
                    )

                    self._gpt_service = GPTService()
                    self.openai_available = bool(
                        getattr(self._gpt_service, "gpt_available", False)
                    )
                except Exception:
                    self._gpt_service = None
                    self.openai_available = False
        ...
        try:
            self.openai_client = getattr(
                getattr(self._gpt_service, "_wrapper", None), "client", None
            )
        except Exception:
            self.openai_client = None

        logger.info(
            f"Advisory Generator initialized - OpenAI available: {self.openai_available}"
        )

    def generate_advisory(
        self, file_path: Optional[str] = None, data: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if file_path:
                ...
                return self._create_error_response(
                    "No file path or data provided", start_time
                )
                ...
                try:
                    ...
                    if derived_score is not None:
                        if existing is None:
                            ...
                            analysis_results["overall_quality_score"] = min(
                                existing, derived_score
                            )
                except Exception:
            ...
            messages = self._generate_messages(
                validation_result, analysis_results, data
            )

            # Create human-readable summary without invoking OpenAI in tests
            if not openai_live:
                human_readable = self._get_template_summary(
                    messages, validation_result, analysis_results
                )
                openai_used_flag = False
            else:
                if self.openai_available:
                    try:
                        human_readable = self._get_openai_summary(
                            messages, validation_result, analysis_results
                        )
                    except Exception:
                        ...
                        openai_used_flag = isinstance(
                            human_readable, str
                        ) and human_readable.startswith("AI Summary:")
                else:
                    human_readable = self._get_template_summary(
                        messages, validation_result, analysis_results
                    )
            ...
            if (
                openai_used_flag
                and isinstance(human_readable, str)
                and not human_readable.startswith("AI Summary:")
            ):
                human_readable = f"AI Summary: {human_readable}"

            # Generate JSON for downstream ML
            ml_json = self._create_ml_json(
                messages, validation_result, analysis_results
            )
            ...
            logger.info(
                f"Advisory generated successfully - {len(messages)} messages, "
    ...
    def _create_human_readable_summary(
        self,
        messages: List[Dict],
        validation_result: Optional[Dict],
        analysis_results: Optional[Dict],
        ...
        if self.openai_available:
            summary = self._get_openai_summary(
                messages, validation_result, analysis_results
            )
            return summary, True
        else:
    ...
    def _get_openai_summary(
        ...
        try:
            ...
            context = {
                ...
                "categories": {},
                "validation_status": (
                    validation_result.get("valid", True) if validation_result else True
                ),
                "quality_score": (
                    analysis_results.get("overall_quality_score", 100)
                    if analysis_results
                    else 100
                ),
            }
            ...
            for msg in messages:
                ...
                context["message_types"][msg_type] = (
                    context["message_types"].get(msg_type, 0) + 1
                )
                context["categories"][category] = (
                    context["categories"].get(category, 0) + 1
                )
...
"""
            ...
            return self._get_template_summary(
                messages, validation_result, analysis_results
            )

    def _get_template_summary(
        self,
        messages: List[Dict],
        validation_result: Optional[Dict],
        analysis_results: Optional[Dict],
        ...
        # Count message types
        critical_count = len(
            [m for m in messages if m["type"] == MessageType.CRITICAL.value]
        )
        warning_count = len(
    ...
    def _create_ml_json(
        ...
        return {
            ...
            "summary": {
                ...
                "info_count": len(
                    [m for m in messages if m["type"] == MessageType.INFO.value]
                ),
                "validation_passed": (
                    validation_result.get("valid", True) if validation_result else True
                ),
            ...
            "feature_flags": {
                "has_validation_errors": validation_result
                and not validation_result.get("valid", True),
                "has_quality_issues": analysis_results
                and analysis_results.get("total_issues_detected", 0) > 0,
                "low_quality_score": (
                    analysis_results.get("overall_quality_score", 100)
                    if analysis_results
                    else 100
