...
def parse_race_csv_correct(csv_path):
    ...
    for dog_name, data in participants.items():
        ...
        consistency = 1.0 / (1.0 + (max(positions) - min(positions))) if len(positions) > 1 else 0.0
        ...
        enriched_dog = {
            ...
            "confidence": min(0.9, 0.3 + (total * 0.05) + (win_rate * 0.3) + (place_rate * 0.2)),
...
def main():
    ...
    for dog in sorted(enriched_dogs, key=lambda x: x['box_number']):
        ...
        print(f"  Career: {hist['career_starts']} starts, {hist['career_wins']} wins, {hist['career_places']} places")
        print(f"  Win Rate: {hist['win_rate']:.1%}, Place Rate: {hist['place_rate']:.1%}")
