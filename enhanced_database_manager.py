...
class EnhancedDatabaseManager:
    ...
    def format_race_data(self, race_row: sqlite3.Row) -> Dict:
        ...
        for key, value in race_dict.items():
            if value is None or (isinstance(value, str) and value.lower() in ['nan', 'null', 'none', '']):
                race_dict[key] = None
    ...
    def generate_race_url(self, venue: str, race_date: str, race_number: int) -> Optional[str]:
    ...
    def get_paginated_races(self, page: int = 1, per_page: int = 20, filters: Dict = None) -> Dict:
        """Get paginated races with advanced filtering and optimization"""
        cache_key = self.get_cache_key('get_paginated_races', (page, per_page, str(filters or {})))
            ...
            with self.get_connection() as conn:
                ...
                if filters:
                    ...
                    if filters.get('has_winner'):
                        where_conditions.append("winner_name IS NOT NULL AND winner_name != '' AND winner_name != 'nan'")
                ...
                self.logger.info(f"Retrieved {len(result['races'])} paginated races (page {page})")
                return result
    ...
    def get_database_stats(self) -> Dict:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Date ranges
                cursor.execute("SELECT MIN(race_date), MAX(race_date) FROM race_metadata WHERE race_date IS NOT NULL")
                ...
                if quality_data[0] > 0:
                    stats['data_completeness'] = {
                        'winner_completion_rate': round((quality_data[1] / quality_data[0]) * 100, 2),
                        'track_condition_rate': round((quality_data[2] / quality_data[0]) * 100, 2),
                        'weather_data_rate': round((quality_data[3] / quality_data[0]) * 100, 2)
                    }
                else:
                    stats['data_completeness'] = {
    ...
    def search_races(self, query: str, filters: Dict = None, limit: int = 50) -> List[Dict]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                self.logger.info(f"Search returned {len(result)} races for query: {query}")
                return result
        ...
        try:
            with self.get_connection() as conn:
                ...
                for venue in venues:
                    ...
                    venue_dict['completion_rate'] = round(
                        (venue_dict['completed_races'] / venue_dict['total_races']) * 100, 2
                    ) if venue_dict['total_races'] > 0 else 0
    ...
    def get_training_data_stats(self) -> Dict:
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("SELECT COUNT(*) FROM race_metadata WHERE winner_name IS NOT NULL")
...
def create_enhanced_database_manager(db_path: str = "greyhound_racing_data.db") -> EnhancedDatabaseManager:
