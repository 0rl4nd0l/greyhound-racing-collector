...
def load_model_contract():
    ...
    if not contract_path.exists():
        print("❌ Model contract file not found. Run inspect_model_features.py first.")
        return None
...
def analyze_current_features():
    ...
    print("🔍 Analyzing current feature generation...")
    ...
    print(f"📊 Current feature builder generates {len(current_features)} features:")
...
def compare_features(expected_features, current_features):
    ...
    print(f"✅ Features correctly implemented: {len(common_features)}")
    for feature in sorted(common_features):
        print(f"   ✓ {feature}")

    print(
        f"\n❌ Missing features (model expects but not generated): {len(missing_features)}"
    )
    for feature in sorted(missing_features):
        print(f"   ❌ {feature}")

    print(
        f"\n⚠️ Extra features (generated but model doesn't use): {len(extra_features)}"
    )
...
def suggest_fixes(missing_features):
    ...
    for feature in sorted(missing_features):
        if feature in direct_mapping:
            print(f"✅ {feature}: {direct_mapping[feature]}")
        elif feature in already_implemented:
            print(
                f"🔄 {feature}: {already_implemented[feature]} - Check naming/mapping"
            )
        else:
            print(f"🚨 {feature}: NEW IMPLEMENTATION REQUIRED")

    print("\n📝 Key Actions Needed:")
    print(
        "1. Update feature_store.py to load the model contract and enforce column alignment"
    )
...
def main():
    print("🔍 Starting comprehensive feature gap analysis...")
    ...
    print(f"✅ Correctly implemented: {len(comparison['common'])}")
    print(f"❌ Missing: {len(comparison['missing'])}")
    print(f"⚠️ Extra: {len(comparison['extra'])}")

    if comparison["missing"]:
        print(
            f"\n🎯 Priority: Fix {len(comparison['missing'])} missing features to resolve model prediction failures"
        )
    else:
        print("\n🎉 All expected features are being generated!")
