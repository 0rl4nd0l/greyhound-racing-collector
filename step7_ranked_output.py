...
def process_win_probabilities(df):
    ...
    if total_probability > 0:
        dogs_data['win_probability_pct_rescaled'] = (dogs_data['win_probability_pct'] / total_probability * 100).round(1)
        ...
        if current_sum != 100.0:
            ...
            dogs_data.iloc[0, dogs_data.columns.get_loc('win_probability_pct_rescaled')] += diff
            dogs_data['win_probability_pct_rescaled'] = dogs_data['win_probability_pct_rescaled'].round(1)
    else:
        dogs_data['win_probability_pct_rescaled'] = dogs_data['win_probability_pct']
    
    # Create final output dataframe
    output_df = dogs_data[['dog_clean_name', 'rank', 'win_probability_pct_rescaled']].copy()
...
def output_results(df):
    ...
    print(f"\nSUMMARY STATISTICS:")
    print(f"Top 5 dogs account for {df.head(5)['WinProbability%'].sum():.1f}% of total probability")
    print(f"Highest probability: {df.iloc[0]['Dog']} ({df.iloc[0]['WinProbability%']:.1f}%)")
    print(f"Lowest probability: {df.iloc[-1]['Dog']} ({df.iloc[-1]['WinProbability%']:.1f}%)")
