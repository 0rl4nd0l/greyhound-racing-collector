...
class ModelValidator:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db", models_dir: str = "./model_registry/models"):
        ...
        for pattern in model_patterns:
            for model_file in self.models_dir.glob(pattern):
                try:
                    ...
                    scaler_file = model_file.parent / model_file.name.replace('_model.joblib', '_scaler.joblib')
                    ...
                    logger.info(f"   Found model: {model_file.name} ({model_info['file_size']} bytes)")
                except Exception as e:
    ...
    def load_historical_data(self, limit: int = 10000) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            ...
            logger.info(f"   Winners: {df['won'].sum()}, Non-winners: {len(df) - df['won'].sum()}")
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'race_date' in features.columns:
                features['race_date'] = pd.to_datetime(features['race_date'], errors='coerce')
            ...
            logger.info(f"✅ Feature preparation complete: {len(features.columns)} features")
    ...
    def validate_model(self, model_info: Dict[str, Any], features: pd.DataFrame, target: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if model_path.endswith('.joblib'):
                    ...
                    validation_result['issues'].append(f"Unsupported model format: {model_path}")
            ...
            if len(features) < 100:
                validation_result['issues'].append(f"Insufficient data for validation: {len(features)} samples")
                return validation_result
            ...
            try:
                ...
                logger.info(f"✅ Sample predictions successful: shape {sample_predictions.shape}")
            ...
            if actual_samples == expected_samples:
                ...
                validation_result['issues'].append(f"Output length mismatch: expected {expected_samples}, got {actual_samples}")
                return validation_result
            
            # Check 4: Probability range validation
            prob_min, prob_max = val_probs.min(), val_probs.max()
            if prob_min >= 0 and prob_max <= 1:
                logger.info(f"✅ Probability range valid: [{prob_min:.3f}, {prob_max:.3f}]")
                validation_result['checks']['probability_range'] = True
            else:
                validation_result['issues'].append(f"Invalid probability range: [{prob_min:.3f}, {prob_max:.3f}]")
            ...
            try:
                ...
                if np.allclose(val_probs, val_probs_2, rtol=1e-10):
                    ...
                    validation_result['issues'].append("Predictions are not deterministic")
            ...
            if model_type in baseline_metrics.get('model_metrics', {}):
                ...
                max_drift = baseline_metrics['validation_criteria']['max_drift_percentage']
                
                # Calculate drift percentages
                auc_drift = abs(roc_auc - baseline['roc_auc']) / baseline['roc_auc'] * 100
                loss_drift = abs(log_loss_score - baseline['log_loss']) / baseline['log_loss'] * 100
                acc_drift = abs(accuracy - baseline['accuracy']) / baseline['accuracy'] * 100
                ...
                if max_observed_drift <= max_drift:
                    logger.info(f"✅ Drift check passed: max drift {max_observed_drift:.2f}% <= {max_drift}%")
                    validation_result['checks']['drift_acceptable'] = True
                else:
                    validation_result['issues'].append(f"Drift too high: {max_observed_drift:.2f}% > {max_drift}%")
                    logger.warning(f"⚠️ Drift check failed: {max_observed_drift:.2f}% > {max_drift}%")
            else:
                logger.warning(f"⚠️ No baseline metrics found for model type: {model_type}")
                validation_result['drift_analysis'] = {'no_baseline': True}
            
            # Overall success check
            required_checks = ['model_loaded', 'can_predict', 'full_prediction', 'output_length', 'probability_range', 'metrics_calculated']
            passed_checks = sum(1 for check in required_checks if validation_result['checks'].get(check, False))
            
            if passed_checks == len(required_checks) and not validation_result['issues']:
                validation_result['success'] = True
                logger.info(f"✅ Model validation successful: {model_info['file_name']}")
            else:
                logger.warning(f"⚠️ Model validation incomplete: {passed_checks}/{len(required_checks)} checks passed")
    ...
    def simulate_race_prediction(self, model_info: Dict[str, Any], features: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(features) < num_dogs:
                return {'success': False, 'error': f'Not enough data: need {num_dogs}, have {len(features)}'}
                ...
                'predictions': [
                    {
                        ...
                        'predicted_rank': int(np.argsort(np.argsort(-win_probs))[i]) + 1
                    }
            ...
            if not (criteria['min_dogs_per_race'] <= num_dogs <= criteria['max_dogs_per_race']):
                issues.append(f"Invalid number of dogs: {num_dogs}")
            
            # Check probability sum
            if not (criteria['min_probability_sum'] <= prob_sum <= criteria['max_probability_sum']):
            ...
            if race_result['validation_passed']:
                logger.info(f"✅ Race simulation successful: {num_dogs} dogs, prob_sum={prob_sum:.3f}")
            else:
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        for model_info in models:
            ...
            race_simulation = self.simulate_race_prediction(model_info, prepared_features)
        ...
        logger.info(f"Success rate: {validation_results['summary']['success_rate']:.1f}%")
        logger.info(f"Total time: {validation_results['summary']['total_validation_time']:.2f}s")
        logger.info(f"Overall result: {'✅ PASSED' if validation_results['summary']['overall_success'] else '❌ FAILED'}")
        
        return validation_results

    def save_validation_report(self, results: Dict[str, Any], output_file: str = "ml_pipeline_validation_report.json"):
...
def main():
    ...
    parser.add_argument("--models-dir", default="./model_registry/models", help="Models directory")
    parser.add_argument("--output", default="ml_pipeline_validation_report.json", help="Output report file")
