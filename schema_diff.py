...
def parse_create_table(sql_statement):
    ...
    table_name_match = re.search(r'CREATE TABLE IF NOT EXISTS (\w+)', sql_statement, re.IGNORECASE)
    ...
    for line in lines[start_idx+1:end_idx]:
        ...
        elif parts[0].upper() not in ['PRIMARY', 'UNIQUE', 'CONSTRAINT'] and len(parts) >= 2:
...
def parse_create_index(sql_statement):
    ...
    match = re.search(r'CREATE INDEX IF NOT EXISTS (\w+) ON (\w+)\s*\((.*?)\)', sql_statement, re.IGNORECASE)
    if match:
        return {
            'name': match.group(1),
            'table': match.group(2),
...
def load_expected_schema_from_multiple_sources():
    ...
    create_table_matches = re.findall(r'cursor\.execute\(\'\'\'\s*(.*?)\s*\'\'\'\)', init_content, re.DOTALL | re.IGNORECASE)
    ...
    unified_matches = re.findall(r'cursor\.executescript\("""(.*?)"""\)', unified_content, re.DOTALL | re.IGNORECASE)
...
def compare_schemas(expected_schema, actual_schema):
    ...
    for table_name in expected_tables.intersection(actual_tables):
        ...
        table_diff['missing_columns'] = [expected_cols[name] for name in missing_col_names]
        ...
        for col_name in set(expected_cols.keys()).intersection(set(actual_cols.keys())):
            ...
            if expected_type != actual_type and not (expected_type == 'DECIMAL' and 'DECIMAL' in actual_type):
                mismatches.append(f"type mismatch (expected: {expected_type}, actual: {actual_type})")
            
            if expected_col['notnull'] != actual_col['notnull']:
                mismatches.append(f"nullability mismatch (expected: {expected_col['notnull']}, actual: {actual_col['notnull']})")
                
            if expected_col['pk'] != actual_col['pk']:
                 mismatches.append(f"PK mismatch (expected: {expected_col['pk']}, actual: {actual_col['pk']})")
        ...
        expected_fks = [(fk['from'], fk['table'], fk['to']) for fk in expected_table['foreign_keys']]
        actual_fks = [(fk['from'], fk['table'], fk['to']) for fk in actual_table['foreign_keys']]
        
        table_diff['missing_foreign_keys'] = [fk for fk in expected_fks if fk not in actual_fks]
        table_diff['extra_foreign_keys'] = [fk for fk in actual_fks if fk not in expected_fks]
        
        # Compare indexes
        expected_idxs = [(idx['name'], tuple(sorted(idx['columns']))) for idx in expected_table['indexes']]
        actual_idxs_raw = actual_table.get('indexes', [])
        actual_idxs = [(idx['name'], tuple(sorted([c['name'] for c in idx['columns']]))) for idx in actual_idxs_raw if not idx['name'].startswith('sqlite_autoindex')]
        
        table_diff['missing_indexes'] = [idx for idx in expected_idxs if idx not in actual_idxs]
        ...
        for table, table_diff in diff['table_diffs'].items():
            ...
            if table_diff['column_mismatches']:
                ...
                for mismatch in table_diff['column_mismatches']:
                    report.append(f"- **`{mismatch['column']}`**: { ', '.join(mismatch['mismatches']) }")
...
def generate_migration_sql(diff, sql_path):
    ...
    for table, table_diff in diff['table_diffs'].items():
        if table_diff['missing_columns']:
            ...
            for col in table_diff['missing_columns']:
                sql.append(f"ALTER TABLE {table} ADD COLUMN {col['name']} {col['type']};")

        if table_diff['missing_indexes']:
            sql.append(f"\n-- Create missing indexes for {table}")
            for idx in table_diff['missing_indexes']:
                sql.append(f"CREATE INDEX IF NOT EXISTS {idx[0]} ON {table} ({ ', '.join(idx[1]) });")
...
if __name__ == "__main__":
    ...
    print(f"Expected schema found {len(expected_schema)} tables: {list(expected_schema.keys())}")
