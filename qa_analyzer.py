...
class QAAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"🔍 QA Analyzer initialized - confidence_threshold={confidence_threshold}, "
    ...
    def analyze_low_confidence_and_variance(
        ...
        try:
            ...
            if "predictions" not in predictions:
                return self._create_error_result(
                    "No predictions found in input", analysis_start
                )

            pred_data = predictions["predictions"]
            if not pred_data:
                return self._create_error_result(
                    "Empty predictions list", analysis_start
                )
            ...
            if "win_prob" not in df.columns:
                ...
                if "win_prob" not in df.columns:
                    return self._create_error_result(
                        "No win_prob column found", analysis_start
                    )
            ...
            result = {
                ...
                "low_confidence_percentage": float(
                    low_confidence_count / len(df) * 100
                ),
                ...
                "processing_time_ms": (datetime.now() - analysis_start).total_seconds()
                * 1000,
            }
        ...
        try:
            ...
            result = {
                ...
                "dominant_runner": {
                    ...
                    "dog_name": df.iloc[dominant_runner_idx].get(
                        "dog_name", f"Dog_{dominant_runner_idx}"
                    ),
    ...
    def analyze_calibration_drift(
        self, predictions: Dict[str, Any], actual_outcomes: Optional[List[int]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if rolling_brier_scores:
                recent_mean_brier = np.mean(
                    rolling_brier_scores[-self.calibration_window :]
                )
            ...
            result = {
                ...
                "recent_mean_brier": (
                    float(recent_mean_brier) if recent_mean_brier is not None else None
                ),
                ...
                "issues_detected": significant_drift
                or abs(calibration_slope - 1.0) > 0.3,
                "processing_time_ms": (datetime.now() - analysis_start).total_seconds()
                * 1000,
            }
    ...
    def detect_leakage_and_date_drift(
        self, predictions: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if race_datetime:
                ...
                if race_datetime > current_time + timedelta(days=max_future_days):
                    ...
                    errors.append(
                        f"Race date is too far in the future: {race_datetime} (max {max_future_days} days ahead)"
                    )
            ...
            if extraction_datetime:
                ...
                if extraction_datetime > current_time:
                    ...
                    errors.append(
                        f"Extraction time is in the future: {extraction_datetime}"
                    )
                ...
                if extraction_datetime < current_time - timedelta(days=max_age_days):
                    warnings.append(
                        f"Extraction time is very old: {extraction_datetime} (>{max_age_days} days ago)"
                    )
            ...
            if race_datetime and extraction_datetime:
                ...
                if extraction_datetime > race_datetime + timedelta(hours=2):
                    ...
                    errors.append(
                        f"Extraction time ({extraction_datetime}) is significantly after race time ({race_datetime})"
                    )
                    ...
                    if "win_prob" in pred:
                        ...
                        if prob < 0 or prob > 1:
                            data_quality_issues.append(
                                f"Invalid probability for prediction {i}: {prob}"
                            )
                        elif prob == 0 or prob == 1:
                            warnings.append(
                                f"Extreme probability for prediction {i}: {prob}"
                            )
            ...
            if "predictions" in predictions:
                ...
                for i, pred in enumerate(pred_data):
                    if (
                        "finish_position" in pred
                        and pred["finish_position"] is not None
                    ):
                        leakage_indicators.append(
                            f"Finish position present in prediction {i} - potential leakage"
                        )
                    if "actual_time" in pred and pred["actual_time"] is not None:
                        leakage_indicators.append(
                            f"Actual race time present in prediction {i} - potential leakage"
                        )
            ...
            result = {
                ...
                "extraction_datetime": (
                    extraction_datetime.isoformat() if extraction_datetime else None
                ),
                ...
                "temporal_analysis": {
                    "race_age_hours": (
                        ((current_time - race_datetime).total_seconds() / 3600)
                        if race_datetime
                        else None
                    ),
                    "extraction_age_hours": (
                        ((current_time - extraction_datetime).total_seconds() / 3600)
                        if extraction_datetime
                        else None
                    ),
                    "extraction_to_race_hours": (
                        ((race_datetime - extraction_datetime).total_seconds() / 3600)
                        if race_datetime and extraction_datetime
                        else None
                    ),
                },
    ...
    def comprehensive_qa_analysis(
        ...
        try:
            logger.info(
                f"🔍 Starting comprehensive QA analysis for race: {predictions.get('race_id', 'unknown')}"
            )
            ...
            if not self.lightweight_mode:
                calibration_analysis = self.analyze_calibration_drift(
            ...
            logger.info(
                f"✅ QA analysis complete - Quality Score: {quality_score}/100, Issues: {len(all_issues)}"
            )
    ...
    def _match_actual_outcomes(
        ...
        try:
            ...
            race_outcomes = self.stored_outcomes[
                self.stored_outcomes["race_id"] == race_id
            ]
            ...
            for pred in predictions.get("predictions", []):
                ...
                if dog_name:
                    ...
                    if not match.empty:
                        outcomes.append(
                            1 if match.iloc[0]["finish_position"] == 1 else 0
                        )
                    else:
    ...
    def _calculate_rolling_brier_scores(self) -> List[float]:
        ...
        try:
            ...
            for race_id in self.stored_outcomes["race_id"].unique()[
                :50
            ]:  # Last 50 races
                race_data = self.stored_outcomes[
                    self.stored_outcomes["race_id"] == race_id
                ]
    ...
    def _calculate_calibration_slope(
        self, predicted_probs: np.ndarray, actual_outcomes: List[int]
    ) -> float:
    ...
    def _generate_recommendations(self, issues: List[str]) -> List[str]:
        ...
        if "low_confidence_variance" in issues:
            recommendations.append(
                "Review model training data and feature engineering for low-confidence predictions"
            )
            recommendations.append(
                "Consider ensemble methods to improve prediction confidence"
            )

        if "class_imbalance" in issues:
            recommendations.append(
                "Check for data quality issues causing extreme probability distributions"
            )
            recommendations.append(
                "Review odds scaling and probability normalization methods"
            )

        if "calibration_drift" in issues:
            recommendations.append("Retrain or recalibrate prediction models")
            recommendations.append(
                "Investigate changes in underlying data distribution"
            )

        if "leakage_date_drift" in issues:
            recommendations.append(
                "Review data extraction and feature engineering pipelines for temporal leakage"
            )
            recommendations.append("Validate data timestamps and processing workflows")

        if not issues:
            recommendations.append(
                "Prediction quality looks good - continue monitoring"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="QA Analyzer for prediction quality assessment"
    )
    parser.add_argument(
        "--db-path", default="greyhound_racing_data.db", help="Database path"
    )
    parser.add_argument(
        "--confidence-threshold", type=float, default=0.3, help="Confidence threshold"
    )
    parser.add_argument(
        "--variance-threshold", type=float, default=0.05, help="Variance threshold"
    )
