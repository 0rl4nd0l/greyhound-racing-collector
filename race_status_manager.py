...
class RaceStatusManager:
    ...
    def get_status_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "completion_rate": complete_races / total_races if total_races > 0 else 0,
    ...
    def update_race_status(self, race_id: str, new_status: str, winner_name: str = None, 
                          winner_source: str = None, note: str = None) -> bool:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM race_metadata WHERE race_id = ?", (race_id,))
    ...
    def bulk_update_status(self, filter_criteria: Dict[str, Any], new_status: str, 
                          winner_source: str = None, note: str = None) -> int:
        ...
        try:
            ...
            # Confirm bulk update
            print(f"⚠️  About to update {count_to_update} races to status '{new_status}'")
            ...
            if note is not None:
                updates.append("data_quality_note = ?")
                update_values.insert(-len(values) if values else 1, note)  # Insert before WHERE values
            
            update_query = f"UPDATE race_metadata SET {', '.join(updates)} WHERE {where_clause}"
    ...
    def print_status_report(self):
        ...
        print(f"   Complete: {overview['complete_races']:,} ({overview['completion_rate']:.1%})")
        ...
        if problems:
            ...
            for problem in problems[:15]:
                print(f"   {problem['race_id']}: {problem['issue_type']} - {problem['details']}")
        ...
        if overview['has_winner_but_pending'] > 0:
            print(f"   🔧 {overview['has_winner_but_pending']} races have winners but wrong status")
...
def main():
    ...
    update_parser.add_argument('status', choices=['pending', 'complete', 'partial_scraping_failed'], 
                             help='New status')
    update_parser.add_argument('--winner', help='Winner name')
    update_parser.add_argument('--source', choices=['scrape', 'inferred', 'manual'], help='Winner source')
    ...
    bulk_parser.add_argument('status', choices=['pending', 'complete', 'partial_scraping_failed'],
                           help='New status')
    bulk_parser.add_argument('--venue', help='Filter by venue')
    bulk_parser.add_argument('--current-status', help='Filter by current status')
    bulk_parser.add_argument('--min-attempts', type=int, help='Minimum scraping attempts')
    bulk_parser.add_argument('--max-attempts', type=int, help='Maximum scraping attempts')
    bulk_parser.add_argument('--from-date', help='From date (YYYY-MM-DD)')
    bulk_parser.add_argument('--to-date', help='To date (YYYY-MM-DD)')
    bulk_parser.add_argument('--source', choices=['scrape', 'inferred', 'manual'], help='Winner source')
    bulk_parser.add_argument('--note', help='Data quality note')
    
    # Fix common issues
    fix_parser = subparsers.add_parser('fix-winner-status', help='Fix races with winners but wrong status')
    
    # Show problematic races
    problems_parser = subparsers.add_parser('problems', help='Show races needing attention')
    problems_parser.add_argument('--limit', type=int, default=50, help='Maximum races to show')
    ...
    if args.command == 'status':
        ...
        manager.update_race_status(
            args.race_id, args.status, 
            args.winner, args.source, args.note
        )
        ...
        count = manager.bulk_update_status(criteria, 'complete', 'inferred', 
