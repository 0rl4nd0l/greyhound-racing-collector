...
def get_file_categories():
    ...
    return {
        'tools': {
            'patterns': [
                'fix_*.py', 'check_*.py', 'repair_*.py', 'cleanup_*.py',
                'analyze_*.py', 'validate_*.py', 'assess_*.py'
            ],
            'description': 'Utility and maintenance tools'
        },
        'scripts': {
            'patterns': [
                'create_*.py', 'setup_*.py', 'run_*.py', 'start_*.py',
                'batch_*.py', 'bulk_*.py', 'daily_*.py'
            ],
            'description': 'Operational scripts'
        },
        'tests': {
            'patterns': [
                'test_*.py', '*_test.py', 'conftest.py', '*TESTING*.py'
            ],
            'description': 'Test files'
        },
        'archive': {
            'patterns': [
                'old_*.py', 'legacy_*.py', 'deprecated_*.py', 'backup_*.py',
                'temp_*.py', 'experimental_*.py', '*_old.py', '*_backup.py'
            ],
...
def should_archive_file(filename, content_sample=""):
    ...
    archive_indicators = [
        # Filename patterns
        'old_', 'legacy_', 'deprecated_', 'backup_', 'temp_',
        '_old', '_backup', '_v1', '_v2',
        'experimental_', 'test_old', 'draft_',
        
        # Content-based (if we have sample)
        'TODO: Remove this', 'DEPRECATED', 'OBSOLETE'
    ]
...
def cleanup_repository():
    ...
    if categorized['archive']:
        archive_dir = f"archive/scripts/cleanup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
