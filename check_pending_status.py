    ...
    try:
        ...
        if by_venue:
            ...
            for venue, count in sorted(
                by_venue.items(), key=lambda x: x[1], reverse=True
            ):
        ...
        if by_attempts:
            ...
            for attempts, count in sorted(by_attempts.items()):
                if attempts == 0:
                    ...
                    print(
                        f"   {attempts} attempt{'s' if attempts > 1 else ''}: {count} races"
                    )
            print()
        ...
        if args.detailed:
            ...
            if recent_sample:
                ...
                for race in recent_sample:
                    ...
                    attempts_str = (
                        f"{attempts} attempts" if attempts > 0 else "not attempted"
                    )
                    print(
                        f"   📅 {race_date}: {venue} Race {race_number} - {attempts_str}"
                    )
                print()
        ...
        if total_pending > 0:
            print(
                f"   🔄 Run backfill process to attempt scraping winners for {total_pending} pending races"
            )
            print(
                f"   ⚙️  Command: python run_backfill.py --max-races {min(total_pending, 50)}"
            )
            ...
            if max_attempts >= 3:
                ...
                print(
                    f"   ⚠️  {max_attempt_count} races have reached maximum attempts ({max_attempts})"
                )
                print(
                    f"   🔧 Consider increasing max-retries or manual intervention for these races"
                )
