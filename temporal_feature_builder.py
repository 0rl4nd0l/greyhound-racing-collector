...
class TemporalFeatureBuilder:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db"):
        ...
        try:
            ...
            enable_results_scrapers = os.getenv("ENABLE_RESULTS_SCRAPERS", "0") not in (
                "0",
                "false",
                "False",
            )
        ...
        if allow_tgr and not use_scraper and not _tgr_tables_present(self.db_path):
            ...
            allow_tgr = False
            logger.info(
                "ℹ️ TGR integration disabled automatically: required DB tables not found (DB-only mode)"
            )
        if TGRPredictionIntegrator and allow_tgr:
            try:
                ...
                reason = (
                    "TGR_ENABLED=1"
                    if tgr_flag
                    else f"mode={mode}, ENABLE_RESULTS_SCRAPERS={enable_results_scrapers}"
                )
                if use_scraper:
                    logger.info(
                        f"✅ TGR integration initialized (scraper enabled) via {reason}"
                    )
                else:
                    logger.info(
                        f"✅ TGR integration initialized in DB-only mode via {reason}"
                    )
                # Cache full TGR feature name set for consistent inclusion
                try:
                    self._tgr_all_feature_names = list(
                        self.tgr_integrator.get_feature_names()
                    )
                except Exception:
                    self._tgr_all_feature_names = []
    ...
    def create_historical_features(
        ...
        if "distance" in historical_data.columns:
            try:
                historical_data["distance_numeric"] = historical_data["distance"].apply(
                    self._to_meters
                )
            except Exception:
        ...
        if target_venue and target_venue.upper() in [
            ...
            weights = weights * (1 + venue_boost)
            logger.debug(
                f"Applied venue boost to {venue_boost.sum()} races for {target_venue}"
            )
        ...
        if target_grade:
            ...
            logger.debug(
                f"Applied grade boost to {grade_boost.sum()} races for grade {target_grade}"
            )
        ...
        if target_distance and "distance" in historical_data.columns:
            ...
            distance_matches = (
                abs(historical_data["distance_numeric"] - target_distance)
                <= distance_tolerance
            ).astype(float)
            distance_boost = distance_matches * 0.7
            weights = weights * (1 + distance_boost)
            logger.debug(
                f"Applied distance boost to {distance_boost.sum()} races for distance {target_distance}m"
            )

        weights = weights / weights.sum()  # Normalize weights

        logger.debug(
            f"Total contextual weight adjustments applied to {len(weights)} historical races"
        )
        ...
        valid_positions = historical_data["finish_position_numeric"][
            valid_position_mask
        ]
        ...
        if len(valid_times) > 0:
            ...
            if target_distance:
                ...
                valid_time_distances = historical_data["distance_numeric"][
                    valid_time_mask
                ]
                ...
                for i, (time_val, distance_val, weight_val) in enumerate(
                    zip(valid_times, valid_time_distances, time_weights)
                ):
                    if pd.notna(distance_val) and distance_val > 0:
                        adjusted_time = adjust_time_for_distance(
                            time_val, distance_val, target_distance
                        )
                ...
                logger.debug(
                    f"Applied distance adjustment for target distance {target_distance}m to {len(adjusted_times)} times"
                )
            else:
        ...
        if "distance" in historical_data.columns:
            ...
            for distance in historical_data["distance"].unique():
                if pd.notna(distance):
                    dist_races = historical_data[
                        historical_data["distance"] == distance
                    ]
                    if len(dist_races) > 0:
                        dist_positions = pd.to_numeric(
                            dist_races["finish_position"], errors="coerce"
                        ).dropna()
            ...
            if distance_performance:
                best_distance = min(
                    distance_performance.items(), key=lambda x: x[1]["avg_position"]
                )
                features["best_distance_avg_position"] = best_distance[1][
                    "avg_position"
                ]
    ...
    def build_features_for_race(
        ...
        for _, dog_row in race_data.iterrows():
            ...
            csv_features = {}
            try:
                csv_races = pd.to_numeric(
                    dog_row.get("csv_historical_races"), errors="coerce"
                )
                if pd.notna(csv_races) and int(csv_races) > 0:
                    if pd.notna(dog_row.get("csv_avg_finish_position")):
                        csv_features["historical_avg_position"] = float(
                            dog_row.get("csv_avg_finish_position")
                        )
                    if pd.notna(dog_row.get("csv_best_finish_position")):
                        csv_features["historical_best_position"] = float(
                            dog_row.get("csv_best_finish_position")
                        )
                    if pd.notna(dog_row.get("csv_win_rate")):
                        csv_features["historical_win_rate"] = float(
                            dog_row.get("csv_win_rate")
                        )
                    if pd.notna(dog_row.get("csv_place_rate")):
                        csv_features["historical_place_rate"] = float(
                            dog_row.get("csv_place_rate")
                        )
                    if pd.notna(dog_row.get("csv_avg_time")):
                        csv_features["historical_avg_time"] = float(
                            dog_row.get("csv_avg_time")
                        )
                    if pd.notna(dog_row.get("csv_best_time")):
                        # Best time may be string like "30.12"; coerce to float if possible
                        try:
                            csv_features["historical_best_time"] = float(
                                dog_row.get("csv_best_time")
                            )
                        except Exception:
                            pass
            except Exception:
            ...
            historical_data = self.load_dog_historical_data(
                dog_row["dog_clean_name"], target_timestamp
            )
            ...
            if csv_features:
                features.update(
                    {k: v for k, v in csv_features.items() if v is not None}
                )
            ...
            if self.tgr_integrator and getattr(self, "_tgr_runtime_enabled", False):
                try:
                    ...
                    try:
                        expected_tgr = (
                            self._tgr_all_feature_names
                            or self.tgr_integrator.get_feature_names()
                        )
                    ...
                    logger.debug(
                        f"Added {len(tgr_features)} TGR features for {dog_row['dog_clean_name']}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to get TGR features for {dog_row['dog_clean_name']}: {e}"
                    )
        ...
        logger.info(
            f"Built features for {len(result_df)} dogs in race {target_race_id}"
        )
    ...
    def assert_no_target_leakage(features: Dict[str, Any], race_id: str, dog_name: str):
        ...
        if race_date_str:
            try:
                ...
                logger.debug(
                    f"Could not parse race date '{race_date_str}' for race {race_id}: {e}"
                )
