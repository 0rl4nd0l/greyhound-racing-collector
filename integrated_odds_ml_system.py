...
class IntegratedOddsMLSystem:
                    ...
                    if 'predictions' in data:
                        for pred in data['predictions']:
                            predictions_data.append({
                                ...
                                'dog_clean_name': self.clean_dog_name(pred.get('dog_name', '')),
                                'predicted_probability': pred.get('probability', pred.get('confidence', 0)),
                                'confidence_score': pred.get('confidence', 0),
                                'model_features': pred.get('features', {}),
                                'prediction_timestamp': data.get('timestamp', datetime.now().isoformat())
                            })
                except Exception as e:
                    self.logger.warning(f"Failed to load prediction file {file_path}: {e}")
        ...
        for url in race_urls:
            try:
                ...
                if metadata['success'] and odds_df is not None:
                    ...
                    odds_df['dog_clean_name'] = odds_df['selection_name'].apply(self.clean_dog_name)
                    ...
                    self.logger.warning(f"Failed to scrape odds from {url}: {metadata.get('error_message', 'Unknown error')}")
    ...
    def identify_value_opportunities(self, predictions_df: pd.DataFrame, odds_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate value
        merged_df['value_percentage'] = (merged_df['predicted_probability'] - merged_df['implied_probability']) * 100
        merged_df['kelly_fraction'] = np.maximum(
            (merged_df['predicted_probability'] * merged_df['odds'] - 1) / (merged_df['odds'] - 1),
            0
        )
        merged_df['recommended_stake'] = np.minimum(merged_df['kelly_fraction'] * 0.25, 0.05)  # Max 5% stake
        ...
        if not value_bets.empty:
            ...
            self.logger.info(f"Identified {len(value_bets)} value betting opportunities")
    ...
    def run_complete_analysis(self, race_urls: List[str]) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Analysis complete: {results['value_opportunities']} opportunities found")
    ...
    def send_value_alerts(self, value_bets_df: pd.DataFrame):
        ...
        high_value_bets = value_bets_df[value_bets_df['value_percentage'] > 10]  # >10% edge
    ...
    def start_automated_monitoring(self, race_urls: List[str], interval_minutes: int = 15):
        ...
        self.logger.info(f"Started automated monitoring every {interval_minutes} minutes")
    ...
    def get_performance_report(self, days: int = 7) -> Dict:
        ...
        report = {
            ...
            'avg_value_percentage': opportunities_df['value_percentage'].mean() if not opportunities_df.empty else 0,
            'high_value_bets': len(opportunities_df[opportunities_df['value_percentage'] > 10]),
            'sportsbooks_monitored': opportunities_df['sportsbook'].nunique() if not opportunities_df.empty else 0,
            'top_dogs': opportunities_df['dog_clean_name'].value_counts().head(5).to_dict() if not opportunities_df.empty else {}
        }
...
def demo_integrated_system():
    ...
    if results['top_bets']:
        ...
        for i, bet in enumerate(results['top_bets'][:3], 1):
            print(f"   {i}. {bet.get('dog_name', 'Unknown')}: {bet.get('value_percentage', 0):.1f}% value")
