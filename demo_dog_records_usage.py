...
def compare_dogs(dog_records):
    ...
    for dog_name, dog_data in dog_records.items():
        ...
        comparison_data.append({"dog_name": dog_name, **analysis})
    ...
    for i, (_, dog) in enumerate(ranking.iterrows(), 1):
        print(
            f"{i}. {dog['dog_name']} - Avg Place: {dog['avg_place']:.2f}, "
...
def demonstrate_per_dog_view():
    ...
    if dog_records:
        ...
        print(
            sample_data[
                ["race_date", "place", "track_code", "race_time", "starting_price"]
            ].head()
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists("dog_records") or not os.listdir("dog_records"):
        ...
        print(
            "Please run 'python process_upcoming_races.py' first to generate the data."
        )
        exit(1)
