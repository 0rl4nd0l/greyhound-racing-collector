...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "expert_selections", sa.Text(), nullable=True
        ),  # JSON stored as TEXT in SQLite
        sa.Column(
            "confidence_ratings", sa.Text(), nullable=True
        ),  # JSON stored as TEXT in SQLite
        sa.Column(
            "key_insights", sa.Text(), nullable=True
        ),  # JSON stored as TEXT in SQLite
        ...
        sa.Column(
            "data_source", sa.Text(), nullable=True, server_default="fasttrack_expert"
        ),
        sa.Column(
            "processing_status", sa.Text(), nullable=True, server_default="pending"
    ...
    op.create_foreign_key(
        "fk_expert_form_analysis_race_id",
        "expert_form_analysis",
        "races",
        ["race_id"],
        ["id"],
    ...
    op.create_index(
        "idx_expert_form_analysis_race_id", "expert_form_analysis", ["race_id"]
    )
    op.create_index(
        "idx_expert_form_analysis_expert_name", "expert_form_analysis", ["expert_name"]
    )
    op.create_index(
        "idx_expert_form_analysis_analysis_date",
        "expert_form_analysis",
        ["analysis_date"],
    )
    op.create_index(
        "idx_expert_form_analysis_processing_status",
        "expert_form_analysis",
        ["processing_status"],
    )
...
SAMPLE_EXPERT_SELECTIONS = {
    ...
    "exotic_bets": {"first_four": [[1, 4, 6, 8]], "running_double": [1, 3]},
}
