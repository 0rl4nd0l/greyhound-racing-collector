...
Examples:
  ...
  python batch_prediction_cli.py --list-jobs
    ...
    parser.add_argument(
        "--input", "-i", help="Input directory containing CSV files", type=str
    )

    parser.add_argument("--output", "-o", help="Output directory for results", type=str)
    ...
    parser.add_argument(
        "--job-status", help="Check status of a specific job by ID", type=str
    )

    parser.add_argument("--cancel-job", help="Cancel a running job by ID", type=str)

    parser.add_argument("--list-jobs", help="List all batch jobs", action="store_true")
    ...
    parser.add_argument(
        "--quiet", "-q", help="Suppress progress output", action="store_true"
    )
...
def progress_callback(job):
    ...
    if not args.quiet:
        ...
        print(
            f"\r🚀 Job {job.job_id[:8]}: [{progress_bar}] {job.progress:.1f}% "
...
def main():
    ...
    if not any([args.input, args.job_status, args.cancel_job, args.list_jobs]):
        parser.error(
            "Must specify --input for batch processing, or use job management options"
        )
    ...
    try:
        ...
        if args.list_jobs:
            ...
            if args.format == "json":
                output = json.dumps(
                    [job.__dict__ for job in jobs], indent=2, default=str
                )
                print(output)
            else:
                if not jobs:
                    ...
                    for job in jobs:
                        ...
                        print(
                            f"{status_icon} {job.job_id[:12]} | {job.name} | {job.status} | "
                            f"{job.completed_files}/{job.total_files} files"
                        )
            return
