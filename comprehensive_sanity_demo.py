...
class PredictionProcessor:
    ...
    def process_predictions(self, predictions: List[Dict], auto_fix: bool = True) -> Dict:
        """
        ...
        if validation_results['flags'] and auto_fix:
            ...
            fixed_validation = self.sanity_checker.validate_predictions(fixed_predictions)
            result['fixed_validation_results'] = fixed_validation
            
            if fixed_validation['flags']:
                logger.warning(f"⚠️  {len(fixed_validation['flags'])} issues remain after fixes")
            else:
        ...
        result['processing_summary'] = {
            ...
            'issues_remaining': len(result.get('fixed_validation_results', {}).get('flags', [])),
    ...
    def process_prediction_file(self, file_path: str, save_fixed: bool = True) -> Dict:
        ...
        try:
            ...
            if save_fixed and result['fixes_applied']:
                ...
                logger.info(f"💾 Fixed predictions saved to: {Path(fixed_file_path).name}")
    ...
    def batch_process_directory(self, directory_path: str, max_files: int = 10) -> List[Dict]:
        """
        ...
        logger.info(f"🗂️  Found {len(json_files)} JSON files, processing {len(json_files)}...")
...
def demonstrate_all_scenarios():
    ...
    perfect_predictions = [
        {'dog_name': 'Champion', 'win_probability': 0.4, 'place_probability': 0.7, 'predicted_rank': 1},
        {'dog_name': 'Runner-up', 'win_probability': 0.3, 'place_probability': 0.6, 'predicted_rank': 2},
        {'dog_name': 'Third Place', 'win_probability': 0.2, 'place_probability': 0.5, 'predicted_rank': 3},
        {'dog_name': 'Fourth', 'win_probability': 0.1, 'place_probability': 0.4, 'predicted_rank': 4}
    ]
    ...
    problematic_predictions = [
        {'dog_name': 'Bad Prob', 'win_probability': 1.5, 'place_probability': -0.1, 'predicted_rank': 2},
        {'dog_name': 'Duplicate Rank', 'win_probability': 0.8, 'place_probability': 0.9, 'predicted_rank': 1},
        {'dog_name': 'Wrong Rank', 'win_probability': 0.9, 'place_probability': 0.8, 'predicted_rank': 1},
        {'dog_name': 'NaN Issue', 'win_probability': float('nan'), 'place_probability': 0.5, 'predicted_rank': 3}
    ]
