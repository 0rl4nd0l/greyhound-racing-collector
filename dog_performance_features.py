...
class DogPerformanceFeatureEngineer:
    ...
    def extract_all_dog_features(self, dog_names: List[str] = None) -> pd.DataFrame:
        ...
        logger.info(f"Extracted features for {len(features_df)} dogs with {len(features_df.columns)} features each")
        return features_df
    ...
    def _calculate_time_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        features['time_variance'] = float(time_data.var()) if len(time_data) > 1 else 0.0
        features['time_range'] = features['worst_race_time'] - features['best_race_time']
    ...
    def _calculate_positional_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'margin' in data.columns:
            ...
            features['avg_beaten_lengths'] = float(max(0, (features['mean_position'] - 1) * 2.5))
    ...
    def _calculate_margin_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'margin' in data.columns:
            ...
            if len(losing_races) > 0:
                losing_margins = pd.to_numeric(losing_races['margin'], errors='coerce').dropna()
            ...
            if len(winning_races) > 0:
                winning_margins = pd.to_numeric(winning_races['margin'], errors='coerce').dropna()
            ...
            if len(margin_data) > 0:
                features['margin_std'] = float(margin_data.std()) if len(margin_data) > 1 else 0.0
                features['best_winning_margin'] = float(margin_data.min()) if len(margin_data) > 0 else 0.0
                features['worst_losing_margin'] = float(margin_data.max()) if len(margin_data) > 0 else 10.0
            else:
    ...
    def _calculate_early_speed_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'first_section_time' in data.columns:
            data['first_section_clean'] = pd.to_numeric(data['first_section_time'], errors='coerce')
            ...
            if len(first_section_data) > 0:
                ...
                features['first_section_consistency'] = float(1.0 / (1.0 + first_section_data.std())) if len(first_section_data) > 1 else 1.0
                ...
                if 4.0 <= avg_section <= 6.0:
                    ...
                    features['early_speed_rank'] = float(1 + 7 * (avg_section - 4.0) / 2.0)
                else:
        ...
        section_score = max(0, 1 - (features['mean_first_section'] - 4.5) / 1.0)  # Normalize around 4.5s
        consistency_score = features['first_section_consistency']
        features['early_speed_score'] = float(0.7 * section_score + 0.3 * consistency_score)
        ...
        if len(last_5_runs) >= 3:
            ...
            if len(positions) >= 3:
                ...
                try:
                    ...
                    features['recent_position_trend'] = float(-slope)  # Negative slope = improving
            ...
            if len(times) >= 3:
                ...
                try:
                    ...
                    features['recent_time_trend'] = float(-slope)  # Negative slope = improving times
                except:
    ...
    def _calculate_venue_specific_features(self, data: pd.DataFrame, target_venue: str = None) -> Dict[str, Any]:
        ...
        if target_venue:
            ...
            is_ballarat = target_venue.upper() in [code.upper() for code in ballarat_codes]
            
            if is_ballarat:
                # Filter for Ballarat races with higher emphasis
                if 'track_code' in data.columns:
                    ballarat_data = data[data['track_code'].str.upper().isin([code.upper() for code in ballarat_codes])]
                elif 'venue' in data.columns:
                    ballarat_data = data[data['venue'].str.upper().isin([code.upper() for code in ballarat_codes])]
                else:
        ...
        if len(ballarat_data) > 0:
            ...
            ballarat_times = pd.to_numeric(ballarat_data['race_time'], errors='coerce').dropna()
            if len(ballarat_times) > 0:
                ...
                features['ballarat_time_consistency'] = float(1.0 / (1.0 + ballarat_times.std())) if len(ballarat_times) > 1 else 1.0
            else:
            ...
            ballarat_positions = pd.to_numeric(ballarat_data['place'], errors='coerce').dropna()
            if len(ballarat_positions) > 0:
                features['ballarat_mean_position'] = float(ballarat_positions.mean())
                features['ballarat_win_rate'] = float((ballarat_positions == 1).sum() / len(ballarat_positions))
                features['ballarat_place_rate'] = float((ballarat_positions <= 3).sum() / len(ballarat_positions))
            else:
    ...
    def _calculate_distance_specific_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'distance_m' in data.columns:
            ...
            if distance_performance:
                best_distance = min(distance_performance.keys(), 
                                  key=lambda d: distance_performance[d]['mean_position'])
                
                features['best_distance'] = float(best_distance)
                features['best_distance_win_rate'] = float(distance_performance[best_distance]['win_rate'])
                features['best_distance_mean_time'] = float(distance_performance[best_distance]['mean_time'])
                features['distance_specialization'] = float(len(distance_performance))
                
                # Most common distance
                most_common_distance = max(distance_performance.keys(),
    ...
    def _calculate_race_context_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'grade' in data.columns:
            ...
            features['most_common_grade'] = str(grade_counts.index[0]) if len(grade_counts) > 0 else 'M'
        ...
        if 'starting_price' in data.columns:
            ...
            if len(prices) > 0:
                ...
                features['market_support'] = float((prices <= 5.0).sum() / len(prices))  # Percentage of races as favorite
            else:
        ...
        if 'race_date' in data.columns:
            try:
                ...
                if len(dates) > 1:
                    ...
                    features['racing_frequency'] = float(365.0 / max(features['avg_days_between_races'], 1))
                else:
        ...
        features['career_earnings_proxy'] = float(features['total_races'] * features.get('win_rate', 0.1) * 1000)  # Estimate
    ...
    def _get_all_dog_names(self) -> List[str]:
        ...
        if not os.path.exists(self.historical_data_path):
            logger.error(f"Historical data path does not exist: {self.historical_data_path}")
            return []
    ...
    def save_features_to_file(self, features_df: pd.DataFrame, filename: str = None) -> str:
        """
...
def main():
    ...
    if not features_df.empty:
        ...
        for dog_name in features_df.index[:3]:
            print(f"\n{dog_name}:")
            print(f"  Mean Race Time: {features_df.loc[dog_name, 'mean_race_time']:.2f}s")
            print(f"  Best Race Time: {features_df.loc[dog_name, 'best_race_time']:.2f}s")
            print(f"  Win Rate: {features_df.loc[dog_name, 'win_rate']:.3f}")
            print(f"  Place Rate (Top 3): {features_df.loc[dog_name, 'place_rate_top3']:.3f}")
            print(f"  Recent Form Trend: {features_df.loc[dog_name, 'recent_position_trend']:.3f}")
            print(f"  Early Speed Score: {features_df.loc[dog_name, 'early_speed_score']:.3f}")
            print(f"  Ballarat Experience: {features_df.loc[dog_name, 'ballarat_experience']} races")
    else:
