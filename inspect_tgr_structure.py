...
def inspect_tgr_page():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            mayfield_elements = soup.find_all(
                string=lambda text: text and "mayfield star" in text.lower()
            )
            ...
            for i, element in enumerate(mayfield_elements):
                ...
                if parent:
                    logger.info(
                        f"  Parent tag: <{parent.name}> with classes: {parent.get('class', [])}"
                    )
                    ...
                    for j, sibling in enumerate(parent.find_next_siblings()):
                        ...
                        if len(sibling_text) > 0:
                            logger.info(
                                f"    {j+1}. <{sibling.name}> {sibling.get('class', [])}: {sibling_text[:100]}..."
                            )
                    ...
                    for j, child in enumerate(
                        children[:10]
                    ):  # Limit to first 10 children
                        child_text = child.get_text(strip=True)
                        if len(child_text) > 0:
                            logger.info(
                                f"    {j+1}. <{child.name}> {child.get('class', [])}: {child_text[:100]}..."
                            )
            ...
            for pattern in date_patterns:
                ...
                for i, match in enumerate(matches[:5]):  # Show first 5
                    ...
                    if parent:
                        logger.info(
                            f"     In: <{parent.name}> {parent.get('class', [])}"
                        )
            ...
            logger.info(
                f"Found {len(racing_list_items)} list items with racing content:"
            )
            ...
            logger.info(
                f"Found {len(racing_table_rows)} table rows with racing content:"
            )
            for i, tr in enumerate(racing_table_rows[:5]):  # Show first 5
                cells = tr.find_all(["td", "th"])
                logger.info(
                    f"  {i+1}. {len(cells)} cells: {[cell.get_text(strip=True) for cell in cells[:6]]}"
                )
            ...
            for div in divs_with_classes[:20]:  # Check first 20 divs with classes
                ...
                if "mayfield star" in text or any(
                    venue in text for venue in ["ballarat", "bendigo", "sandown"]
                ):
                    logger.info(
                        f"Relevant div with class {div.get('class')}: {text[:100]}..."
                    )
