...
class FormGuideValidator:
    ...
    def preprocess_csv_content(self, content: str) -> Tuple[str, List[GuideIssue]]:
        ...
        cleaned_content, invisible_char_issues = self._clean_invisible_characters(content)
    ...
    def forward_fill_continuation_rows(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[GuideIssue]]:
        ...
        for idx, row in df_copy.iterrows():
            ...
            if dog_name_col and (pd.isna(row[dog_name_col]) or str(row[dog_name_col]).strip() == ''):
    ...
    def validate_dog_data(self, dog_data: Dict[str, Any], row_number: int = None) -> List[GuideIssue]:
        """Validate dog block data according to FORM_GUIDE_SPEC.md requirements."""
        issues = []
        
        # Dog name validation (required field)
        dog_name = dog_data.get('dog_name') or dog_data.get('Dog Name') or dog_data.get('name')
    ...
    def validate_csv_file(self, file_path: Path) -> Tuple[pd.DataFrame, List[GuideIssue]]:
...
def validate_only(file_path: Path):
    ...
    report = {
        ...
        "sample_data": df.head(3).to_dict('records') if not df.empty else []  # First 3 records as sample
    }
