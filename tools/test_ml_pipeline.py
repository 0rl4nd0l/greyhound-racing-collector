...
def test_ml_pipeline():
    ...
    try:
        ...
        results["tests"]["import_ml_system"] = {
            "status": "pass",
            "details": "MLSystemV4 imported successfully",
        }
    ...
    try:
        ...
        results["tests"]["import_temporal_builder"] = {
            "status": "pass",
            "details": "TemporalFeatureBuilder imported successfully",
        }
    ...
    try:
        ...
        results["tests"]["instantiate_ml_system"] = {
            "status": "pass",
            "details": f"MLSystemV4 instantiated with DB: {ml_system.db_path}",
        }
    ...
    try:
        has_temporal_builder = (
            hasattr(ml_system, "temporal_builder")
            and ml_system.temporal_builder is not None
        )
    ...
    try:
        has_pipeline = (
            ml_system.pipeline is not None or ml_system.calibrated_pipeline is not None
        )
        results["tests"]["existing_model_check"] = {
            ...
            "details": {
                ...
                "feature_columns_count": (
                    len(ml_system.feature_columns) if ml_system.feature_columns else 0
                ),
            },
        }
        ...
        print(
            f"   Feature columns: {len(ml_system.feature_columns) if ml_system.feature_columns else 0}"
        )
    except Exception as e:
    ...
    try:
        ...
        results["tests"]["data_preparation"] = {
            ...
            "details": {
                ...
                "train_races": (
                    len(train_data["race_id"].unique()) if train_size > 0 else 0
                ),
                "test_races": (
                    len(test_data["race_id"].unique()) if test_size > 0 else 0
                ),
            },
        }
        ...
        if train_size > 0:
            ...
            print(
                f"   Train: {train_size} samples, {results['tests']['data_preparation']['details']['train_races']} races"
            )
            print(
                f"   Test: {test_size} samples, {results['tests']['data_preparation']['details']['test_races']} races"
            )
    ...
    try:
        ...
        is_valid_prediction = (
            ...
            and "win_probability" in prediction
            and "confidence" in prediction
            and isinstance(prediction["win_probability"], (int, float))
            and isinstance(prediction["confidence"], (int, float))
            and 0 <= prediction["win_probability"] <= 1
    ...
    if results["overall_status"] == "poor":
        results["recommendations"].append(
            "HIGH: Major issues with ML pipeline - requires immediate attention"
        )
        results["recommendations"].append(
            "Consider retraining models or fixing data pipeline issues"
        )
    elif results["overall_status"] == "partial":
        results["recommendations"].append(
            "MED: Some pipeline components working but issues present"
        )
        results["recommendations"].append(
            "Address data preparation and model loading issues"
        )
    else:
        results["recommendations"].append("LOW: ML pipeline appears functional")
        results["recommendations"].append(
            "Consider performance optimization and feature engineering improvements"
        )
