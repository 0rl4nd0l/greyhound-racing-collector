        ...
        for table in tables:
            ...
            if table_info.get("issues"):
                assessment["issues"].extend(
                    [f"{table}: {issue}" for issue in table_info["issues"]]
                )
...
def assess_table(conn, table_name):
    ...
    try:
        ...
        for col in columns:
            ...
            info["columns"][col_name]["null_rate"] = null_count / max(
                info["row_count"], 1
            )
        ...
        if high_null_columns:
            info["issues"].append(
                f"High null rates in columns: {', '.join(high_null_columns)}"
            )
...
def assess_dog_race_data(conn):
    ...
    try:
        ...
        if dup_count > 0:
            issues.append(
                f"Found {dup_count} duplicate (race_id, box_number) combinations"
            )
        ...
        missing_odds = conn.execute(
            "SELECT COUNT(*) FROM dog_race_data WHERE starting_price IS NULL"
        ).fetchone()[0]
    ...
    try:
        ...
        try:
            venue_count = conn.execute(
                "SELECT COUNT(DISTINCT venue) FROM race_metadata WHERE venue IS NOT NULL"
            ).fetchone()[0]
...
def generate_recommendations(assessment):
    ...
    if total_issues > 10:
        recommendations.append(
            "HIGH: Critical data quality issues detected - prioritize data cleaning"
        )
    ...
    if "dog_race_data" in tables:
        ...
        if dog_info.get("dog_race_stats", {}).get("missing_odds_count", 0) > 1000:
            recommendations.append(
                "MED: High number of missing starting prices - affects EV calculations"
            )
        if dog_info.get("dog_race_issues"):
            recommendations.append(
                "HIGH: Fix duplicate race/box combinations and invalid positions"
            )
    ...
    if "race_metadata" in tables:
        ...
        if race_info.get("race_metadata_issues"):
            recommendations.append(
                "HIGH: Fix missing winner names - required for proper labeling"
            )
    ...
    if data_quality.get("orphaned_dog_records", 0) > 100:
        recommendations.append(
            "MED: Many dog records without race metadata - affects feature engineering"
        )
    if data_quality.get("winner_mismatches", 0) > 50:
        recommendations.append(
            "HIGH: Winner name mismatches - verify race result scraping accuracy"
        )
...
if __name__ == "__main__":
    ...
    print("\n📋 Assessment Summary:")
    print(
        f"Database: {assessment['database_path']} ({assessment.get('file_size_mb', 0)}MB)"
    )
