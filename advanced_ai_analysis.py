...
class AdvancedAIAnalyzer:
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["track_condition_encoded"] = le_track_condition.fit_transform(
        ...
        df["box_number_group"] = df["box_number"].apply(
            lambda x: "inside" if x <= 4 else "outside"
        )
        df["weight_normalized"] = df["weight"] / df["weight"].mean()
        df["odds_inverse"] = 1 / (
            df["starting_price"] + 0.001
    ...
    def train_winner_prediction_model(self, df: pd.DataFrame) -> dict:
        ...
        ensemble_accuracy = accuracy_score(
            y_test, (ensemble_prediction > 0.5).astype(int)
        )
        print(f"✨ Ensemble model accuracy: {ensemble_accuracy:.3f}")

        # Save ensemble model as best
        self.models["winner_prediction"] = lambda x: np.mean(
            [model.predict(x) for model, _ in models.values()], axis=0
        )
        return results
    ...
    def calculate_track_bias(self, venue_data: pd.DataFrame) -> dict:
        ...
        inside_win_rate = (
            len(inside_boxes[inside_boxes["finish_position"] == "1"])
            / len(inside_boxes)
            if len(inside_boxes) > 0
            else 0
        )
        outside_win_rate = (
            len(outside_boxes[outside_boxes["finish_position"] == "1"])
            / len(outside_boxes)
        ...
        for weather in venue_data["weather"].dropna().unique():
            ...
            if len(weather_data) > 5:  # Minimum threshold
                avg_time = pd.to_numeric(
                    weather_data["individual_time"], errors="coerce"
                ).mean()
                weather_impact[weather] = {
                    "races": len(weather_data),
                    "avg_time": avg_time,
                    "avg_winning_odds": weather_data[
                        weather_data["finish_position"] == "1"
                    ]["starting_price"].mean(),
                }
    ...
    def analyze_trainer_performance(self, df: pd.DataFrame) -> dict:
        ...
        for trainer in df["trainer_name"].dropna().unique():
            ...
            if len(trainer_data) >= 10:  # Minimum threshold
                ...
                places = len(
                    trainer_data[trainer_data["finish_position"].isin(["1", "2", "3"])]
                )
        ...
        # Sort by win rate
        trainer_stats = dict(
            sorted(trainer_stats.items(), key=lambda x: x[1]["win_rate"], reverse=True)
        )
    ...
    def generate_race_predictions(self, df: pd.DataFrame) -> dict:
        ...
        for race_id in recent_races["race_id"].unique():
            ...
            race_predictions.sort(
                key=lambda x: x["predicted_win_probability"], reverse=True
            )
    ...
    def generate_comprehensive_report(
        ...
        report_path = os.path.join(
            self.results_dir, f"ai_analysis_report_{timestamp}.md"
        )

        with open(report_path, "w") as f:
            f.write(
                f"""# Advanced AI Analysis Report
...
"""
            ...
            if ml_results:
                ...
                for model_name, results in ml_results.items():
                    f.write(
                        f"- **{model_name}**: {results['accuracy']:.3f} accuracy (CV: {results['cv_mean']:.3f} ± {results['cv_std']:.3f})\n"
                    )

                f.write("\n### Feature Importance (Top 10)\n")
                best_model = max(
                    ml_results.keys(), key=lambda x: ml_results[x]["accuracy"]
                )
            ...
            for venue, analysis in venue_analysis.items():
                ...
                f.write(
                    f"- **Average Winning Odds**: ${analysis['avg_winning_odds']:.2f}\n"
                )
                f.write(
                    f"- **Inside Box Win Rate**: {analysis['track_bias']['inside_win_rate']:.2%}\n"
                )
                f.write(
                    f"- **Outside Box Win Rate**: {analysis['track_bias']['outside_win_rate']:.2%}\n"
                )
                f.write(
                    f"- **Track Bias**: {analysis['track_bias']['bias_factor']:.3f}\n\n"
                )

            f.write(f"\n## Top Trainers (by Win Rate)\n")
            for i, (trainer, stats) in enumerate(list(trainer_stats.items())[:10]):
                f.write(
                    f"{i+1}. **{trainer}** - {stats['win_rate']:.2%} ({stats['wins']}/{stats['total_starts']})\n"
                )
            ...
            if predictions:
                ...
                for race_id, pred in list(predictions.items())[:5]:
                    f.write(
                        f"### {pred['race_info']['venue']} Race {pred['race_info']['race_number']}\n"
                    )
                    f.write(f"Date: {pred['race_info']['race_date']}\n")
                    f.write("**Top 3 Predictions:**\n")
                    for i, dog in enumerate(pred["predictions"][:3]):
                        f.write(
                            f"{i+1}. **{dog['dog_name']}** (Box {dog['box_number']}) - {dog['predicted_win_probability']:.2%}\n"
                        )
                    f.write("\n")

            f.write(f"\n## Statistical Insights\n")
            f.write(
                f"- **Most Successful Box**: {df[df['finish_position'] == '1']['box_number'].mode().iloc[0] if len(df[df['finish_position'] == '1']) > 0 else 'N/A'}\n"
            )
            f.write(
                f"- **Average Winning Odds**: ${df[df['finish_position'] == '1']['starting_price'].mean():.2f}\n"
            )
            f.write(
                f"- **Longest Shot Winner**: ${df[df['finish_position'] == '1']['starting_price'].max():.2f}\n"
            )
            f.write(
                f"- **Shortest Odds Winner**: ${df[df['finish_position'] == '1']['starting_price'].min():.2f}\n"
            )
    ...
    def run_comprehensive_analysis(self):
        ...
        report_path = self.generate_comprehensive_report(
            df, ml_results, venue_analysis, trainer_stats, predictions
        )

        # Save to database
        self.save_analysis_to_database(
            ml_results, venue_analysis, trainer_stats, predictions
        )
