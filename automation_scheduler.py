...
class AutomationScheduler:
    ...
    def run_task(self, task_name: str, command: List[str], timeout: int = 300) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.info(
                    f"✅ {task_name} completed successfully in {duration:.1f}s"
                )
                ...
                self.logger.error(
                    f"❌ {task_name} failed with code {result.returncode}"
                )
    ...
    def run_predictions(self):
        ...
        self.logger.info(
            f"🏆 Predictions completed: {success_count}/{len(race_files[:5])} successful"
        )
...
""",
    ...
    def cleanup_old_files(self):
        ...
        try:
            ...
            file_counts = {
                ...
                "predictions": (
                    len(list((self.base_dir / "predictions").glob("*.json")))
                    if (self.base_dir / "predictions").exists()
                    else 0
                ),
                "csv_files": len(list(self.base_dir.rglob("*.csv"))),
                "databases": len(list(self.base_dir.rglob("*.db"))),
                "backups": (
                    len(list((self.base_dir / "backups").glob("*.db")))
                    if (self.base_dir / "backups").exists()
                    else 0
                ),
            }

            self.logger.info(
                f"🧹 Cleanup completed - removed {temp_cleaned} temporary files"
            )
            self.logger.info(f"💾 Total storage: {total_size_mb:.1f} MB")
            self.logger.info(
                f"📊 Data files: {file_counts['logs']} logs, {file_counts['predictions']} predictions, {file_counts['csv_files']} CSVs, {file_counts['databases']} DBs, {file_counts['backups']} backups"
            )
    ...
    def backup_database(self):
        ...
        try:
            ...
            backup_file = (
                backup_dir
                / f"greyhound_racing_data_backup_{current_date.strftime('%Y%m%d')}.db"
            )
            ...
            if removed_count > 0:
                self.logger.info(
                    f"🗂️ Cleaned {removed_count} old backups (kept {len(keep_files)} backups)"
                )
    ...
    def morning_routine(self):
        ...
        self.logger.info(
            f"🌅 Morning routine completed: {success_count}/{len(tasks)} tasks successful"
        )
        self.save_status()
    ...
    def afternoon_routine(self):
        ...
        self.logger.info(
            f"🌞 Afternoon routine completed: {success_count}/{len(tasks)} tasks successful"
        )
        self.save_status()
    ...
    def evening_routine(self):
        ...
        self.logger.info(
            f"🌙 Evening routine completed: {success_count}/{len(tasks)} tasks successful"
        )
        self.save_status()
