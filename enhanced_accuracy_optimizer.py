...
class AdvancedEnsemblePredictor:
    ...
    def load_primary_model(self) -> bool:
        ...
        try:
            ...
            self.models = {metadata.model_id: {'model': model, 'scaler': scaler, 'metadata': metadata, 'weight': getattr(metadata, 'accuracy', 0.5)}}
            ...
            logger.info(f"🎯 Loaded primary model: {metadata.model_id} (accuracy: {metadata.accuracy:.3f})")
            return True
        except Exception as e:
    ...
    def _predict_with_model(self, model_id: str, model_data: Dict[str, Any], base_features: pd.DataFrame) -> np.ndarray:
        ...
        def _as_model_input(m, df: pd.DataFrame):
            try:
                if hasattr(m, 'feature_names_in_') and getattr(m, 'feature_names_in_') is not None:
                    return df
            except Exception:
                pass
            try:
                ...
                if isinstance(m, _CCC):
                    ...
                    if base is not None and getattr(base, 'feature_names_in_', None) is not None:
                        return df
            except Exception:
                pass
            return df.to_numpy()
    ...
    def predict_with_ensemble(self, features: pd.DataFrame, race_id: str) -> List[Dict[str, Any]]:
        ...
        for model_id, model_data in self.models.items():
            try:
                win_probs = self._predict_with_model(model_id, model_data, base_features)
    ...
    def predict_with_primary_then_fallback(self, features: pd.DataFrame, race_id: str) -> List[Dict[str, Any]]:
        ...
        primary_id, primary_data = next(iter(self.models.items())) if not self._primary else self._primary

        try:
            primary_probs = self._predict_with_model(primary_id, primary_data, base_features)
            model_predictions = {primary_id: primary_probs}
        except Exception as e:
            logger.warning(f"Primary model {primary_id} failed: {e}. Falling back to secondary models (ensemble)...")
            ...
            if primary_id in self.models:
                all_items = [(mid, mdata) for mid, mdata in self.models.items() if mid != primary_id]
            else:
        ...
        for i, (_, row) in enumerate(base_features.iterrows()):
            try:
                ...
                if ensemble_count > 1:
                    ...
                    confidence = min(1.0, max(0.1, 0.7 * model_agreement + 0.3 * extremeness))
                predictions.append({
                    'dog_clean_name': (names[i] if names and i < len(names) else row.get('dog_clean_name', f'Dog_{i}')),
                    'box_number': int(row.get('box_number', i + 1)),
                    'win_probability': round(float(calibrated_win_prob), 4),
...
class AccuracyOptimizer:
    ...
    def _load_optimization_config(self) -> Dict:
        ...
        default_config = {
            ...
            'ensemble_mode': os.getenv('ENSEMBLE_MODE', 'primary_with_fallback').strip().lower()
        }
        ...
        try:
            em = str(default_config.get('ensemble_mode', 'primary_with_fallback')).strip().lower()
    ...
    def generate_optimized_predictions(self, features: pd.DataFrame, race_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            mode = str(self.config.get('ensemble_mode', 'primary_with_fallback')).lower()
            if mode == 'full_ensemble':
                predictions = self.ensemble_predictor.predict_with_ensemble(features, race_id)
            else:
                predictions = self.ensemble_predictor.predict_with_primary_then_fallback(features, race_id)
            ...
            try:
                if isinstance(result.get('model_ids_used'), list) and len(result['model_ids_used']) == 1:
