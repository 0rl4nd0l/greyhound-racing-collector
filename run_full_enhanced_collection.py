    ...
    def discover_race_files():
        ...
        for cache_file in cached_files:
            try:
                ...
                has_form_guide = (
                    soup.find(class_="form-guide-meeting__heading") is not None
                )
                ...
                if has_form_guide and len(dog_sections) > 0:
                    ...
                    heading_text = (
                        heading.get_text(strip=True) if heading else "Unknown"
                    )

                    # Count non-vacant dogs
                    active_dogs = 0
                    for section in dog_sections:
                        if "form-guide-long-form-selection--vacant" not in section.get(
                            "class", []
                        ):
                            active_dogs += 1
        ...
        for i, race_info in enumerate(race_files):
            ...
            try:
                ...
                if race_details and race_details.get("dogs"):
                    ...
                    logger.info(
                        f"   ✅ Extracted {len(dogs)} dogs from {race_details.get('venue')} ({race_details.get('date')})"
                    )
                    total_dogs += len(dogs)
                    ...
                    for dog in dogs:
                        ...
                        all_dogs_data[dog_name]["total_history_records"] += len(
                            racing_history
                        )
                        ...
                        for race in racing_history:
                            ...
                            if race.get("distance"):
                                all_dogs_data[dog_name]["distances"].add(
                                    str(race["distance"])
                                )

                else:
                    logger.warning(
                        f"   ⚠️ No dogs extracted from {race_info['heading']}"
                    )
    ...
    def calculate_enhanced_analytics(all_dogs_data):
        ...
        for dog_name, dog_data in all_dogs_data.items():
            if dog_data["total_history_records"] > 0:
                ...
                analytics["summary"]["total_history_records"] += dog_data[
        ...
        analytics["summary"]["unique_venues"] = list(
            analytics["summary"]["unique_venues"]
        )
        analytics["summary"]["unique_grades"] = list(
            analytics["summary"]["unique_grades"]
        )
        analytics["summary"]["unique_distances"] = list(
            analytics["summary"]["unique_distances"]
        )
        ...
        analytics["summary"]["distance_count"] = len(
            analytics["summary"]["unique_distances"]
        )

        # Top dogs analysis
        dogs_by_appearances = sorted(
            all_dogs_data.items(), key=lambda x: len(x[1]["appearances"]), reverse=True
        )
        dogs_by_history = sorted(
            all_dogs_data.items(),
            key=lambda x: x[1]["total_history_records"],
            reverse=True,
        )
        dogs_by_venues = sorted(
            all_dogs_data.items(), key=lambda x: len(x[1]["venues"]), reverse=True
        )
        ...
        analytics["top_dogs"]["most_venues"] = [
            ...
            for name, data in dogs_by_venues[:10]
    ...
    def run_full_enhanced_collection():
        ...
        def _parse_race_from_soup(self, soup, url):
            ...
            try:
                ...
                for section in dog_sections:
                    if "form-guide-long-form-selection--vacant" in section.get(
                        "class", []
                    ):
                        continue

                    header = section.find(
                        class_="form-guide-long-form-selection__header"
                    )
                    if not header:
                        continue
        ...
        try:
            ...
            all_race_data, all_dogs_data, total_dogs, total_races = (
                extract_comprehensive_race_data(scraper, race_files)
            )
            ...
            logger.info(
                f"   • Unique dogs found: {analytics['summary']['total_unique_dogs']}"
            )
            logger.info(
                f"   • Dogs with racing histories: {analytics['summary']['dogs_with_histories']}"
            )
            logger.info(
                f"   • Total race history records: {analytics['summary']['total_history_records']}"
            )
            logger.info(
                f"   • Average history per dog: {analytics['summary']['average_history_per_dog']:.1f}"
            )
            logger.info(
                f"   • Unique venues covered: {analytics['summary']['venue_count']}"
            )
            logger.info(
                f"   • Unique grades found: {analytics['summary']['grade_count']}"
            )
            logger.info(
                f"   • Unique distances found: {analytics['summary']['distance_count']}"
            )
            ...
            if analytics["top_dogs"]["most_history_records"]:
                ...
                logger.info(
                    f"   • Most race records: {top_dog['dog_name']} ({top_dog['history_records']} records)"
                )

            if analytics["top_dogs"]["most_venues"]:
                versatile_dog = analytics["top_dogs"]["most_venues"][0]
                logger.info(
                    f"   • Most venues: {versatile_dog['dog_name']} ({versatile_dog['venue_count']} venues)"
                )
            ...
            if len(analytics["summary"]["unique_venues"]) > 10:
                logger.info(
                    f"   • ... and {len(analytics['summary']['unique_venues']) - 10} more venues"
                )
