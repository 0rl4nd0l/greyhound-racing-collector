...
class ModelMonitoringService:
    ...
    def start_monitoring(self):
        ...
        logger.info(
            f"   🔄 Checking for updates every {self.check_interval_minutes} minutes"
        )
    ...
    def _check_for_model_updates(self):
        ...
        try:
            ...
            if (
                ...
                or self.current_model_info.get("model_id") != metadata.model_id
            ):

                logger.info(f"🔄 New best model detected: {metadata.model_id}")
                logger.info(
                    f"   📊 Performance: Acc={metadata.accuracy:.3f}, AUC={metadata.auc:.3f}"
                )
    ...
    def _analyze_performance_drift(self):
        ...
        try:
            ...
            if (
                ...
                < self.thresholds["min_predictions_for_analysis"]
            ):
            ...
            cutoff_time = datetime.now() - timedelta(
                hours=self.thresholds["performance_window_hours"]
            )
            recent_performance = [
                entry
            ...
            accuracies = [
                entry["accuracy"] for entry in recent_performance if "accuracy" in entry
            ]
            predictions_count = sum(
                entry.get("predictions_count", 0) for entry in recent_performance
            )
            ...
            if self.current_model_info and "accuracy" in self.current_model_info:
                ...
                if accuracy_drop > self.thresholds["accuracy_drop_critical"]:
                    ...
                    logger.warning(
                        f"🚨 CRITICAL: Performance drop of {accuracy_drop:.3f}"
                    )
                    ...
                    logger.warning(
                        f"⚠️ WARNING: Performance drop of {accuracy_drop:.3f}"
                    )
    ...
    def record_prediction_performance(
        self, predictions: List[Dict], actual_results: List[int] = None
    ):
        ...
        try:
            ...
            performance_entry = {
                ...
                "average_confidence": sum(
                    p.get("confidence_score", 0.5) for p in predictions
                )
                / len(predictions),
