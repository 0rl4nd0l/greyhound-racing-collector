...
class DataMonitor:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db", config_path: str = "config/monitoring.json"):
    ...
    def check_integrity_violations(self) -> Dict:
        ...
        with self.integrity_manager:
            integrity_report = self.integrity_manager.run_comprehensive_integrity_check()
    ...
    def calculate_data_quality_score(self) -> float:
        ...
        try:
            ...
            if total_dog_races > 0:
                quality_metrics['consistency'] = (consistent_dog_data / total_dog_races) * 100
            else:
            ...
            if total_expert_data > 0:
                quality_metrics['validity'] = (valid_positions / total_expert_data) * 100
            else:
        ...
        if os.path.exists(history_file):
            try:
                ...
                if len(recent_data) >= self.config['trend_analysis']['min_data_points']:
                    # Calculate trends
                    quality_scores = [entry.get('data_quality_score', 0) for entry in recent_data]
                    violation_counts = [len(entry.get('violations', {})) for entry in recent_data]
                    
                    trends['data_quality_trend'] = 'improving' if quality_scores[-1] > quality_scores[0] else 'declining'
                    trends['violation_trend'] = 'improving' if violation_counts[-1] < violation_counts[0] else 'increasing'
                    trends['avg_quality_score'] = sum(quality_scores) / len(quality_scores)
                    trends['avg_violations'] = sum(violation_counts) / len(violation_counts)
...
"""
        ...
        try:
            ...
            self.logger.info(f"Email alert sent to {len(email_config['recipient_emails'])} recipients")
    ...
    def run_monitoring_check(self) -> Dict:
        ...
        self.logger.info(f"Monitoring check complete: {len(alerts)} alerts, quality score: {quality_score}%")
