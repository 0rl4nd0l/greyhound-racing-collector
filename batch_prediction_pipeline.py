...
class BatchPredictionPipeline:
    ...
    def _initialize_systems(self):
        ...
        if ML_SYSTEM_V3_AVAILABLE:
            ...
            if not under_pytest or enable_ml_v3_tests:
                ...
                self.logger.info("⏭️ Skipping MLSystemV3 initialization under tests (set ENABLE_ML_V3_IN_TESTS=1 to enable)")
        ...
        if CSV_INGESTION_AVAILABLE:
            try:
                ...
                self.csv_ingestor = FormGuideCsvIngestor(validation_level=validation_level)
            ...
            for file_path in input_files:
                try:
                    if os.path.exists(file_path):
                        ...
                        last = self.processed_manifest.get(os.path.basename(file_path), 0)
                        if mtime > last:
    ...
    def run_batch_job(self, job_id: str) -> BatchJob:
        ...
        if job.status == "cancelled":
            self.logger.info(f"Job {job_id} was cancelled before start; skipping execution")
            return job
