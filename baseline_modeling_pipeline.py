...
class BaselineModelingPipeline:
    ...
    def load_data(self):
        ...
        data["win"] = (
            pd.to_numeric(data["finish_position"], errors="coerce") == 1
        ).astype(int)
        data["place"] = (
            pd.to_numeric(data["finish_position"], errors="coerce") <= 3
        ).astype(int)
    ...
    def prepare_features(self, target="win"):
        ...
        categorical_features = ["venue_encoded", "grade_encoded"]
    ...
    def temporal_split(self, X, y, train_ratio=0.6, val_ratio=0.2):
        ...
        print(
            f"  - Train: {len(X_train)} samples ({dates_train.min()} to {dates_train.max()})"
        )
        print(
            f"  - Validation: {len(X_val)} samples ({dates_val.min()} to {dates_val.max()})"
        )
        print(
            f"  - Test: {len(X_test)} samples ({dates_test.min()} to {dates_test.max()})"
        )
    ...
    def compare_to_benchmarks(self, results):
        ...
        print("=" * 80)
        print(
            f"{'Model':<25} {'Accuracy':<10} {'AUC':<10} {'Log Loss':<10} {'Brier':<10}"
        )
        print("=" * 80)
        ...
        for name, metrics in benchmarks.items():
            print(
                f"{name:<25} {metrics['accuracy']:<10.4f} {metrics['auc']:<10.4f} "
        ...
        for name, result in results.items():
            print(
                f"{name:<25} {result['accuracy']:<10.4f} {result['auc']:<10.4f} "
        ...
        for name, result in results.items():
            ...
            status = (
                "🟢 IMPROVED" if acc_change > 0 and auc_change > 0 else "🔴 DEGRADED"
            )
            print(f"{name}: {status}")
