...
def diagnose_variance():
    ...
    try:
        ...
        # Clean dog names
        race_data['dog_clean_name'] = race_data['Dog Name'].str.replace(r'^\d+\.\s*', '', regex=True)
        ...
        # Build features
        race_features = ml_system.temporal_builder.build_features_for_race(race_data, race_id)
        ...
        for cat_col in categorical_features:
            if cat_col in X_pred.columns:
                ...
                X_pred[cat_col] = X_pred[cat_col].apply(
                    lambda x: default_values.get(cat_col, 'Unknown') if (pd.isna(x) or x == 0 or x == '0') else str(x)
                )
        ...
        for i, dog_name in enumerate(race_features['dog_clean_name']):
            ...
            print(f"     {dog_name}: {non_zero}/{len(X_pred.columns)} ({non_zero/len(X_pred.columns)*100:.1f}%)")
