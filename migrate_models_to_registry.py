...
def find_existing_models() -> List[Dict[str, Any]]:
    ...
    for i, model in enumerate(model_files[:10], 1):  # Show first 10
        print(
            f"   {i}. {model['file_name']} ({model['file_size_mb']:.1f} MB, {model['modification_time'].strftime('%Y-%m-%d %H:%M')})"
        )
...
def migrate_model_to_registry(
    model_metadata: Dict[str, Any], file_info: Dict[str, Any]
) -> bool:
    ...
    try:
        ...
        if models:
            ...
            for i, model in enumerate(models[:10], 1):
                ...
                print(
                    f"      Performance: Acc={model.accuracy:.3f}, AUC={model.auc:.3f}"
                )
                print(f"      Training: {model.training_timestamp[:19]}")
                print(
                    f"      Notes: {model.notes[:50]}{'...' if len(model.notes) > 50 else ''}"
                )
...
def main():
    ...
    if successful_migrations > 0:
        ...
        print(
            f"   {successful_migrations} models are now available in the centralized registry"
        )
