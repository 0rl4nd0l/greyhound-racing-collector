...
class EnhancedRaceAnalyzer:
    ...
    def load_data(self):
        ...
        self.data['race_date'] = pd.to_datetime(self.data['race_date'], format='mixed', dayfirst=False)
        
        # Remove any remaining N/A finish positions and convert to numeric
        self.data = self.data[self.data['finish_position'].notna() & (self.data['finish_position'] != 'N/A')]
        self.data['finish_position'] = pd.to_numeric(self.data['finish_position'], errors='coerce')
        self.data = self.data[self.data['finish_position'].notna()]  # Remove any that couldn't be converted
    ...
    def engineer_features(self):
        ...
        dog_stats = self.data.groupby('dog_name').agg({
        ...
        dog_stats.columns = ['dog_name', 'race_count', 'first_race', 'last_race', 
                           'avg_position', 'position_std', 'median_position',
                           'avg_weight', 'avg_starting_price']
        
        # Calculate career span
        dog_stats['career_span_days'] = (dog_stats['last_race'] - dog_stats['first_race']).dt.days
        dog_stats['races_per_month'] = dog_stats['race_count'] / (dog_stats['career_span_days'] / 30.44)
        dog_stats['races_per_month'] = dog_stats['races_per_month'].fillna(dog_stats['race_count'])
        
        # Add recency metrics
        latest_date = self.data['race_date'].max()
        dog_stats['days_since_last_race'] = (latest_date - dog_stats['last_race']).dt.days
    ...
    def add_race_condition_features(self):
        ...
        self.data['grade_numeric'] = pd.to_numeric(
            self.data['grade'].str.extract(r'(\d+)')[0], 
            errors='coerce'
        ).fillna(999)  # High number for non-numeric grades
        
        # Track condition normalized
        track_mapping = {
            'FAST': 1, 'GOOD': 2, 'SLOW': 3, 'HEAVY': 4,
            'Fast': 1, 'Good': 2, 'slow': 3, 'Heavy': 4
        }
        self.data['track_condition_numeric'] = self.data['track_condition'].map(track_mapping).fillna(2)  # Default to 'Good'
        ...
        self.data['venue_avg_performance'] = self.data['venue'].map(venue_performance_avg)
        ...
        for dog_name in self.data['dog_name'].unique():
            ...
            distance_performance = dog_data.groupby('distance_category')['performance_score'].mean()
            for dist_cat in ['Short', 'Medium', 'Long', 'Extra_Long']:
                self.data.loc[dog_mask, f'dog_performance_{dist_cat.lower()}'] = distance_performance.get(dist_cat, 0.5)
            
            # Performance by grade
            grade_performance = dog_data.groupby('grade_normalized')['performance_score'].mean()
            self.data.loc[dog_mask, 'dog_avg_grade_performance'] = dog_data['performance_score'].mean()
            
            # Performance by track condition
            track_performance = dog_data.groupby('track_condition_numeric')['performance_score'].mean()
            self.data.loc[dog_mask, 'dog_avg_track_performance'] = dog_data['performance_score'].mean()
            
            # Performance by venue (dog-specific venue performance)
            venue_performance = dog_data.groupby('venue')['performance_score'].mean()
            self.data.loc[dog_mask, 'dog_venue_avg_performance'] = dog_data['performance_score'].mean()
        ...
        try:
            ...
            if len(weather_data) > 0:
                ...
                weather_encoding = {
                    ...
                    'overcast': 4, 'rain': 5, 'heavy_rain': 6, 'fog': 7
                }
                self.data['weather_condition_encoded'] = self.data['weather_condition'].map(weather_encoding).fillna(0)
                
                # Temperature features
                self.data['temperature'] = pd.to_numeric(self.data['temperature'], errors='coerce').fillna(20)
                self.data['temp_deviation_from_optimal'] = abs(self.data['temperature'] - 20)  # 20°C as optimal
                self.data['is_extreme_temperature'] = ((self.data['temperature'] < 10) | 
                                                     (self.data['temperature'] > 30)).astype(int)
                ...
                self.data['humidity'] = pd.to_numeric(self.data['humidity'], errors='coerce').fillna(50)
                self.data['wind_speed'] = pd.to_numeric(self.data['wind_speed'], errors='coerce').fillna(0)
                self.data['pressure'] = pd.to_numeric(self.data['pressure'], errors='coerce').fillna(1013)
                
                # Rain indicator
                self.data['is_rain'] = self.data['weather_condition'].str.contains('rain', case=False, na=False).astype(int)
                ...
                for dog_name in self.data['dog_name'].unique():
                    ...
                    weather_performance = dog_data.groupby('weather_condition')['performance_score'].mean()
                    
                    # Rain performance
                    rain_races = dog_data[dog_data['is_rain'] == 1]
                    if len(rain_races) > 0:
                        self.data.loc[dog_mask, 'rain_performance_avg'] = rain_races['performance_score'].mean()
                        self.data.loc[dog_mask, 'rain_experience'] = len(rain_races)
                    else:
                        self.data.loc[dog_mask, 'rain_performance_avg'] = dog_data['performance_score'].mean()
                        self.data.loc[dog_mask, 'rain_experience'] = 0
                    
                    # Temperature performance analysis
                    temp_groups = pd.cut(dog_data['temperature'], bins=[0, 15, 25, 50], labels=['Cold', 'Optimal', 'Hot'])
                    temp_performance = dog_data.groupby(temp_groups)['performance_score'].mean()
                    
                    for temp_cat in ['Cold', 'Optimal', 'Hot']:
                        self.data.loc[dog_mask, f'temp_performance_{temp_cat.lower()}'] = temp_performance.get(temp_cat, dog_data['performance_score'].mean())
                ...
                self.data['is_rain'] = 0
                self.data['rain_performance_avg'] = self.data.get('performance_score', 0.5)
    ...
    def normalize_performance(self):
        ...
        print(f"📊 Field size stats before validation: min={self.data['field_size'].min()}, max={self.data['field_size'].max()}, mean={self.data['field_size'].mean():.2f}")
        
        # Fix unrealistic field sizes - use actual maximum finish position per race as field size
        race_actual_field_sizes = self.data.groupby('race_id')['finish_position'].max().reset_index()
        ...
        self.data['corrected_field_size'] = self.data.apply(
            lambda row: max(3, row['field_size'], row.get('actual_field_size', row['field_size'])),
            axis=1
        )
        
        print(f"📊 Field size stats after validation: min={self.data['corrected_field_size'].min()}, max={self.data['corrected_field_size'].max()}, mean={self.data['corrected_field_size'].mean():.2f}")
        ...
        self.data['performance_score'] = (self.data['corrected_field_size'] - self.data['finish_position'] + 1) / self.data['corrected_field_size']
        self.data['weighted_performance'] = self.data['performance_score'] * self.data['frequency_weight']
        ...
        self.data['recent_form'] = self.data.groupby('dog_name')['performance_score'].transform(
            lambda x: x.rolling(window=3, min_periods=1).mean()
        )
        
        # Form trend (improvement/decline over last 5 races)
        self.data['form_trend'] = self.data.groupby('dog_name')['performance_score'].transform(
        ...
        numeric_cols = ['frequency_weight', 'performance_score', 'weighted_performance', 'consistency_score', 'recent_form']
    ...
    def temporal_analysis(self):
        ...
        self.data['performance_score'] = pd.to_numeric(self.data['performance_score'], errors='coerce')
        self.data['finish_position'] = pd.to_numeric(self.data['finish_position'], errors='coerce')
        self.data['individual_time'] = pd.to_numeric(self.data['individual_time'], errors='coerce')
        ...
        monthly_stats.columns = ['year_month', 'avg_performance', 'performance_std', 
                               'total_entries', 'unique_races']
        ...
        venue_stats.columns = ['venue', 'avg_performance', 'performance_std', 
                             'race_count', 'avg_finish_pos', 'avg_field_size']
    ...
    def analyze_race_conditions(self):
        ...
        # Distance analysis
        distance_stats = self.data.groupby('distance_category').agg({
            'performance_score': ['mean', 'std', 'count'],
            'finish_position': 'mean',
            'individual_time': 'mean'
        }).reset_index()
        distance_stats.columns = ['distance_category', 'avg_performance', 'performance_std', 
                                'race_count', 'avg_finish_pos', 'avg_time']
        
        # Grade analysis
        grade_stats = self.data.groupby('grade_normalized').agg({
            'performance_score': ['mean', 'std', 'count'],
            'finish_position': 'mean',
            'field_size': 'mean'
        }).reset_index()
        grade_stats.columns = ['grade', 'avg_performance', 'performance_std', 
                             'race_count', 'avg_finish_pos', 'avg_field_size']
        
        # Track condition analysis
        track_stats = self.data[self.data['track_condition'].notna()].groupby('track_condition').agg({
            'performance_score': ['mean', 'std', 'count'],
            'finish_position': 'mean',
            'individual_time': 'mean'
        }).reset_index()
        track_stats.columns = ['track_condition', 'avg_performance', 'performance_std', 
                             'race_count', 'avg_finish_pos', 'avg_time']
        
        # Venue analysis
        venue_performance_stats = self.data.groupby('venue').agg({
            'performance_score': ['mean', 'std', 'count'],
            'finish_position': 'mean',
            'individual_time': 'mean',
            'field_size': 'mean'
        }).reset_index()
        venue_performance_stats.columns = ['venue', 'avg_performance', 'performance_std', 
                                         'race_count', 'avg_finish_pos', 'avg_time', 'avg_field_size']
        
        # Distance vs Performance correlation
        distance_correlation = self.data['distance_numeric'].corr(self.data['performance_score'])
        
        # Weather analysis
        weather_stats = None
        weather_performance_impact = None
        if 'weather_condition' in self.data.columns and self.data['weather_condition'].notna().any():
            ...
            weather_stats.columns = ['weather_condition', 'avg_performance', 'performance_std', 
                                   'race_count', 'avg_finish_pos', 'avg_time', 'avg_adjustment_factor']
            
            # Weather performance impact analysis
            weather_performance_impact = {
                'clear_vs_rain': {
                    'clear_avg': self.data[self.data['weather_condition'] == 'clear']['performance_score'].mean(),
                    'rain_avg': self.data[self.data['is_rain'] == 1]['performance_score'].mean(),
                    'difference': self.data[self.data['weather_condition'] == 'clear']['performance_score'].mean() - 
                                self.data[self.data['is_rain'] == 1]['performance_score'].mean()
                },
                'temperature_correlation': self.data['temperature'].corr(self.data['performance_score']),
                'humidity_correlation': self.data['humidity'].corr(self.data['performance_score']),
                'wind_correlation': self.data['wind_speed'].corr(self.data['performance_score'])
            }
        
        return {
            'distance_stats': distance_stats,
            'grade_stats': grade_stats.sort_values('race_count', ascending=False).head(20),
            'track_stats': track_stats,
            'venue_stats': venue_performance_stats.sort_values('race_count', ascending=False),
    ...
    def identify_top_performers(self, min_races=3):
        ...
        dog_rankings = frequent_dogs.groupby('dog_name').agg({
        ...
        dog_rankings[score_features] = scaler.fit_transform(dog_rankings[score_features])
    ...
    def predict_performance(self, upcoming_races_df=None):
        ...
        feature_cols = [
            'box_number', 'field_size', 'race_count', 'avg_position',
            'position_std', 'avg_weight', 'races_per_month', 'race_sequence',
            'consistency_score', 'recent_form', 'distance_numeric', 'grade_numeric',
            'track_condition_numeric', 'venue_encoded', 'venue_avg_performance',
            'dog_performance_short', 'dog_performance_medium', 'dog_performance_long', 
            'dog_avg_grade_performance', 'dog_avg_track_performance', 'dog_venue_avg_performance'
        ]
        
        # Add weather features if available
        weather_features = [
            'weather_condition_encoded', 'temperature', 'temp_deviation_from_optimal',
            'is_extreme_temperature', 'weather_adjustment_factor', 'humidity',
            'wind_speed', 'pressure', 'is_rain', 'rain_performance_avg', 'rain_experience'
        ]
        
        # Only add weather features that exist in the data
        available_weather_features = [f for f in weather_features if f in self.data.columns]
        feature_cols.extend(available_weather_features)
        
        if available_weather_features:
            print(f"🌤️ Including {len(available_weather_features)} weather features in ML model")
    ...
    def generate_insights(self):
        ...
        insights = {
            ...
            'frequency_analysis': {
                'single_race_dogs': len(self.data[self.data['race_count'] == 1]['dog_name'].unique()),
                'frequent_racers': len(self.data[self.data['race_count'] > 3]['dog_name'].unique()),
            ...
            'performance_metrics': {
                ...
                'consistency_leader': self.data.loc[self.data['consistency_score'].idxmax(), 'dog_name']
            }
        }
    ...
    def run_comprehensive_analysis(self):
        ...
        print(f"  Distance correlation with performance: {race_condition_analysis['distance_correlation']:.3f}")
        print("\nDistance Categories:")
        ...
        if race_condition_analysis.get('weather_stats') is not None:
            ...
            if race_condition_analysis.get('weather_performance_impact'):
                ...
                if clear_vs_rain:
                    print(f"  Clear weather avg: {clear_vs_rain.get('clear_avg', 0):.3f}")
                    print(f"  Rain avg: {clear_vs_rain.get('rain_avg', 0):.3f}")
                    print(f"  Performance difference: {clear_vs_rain.get('difference', 0):.3f}")
                
                print(f"  Temperature correlation: {impact.get('temperature_correlation', 0):.3f}")
                print(f"  Humidity correlation: {impact.get('humidity_correlation', 0):.3f}")
        ...
        for i, row in top_performers.head(10).iterrows():
            print(f"  {row['dog_name']}: Score {row['composite_score']:.3f} "
