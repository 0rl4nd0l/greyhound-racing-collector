...
def get_pending_race_statistics(db_path: str = "greyhound_racing_data.db") -> Dict[str, Any]:
...
def print_status_report(stats: Dict[str, Any]):
    ...
    for status, count in stats['status_breakdown'].items():
        percentage = (count / stats['total_races']) * 100 if stats['total_races'] > 0 else 0
    ...
    if stats['recent_complete']:
        ...
        for race in stats['recent_complete']:
            source_display = f" ({race['winner_source']})" if race['winner_source'] else ""
    ...
    if stats['total_pending'] > 0:
        high_priority = sum(count for attempts, count in stats['pending_by_attempts'].items() if attempts == 0)
        medium_priority = sum(count for attempts, count in stats['pending_by_attempts'].items() if 1 <= attempts <= 2)
        low_priority = sum(count for attempts, count in stats['pending_by_attempts'].items() if attempts >= 3)
        
        if high_priority > 0:
            print(f"   🔴 HIGH PRIORITY: {high_priority:,} races with 0 scraping attempts")
        ...
        if completion_rate < 0.5:
            print(f"   ⚠️  Low completion rate ({completion_rate:.1%}) - consider running comprehensive backfill")
        elif completion_rate < 0.8:
            print(f"   ✅ Good completion rate ({completion_rate:.1%}) - run targeted backfill on high priority races")
        else:
            print(f"   🎉 Excellent completion rate ({completion_rate:.1%}) - minimal backfill needed")
    else:
