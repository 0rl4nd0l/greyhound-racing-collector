    ...
    def create_features(self, data):
        ...
        if track_condition in track_performance:
            ...
            if isinstance(condition_data, dict) and condition_data.get('races', 0) >= 2:
                features['track_condition_win_rate'] = condition_data.get('win_rate', 0.125)
                features['track_condition_avg_position'] = condition_data.get('avg_position', 4.5)
                features['track_condition_experience'] = min(1.0, condition_data.get('races', 0) / 10)
            else:
        ...
        if wind_speed:
            features['wind_impact'] = max(0, 1 - wind_speed / 20)  # Wind over 20km/h has significant impact
        else:
