...
class V3TrainerFeatures:
    ...
    def create_features(self, data):
        ...
        if trainer and trainer in trainer_stats:
            ...
            features['trainer_experience'] = min(1.0, trainer_data.get('total_races', 0) / 100)
            
            # Trainer recent form (last 30 days)
            recent_performance = trainer_data.get('recent_performance', {})
            features['trainer_recent_win_rate'] = recent_performance.get('win_rate', 0.125)
            features['trainer_recent_strike_rate'] = recent_performance.get('strike_rate', 0.3)
        else:
        ...
        if trainer and dog_name:
            ...
            combination_stats = data.get('trainer_dog_combinations', {}).get(combination_key, {})
            features['trainer_dog_win_rate'] = combination_stats.get('win_rate', 0.125)
            features['trainer_dog_races'] = min(1.0, combination_stats.get('races', 0) / 20)
        else:
        ...
        features['trainer_effectiveness'] = np.mean([
            ...
            1 - (features['trainer_avg_position'] - 1) / 7,  # Convert position to score
