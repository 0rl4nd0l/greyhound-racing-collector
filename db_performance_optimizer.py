...
class SQLiteConnectionPool:
    ...
    def _create_connection(self) -> sqlite3.Connection:
        ...
        try:
            ...
            if os.environ.get("FLASK_ENV", "").lower() == "testing" or os.environ.get(
                "TESTING", "0"
            ).lower() in ("1", "true", "yes"):
                disable_wal = True
        except Exception:
...
class LazyModelLoader:
    ...
    def get_race_metadata(
        self, race_id: str = None, venue: str = None, limit: int = 100
    ):
        ...
        logger.info(
            f"🔍 Loaded {len(data)} race metadata records in {(end_time - start_time)*1000:.2f}ms"
        )
        return data
    ...
    def get_dog_data(self, dog_name: str = None, limit: int = 100):
        ...
        logger.info(
            f"🐕 Loaded {len(data)} dog records in {(end_time - start_time)*1000:.2f}ms"
        )
        return data
    ...
    def get_recent_race_performance(self, dog_name: str, days: int = 30):
        ...
        logger.info(
            f"📈 Loaded {len(data)} recent performance records for {dog_name} in {(end_time - start_time)*1000:.2f}ms"
        )
        return data
...
def initialize_db_optimization(
    database_path: str = "greyhound_racing_data.db", pool_size: int = 20
):
