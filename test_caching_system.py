...
"""
        ...
        cursor.execute("SELECT file_hash, race_date, venue, race_no, status FROM processed_race_files")
        rows = cursor.fetchall()
        for row in rows:
            print(f"  Hash: {row[0][:8]}..., Date: {row[1]}, Venue: {row[2]}, Race: {row[3]}, Status: {row[4]}")
        ...
        different_csv_content = test_csv_content.replace("Test Dog 1", "Different Dog 1")
