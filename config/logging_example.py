...
def example_prediction_logging():
    ...
    # Model training logs
    log_prediction("Starting model training pipeline", level="INFO",
                   action="train_model", outcome="started",
                   details={"model_type": "ExtraTreesClassifier", "version": "v4", "dataset_size": 10000})
    
    log_prediction("Model training completed", level="INFO", 
                   action="train_model", outcome="success",
                   cache_status="stored",
                   details={"accuracy": 0.85, "training_time": "2.5 minutes"})
    
    # Inference logs
    log_prediction("Running batch prediction", level="INFO",
                   action="batch_predict", outcome="in_progress",
...
def example_test_logging():
    ...
    log_test("Running comprehensive test suite", level="INFO",
             action="run_tests", outcome="started",
             details={"test_types": ["unit", "integration", "regression"], "total_tests": 250})
    
    log_test("Test suite completed", level="INFO",
             action="run_tests", outcome="success", 
             validation_errors=[],
             details={"passed": 248, "failed": 2, "skipped": 0})
    
    # Error case
    log_test("Test failure detected", level="WARNING",
             action="test_failure", outcome="failed",
             validation_errors=["test_prediction_accuracy: AssertionError"],
             details={"test_name": "test_prediction_accuracy", "expected": 0.8, "actual": 0.75})

def example_qa_logging():
    """Example of QA-related logging"""
    print("🔍 Example: QA logging")
    
    log_qa("Starting data integrity check", level="INFO",
           action="integrity_check", outcome="started",
           details={"tables_to_check": ["races", "dogs", "results"], "check_type": "full_scan"})
    
    log_qa("Data integrity check completed", level="INFO",
           action="integrity_check", outcome="success",
           validation_errors=[],
           details={"records_validated": 50000, "errors_found": 0})
    
    # QA issue detected
    log_qa("Data quality issue detected", level="WARNING",
           action="quality_check", outcome="issues_found",
...
def example_gpt_assistant_logging():
    ...
    log_gpt_assistant("Processing GPT enhancement request", level="INFO",
                      action="gpt_enhance", outcome="processing",
                      details={"request_type": "prediction_analysis", "model": "gpt-4"})
    
    log_gpt_assistant("GPT analysis completed", level="INFO",
                      action="gpt_enhance", outcome="success",
                      cache_status="cached",
...
def example_config_logging():
    ...
    log_config("Loading system configuration", level="INFO",
               action="config_load", outcome="started",
               details={"config_files": ["defaults.yaml", "logging_config.py"]})
    
    log_config("Configuration validation passed", level="INFO",
               action="config_validate", outcome="success",
               validation_errors=[],
...
def example_component_detection():
    ...
    logger.info("Running unit tests for data validation",
    ...
    logger.info("AI enhancement processing",
...
def example_debug_logging():
    ...
    log_prediction("Debug: Feature extraction details", level="DEBUG",
                   action="extract_features", outcome="debug_info",
                   details={"features_extracted": 33, "processing_time": 0.05})
    
    log_test("Debug: Test setup information", level="DEBUG",
             action="test_setup", outcome="debug_info", 
