...
class ComponentLogger:
    ...
    def _get_component_from_context(self, **kwargs) -> str:
        ...
        if any(keyword in action for keyword in ['predict', 'model', 'train', 'ml', 'inference', 'calibrat']):
            return 'prediction'
        
        # Test-related keywords  
        if any(keyword in action for keyword in ['test', 'validate', 'assert', 'check']):
            return 'test'
        
        # QA-related keywords
        if any(keyword in action for keyword in ['quality', 'integrity', 'audit', 'verify']):
            return 'qa'
...
class StructuredLoggingConfig:
    ...
    def __init__(self, log_dir: str = "./logs", debug_mode: bool = None):
        self.log_dir = Path(log_dir)
        self.debug_mode = debug_mode if debug_mode is not None else self._check_debug_mode()
...
if __name__ == "__main__":
    ...
    log_prediction("Model training initiated", level="INFO", 
                   action="train_model", outcome="started",
                   details={"model_type": "ExtraTreesClassifier", "version": "v4"})
    
    log_test("Running unit tests", level="INFO",
             action="run_tests", outcome="in_progress", 
    ...
    log_config("System configuration updated", level="INFO",
               action="config_update", outcome="success",
               details={"config_file": "logging_config.py"})
    
    print("✅ Structured logging test completed - check log files in respective directories")
