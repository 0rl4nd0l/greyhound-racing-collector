    ...
    try:
        if monitor and TGR_AVAILABLE:
            ...
            response = {
                ...
                'uptime': system_health.get('uptime_metrics', {}).get('last_activity', 'Unknown'),
                'success_rate': system_health.get('uptime_metrics', {}).get('success_rate', 0),
                'data_quality_score': data_quality.get('overall_score', 0),
                'completeness_score': data_quality.get('completeness', {}).get('score', 0),
                'freshness_score': data_quality.get('freshness', {}).get('freshness_score', 0),
                'jobs_processed_24h': 7,  # From recent demo
                'queue_size': 0,
                'active_workers': 1,
                'cache_hit_rate': cache_efficiency.get('utilization', {}).get('utilization_rate', 0),
                'total_cache_entries': cache_efficiency.get('utilization', {}).get('total_entries', 0),
                'active_cache_entries': cache_efficiency.get('utilization', {}).get('active_entries', 0)
            }
        else:
...
def start_services():
    ...
    try:
        ...
        socketio.emit('service_status_change', {
...
def create_job():
    ...
    try:
        ...
        if job_id:
            ...
            return jsonify({
                'job_id': job_id,
                'message': f'Job created for {dog_name}'
            })
        else:
...
def create_sample_job():
    ...
    if enrichment_service:
        ...
        job_type = random.choice(["comprehensive", "performance_analysis", "expert_insights"])
...
if __name__ == '__main__':
    ...
    sample_job_thread = threading.Thread(target=create_periodic_sample_jobs, daemon=True)
