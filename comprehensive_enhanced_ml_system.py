                ...
                try:
                    ...
                    with open(csv_file, "r", encoding="utf-8", errors="ignore") as f:
                        ...
                        if "DOCTYPE html" in first_line or "<html>" in first_line:
                            print(f"🗑️ Deleting HTML file: {csv_file.name}")
                            csv_file.unlink()  # Delete the file
                            skipped_files += 1
                            continue
                        if not first_line.startswith("Dog Name"):
                            print(
                                f"🗑️ Deleting non-standard CSV: {csv_file.name} (starts with: {first_line[:50]}...)"
                            )
                            csv_file.unlink()  # Delete the file
                            skipped_files += 1
                            continue
        ...
        try:
            ...
            for _, row in race_results_df.iterrows():
                ...
                if len(db_historical_data) >= 1 or len(historical_form_data) >= 1:
                    ...
                    temperature = safe_float_convert(row.get("temperature"), 15.0)
                    humidity = safe_float_convert(row.get("humidity"), 60.0)
                    wind_speed = safe_float_convert(row.get("wind_speed"), 10.0)
                    pressure = safe_float_convert(row.get("pressure"), 1013.0)
                    weather_adjustment_factor = safe_float_convert(
                        row.get("weather_adjustment_factor"), 1.0
                    )
