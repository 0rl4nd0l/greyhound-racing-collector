...
class ComprehensiveEnhancedMLSystem:
    def __init__(self, db_path="greyhound_racing_data.db"):
        ...
        self.base_models = {
            ...
            'calibrated_random_forest': {
                ...
                'params': {
                    'n_estimators': 300, 'max_depth': 20, 'min_samples_split': 3,
                    'class_weight': {0: 0.6, 1: 1.4}, 'random_state': 42
                }
            },
            ...
            'neural_network': {
                'model': MLPClassifier,
                'params': {'hidden_layer_sizes': (150, 100, 50), 'max_iter': 2000, 'random_state': 42}
            }
        }
    ...
    def parse_historical_race_data(self, race_data_str):
        ...
        try:
            ...
            if len(parts) >= 10:
                try:
                    race_info['position'] = int(parts[2]) if parts[2].isdigit() else None
                    race_info['box'] = int(parts[3]) if parts[3].isdigit() else None
                    race_info['weight'] = float(parts[4]) if parts[4].replace('.', '').isdigit() else None
                    race_info['distance'] = int(parts[5]) if parts[5].isdigit() else None
                    ...
                    race_info['time'] = float(parts[9]) if parts[9].replace('.', '').isdigit() else None
                    race_info['starting_price'] = float(parts[-1]) if parts[-1].replace('.', '').isdigit() else None
                except:
                    pass
        ...
        try:
            ...
            for _, row in race_results_df.iterrows():
                ...
                if len(db_historical_data) >= 1 or len(historical_form_data) >= 1:
                    ...
                    for form_entry in historical_form_data[:20]:  # Use up to 20 historical races
                        try:
                            # Parse position
                            ...
                            if time_str:
                                try:
                                    ...
                                    if 15.0 <= time_val <= 60.0:  # Reasonable time range
                            ...
                            if weight_str:
                                try:
                                    ...
                                    if 20.0 <= weight_val <= 40.0:  # Reasonable weight range
                            ...
                            if dist_str:
                                try:
                                    ...
                                    if 300 <= dist_val <= 800:  # Reasonable distance range
                    ...
                    recent_form_avg = np.mean(all_positions[:8]) if len(all_positions) >= 8 else np.mean(all_positions)
                    win_rate = sum(1 for p in all_positions if p == 1) / len(all_positions)
                    place_rate = sum(1 for p in all_positions if p <= 3) / len(all_positions)
                    
                    # Time consistency (stable feature)
                    time_consistency = 1 / (np.std(all_times) + 0.1) if len(all_times) > 1 else 0.5
                    
                    # Market confidence (high impact)
                    market_confidence = 1 / (np.mean(all_odds) + 1) if all_odds else 0.1
                    try:
                        current_odds_log = np.log(float(row['starting_price']) + 1) if pd.notna(row['starting_price']) else np.log(10)
                    except (ValueError, TypeError):
                    ...
                    if len(all_positions) >= 10:
                        ...
                        older_10 = np.mean(all_positions[10:20]) if len(all_positions) >= 20 else np.mean(all_positions[10:])
                        long_term_form_trend = (older_10 - recent_10) / (older_10 + 1) if older_10 > 0 else 0
                    else:
                    ...
                    if all_times:
                        ...
                        time_improvement_trend = -np.polyfit(range(len(all_times)), all_times, 1)[0] if len(all_times) > 2 else 0
                    else:
                        avg_time = 30.0
                        best_time = 28.0
                    ...
                    if all_weights:
                        ...
                        try:
                            current_weight = float(row['weight']) if pd.notna(row['weight']) else avg_weight
                        ...
                        try:
                            current_weight = float(row['weight']) if pd.notna(row['weight']) else 30.0
                        except (ValueError, TypeError):
                            current_weight = 30.0
                        weight_vs_avg = 0
                    ...
                    try:
                        current_distance = float(str(row['distance']).replace('m', '')) if pd.notna(row['distance']) else 500.0
                    except (ValueError, TypeError):
                        current_distance = 500.0
                    if form_distances:
                        distance_experience_count = sum(1 for d in form_distances if abs(d - current_distance) <= 50)
                        distance_specialization = distance_experience_count / len(form_distances) if form_distances else 0
                    else:
                        distance_specialization = 0.1
                    
                    # Track/Venue experience
                    current_venue = row['venue']
                    venue_experience = len([r for _, r in db_historical_data.iterrows() if r['venue'] == current_venue])
                    venue_experience += sum(1 for track in form_tracks if track and current_venue in str(track))
                    
                    # Grade/Class analysis
                    current_grade = row['grade']
                    grade_experience = len([r for _, r in db_historical_data.iterrows() if r['grade'] == current_grade])
                    
                    # Recent activity pattern
                    try:
                        if len(db_historical_data) > 0:
                            last_race_date = datetime.fromisoformat(db_historical_data.iloc[0]['race_date'])
                    ...
                    fitness_score = (1 / (1 + days_since_last/30)) * (1 + place_rate) * (1 + win_rate)
                    
                    # Competition strength
                    try:
                        field_size = int(row['field_size']) if pd.notna(row['field_size']) else 6
                    except (ValueError, TypeError):
                        field_size = 6
                    ...
                    try:
                        current_box = int(row['box_number']) if pd.notna(row['box_number']) else 4
                    except (ValueError, TypeError):
                        current_box = 4
                    box_win_rate = 0
                    if len(db_historical_data) > 0:
                        same_box_races = db_historical_data[db_historical_data['box_number'] == current_box]
                        if len(same_box_races) > 0:
                            same_box_positions = [int(''.join(filter(str.isdigit, str(p)))) 
                                                for p in same_box_races['finish_position'] 
                                                if pd.notna(p) and ''.join(filter(str.isdigit, str(p)))]
                            box_win_rate = sum(1 for p in same_box_positions if p == 1) / len(same_box_positions) if same_box_positions else 0
                    ...
                    if TRADITIONAL_ANALYSIS_AVAILABLE:
                        try:
                            ...
                            if processed_count % 50 == 0:  # Log occasionally
                                print(f"     Traditional analysis for {dog_name}: {traditional_features.get('traditional_overall_score', 0):.3f}")
                                
                        except Exception as e:
                            # Don't let traditional analysis errors stop the training
                            if processed_count % 100 == 0:  # Log errors occasionally
                                print(f"     ⚠️ Traditional analysis failed for {dog_name}: {e}")
                            traditional_features = {}
                    
                    # WEATHER FEATURES (Priority 2 - Environmental Impact)
                    weather_condition = str(row.get('weather_condition', '')).strip().lower()
                    temperature = float(row.get('temperature', 15.0)) if pd.notna(row.get('temperature')) else 15.0
                    humidity = float(row.get('humidity', 60.0)) if pd.notna(row.get('humidity')) else 60.0
                    wind_speed = float(row.get('wind_speed', 10.0)) if pd.notna(row.get('wind_speed')) else 10.0
                    pressure = float(row.get('pressure', 1013.0)) if pd.notna(row.get('pressure')) else 1013.0
                    weather_adjustment_factor = float(row.get('weather_adjustment_factor', 1.0)) if pd.notna(row.get('weather_adjustment_factor')) else 1.0
                    
                    # Weather condition encoding
                    weather_clear = 1 if 'clear' in weather_condition or 'sunny' in weather_condition else 0
                    weather_cloudy = 1 if 'cloud' in weather_condition or 'overcast' in weather_condition else 0
                    weather_rain = 1 if 'rain' in weather_condition or 'shower' in weather_condition else 0
                    weather_fog = 1 if 'fog' in weather_condition or 'mist' in weather_condition else 0
                    ...
                    if len(db_historical_data) > 0:
                        similar_weather_races = db_historical_data[
                            (db_historical_data['weather_condition'].str.lower().str.contains(weather_condition, na=False)) |
                            (db_historical_data['weather_condition'].isnull() & (weather_condition == ''))
                        ]
                        if len(similar_weather_races) > 0:
                            weather_experience_count = len(similar_weather_races)
                            weather_positions = [int(''.join(filter(str.isdigit, str(p)))) 
            ...
            print(f"✅ Created comprehensive features for {len(enhanced_records)} records")
    ...
    def prepare_comprehensive_features(self, df):
        ...
        try:
            ...
            priority_1_features = [  # Stable, high-impact features
                'avg_position', 'recent_form_avg', 'market_confidence', 'current_odds_log',
                'venue_experience', 'place_rate', 'current_weight', 'time_consistency',
                # Traditional analysis core features (Priority 1 - Expert Knowledge)
                'traditional_overall_score', 'traditional_performance_score', 'traditional_form_score',
                'traditional_consistency_score', 'traditional_confidence_level'
            ]
            
            priority_2_features = [  # Enhanced comprehensive features
                'win_rate', 'long_term_form_trend', 'position_consistency', 'avg_time', 
                'best_time', 'time_improvement_trend', 'avg_weight', 'weight_consistency',
                'weight_vs_avg', 'distance_specialization', 'grade_experience', 'fitness_score',
                # Traditional analysis extended features
                'traditional_class_score', 'traditional_fitness_score', 'traditional_experience_score',
                'traditional_trainer_score', 'traditional_track_condition_score', 'traditional_distance_score',
                'traditional_key_factors_count', 'traditional_risk_factors_count',
                # Weather features
                'temperature', 'humidity', 'wind_speed', 'pressure', 'weather_adjustment_factor',
                'weather_clear', 'weather_cloudy', 'weather_rain', 'weather_fog',
                'temp_cold', 'temp_cool', 'temp_optimal', 'temp_warm', 'temp_hot',
                'wind_calm', 'wind_light', 'wind_moderate', 'wind_strong',
                'humidity_low', 'humidity_normal', 'humidity_high',
                'weather_experience_count', 'weather_performance'
            ]
            
            priority_3_features = [  # Context and situational features
                'days_since_last', 'competition_strength', 'box_win_rate', 'current_box', 
                'field_size', 'historical_races_count'
            ]
            ...
            df['track_condition_encoded'] = le_condition.fit_transform(df['track_condition'].fillna('Good'))
            ...
            feature_columns = (priority_1_features + priority_2_features + 
                             priority_3_features + ['venue_encoded', 'track_condition_encoded', 
            ...
            target_columns = ['is_winner', 'is_placer', 'race_date', 'race_id', 'dog_name']
            ...
            complete_df[available_features] = imputer.fit_transform(complete_df[available_features])
    ...
    def train_models(self):
        ...
        place_accuracy = self.evaluate_ensemble(place_models, X_test, y_place_test, 'place')
        ...
        win_status = "🟢 EXCELLENT" if win_accuracy > 0.65 else "🟡 GOOD" if win_accuracy > 0.55 else "🔴 NEEDS WORK"
        place_status = "🟢 EXCELLENT" if place_accuracy > 0.70 else "🟡 GOOD" if place_accuracy > 0.60 else "🔴 NEEDS WORK"
    ...
    def run_comprehensive_analysis(self):
        ...
        try:
            ...
            prepared_df, feature_columns = self.prepare_comprehensive_features(enhanced_df)
            if prepared_df is None:
                print("❌ Comprehensive feature preparation failed")
                return None
            
            print(f"📊 Comprehensive dataset: {len(prepared_df)} samples, {len(feature_columns)} features")
            ...
            results = {
                ...
                'data_summary': {
                    ...
                    'avg_historical_races': np.mean([len(races) for races in form_data.values()]) if form_data else 0
                },
                'model_results': {},
                'ensemble_results': {},
            ...
            ensemble_models = self.create_advanced_ensemble_models(X_train_scaled, y_train)
            ...
            try:
                ...
                if update_success:
                    print("✅ Prediction system automatically updated with latest insights!")
                else:
    ...
    def auto_optimize_model_parameters(self, prepared_df, feature_columns):
        ...
        try:
            ...
            print(f"📊 Training: {len(train_df)} races, Validation: {len(val_df)} races")
            ...
            for weight_name, weight_config in class_weight_configs.items():
                model_configs[f'rf_{weight_name}'] = {
                    ...
                    'params': {
                        'n_estimators': 300, 'max_depth': 20, 'min_samples_split': 3,
                        'class_weight': weight_config, 'random_state': 42
                    },
                    'type': 'RandomForest',
            ...
            for weight_name, weight_config in list(class_weight_configs.items())[:3]:  # Test top 3
            ...
            for config_name, config in model_configs.items():
                try:
                    ...
                    realistic_probs = np.sum((y_pred_proba >= 0.05) & (y_pred_proba <= 0.8)) / len(y_pred_proba)
                    ...
                    print(f"Acc: {accuracy:.3f}, AUC: {auc:.3f}, F1: {f1:.3f}, Prob Range: {prob_range:.3f}")
            ...
            for result in validation_results:
                ...
                composite_score = (
                    ...
                    min(result['prob_range'] / 0.5, 1.0) * 0.1  # Good probability spread
                )
            ...
            print("-" * 70)
            print(f"{'Rank':<4} {'Model':<20} {'Accuracy':<8} {'AUC':<6} {'F1':<6} {'ProbRange':<10} {'Composite':<10}")
            print("-" * 70)
            
            for i, result in enumerate(validation_results[:5]):
                print(f"{i+1:<4} {result['config_name']:<20} {result['accuracy']:<8.3f} {result['auc']:<6.3f} {result['f1_score']:<6.3f} {result['prob_range']:<10.3f} {result['composite_score']:<10.3f}")
            ...
            print(f"   📊 Realistic Probabilities: {best_result['realistic_prob_ratio']:.1%}")
    ...
    def validate_model_on_historical_races(self, model, scaler, feature_columns, test_df):
        ...
        try:
            ...
            for result in threshold_results:
                print(f"{result['threshold']:<10.1f} {result['accuracy']:<10.3f} {result['precision']:<10.3f} {result['recall']:<10.3f} {result['f1_score']:<10.3f}")
            ...
            print(f"{'Prob Range':<12} {'Count':<8} {'Accuracy':<10} {'Actual WR':<12} {'Pred WR':<12} {'Cal Error':<12}")
            print("-" * 75)
            
            for result in bin_analysis:
                print(f"{result['prob_range']:<12} {result['count']:<8} {result['accuracy']:<10.3f} {result['actual_win_rate']:<12.3f} {result['predicted_win_rate']:<12.3f} {result['calibration_error']:<12.3f}")
            
            # Overall calibration quality
            avg_calibration_error = np.mean([r['calibration_error'] for r in bin_analysis])
            print(f"\n📊 AVERAGE CALIBRATION ERROR: {avg_calibration_error:.3f}")
            
            # Top prediction analysis
            top_predictions_mask = y_pred_proba >= np.percentile(y_pred_proba, 90)  # Top 10% predictions
            if np.sum(top_predictions_mask) > 0:
                top_prediction_accuracy = y_test[top_predictions_mask].mean()
                print(f"📊 TOP 10% PREDICTIONS WIN RATE: {top_prediction_accuracy:.1%} (Expected: ~{y_pred_proba[top_predictions_mask].mean():.1%})")
    ...
    def create_advanced_ensemble_models(self, X_train, y_train):
        ...
        try:
            ...
            bagging_clf = BaggingClassifier(
                estimator=ExtraTreesClassifier(n_estimators=50, max_depth=10, class_weight='balanced', random_state=42),
                n_estimators=10,
                random_state=42
            )
            ...
            if self.imbalanced_models:
                for name, config in self.imbalanced_models.items():
                    ensemble_models[f'ensemble_{name}'] = config['model'](**config['params'])
    ...
    def _run_comprehensive_validation(self, test_df, feature_columns, best_model, scaler):
        ...
        try:
            ...
            if TRADITIONAL_ANALYSIS_AVAILABLE and 'traditional_overall_score' in test_df.columns:
                traditional_scores = test_df['traditional_overall_score'].fillna(0.5).tolist()
                
                # Convert traditional scores to binary predictions (threshold = 0.6)
                traditional_threshold = 0.6
                traditional_predictions = [1 if score >= traditional_threshold else 0 for score in traditional_scores]
                ...
                if len(traditional_predictions) > 0:
                    ...
                    print(f"   📊 Pure Traditional Accuracy: {traditional_accuracy:.3f}")
                    print(f"   📊 Pure Traditional AUC: {traditional_auc:.3f}")
                    print(f"   📊 Average Traditional Score: {np.mean(traditional_scores):.3f}")
                    ...
                    validation_results['pure_traditional'] = {'error': 'No valid predictions'}
            else:
                print("   ⚠️ Traditional analysis features not available in test data")
                validation_results['pure_traditional'] = {'error': 'Traditional features missing'}
            ...
            if traditional_scores and len(traditional_scores) == len(ml_probabilities):
                ...
                combined_predictions = [1 if score >= combined_threshold else 0 for score in combined_scores]
                ...
                print(f"   📊 ML Weight: {ml_weight}, Traditional Weight: {traditional_weight}")
                ...
                improvement_vs_traditional = combined_accuracy - validation_results.get('pure_traditional', {}).get('accuracy', 0)
                
                print(f"   📈 Combined vs Pure ML: {improvement_vs_ml:+.3f} accuracy")
                if 'pure_traditional' in validation_results and 'accuracy' in validation_results['pure_traditional']:
                    print(f"   📈 Combined vs Pure Traditional: {improvement_vs_traditional:+.3f} accuracy")
                ...
                validation_results['performance_comparison'] = {
                    ...
                    'best_approach': 'combined' if combined_accuracy >= ml_accuracy else 'ml'
                }
                
            else:
                print("   ⚠️ Cannot test combined approach - traditional scores not available")
                validation_results['combined'] = {'error': 'Traditional scores not available for combination'}
            ...
            if TRADITIONAL_ANALYSIS_AVAILABLE:
                traditional_feature_columns = [col for col in feature_columns if col.startswith('traditional_')]
                print(f"   📊 Traditional features in model: {len(traditional_feature_columns)}")
            ...
            if traditional_scores and len(traditional_scores) == len(ml_probabilities):
                ...
                for i, (ml_prob, trad_score, actual) in enumerate(zip(ml_probabilities, traditional_scores, y_test)):
                ...
                print(f"   📊 Found {len(divergent_cases)} cases with divergence >= {divergence_threshold}")
                ...
                if divergent_cases:
                    ...
                    for case in divergent_cases:
                        ...
                        traditional_prediction = 1 if case['traditional_score'] >= 0.6 else 0
                    ...
                    traditional_divergent_accuracy = traditional_correct_divergent / len(divergent_cases)
                    
                    print(f"   📊 ML accuracy in divergent cases: {ml_divergent_accuracy:.3f}")
                    print(f"   📊 Traditional accuracy in divergent cases: {traditional_divergent_accuracy:.3f}")
                    
                    # Show some examples
                    print(f"   📊 Example divergent cases (first 3):")
                    for i, case in enumerate(divergent_cases[:3]):
                        print(f"      {i+1}. {case['dog_name']} - ML: {case['ml_score']:.3f}, Traditional: {case['traditional_score']:.3f}, Actual: {case['actual_winner']}")
    ...
    def predict_race(self, race_data):
        ...
        for i, (_, dog_data) in enumerate(race_data.iterrows(), 1):
            if i % 3 == 1 or len(race_data) <= 8:  # Show progress for every 3rd dog or all if ≤8 dogs
                print(f"      [{i}/{len(race_data)}] {dog_data.get('dog_name', f'Dog {i}')}...")
            ...
            try:
                ...
                print(f"      ⚠️ Error predicting {dog_data.get('dog_name', f'Dog {i}')}: {e}")
                continue
        ...
        print("═" * 80)
        print(f"{'Rank':<4} {'Dog Name':<20} {'Win Prob':<10} {'Place Prob':<11} {'Confidence':<12} {'Status':<8}")
        print("─" * 80)
        ...
        for i, (_, pred) in enumerate(predictions_df.iterrows()):
            ...
            print(f"{rank:<4} {pred['dog_name']:<20} {pred['win_probability']:.3f}      {pred['place_probability']:.3f}       {confidence_level:<12} {status}")
        
        # Summary statistics
        avg_win_prob = predictions_df['win_probability'].mean()
        top_dog_advantage = predictions_df.iloc[0]['win_probability'] - predictions_df.iloc[1]['win_probability'] if len(predictions_df) > 1 else 0
        
        print("═" * 80)
        print(f"📈 RACE ANALYSIS SUMMARY:")
        print(f"   🎯 Favorite: {predictions_df.iloc[0]['dog_name']} ({predictions_df.iloc[0]['win_probability']:.3f})")
        print(f"   📊 Average Win Probability: {avg_win_prob:.3f}")
        print(f"   ⚡ Top Dog Advantage: {top_dog_advantage:.3f}")
        print(f"   🏁 Field Competitiveness: {'TIGHT' if top_dog_advantage < 0.1 else 'MODERATE' if top_dog_advantage < 0.2 else 'CLEAR FAVORITE'}")
    ...
    def _create_dog_features_for_prediction(self, dog_info, dog_historical, dog_form_data, race_info, feature_columns):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Comprehensive Enhanced ML Model System')
    parser.add_argument('--command', choices=['analyze', 'status'], 
                       default='analyze', help='Command to execute')
    ...
    if args.command == 'analyze':
        ...
        result_files = len(list(system.results_dir.glob('comprehensive_analysis_*.json')))
        form_files = len(list(system.downloaded_forms_dir.glob('*.csv'))) if system.downloaded_forms_dir.exists() else 0
