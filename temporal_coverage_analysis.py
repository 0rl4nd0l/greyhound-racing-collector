...
class TemporalCoverageAnalyzer:
    ...
    def build_race_timelines(self, df):
        ...
        # Daily timeline
        timelines['daily'] = df.groupby('race_date').agg({
            'race_id': 'count',
            'venue': 'nunique',
            'field_size': ['mean', 'sum']
        }).round(2)
        timelines['daily'].columns = ['race_count', 'unique_venues', 'avg_field_size', 'total_dogs']
        ...
        timelines['weekly'].columns = ['race_count', 'unique_venues', 'week_start', 'week_end']
        ...
        timelines['monthly'].columns = ['race_count', 'unique_venues', 'month_start', 'month_end']
        ...
        timelines['yearly'].columns = ['race_count', 'unique_venues', 'year_start', 'year_end']
        
        # Venue-specific timeline
        timelines['by_venue'] = df.groupby(['venue', 'year_month']).agg({
            'race_id': 'count'
    ...
    def detect_extraction_anomalies(self, df):
        ...
        daily_extractions = df_extract.groupby('extraction_date').agg({
            'race_id': 'count',
            'extraction_timestamp': ['min', 'max']
        })
        daily_extractions.columns = ['records_extracted', 'first_extraction', 'last_extraction']
        ...
        if len(extraction_counts) > 0:
            ...
            unusual_extraction_days = extraction_counts[extraction_counts > upper_threshold]
        else:
    ...
    def verify_monotonicity(self, df):
        ...
        df_time_sorted = df.dropna(subset=['race_date', 'extraction_timestamp']).sort_values('extraction_timestamp')
        ...
        for i in range(1, len(df_time_sorted)):
            ...
            if current_race_date < previous_race_date and current_extract_time > previous_extract_time:
    ...
    def check_timezone_anomalies(self, df):
        ...
        daily_extract_patterns = df_tz.groupby(df_tz['extraction_timestamp'].dt.date).agg({
            'extract_hour': ['min', 'max', 'std'],
            'race_id': 'count'
        })
        daily_extract_patterns.columns = ['min_hour', 'max_hour', 'hour_std', 'record_count']
        
        # Detect unusual hour patterns (possible timezone shifts)
        unusual_hours = daily_extract_patterns[
            (daily_extract_patterns['hour_std'] > 6) |  # High variation in extraction hours
            (daily_extract_patterns['min_hour'] < 3) |  # Very early extractions
            (daily_extract_patterns['max_hour'] > 23)   # Very late extractions
        ]
        ...
        timezone_analysis = {
            ...
            'extraction_hour_distribution': df_tz['extract_hour'].value_counts().to_dict(),
            'time_span_analysis': {
                'total_days': (df_tz['extraction_timestamp'].max() - df_tz['extraction_timestamp'].min()).days,
                'unique_extraction_dates': df_tz['extraction_timestamp'].dt.date.nunique()
            }
        }
    ...
    def generate_visualizations(self, df):
        ...
        venue_timeline = df.groupby(['year_month', 'venue'])['race_id'].count().unstack(fill_value=0)
    ...
    def print_summary_report(self):
        ...
        if 'timelines' in results:
            ...
            if 'yearly' in timelines:
                ...
                for year, data in timelines['yearly'].iterrows():
                    print(f"  {year}: {data['race_count']} races across {data['unique_venues']} venues")
            ...
            if 'monthly' in timelines:
                ...
                for month, data in recent_months.iterrows():
                    print(f"  {month}: {data['race_count']} races, {data['unique_venues']} venues")
        ...
        if 'anomalies' in results:
            ...
            print(f"Consecutive gap periods: {len(anomalies['consecutive_gap_periods'])}")
            
            if anomalies['consecutive_gap_periods']:
                print("  Significant gap periods:")
                for gap in anomalies['consecutive_gap_periods'][:5]:
                    print(f"    {gap['start']} to {gap['end']} ({gap['duration']} days)")
            
            print(f"Unusual spike days: {len(anomalies['spikes'])}")
            if anomalies['spikes']:
                print("  Top spike days:")
                spikes_sorted = sorted(anomalies['spikes'].items(), key=lambda x: x[1], reverse=True)
        ...
        if 'monotonicity' in results:
            ...
            if mono['id_gaps']:
                ...
                for gap in mono['id_gaps'][:3]:
                    print(f"    Gap of {gap['gap_size']} between IDs {gap['after_id']} and {gap['before_id']}")
        ...
        if 'timezone' in results:
            ...
            if 'error' not in tz:
                print(f"Potential DST transition dates: {len(tz['potential_dst_transition_dates'])}")
                print(f"Unusual extraction patterns: {len(tz['unusual_extraction_patterns'])}")
                
                time_span = tz['time_span_analysis']
                print(f"Data collection span: {time_span['total_days']} days")
                print(f"Unique extraction dates: {time_span['unique_extraction_dates']}")
                ...
                if hour_dist:
                    ...
                    print(f"Most common extraction hour: {most_common_hour}:00 ({hour_dist[most_common_hour]} records)")
            else:
                print(f"Timezone analysis error: {tz['error']}")
        
        print("\n" + "=" * 80)
        print("Analysis complete! Check 'temporal_coverage_analysis.png' for visualizations.")
        print("=" * 80)
        ...
        with TemporalCoverageAnalyzer(database_path) as analyzer:
            ...
            simplified_results = {
                'summary': {
                    ...
                    'date_range': {
                        'start': str(results.get('monotonicity', {}).get('extraction_time_range', {}).get('min', '')),
                        'end': str(results.get('monotonicity', {}).get('extraction_time_range', {}).get('max', ''))
                    },
                    'anomalies_found': {
                        'gaps': results.get('anomalies', {}).get('total_gaps', 0),
                        'spikes': len(results.get('anomalies', {}).get('spikes', {})),
                        'id_gaps': len(results.get('monotonicity', {}).get('id_gaps', [])),
                        'timestamp_issues': len(results.get('monotonicity', {}).get('timestamp_anomalies', []))
                    }
                }
            }
