...
class EndpointCache:
    ...
    def __init__(self, default_ttl: int = 30, max_entries: int = 1000):
        ...
        logger.info(
            f"🚀 EndpointCache initialized (TTL: {default_ttl}s, Max entries: {max_entries})"
        )
    ...
    def _evict_lru(self):
        ...
        sorted_entries = sorted(
            self._cache.items(), key=lambda x: (x[1].last_accessed, x[1].created_at)
        )

        entries_to_remove = (
            len(sorted_entries) - self.max_entries + 10
    ...
    def get(
        self, key: str, if_none_match: Optional[str] = None
    ...
    def set(self, key: str, data: Any, ttl: Optional[int] = None) -> str:
        ...
        logger.debug(
            f"📝 Cached data for key '{key}' (ETag: {etag[:8]}..., TTL: {ttl}s)"
        )
        return etag
