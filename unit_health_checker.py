...
class ModuleHealthChecker:
        ...
        for name in dir(module):
            ...
            if (callable(obj) and 
                (name.startswith('test_') or 
                 name.startswith('self_test') or
                 name.startswith('validate') or
                 name.startswith('check_') or
                 name == 'run_tests' or
                 name == 'diagnose')):
                ...
                for method_name in dir(obj):
                    ...
                    if (callable(method) and 
                        (method_name.startswith('test_') or 
                         method_name.startswith('self_test') or
                         method_name.startswith('validate') or
                         method_name == 'run_diagnostics')):
        ...
        for method_name in test_methods:
            try:
                ...
                try:
                    ...
                    results["test_results"][method_name] = {"success": True, "result": str(result)[:200]}
                except TypeError:
                    ...
                    try:
                        ...
                        results["test_results"][method_name] = {"success": False, "error": "Parameter mismatch"}
            except Exception as e:
            ...
            try:
                ...
                for test_method in test_methods:
                    try:
                        ...
                        results["errors"].append(f"{class_name}.{test_method}: {str(e)[:100]}")
    ...
    def check_single_module(self, module_name: str):
        ...
        if not import_success:
            ...
            self.log_result(module_name, "IMPORT_FAILED", {
                "step": "import",
                "error": import_error
            })
            return
        ...
        print(f"📋 Module structure: {len(module_attrs['main_classes'])} classes, {len(module_attrs['main_functions'])} functions")
        
        # Step 3: Run lightweight tests
        test_results = self.run_lightweight_tests(module)
        
        total_tests = len(test_results["module_functions_tested"]) + len(test_results["class_tests_run"])
        ...
        status = "SUCCESS" if import_success and errors_count == 0 else "PARTIAL_SUCCESS" if import_success else "FAILED"
        
        self.log_result(module_name, status, {
            "step": "complete_check",
    ...
    def run_all_checks(self):
        ...
        for module_name in self.modules_to_check:
            try:
                ...
                self.log_result(module_name, "CRITICAL_ERROR", {
                    "step": "health_check",
                    "error": {
                        "error_type": type(e).__name__,
                        "error_message": str(e),
        ...
        # Summary
        print("\n" + "=" * 60)
        print(f"🏁 Health Check Complete: {success_count}/{len(self.modules_to_check)} modules processed successfully")
