...
class OptimizedQueries:
    ...
    def get_comprehensive_system_stats(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            logger.debug(
                f"📊 Comprehensive system stats retrieved in {query_time*1000:.2f}ms"
            )
            return stats
                    ...
                    races.append(
                        {
                            ...
                            "race_number": (
                                r["race_number"] if "race_number" in r.keys() else r[2]
                            ),
                            "race_date": (
                                r["race_date"] if "race_date" in r.keys() else r[3]
                            ),
                            "race_name": (
                                r["race_name"] if "race_name" in r.keys() else r[4]
                            ),
                            "grade": r["grade"] if "grade" in r.keys() else r[5],
                            "distance": (
                                r["distance"] if "distance" in r.keys() else r[6]
                            ),
                            "field_size": (
                                r["field_size"] if "field_size" in r.keys() else r[7]
                            ),
                            "winner_name": (
                                r["winner_name"] if "winner_name" in r.keys() else r[8]
                            ),
                            "winner_odds": (
                                r["winner_odds"] if "winner_odds" in r.keys() else r[9]
                            ),
                            "winner_margin": (
                                r["winner_margin"]
                                if "winner_margin" in r.keys()
                                else r[10]
                            ),
                            "url": r["url"] if "url" in r.keys() else r[11],
                            "extraction_timestamp": (
                                r["extraction_timestamp"]
                                if "extraction_timestamp" in r.keys()
                                else r[12]
                            ),
                            "track_condition": (
                                r["track_condition"]
                                if "track_condition" in r.keys()
                                else r[13]
                            ),
                        }
                    )
