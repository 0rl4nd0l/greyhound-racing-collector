...
def fix_data_quality():
    ...
    try:
        ...
        print(f"   With positions: {with_positions:,} ({with_positions/total_dogs*100:.1f}%)")
        print(f"   Missing positions: {missing_positions:,} ({missing_positions/total_dogs*100:.1f}%)")
        ...
        if races_no_positions:
            ...
            print(f"   ✅ Removed {removed_dogs:,} dog entries from {len(races_no_positions)} races")
        else:
        ...
        if races_partial_missing:
            print(f"   Found {len(races_partial_missing)} races with partial missing positions")
            ...
            for race_id, total_dogs, positioned_dogs in races_partial_missing:
                ...
                """, (race_id,))
                existing_positions = [row[0] for row in cursor.fetchall() if row[0] not in ['N/A', '']]
            ...
            print(f"   ✅ Fixed position assignments for {len(races_partial_missing)} races")
        else:
        ...
        if field_size_removed > 0:
            print(f"   ✅ Removed {field_size_removed:,} entries from races with <3 or >20 dogs")
        else:
        ...
        if final_cleanup > 0:
            print(f"   ✅ Removed {final_cleanup:,} entries from races with remaining position issues")
        else:
        ...
        print(f"   With positions: {final_positioned:,} ({final_positioned/final_dogs*100:.1f}%)")
        print(f"   Missing positions: {final_missing:,} ({final_missing/final_dogs*100:.1f}%)")
        
        # Calculate improvement
        dogs_removed = total_dogs - final_dogs
        position_rate_improvement = (final_positioned/final_dogs*100) - (with_positions/total_dogs*100)
        
        print(f"\n✨ Improvements:")
        print(f"   Dogs removed: {dogs_removed:,}")
        print(f"   Position completeness improved by: {position_rate_improvement:+.1f}%")
