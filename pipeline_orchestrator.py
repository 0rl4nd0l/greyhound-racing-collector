...
class PipelineOrchestrator:
    ...
    def __init__(self):
        ...
        # Create directories
        for directory in [self.unprocessed_dir, self.processed_dir, self.upcoming_dir, self.predictions_dir]:
    ...
    def collect_upcoming_races(self) -> bool:
        ...
        try:
            ...
            races_file = self.upcoming_dir / f"upcoming_races_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                ...
                for race_file in upcoming_files:
                    try:
                        ...
                        if result.get('success'):
                            ...
                            logger.warning(f"⚠️ Prediction failed for {race_file.name}: {result.get('error')}")
                    except Exception as e:
                        logger.error(f"❌ Error predicting {race_file.name}: {e}")
                
                self.stats['predictions_generated'] = successful_predictions
                logger.info(f"🏁 Generated {successful_predictions}/{len(upcoming_files)} predictions")
    ...
    def run_data_validation(self) -> bool:
        ...
        try:
            ...
            if db_stats['total_races'] < 5:
                issues.append(f"Low race count: {db_stats['total_races']} (minimum 5 recommended)")
            
            if db_stats['total_dogs'] < db_stats['total_races'] * 3:
                issues.append(f"Low dog-to-race ratio: {db_stats['total_dogs']}/{db_stats['total_races']}")
    ...
    def run_full_pipeline(self, collect_historical: bool = True) -> Dict[str, Any]:
        ...
        results = {
            'success': all([historical_success, upcoming_success, processing_success, prediction_success, validation_success]),
            'steps': {
        ...
        logger.info(f"📊 Final stats: {final_db_stats['total_races']} races, {final_db_stats['total_dogs']} dogs")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Greyhound Racing Data Pipeline Orchestrator')
    parser.add_argument('--mode', choices=['full', 'quick', 'historical', 'process', 'predict'], 
                       default='full', help='Pipeline execution mode')
    parser.add_argument('--days-back', type=int, default=7, 
                       help='Days back for historical collection')
