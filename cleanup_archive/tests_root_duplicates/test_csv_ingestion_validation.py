...
def test_csv_ingestion_basic():
    ...
    try:
        from csv_ingestion import create_ingestor, ValidationLevel
    ...
    try:
        ...
        if test_dir.exists():
            ...
            for csv_file in csv_files:
                ...
                print(f"      Columns found: {len(validation_result.available_columns)}")
        ...
        if test_dir.exists():
            ...
            if csv_files:
                ...
                if processed_data:
                    ...
                    dog_name_count = sum(1 for record in processed_data if 'dog_name' in record)
                    print(f"   ✓ Records with dog_name field: {dog_name_count}/{len(processed_data)}")
                    ...
                    print(f"   📋 Sample dog_name mappings ({len(unique_dogs)} unique dogs):")
                    for i, (dog_name, record) in enumerate(unique_dogs.items()):
                        if i < 3:  # Show first 3
                            print(f"      {i+1}. '{dog_name}' -> dog_name: '{record.get('dog_name', 'MISSING')}'")
            ...
            for dog_name in sample_dogs:
                ...
                if races:
                    ...
                    if 'dog_name' in sample_race:
                        print(f"   ✓ {dog_name}: dog_name field present = '{sample_race['dog_name']}'")
                    else:
...
def test_fallback_system():
    ...
    try:
        ...
        if legacy_form_data:
            print(f"   ✅ Legacy system loaded form data for {len(legacy_form_data)} dogs")
