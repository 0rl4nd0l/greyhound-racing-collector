    ...
    try:
        ...
        if not features.empty:
            ...
            standard_features = [
                ...
                if not f.startswith("tgr_")
                and f not in ["race_id", "target_timestamp", "dog_clean_name"]
            ]
            ...
            if tgr_features:
                ...
                for feat in tgr_features[:5]:
                    value = (
                        features[feat].iloc[0] if feat in features.columns else "N/A"
                    )
                    print(f"      {feat}: {value}")

            print(f"\n   📋 Feature composition for training:")
            print(
                f"      🔢 Numerical: {len([f for f in all_features if f not in ['venue', 'grade', 'track_condition', 'weather', 'trainer_name', 'race_id', 'target_timestamp', 'dog_clean_name']])}"
            )
            print(
                f"      🏷️  Categorical: {len([f for f in all_features if f in ['venue', 'grade', 'track_condition', 'weather', 'trainer_name']])}"
            )
        else:
        ...
        print(
            f"   🎯 New model will handle {feature_count + len(tgr_features)} features"
        )
