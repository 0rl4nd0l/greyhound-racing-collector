...
class DataMonitoringSystem:
        ...
        try:
            with self.connect_db() as conn:
                ...
                if exceeds_threshold:
                    logger.warning(
                        f"{rule_name} ({severity}): {violation_count} violations (threshold: {threshold})"
                    )
                else:
                    logger.info(
                        f"{rule_name} ({severity}): {violation_count} violations (threshold: {threshold})"
    ...
    def create_monitoring_report(self, validation_results):
        ...
        with open(report_path, "w") as f:
            ...
            for result in validation_results["results"]:
                ...
                f.write(
                    f"**Violations**: {result['violation_count']} (threshold: {result['threshold']})\n"
                )
    ...
    def check_csv_data_quality(self):
        ...
        logger.info(
            f"CSV Quality Check: {single_dog_files}/{total_files} files have single dog entries"
        )

        return {
            "total_files": total_files,
            "single_dog_files": single_dog_files,
            "single_dog_percentage": (
                (single_dog_files / total_files * 100) if total_files > 0 else 0
            ),
...
def main():
    ...
    logger.info(
        f"  CSV Single-Dog Files: {csv_quality['single_dog_files']}/{csv_quality['total_files']} ({csv_quality['single_dog_percentage']:.1f}%)"
    )
