...
class BatchCLI:
    ...
    def run_job_with_progress(self, job_id: str):
        ...
        def create_status_table(job: BatchJob) -> Table:
            ...
            table.add_row("Remaining", str(job.total_files - job.completed_files - job.failed_files))
        ...
        import threading
        job_thread = threading.Thread(target=self.pipeline.run_batch_job, args=(job_id,))
    ...
    def display_job_summary(self, job: BatchJob):
        ...
        console.print(f"Duration: {(datetime.now() - job.created_at).total_seconds():.1f}s")
        ...
        try:
            while True:
                ...
                if current_job:
                    ...
                    console.print(f"🔄 [bold blue]Live Job Monitor - {current_job.name}[/bold blue]")
                    ...
                    table.add_row("Created At", current_job.created_at.strftime("%Y-%m-%d %H:%M:%S"))
                    ...
                    if current_job.status in ["completed", "completed_with_errors", "failed", "cancelled"]:
...
@click.option('--validation-level', default='moderate', type=click.Choice(['strict', 'moderate', 'lenient']))
@click.pass_context
...
@click.option('--recursive/--no-recursive', default=True, help='Search recursively for CSV files')
@click.pass_context
