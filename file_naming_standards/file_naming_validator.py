...
def validate_filename(filename, standards):
    ...
    if re.search(r"_\d{2}_\d{2}", filename) or re.search(
        r"_\d{2}_\d{2}_\d{2}", filename
    ):
...
def validate_directory(directory_path):
    ...
    results = {"compliant": [], "non_compliant": [], "total_files": 0}
    ...
    for file_path in base_path.rglob("*.csv"):
        ...
        if is_compliant:
            ...
            results["non_compliant"].append({"file": str(file_path), "reason": message})
...
if __name__ == "__main__":
    ...
    compliance_rate = (
        len(results["compliant"]) / results["total_files"] * 100
