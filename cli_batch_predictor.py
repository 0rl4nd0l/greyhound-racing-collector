...
try:
    ...
    print(
        "❌ Batch prediction pipeline not available. Please ensure batch_prediction_pipeline.py exists."
    )
...
def print_progress_bar(
    progress, total, prefix="Progress", suffix="Complete", length=40
):
    ...
    try:
        while True:
            ...
            if job.status in ["completed", "failed", "cancelled"]:
                ...
                print(
                    f"📈 Results: {job.completed_files} completed, {job.failed_files} failed"
                )
...
def predict_batch_directory(directory_path, pipeline=None, historical=False):
    ...
    try:
        ...
        job_thread = threading.Thread(
            target=pipeline.run_batch_job, args=(job_id,), daemon=True
        )
        job_thread.start()
...
def show_job_status(job_id, pipeline=None):
    ...
    print(
        f"Files: {job.completed_files}/{job.total_files} (Failed: {job.failed_files})"
    )
...
def list_jobs(pipeline=None):
    ...
    for job in jobs:
        ...
        print(
            f"{job_id_short:<12} {name_short:<25} {job.status:<12} {job.progress:<10.1f}% {files_info:<10}"
        )
...
""",
    ...
    parser.add_argument("--file", "-f", help="Predict a single CSV file")
    parser.add_argument("--batch", "-b", help="Predict all CSV files in directory")
    parser.add_argument(
        "--upcoming-races", "-u", action="store_true", help="Predict all upcoming races"
    )
    parser.add_argument("--job-status", "-s", help="Show status of specific job")
    parser.add_argument(
        "--list-jobs", "-l", action="store_true", help="List all batch jobs"
    )
    parser.add_argument(
        "--progress-callback",
        action="store_true",
    ...
    if not any(
        [args.file, args.batch, args.upcoming_races, args.job_status, args.list_jobs]
    ):
        parser.print_help()
        sys.exit(1)
