...
captures remain consumed. The original authorization file, request ceiling
(48,000), total charged live-time ceiling (10,800 seconds), prior STOPs and ledger
are unchanged. New packages bind the original authorization plus its hash-bound
amendment. Each race/window remains single-use. The source-operation ceiling is
prospectively 192 across this recovery, less the three operations already used.
Measured ~50-second refreshes and the 19-second redundant wait imply that a
repaired 90-minute cycle could exceed the old eight remaining captures and 128
source operations. These finite bounds cover that test; they do not assert a
provider-approved rate. Per-minute admission and zero automatic denial retries
remain unchanged.

An independent input-only check used retained Gosford R8 forms while still
pre-jump, the real history sealer and pinned frozen-feature subprocess under
kernel network denial. Eight runners and 16 features generated in 2.174489s
(history 1.564748s, feature subprocess 0.606987s), with three retained prior history
rows and zero target rows materialized. Feature SHA256:
5ee09f839b640efda825f43ae2f4cfc1c7c4a71f5e96a8cd4a0abef48d153bcf.
No receipt, prediction or research admission was generated by this diagnostic.

After the user challenged the two-hour wait, the next comparison's engineering
quiet period was explicitly revised to 45 minutes, with a 15:30 AEST launch.
The inherited exponential delay is not provider guidance. This revision is
justified by the tested, materially different readiness implementation and a
quiet period far longer than normal admission cadence. It is not an unchanged
