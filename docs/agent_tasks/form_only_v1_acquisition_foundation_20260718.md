...
exit_predicate: Candidate counts and source identity are reproduced; every included and excluded race and runner is frozen with source hashes, bytes, capture and jump times, and label provenance; all 530 overlap runners are reconciled with zero unexplained mismatches; development and Jul 11 through Aug 9 input-only packets rebuild byte-identically; leakage and forbidden-field guards pass; post-2026-07-09 outcomes remain unopened; and no model, database, runtime, service, timer, activation, betting, or PR 46 through 48 mutation occurs.
