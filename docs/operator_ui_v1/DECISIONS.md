...
## 2026-08-03 — DEC-GHU-035C4-REJECTED-C5-DETERMINISTIC-DEADLINE-EVIDENCE

- Context: C4 commit `29742e3c4c8309830400da4e9fa22a7fe9f1bb9a`, tree `331dd738e56ddf0952b6925d8b2fcd21555bf4bf`, run `20260803T075059Z-e3c2405804-c77d64`, session `019fc69b-b181-7751-a623-55b18442d5e9`, passed its three focused tests. The parent's one-time full bound command over `tests/operator_ui/test_r3_e2e_safety.py tests/race_collection/test_synchronous_manual_capture.py tests/race_collection/test_manual_prediction_collector_request.py tests/operator_ui/test_live_adapters.py tests/operator_ui/test_job_store.py tests/operator_ui/test_prediction_worker.py tests/operator_ui/test_r3_api.py tests/operator_ui/test_security.py` completed `2 failed, 611 passed, 1 warning in 360.40s`; only two whole-worker `<0.5s` elapsed assertions failed at `0.862093s` and `0.573822s`.
- Decision: Reject C4 as a validation candidate but preserve its stable-identity correction. Classify the two failures as nondeterministic loaded-execution test design, not a reachable cleanup-deadline product defect. C5 changes tests only: directly observe that closer joins receive no more than the configured shared cleanup budget, prove return while blocking close remains blocked, and retain durable start-failure evidence. Do not relax the configured deadline or change production.
- Evidence: `_LifetimeOwner.cleanup` creates one absolute deadline, uses only `max(0, deadline - monotonic())` for process/reader/closer joins, and delegates pipe close to daemon threads. The rejected assertions instead included store setup, audit transitions, persistence, and scheduling in total `run_once` wall time.
- Consequences: C5 is `REVIEW`, pending parent validation and independent review. The unchanged broad suite is not rerun. Cancellation/reap, bounded pipe cleanup, one process/no retry, durable error, canonical stable race identity, audit constraints, and all 23 fixture bindings remain unchanged. No GHU-036, deployment, live action, runtime/data/model mutation, outcome access, training, promotion, EV, betting, push, or merge is authorized or claimed.
...
## 2026-08-03 — DEC-GHU-035C2-REJECTED-C3-CORRECTION

- Context: Parent executable validation of C2 commit `b8cd217e3483fbbc15dc89880c6358945f903bab`, tree `9cc09d08b637a0f198678ec44320a5957a002836`, from implementer run `20260803T073521Z-49cb6e5328-5b931a`, session `019fc68c-b1f7-7d42-beb2-0b34861510d1`, ran `uv run --with-requirements requirements/all.in pytest -q tests/operator_ui/test_r3_e2e_safety.py` and returned exit 1 with `1 failed, 1 passed`; the first POST returned 202, the duplicate returned 200, and exactly one launch occurred, but the job was `FAILED / PROCESS_OUTPUT_INVALID`.
- Decision: Preserve C2 as rejected evidence and correct only its hand-built partial process JSON as `GHU-035C3` by reusing the genuine sealed-v2 blocker helper from the real worker tests. Preserve the verifier-only finalization boundary: a valid sealed blocker is `PRODUCER_COMPLETED / PRODUCER_PREDICTION_BLOCKED:POST_JUMP`, never false `PREDICTION_READY`.
- Consequences: C3 is `REVIEW`, pending parent executable validation, freeze, and independent review. All 23 genuine fixture bindings, protocol race identity, one submission/one invocation, duplicate reuse, and exact blocker truth remain unchanged. No production change, GHU-036, deployment, live action, runtime/data/model mutation, outcome access, training, promotion, EV, betting, staging, commit, push, or merge is authorized or claimed.

## 2026-08-03 — DEC-GHU-035C3-REJECTED-C4-STABLE-IDENTITY-CORRECTION

- Context: Parent validation of C3 commit `e3c2405804a03484de9a92e7d92a6379e870b8d2`, tree `dd11aa8e6dec356f1e9581239839cc9790311033`, run `20260803T074017Z-b8cd217e34-b4650b`, child `c08d284522fbd047033d5d2afbaf8df9`, session `019fc691-42a1-7a43-920a-38dec213ccf3`, returned `1 failed, 1 passed`; isolated validation reported `PREDICTION_BUNDLE_INVALID` at `result.race.identity`.
- Decision: Preserve C3 as rejected evidence. C4 corrects the reachable non-security identity-adapter inconsistency: the collector's canonical `stable_race_id` passes unchanged through bootstrap and worker and is required by sealed-result v2, so operation audit must record that same bounded printable identity rather than require the unrelated synthetic `race-` prefix. This applies the accepted identity contract consistently and changes no authorization or audit-policy decision.
- Consequences: C4 is `REVIEW`, pending parent executable validation, freeze, and independent review. Authentication, CSRF, Level-2 authority, audit append-only confirmation and hash-chain verification, idempotency, collector ownership, stable race equality, verifier-only finalization, and leakage controls remain unchanged. No GHU-036, deployment, live action, runtime/data/model mutation, outcome access, training, promotion, EV, betting, staging, commit, push, merge, acceptance, or integration is authorized or claimed.

## 2026-08-03 — DEC-GHU-035C5-REJECTED-C6-SHARED-DEADLINE-PROOF

- Context: C5 commit `f981cf38e919c67c18504cd4741cf761b5b8d191`, tree `909cbcf419db2db919a18be527b8ade91e9f2390`, implementation run `20260803T080430Z-29742e3c4c-f34bc6`, session `019fc6a7-73da-7273-9c27-dde575a8c9fc`, was independently reviewed in run `20260803T081216Z-f981cf38e9-b04c26`, session `019fc6ae-8b20-7830-a86f-1c7726668c01`, with verdict `REJECT/CORRECTION_REQUIRED`.
- Decision: Preserve C5 as rejected and correct only its one blocking test-evidence gap as GHU-035C6. Use a controlled monotonic clock and deterministic join consumption to prove later closer timeouts are decreasing remainders from one absolute deadline and total allowed cleanup consumption is exactly the configured 20ms. Production must not change.
- Evidence: The two focused worker tests were attempted once but did not execute because neither `uv` nor an importable `pytest` was available (exit 127). Direct Python compilation, `git diff --check`, and the fixture-binding check each exited 0; all 23 bindings resolved.
- Consequences: C6 remains `REVIEW`, pending parent validation and independent review. The blocked-close and durable-error assertions, all 23 fixture bindings, canonical stable race identity, sealed blocker truth, audit constraints, collector sole authority, and one invocation/no retry remain unchanged. No GHU-036, deployment, live action, outcome access, runtime/data/model mutation, training, promotion, EV, betting, staging, commit, push, or merge is authorized or claimed.

## 2026-08-03 — DEC-GHU-036-GENERATED-DEFAULT-OFF-REVIEW

- Context: GHU-035 was independently accepted in session `019fc6b6-c551-7901-a2e0-6bdaa17545d6` and integrated at commit `881a3cee0c7f93dd26f5ece9185052f59c4c1aed`, tree `226dae42ebc4deea7ce1c7954e8da74fabd37f7a`. Bootstrap already consumes one finite repository-v1 binding, but no repository generator or reversible service package existed.
- Decision: Generate that fixed binding and a distinct `greyhound-operator-ui-r3.service` package only through repository code. Default the feature off; require explicit private/local bind, pinned regular Python, exact deployment/profile/artifact hashes, authoritative DB/evidence/current-index/protocol and producer roots, a separate operations root, and an external mode-0600 secret file. Reject unsafe, symlinked, overlapping, missing, or public inputs before output. Rollback disables the feature and retains all audit and prediction evidence; the existing UI remains untouched.
- Consequences: GHU-036 is `REVIEW`, not accepted, integrated, installed, deployed, or runtime-proven. Service lifecycle, public exposure, live prediction, collector/browser action, runtime/data/model mutation, outcomes, training, promotion, EV, betting, push, and merge remain unauthorized and unclaimed.
