...
| Current release / ticket | R2 and `GHU-030`/`GHU-031` are accepted/integrated. Original `GHU-032`/`033`/`034` and C1 are blocked/rejected; corrected C2 is an unaccepted review candidate. R5 remains deferred. |
...
## GHU-032C1 through GHU-034C1 rejected correction

The original coupled candidate at commit `8262553eafc1dc3513efebe6f0f1d68cc58e044b`, tree `c2a31048fa043e28f1deeccfaa55b8dde9e0fd59`, and the first correction at commit `6a4e193a6677210b0dc8d406bb8c64df72230f86`, tree `7d5fd4da39963a01d82263fb2fbe2fb393c17744`, are rejected/blocked. C1 binary diff SHA-256 is `e43fcdfc3c5b32026de241b18d07669d6599be5537390a64c123669f58508806`; implementer session `019fba50-88d9-7ad2-b727-b0067f0481dc`; reviewer session `019fba5e-ac63-7c21-b109-af800153b461`, verdict `REJECT/CORRECTION_REQUIRED`. Seven findings remain recorded in `TICKETS.md`. C1 validation truth: focused `330 passed`, Operator UI `807 passed`, static gates passed, and broader `tests/race_collection` interrupted after at least one observed failure with no final count/pass.

`GHU-032C2` through `GHU-034C2` depends on accepted/integrated `GHU-030`/`GHU-031`. Parent returned five narrow pre-review repairs: GET-only WAITING rediscovery, authenticated sealed-request binding, expanded executable reconnect behavior plus an accessible retransmission control, pre-existing job-store object safety, and artifact/ledger cleanup. Updated focused validation passed `564`; executable Node state tests passed `7`; Python/Node syntax and `git diff --check` passed; the one updated full Operator UI run passed `814 in 88.84s`. The already-terminal classifier-selected broad run remains exact environment/path-gate evidence: exit 2 after `1 failed, 389 passed in 650.55s`; `test_registration_and_observation_commands_preserve_exact_immutable_authority` rejected the run-worktree basetemp and the run ended with `KeyboardInterrupt`. Parent correction authority classifies that result as environment/path evidence on rejected bytes, not an owner blocker, and it was not rerun. C2 is `REVIEW`, not accepted, integrated, deployed, or runtime-proven.
