...
- Scope: Under the one server-configured private `artifacts/on_demand_prediction_runs` bundle root, add the sole producer-owned canonical atomic index `prediction_bundle_index_v1.json`, its fixed producer-only `prediction_bundle_index_v1.lock`, and strict `on_demand_prediction_bundle_index_v1`, `on_demand_race_prediction_v2`, and `on_demand_prediction_bundle_manifest_v2` schemas defined by P-PREDICTION-BUNDLE-SEALED.; Serialize each read/replace update under bounded descriptor-safe exclusive lock ownership; contention and stale locks fail closed without stealing, and no collector or other lock is authorized.; Seal stable `prediction_id`, explicit nullable `job_id`, every-terminal `generated_at`, exact race/config/model identity, terminal status and blocker stage, and a non-self-referential aggregate logical bundle SHA.; Add the bounded descriptor-relative verifier and producer publication needed to validate the fixed index and every selected detail independently, with exact membership equal to manifest regular files plus only their derived parent directories.; Suggested implementation paths are limited to `scripts/predict_race_now.py`, `src/predictor/on_demand.py`, schema/config files strictly required by the producer-owned contract, and focused tests; unrelated paths are not pre-authorized.
- Non-goals: No UI/read-model adapter; no replay execution, outcome join, result comparison, result access, bundle rewrite or legacy migration; no arbitrary scan/path input, service/browser/collector/runtime action or lock action beyond the fixed producer index-update lock, canonical data write, training, persistence, registration, activation or promotion.; Existing unindexed v1 bundles remain replay-compatible and UI catalog-ineligible and MUST NOT be rewritten.
- Acceptance: The sole fixed index is canonical, atomically replaced, newest-first by `generated_at` then `prediction_id`, contains at most 256 unique entries, and meets the exact byte/file/aggregate/deadline bounds in P-PREDICTION-BUNDLE-SEALED; neither browser nor API can supply a root, directory, filename or path.; Every update holds the one fixed producer index lock across current-index read and replacement, with bounded exclusive descriptor-safe acquisition, verified ownership/type/identity, fail-closed contention/replacement, non-stealing stale-lock handling, and owner-only verified release; it never manipulates a collector or other lock.; Strict schemas reject unknown fields and noncanonical/nonfinite bytes; every terminal bundle binds its directory, index row, result, manifest, optional job, exact race, config and model evidence, with evidence-backed unavailable model artifact hashes allowed only for valid `market_only_v1`.; Every allowed regular file is enumerated once and allowed directories are exactly its derived proper-parent set; bounded descriptor-relative enumeration must match those sets exactly, while every extra/missing directory entry, traversal, duplicate, changed file, symlink, special file, replacement or unbounded tree fails closed; the aggregate logical SHA is computed over the defined manifest payload and entries without requiring a manifest self-hash.; Each detail open is descriptor-relative, no-follow, regular-file and fixed-root constrained, retains and rechecks root/component/file `fstat` identities through finite reads, and rejects component replacement/TOCTOU.; Only verified `PREDICTION_READY` exposes finite normalized probabilities; all other terminal records distinguish protocol, validation and scoring blockers without inventing a stage or outcome.; Index freshness is only listing freshness, while verified bundle bytes support only P-IMMUTABLE-HISTORICAL claims.
- Validation: Focused schema and canonical-byte tests; index lock contention/stale/non-steal/ownership/type/symlink/replacement/deadline/release tests and concurrent-producer lost-entry test; index atomicity/order/uniqueness/count/size/deadline tests; descriptor-root/component/file replacement and symlink/special-file tests; exact derived-directory membership and extra-directory-entry/traversal/duplicate/bounded-enumeration tests; added/missing/changed-file and logical-SHA tests; exact directory/index/result/manifest/job/race/model/config binding tests; market-only nullable-artifact tests; generated-time/status/stage/unknown-field/nonfinite/probability tests; legacy-v1 replay-compatibility and catalog-ineligibility tests; no scan/caller-path/result/outcome/replay/runtime-action or collector-lock audit.
...
- Authority: Level 2 only after all R3 gates; deployment/live proof require separate Level 4/explicit authority.
- Claims supported: Mechanically frozen C5 repository-owned generator/test/ledger correction candidate; C3/C4 history, C4's exact 27-test validation, and C5's exact 54/54 deployment-generator validation are recorded below; default-off and no deployment performed. Mechanically frozen documentation-only C6 was rejected, and documentation-only C7 is in `REVIEW`.
...
- Next safe action: Parent reviews the documentation-only C7 reconciliation and retains acceptance and integration authority.
- Closeout evidence: The initial candidate `d1024b05bae5988b246fb8f2ae0f6e1f7d569153`, tree `1d767f105695d6b38b877b02f3a48bb1584f08b0`, was rejected. C1 was `16e2aae871e5c913d583c8cfa398e132b363d66e`, tree `9f3c36ef005151f2edd3a76d7594b368f0aa55fb`. C2 was `913fc84368e70cb6a2161381a3ee6823812f336a`, tree `a211c4a593a723a0ccf2223189f5b87d00ed8f08`, and was rejected in session `019fc6df-8de0-7852-b20f-d3cb34570e49` for the two recorded secret-authority and ledger findings. C3 was mechanically frozen and committed as rejected correction candidate `1e71261bd5a469abc72d40111575fc51282723be`, tree `60121cc4abe3343f03a8677e1ba3ffdb601bea76`; it was never independently accepted or integrated. C4 was `dd0a87db44530a3ee98b28f9eb677f9d5be81b7f`, tree `aa1f754c996ecee4e05161dc1aa84fd3338c542d`, and changed only the one-line canonical-database secret-alias fixture atop C3; deployment validation passed exactly `27 passed`. Independent C4 review session `019fc6ec-540c-7780-b50c-12559783309c` rejected exactly three issues: permissive/ambiguous external EnvironmentFile parsing allowed override or extra assignments; independent output replacement could leave a partial/mixed four-file package after a reported failure; and the ledgers misstated C3/C4 history/current state. C5 corrects only those findings and is the mechanically frozen candidate at commit `61567704a812969abc39df07c92eb5ab7e858710`, tree `254fbcd541b37d6904a8000391da5488fefa3e26`; its deployment-generator suite passed exactly `54/54`. C6 was mechanically frozen at commit `bd95d7affc6b4b10c9b4b5445ad841d72d773e51`, tree `92cb955331961e0cdc12c9a99ee0725a84b76c04`, and rejected in session `019fc6fc-b9e8-76f3-9cb9-d3a1867db428` for the two documentation findings recorded in the current status summary. C7 is a documentation-only reconciliation in `REVIEW`; its volatile self-identity is intentionally not asserted here. No acceptance, PR, merge, publication, install, deploy, start, restart, or live proof is claimed.
