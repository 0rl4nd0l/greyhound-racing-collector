# DB Inventory and Usage Analysis ()

- Commit: 
- Generated: 2025-09-02T17:36:43
...
1) Enforce DB routing: replace direct sqlite3.connect with open_sqlite_readonly/open_sqlite_writable; ensure writers target staging only.
2) Resolve migration divergence if any (merge heads), and align models.py with Alembic head.
3) Verify critical indexes on race_metadata and dog_race_data; if missing, plan a guarded migration.
4) Ensure analytics DB is complete and up to date; if needed, publish from staging using docs/DB_WORKFLOW.md.
5) Add CI checks to fail on direct sqlite3.connect or writes against analytics DB in read-only contexts.