...
def validate_database_schema():
    ...
    try:
        ...
        if tgr_tables_found > 0:
            print(
                f"   ✅ TGR integration available ({tgr_tables_found}/{len(optional_tables)} tables)"
            )
        else:
            print(
                f"   ⚠️  No TGR tables found - training will use standard features only"
            )
    ...
    try:
        ...
        for _, race in recent_races.iterrows():
            ...
            if race["dog_count"] < 3 or race["dog_count"] > 20:
                issues.append(
                    f"Race {race['race_id']}: unusual field size ({race['dog_count']})"
                )

            # Check for complete position data
            if race["positions_available"] != race["dog_count"]:
                issues.append(
                    f"Race {race['race_id']}: missing positions ({race['positions_available']}/{race['dog_count']})"
                )

            # Check for valid position range
            if race["min_position"] != 1 or race["max_position"] != race["dog_count"]:
                issues.append(
                    f"Race {race['race_id']}: invalid position range ({race['min_position']}-{race['max_position']}, expected 1-{race['dog_count']})"
                )
        ...
        for _, day in temporal_dist.iterrows():
            print(
                f"      {day['date']}: {day['races']} races, {day['total_dogs']} dogs"
            )
...
def validate_feature_building():
    ...
    try:
        ...
        print(
            f"   ✅ Test data: {len(test_data)} samples, {len(test_data['race_id'].unique())} races"
        )
        ...
        features = temporal_builder.build_features_for_race(
            sample_race_data, sample_race_id
        )
        ...
        print(
            f"      Key standard features found: {len(found_standard)}/{len(expected_standard)}"
        )
...
def validate_training_pipeline():
    ...
    try:
        ...
        train_max_date = (
            train_data["race_timestamp"].max()
            if "race_timestamp" in train_data.columns
            else "Unknown"
        )
        test_min_date = (
            test_data["race_timestamp"].min()
            if "race_timestamp" in test_data.columns
            else "Unknown"
        )
        ...
        train_features = ml_system.build_leakage_safe_features(
            train_data.head(100)
