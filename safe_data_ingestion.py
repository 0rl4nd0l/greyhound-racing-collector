...
class SafeDataIngestion:
    ...
    def validate_and_clean_record(self, record: Dict, table_name: str) -> Tuple[Dict, List[str]]:
        ...
        if table_name == 'race_metadata':
            ...
            if 'race_date' in cleaned_record and cleaned_record['race_date']:
                try:
                    ...
                    warnings.append(f"Could not parse race_date: {cleaned_record['race_date']}")
        
        elif table_name == 'dog_race_data':
            # Ensure box_number is valid
            if 'box_number' in cleaned_record and cleaned_record['box_number'] is not None:
                try:
    ...
    def insert_single_record(self, record: Dict, table_name: str) -> Tuple[bool, List[str]]:
        ...
        with self.integrity_manager:
            success, errors = self.integrity_manager.safe_insert_record(table_name, cleaned_record)
        
        if success:
            self.ingestion_stats['records_inserted'] += 1
            self.ingestion_stats['successful_inserts'].append({
                'table': table_name,
                'record_identifier': cleaned_record.get('race_id') or cleaned_record.get('id') or 'unknown',
    ...
    def insert_batch_records(self, records: List[Dict], table_name: str) -> Dict:
        ...
        self.logger.info(f"Starting batch insertion of {len(records)} records into {table_name}")
        ...
        for i, record in enumerate(records):
            try:
                ...
                # Add any warnings
                batch_results['warnings'].extend([msg for msg in messages if 'warning' in msg.lower()])
                ...
                batch_results['failed_inserts'] += 1
                batch_results['errors'].append(f"Unexpected error processing record {i}: {str(e)}")
                self.logger.error(f"Unexpected error processing record {i}: {e}")
        
        self.logger.info(f"Batch insertion complete: {batch_results['successful_inserts']} success, {batch_results['failed_inserts']} failed")
    ...
    def generate_ingestion_report(self) -> str:
        ...
        report = {
            ...
            'success_rate': (
                self.ingestion_stats['records_inserted'] / max(self.ingestion_stats['records_processed'], 1)
            ) * 100,
            'recommendations': []
        }
        ...
        if self.ingestion_stats['records_rejected'] > 0:
            report['recommendations'].append("Review rejected records and fix data quality issues")
        
        if len(self.ingestion_stats['validation_errors']) > 10:
            report['recommendations'].append("Consider improving data validation at the source")
        
        if report['success_rate'] < 95:
            report['recommendations'].append("Data quality issues detected - review ingestion process")
        else:
            report['recommendations'].append("Good data quality - maintain current practices")
...
def main():
    ...
    parser.add_argument('--generate-report', action='store_true', help='Generate ingestion report')
