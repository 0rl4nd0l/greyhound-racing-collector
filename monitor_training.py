...
def get_training_processes():
    ...
    try:
        ...
        training_procs = []
        for line in lines:
            if any(keyword in line.lower() for keyword in ['train', 'ml_system', 'comprehensive_enhanced']) and 'grep' not in line and line.strip():
        ...
        while True:
            ...
            if processes:
                ...
                for i, proc in enumerate(processes, 1):
                    print(f"  {i}. PID: {proc['pid']} | CPU: {proc['cpu']}% | Memory: {proc['mem']}%")
