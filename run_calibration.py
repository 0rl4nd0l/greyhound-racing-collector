...
def run_calibration(model_path: str) -> None:
    ...
    if "raw_win_prob" in cal_data.columns and "actual_win" in cal_data.columns:
        ...
        if win_mask.sum() > 10:
            ...
            if calibrator.win_calibrator is not None:
                ...
                calibrated_win_brier = brier_score_loss(
                    actual_wins, calibrated_win_probs
                )
                ...
                logging.info(
                    f"   Improvement: {((win_brier_score - calibrated_win_brier) / win_brier_score * 100):.2f}%"
                )
                ...
                try:
                    fraction_pos, mean_pred = calibration_curve(
                        actual_wins, calibrated_win_probs, n_bins=10
                    )
                    # Calculate slope of the reliability diagram
                    if len(fraction_pos) > 1:
                        reliability_slope_win = np.polyfit(mean_pred, fraction_pos, 1)[
                            0
                        ]
                        logging.info(
                            f"   Reliability Slope: {reliability_slope_win:.4f}"
                        )
                        ...
                        plt.plot(
                            ...
                            fraction_pos,
                            "o-",
                            label="Win probability calibration",
                        )
                        ...
                        plt.savefig(
                            "calibration_curve_win.png", dpi=150, bbox_inches="tight"
                        )
                        plt.close()
                        logging.info(
                            "   📊 Win probability calibration curve saved to calibration_curve_win.png"
                        )
                except Exception as e:
                    logging.warning(
                        f"Could not calculate reliability slope for win probabilities: {e}"
                    )
            else:
    ...
    if "raw_place_prob" in cal_data.columns and "actual_place" in cal_data.columns:
        ...
        if place_mask.sum() > 10:
            ...
            if calibrator.place_calibrator is not None:
                calibrated_place_probs = calibrator.place_calibrator.predict(
                    raw_place_probs
                )
                calibrated_place_brier = brier_score_loss(
                    actual_places, calibrated_place_probs
                )
                ...
                logging.info(
                    f"   Improvement: {((place_brier_score - calibrated_place_brier) / place_brier_score * 100):.2f}%"
                )
                ...
                try:
                    fraction_pos, mean_pred = calibration_curve(
                        actual_places, calibrated_place_probs, n_bins=10
                    )
                    # Calculate slope of the reliability diagram
                    if len(fraction_pos) > 1:
                        reliability_slope_place = np.polyfit(
                            mean_pred, fraction_pos, 1
                        )[0]
                        logging.info(
                            f"   Reliability Slope: {reliability_slope_place:.4f}"
                        )
                        ...
                        plt.plot(
                            ...
                            fraction_pos,
                            "o-",
                            label="Place probability calibration",
                        )
                        plt.xlabel("Mean Predicted Probability")
                        plt.ylabel("Fraction of Positives")
                        ...
                        plt.grid(True, alpha=0.3)
                        plt.savefig(
                            "calibration_curve_place.png", dpi=150, bbox_inches="tight"
                        )
                        plt.close()
                        logging.info(
                            "   📊 Place probability calibration curve saved to calibration_curve_place.png"
                        )
                except Exception as e:
                    logging.warning(
                        f"Could not calculate reliability slope for place probabilities: {e}"
                    )
            else:
    ...
    artifacts = {
        ...
        "win_brier_score": (
            float(win_brier_score) if win_brier_score is not None else None
        ),
        "place_brier_score": (
            float(place_brier_score) if place_brier_score is not None else None
        ),
        "reliability_slope_win": (
            float(reliability_slope_win) if reliability_slope_win is not None else None
        ),
        "reliability_slope_place": (
            float(reliability_slope_place)
            if reliability_slope_place is not None
            else None
        ),
    ...
    overall_brier = (
        win_brier_score if win_brier_score is not None else place_brier_score
    )
    ...
    if overall_brier > BRIER_SCORE_THRESHOLD:
        logging.error(
            f"❌ WORKFLOW FAILED: Brier score {overall_brier:.4f} exceeds threshold {BRIER_SCORE_THRESHOLD}"
        )
        exit(1)
    else:
        logging.info(
            f"✅ WORKFLOW PASSED: Brier score {overall_brier:.4f} is within acceptable range"
        )
