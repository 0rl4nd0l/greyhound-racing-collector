...
class TemporalIntegrityChecker:
    ...
    def check_temporal_ordering(self, conn):
        ...
        try:
            ...
            if not issues_df.empty:
                self.results['issues'].append({'temporal_violations': issues_df.to_dict('records')})
    ...
    def check_retroactive_inclusion(self, conn):
        ...
        try:
            ...
            if not retro_issues_df.empty:
                self.results['issues'].append({'retroactive_violations': retro_issues_df.to_dict('records')})
                logger.warning(f"Detected {len(retro_issues_df)} retroactive inclusion issues.")
            else:
    ...
    def run_checks(self):
        ...
        try:
            ...
            logger.info(f"Temporal integrity check completed. Status: {self.results['summary']['status']}")
