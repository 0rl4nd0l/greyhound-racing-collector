...
class TheGreyhoundRecorderDBAdapter:
    ...
    def adapt_and_load_meeting(self, meeting_data: Dict[str, Any]):
        ...
        try:
            ...
            # Get the race_id
            cursor.execute(
                "SELECT id FROM race_metadata WHERE race_name = ?",
                (meeting_data.get("meeting_title"),),
            )
            ...
            self._insert_or_update(
                "races_gr_extra", recorder_extra, {"race_id": race_id}
            )

            self.conn.commit()
            logger.info(
                f"Successfully loaded The Greyhound Recorder data for meeting {meeting_data.get('meeting_title')}"
            )
