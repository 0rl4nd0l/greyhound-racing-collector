...
class FastTrackDBAdapter:
    ...
    def _get_existing_id(self, table: str, where_col: str, where_val) -> Optional[int]:
        ...
        try:
            ...
            cursor.execute(f"SELECT id FROM {table} WHERE {where_col} = ?", (where_val,))
    ...
    def adapt_and_load_race(self, race_data: Dict[str, Any]):
        ...
        try:
            ...
            race_id = self._get_existing_id('race_metadata', 'race_name', race_data.get('race_name'))
            if not race_id:
            ...
            for dog_perf in race_data.get('results', []):
                ...
                dog_id = self._get_existing_id('dogs', 'dog_name', dog_perf.get('dog_name'))
                if not dog_id:
                ...
                if not performance_id:
                    logger.warning(f"Performance not found for race {race_id}, dog {dog_id}")
                    continue
                ...
                self._insert_or_update('dog_performance_ft_extra', perf_extra, {'performance_id': performance_id})
    ...
    def _get_existing_performance_id(self, race_id: int, dog_id: int) -> Optional[int]:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute("SELECT id FROM dog_performances WHERE race_id = ? AND dog_id = ?", (race_id, dog_id))
            result = cursor.fetchone()
            return result[0] if result else None
        except Exception as e:
