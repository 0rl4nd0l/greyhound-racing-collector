...
class TheGreyhoundRecorderScraper:
    ...
    def __init__(
        ...
        # Use the shared HTTP session with centralized retry logic
    ...
    def fetch_enhanced_dog_data(
        ...
        Optimizations:
        - Limit number of meetings scanned via TGR_MAX_MEETINGS (default 10)
        - Early-stop after TGR_STOP_ON_FOUND_COUNT races found for the dog (default 5)
        """
        import os
        ...
        try:
            ...
            # Limits and early-stop config
            try:
                max_meetings = int(os.getenv("TGR_MAX_MEETINGS", "10"))
            except Exception:
                max_meetings = 10
            try:
                stop_on = int(os.getenv("TGR_STOP_ON_FOUND_COUNT", "5"))
            except Exception:
                stop_on = 5

            meetings = form_guides.get("meetings", [])[:max_meetings]
            found = 0
            ...
            for meeting in meetings:
                if meeting.get("long_form_url"):
                    ...
                    if dog_entries:
                        ...
                        found += len(dog_entries)
                        # Early stop once sufficient entries are found
                        if found >= stop_on:
                            self.logger.info(
                                f"Early stop: collected {found} entries for {dog_name} (limit={stop_on})"
                            )
                            break
            ...
            self.logger.info(
                f"Enhanced data collected: {len(enhanced_data['form_entries'])} races"
            )
    ...
    def fetch_dog_profile_history(self, dog_name: str) -> List[Dict[str, Any]]:
        """Fetch recent race history directly from the dog's profile page.
        Returns a list of entries with minimal fields compatible with tgr_enhanced_dog_form.
        """
        import re

        entries: List[Dict[str, Any]] = []

        # Build candidate slugs (simple heuristics)
        ...
        candidates = []
        base_slug = slugify(dog_name)
        candidates.append(f"{BASE_URL}/greyhound/{base_slug}")
        candidates.append(f"{BASE_URL}/greyhound/{base_slug}/")
        ...
        for url in candidates:
            ...
            if not soup:
                continue
            # Look for a table that resembles race history (headers with Date/Fin etc.)
            ...
            for table in tables:
                ...
                if not headers:
                    continue
                score = 0
                for key in ("date", "fin", "box", "dis", "trk", "time"):
                    if any(key in h for h in headers):
                        score += 1
                if score < 3:
                    continue
                # Parse rows similar to _extract_dog_racing_history
                ...
                for row in tbody.find_all("tr"):
                    ...
                    # Try to map indices by header keywords
                    def find_idx(keys):
                        for k in keys:
                    ...
                    idx_fin = find_idx(["fin"])
                    idx_box = find_idx(["box"])
                    idx_trk = find_idx(["trk", "track", "venue"])
                    idx_dis = find_idx(["dis", "distance"])
                    # Build entry
                    try:
                        race_date = (
                            cell_texts[idx_date] if idx_date is not None else None
                        )
                    except Exception:
                        race_date = None
                    fin_pos = None
                    try:
                        if idx_fin is not None:
                            m = re.search(r"(\d+)", cell_texts[idx_fin])
                            if m:
                                fin_pos = int(m.group(1))
                    except Exception:
                        pass
                    box_no = None
                    try:
                        if idx_box is not None:
                            btxt = cell_texts[idx_box].replace("(", "").replace(")", "")
                            if btxt.isdigit():
                                box_no = int(btxt)
                    except Exception:
                        pass
                    venue = None
                    try:
                        if idx_trk is not None:
                            venue = cell_texts[idx_trk]
                    except Exception:
                        pass
                    distance = None
                    try:
                        if idx_dis is not None:
                            distance = cell_texts[idx_dis]
                    except Exception:
                        pass
                    # Only accept if date exists
                    if race_date:
                        entries.append(
                            {
                                "dog_name": dog_name,
                                ...
                                "box_number": box_no,
                                "recent_form": (
                                    [str(fin_pos)] if fin_pos is not None else []
                                ),
                                "comments": None,
                                "odds": None,
                                "odds_text": None,
    ...
    def _calculate_performance_metrics(
        ...
        try:
            ...
            if positions:
                ...
                if len(positions) > 1:
                    variance = sum(
                        (p - metrics["average_position"]) ** 2 for p in positions
                    ) / len(positions)
    ...
    def store_enhanced_data_to_db(
        self, enhanced_data: Dict[str, Any], db_path: str = "greyhound_racing_data.db"
    ):
        ...
        try:
            ...
            self.logger.info(
                f"✅ Stored enhanced TGR data for {dog_name}: "
    ...
    def get_race_insights(
        self, venue: str, race_date: str, race_number: int
    ) -> Dict[str, Any]:
        """Get race-level insights for a specific race.

        Default: disabled to avoid placeholder data. Enable only for development by
        setting TGR_ALLOW_PLACEHOLDER=1.
        """
        self.logger.info(
            f"Getting race insights for {venue} Race {race_number} on {race_date}"
        )
        if os.getenv("TGR_ALLOW_PLACEHOLDER", "0").lower() not in ("1", "true", "yes"):
            raise RuntimeError(
                "Race insights placeholder disabled. Implement real scraping or enable dev flag TGR_ALLOW_PLACEHOLDER=1."
            )
        return {
            "venue": venue,
            "race_date": race_date,
