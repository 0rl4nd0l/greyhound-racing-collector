...
class FastTrackScraper:
    ...
    def fetch_race(self, meeting_id: int, race_id: int) -> dict:
        ...
        """
        self.logger.info(
            f"Fetching race result for meeting {meeting_id}, race {race_id}"
        )
    ...
    def _parse_dog(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        if id_elem:
            try:
                dog_details["dog_id"] = int(
                    id_elem.get("value") or id_elem.text.strip()
                )
    ...
    def _parse_watchdog_form_guides(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        for link in soup.select("a[href*='/RaceField/ViewRaces/']"):
            ...
            if meeting_id_match:
                ...
                existing = next(
                    (m for m in meetings if m["meeting_id"] == meeting_id), None
                )
                if not existing:
                    meetings.append(
                        {
    ...
    def _parse_race_meeting(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        for link in soup.select("a[href*='/RaceField/ViewRaces/']"):
            ...
            if match:
                races.append(
                    {"race_id": int(match.group(1)), "race_name": link.text.strip()}
                )
    ...
    def _parse_race(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        if race_id_elem:
            race_id_value = race_id_elem.get("value") or race_id_elem.get(
                "data-race-id"
            )
            if race_id_value:
                try:
        ...
        for row in soup.select("tr.ReportRaceDogLine"):
            try:
                ...
                if box_elem and name_elem:
                    ...
                    position_elem = row.select_one(
                        ".position, .finish-position, td:nth-child(2)"
                    )
                    if position_elem:
                        try:
                            dog_data["finish_position"] = int(
                                position_elem.text.strip()
                            )
                        except (ValueError, TypeError):
                            pass
