...
class JobStore:
    ...
    def verify(self):
        try:
            db=self._connect(); return self._verify_db(db)
    ...
    def get(self,job_id):
        db=self._connect()
        try:
            self._require_valid(db); row=db.execute("SELECT * FROM jobs WHERE job_id=?",(job_id,)).fetchone(); event=db.execute("SELECT * FROM job_events WHERE job_id=? ORDER BY sequence DESC LIMIT 1",(job_id,)).fetchone(); attempt=db.execute("SELECT 1 FROM job_attempts WHERE job_id=?",(job_id,)).fetchone()
