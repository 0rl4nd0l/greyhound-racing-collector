...
def _validate_sealed_protocol(contents: Mapping[str, bytes], result: Mapping[str, Any]) -> None:
    ...
    if exact.get("receipt") != reference or not isinstance(artifacts,Mapping) or set(artifacts) != {"report","form","sidecar"} or any(not isinstance(artifacts[key],Mapping) or set(artifacts[key])!={"path","sha256"} or artifacts[key].get("sha256") != sealed.get(f"source_{key}_sha256") for key in ("report","form","sidecar")) or exact.get("form_name") != Path(artifacts["form"]["path"]).name: raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="protocol.authenticated_receipt")
