...
def _validate_sealed_protocol(contents: Mapping[str, bytes], result: Mapping[str, Any]) -> None:
    ...
    real_protocol = values["request"].get("schema_version") == "manual-prediction-collector-request-v1"
    for name in names:
        hash_name = "authenticated_receipt_sha256" if name == "authenticated_receipt" else f"{name}_sha256"
        if (real_protocol and set(values[name]) != expected_keys[name]) or sha256_bytes(contents[f"protocol/{name}.json"]) != chain[hash_name] or values[name].get("request_id") != chain["request_id"]: raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field=f"protocol.{name}")
    ...
    if request.get("race",{}).get("race_id") != result["race"]["race_id"] or request.get("race",{}).get("jump_timestamp") != result["race"]["jump_timestamp"] or (real_protocol and (request.get("race") != result_race or request.get("expected_runner_set_sha256") != receipt.get("runner_set_sha256") or request.get("expected_runners") != receipt.get("runners") or request.get("attempt_authority") != "one_attempt" or request.get("research_only") is not True)): raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="protocol.request")
    if claim.get("request_sha256") != chain["request_sha256"] or attempt.get("request_sha256") != chain["request_sha256"] or attempt.get("claim_sha256") != chain["claim_sha256"] or response.get("request_sha256") != chain["request_sha256"] or response.get("claim_sha256") != chain["claim_sha256"] or response.get("attempt_sha256") != chain["attempt_sha256"] or (real_protocol and (claim.get("safe_boundary") is not True or attempt.get("collector_run_id") != claim.get("collector_run_id") or attempt.get("attempt_number") != 1 or _timestamp(attempt.get("started_at"),"protocol.attempt.started_at") < _timestamp(claim.get("claimed_at"),"protocol.claim.claimed_at"))): raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="protocol.hash_chain")
    reference = response.get("receipt")
    sealed=receipt.get("sealed_handoff"); source=receipt.get("source_evidence")
    if not isinstance(reference,Mapping) or reference.get("sha256") != chain["receipt_sha256"] or receipt.get("request_sha256") != chain["request_sha256"] or receipt.get("race") != request.get("race") or consume.get("response_sha256") != chain["response_sha256"] or consume.get("consume_once") is not True or consume.get("status") != response.get("status") or (real_protocol and (response.get("race") != request.get("race") or response.get("status") != "RECEIPT_READY" or response.get("reason") is not None or reference != {"schema_version":"manual-prediction-collector-receipt-v1","path":f"receipts/{chain['request_id']}.json","sha256":chain["receipt_sha256"]})): raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="protocol.receipt_chain")
    if not isinstance(sealed,Mapping) or exact.get("race_id") != result["race"]["race_id"] or exact.get("receipt",{}).get("sha256") != chain["receipt_sha256"] or (real_protocol and (not isinstance(source,Mapping) or sealed.get("race") != request.get("race") or sealed.get("race_id") != result["race"]["race_id"] or sealed.get("runner_set_sha256") != receipt.get("runner_set_sha256") or any(source.get(key)!=sealed.get(key) for key in ("source_report_sha256","source_form_sha256","source_sidecar_sha256","capture_attempt_sha256","append_report_sha256")))): raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="protocol.sealed_handoff")
    if real_protocol and (exact.get("receipt") != reference or exact.get("form_name") != Path(exact.get("artifacts",{}).get("form",{}).get("path","")).name or set(exact.get("artifacts",{})) != {"report","form","sidecar"} or any(exact["artifacts"][key].get("sha256") != sealed.get(f"source_{key}_sha256") for key in ("report","form","sidecar"))): raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="protocol.authenticated_receipt")
    ...
    if any(not isinstance(value,str) or _SHA256_RE.fullmatch(value) is None for value in digests) or sha256_bytes(history_raw) != cutoff["history_seal_sha256"] or sha256_bytes(contents["features/sealed_history.db"]) != history.get("sealed_sha256") or history.get("sealed_sha256") != cutoff["sealed_sha256"] or history.get("source_sha256") != cutoff["source_sha256"] or _timestamp(history.get("cutoff_timestamp"),"history.cutoff_timestamp") != jump or _timestamp(cutoff.get("cutoff_timestamp"),"cutoff.cutoff_timestamp") != jump or history.get("target_rows_materialized") != 0 or history.get("at_or_after_cutoff_rows_materialized") != 0 or (real_protocol and (history.get("target_race_id") != result["race"]["race_id"] or not isinstance(history.get("excluded_target_metadata_rows"),int) or history.get("excluded_target_metadata_rows") < 0 or not isinstance(history.get("excluded_at_or_after_cutoff_metadata_rows"),int) or history.get("excluded_at_or_after_cutoff_metadata_rows") < 0)): raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="authenticated_cutoff")
