...
def seal_history_database(
    *,
    ...
    try:
        ...
        metadata_rows = [
            dict(row) for row in source_db.execute("SELECT * FROM race_metadata")
        ]
        ...
        dog_rows = [
            dict(row)
            for row in source_db.execute("SELECT * FROM dog_race_data")
            if str(row[dog_columns.index("race_id")]) in safe_ids
        ]
