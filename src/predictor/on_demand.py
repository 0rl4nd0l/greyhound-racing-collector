...
def _validate_sealed_protocol(contents: Mapping[str, bytes], result: Mapping[str, Any]) -> None:
    ...
    history_keys={"schema_version","cutoff_timestamp","source_sha256","sealed_sha256","target_race_id","target_rows_materialized","at_or_after_cutoff_rows_materialized","excluded_target_metadata_rows","excluded_at_or_after_cutoff_metadata_rows"}
    ...
    if set(history) != history_keys or history.get("schema_version") != "sealed_prediction_history_v1" or any(not isinstance(value,str) or _SHA256_RE.fullmatch(value) is None for value in digests) or sha256_bytes(history_raw) != cutoff["history_seal_sha256"] or sha256_bytes(contents["features/sealed_history.db"]) != history.get("sealed_sha256") or history.get("sealed_sha256") != cutoff["sealed_sha256"] or history.get("source_sha256") != cutoff["source_sha256"] or _timestamp(history.get("cutoff_timestamp"),"history.cutoff_timestamp") != jump or _timestamp(cutoff.get("cutoff_timestamp"),"cutoff.cutoff_timestamp") != jump or history.get("target_rows_materialized") != 0 or history.get("at_or_after_cutoff_rows_materialized") != 0 or history.get("target_race_id") != result["race"]["race_id"] or not isinstance(history.get("excluded_target_metadata_rows"),int) or isinstance(history.get("excluded_target_metadata_rows"),bool) or history.get("excluded_target_metadata_rows") < 0 or not isinstance(history.get("excluded_at_or_after_cutoff_metadata_rows"),int) or isinstance(history.get("excluded_at_or_after_cutoff_metadata_rows"),bool) or history.get("excluded_at_or_after_cutoff_metadata_rows") < 0: raise _blocked("PREDICTION_BUNDLE_IDENTITY_MISMATCH", field="authenticated_cutoff")
