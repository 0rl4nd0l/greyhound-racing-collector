...
def fix_winner_names():
    ...
    logger.info(
        "Step 2: Cross-referencing with dog race data for position 1 finishers..."
    )
    ...
    logger.info(
        f"Overall winner coverage: {winner_coverage:.1f}% ({races_with_winners}/{total_races})"
    )
    logger.info(
        f"CSV ingestion winner coverage: {csv_coverage:.1f}% ({csv_with_winners}/{csv_races})"
    )
