...
def debug_tgr_parsing():
    ...
    for i, meeting in enumerate(form_guides.get("meetings", [])[:3]):
        ...
        if test_dog.upper() in page_text:
            ...
            dog_mentions = soup.find_all(
                string=lambda text: text and test_dog.lower() in text.lower()
            )
            ...
            for j, mention in enumerate(dog_mentions[:5]):  # Show first 5
                ...
                if parent:
                    logger.info(
                        f"         Parent: <{parent.name}> class={parent.get('class', [])}"
                    )
                    ...
                    if hasattr(parent, "find_next_siblings"):
                        ...
                        for k, sibling in enumerate(siblings):
                            ...
                            logger.info(
                                f"           {k+1}. <{sibling.name}> {sibling.get('class', [])}: {sibling_text[:50]}..."
                            )
            ...
            for j, table in enumerate(tables):
                ...
                if len(rows) > 1:  # Has header + data rows
                    ...
                    logger.info(
                        f"      Table {j+1}: {len(rows)} rows, headers: {headers}"
                    )
    ...
    logger.info(
        f"📊 Result: {len(enhanced_data.get('form_entries', []))} race entries found"
    )
