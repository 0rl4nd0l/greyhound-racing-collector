...
try:
    ...
    print(f"⚠️ TGR components not available: {e}")
    TGR_AVAILABLE = False
...
class EnhancedComprehensiveProcessor:
    ...
    def __init__(
        self, db_path="greyhound_racing_data.db", processing_mode="full", batch_size=50
    ):
        ...
        print(
            f"🚀 Enhanced Comprehensive Processor Initialized ({processing_mode} mode)"
        )
    ...
    def cleanup(self):
        """Clean up resources, especially web driver"""
        try:
            if self.driver:
                self.driver.quit()
                print("✅ Chrome driver closed")
        except Exception as e:
            print(f"⚠️ Error closing Chrome driver: {e}")
    ...
    def _migrate_database_schema(self, cursor):
        ...
        try:
            ...
            new_columns = [
                ...
                # New result status tracking columns
    ...
    def process_grouped_dog_data(
        self,
        dog_group: List[pd.Series],
        ...
        try:
            ...
            if hasattr(current_race_row, "name") and current_race_row.name is not None:
                ...
                first_col_value = (
                    str(current_race_row.iloc[0]) if len(current_race_row) > 0 else ""
                )
            ...
            if dog_data:
                ...
                try:
                    ...
                    if historical_data:
                        ...
                        dog_data["win_rate"] = (
                            win_count / len(recent_positions)
                            if recent_positions
                            else 0.0
                        )
    ...
    def process_csv_file(self, csv_file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for dog_group in dog_groups:
                dog_data = self.process_grouped_dog_data(
                    dog_group, race_info, race_results
                )
                if dog_data:
            ...
            self.assign_finish_positions_from_scraped_results(
                race_results, processed_dogs
            )
            ...
            if not enhanced_race_info.get("winner_name"):
                print(
                    f"   ⚠️ No winner found from web scraping - form guides contain only historical data"
                )
                print(
                    f"   📋 Form guide structure: Each dog has multiple historical race rows"
                )
                print(
                    f"   🚫 Cannot determine actual race winner from historical performance data"
                )
                # Do not attempt to determine winner from form guide - it's invalid
            elif enhanced_race_info.get(
                "data_quality_note"
            ) and "mismatch" in enhanced_race_info.get("data_quality_note", ""):
            ...
            print(
                f"   📊 Processed {len(processed_dogs)} dogs (grouped from {len(df)} rows)"
            )

            # STEP 7: Check if race should be excluded - now with relaxed criteria
            ...
            positioned_dogs_count = sum(
                1
                for dog in processed_dogs
                if dog.get("finish_position") not in ["N/A", "", None]
            )
            positioning_ratio = (
                positioned_dogs_count / total_dogs if total_dogs > 0 else 0
            )

            # Check if winner is in the positioned dogs
            winner_name = enhanced_race_info.get("winner_name", "")
            winner_in_data = any(
                dog.get("finish_position") == "1"
                or (
                    winner_name
                    and winner_name.lower() in dog.get("dog_clean_name", "").lower()
                )
                for dog in processed_dogs
            )
            ...
            dead_heat_positions = [
                pos for pos, count in position_counts.items() if count > 1
            ]
                ...
                for dog in processed_dogs:
                    ...
                    if pos and pos not in ["N/A", "", None]:
                        ...
                        if clean_pos in dead_heat_positions and not pos.endswith("="):
                            ...
                            print(
                                f"   🔄 Fixed dead heat: {dog.get('dog_clean_name')} -> {clean_pos}="
                            )
            ...
            non_dead_heat_positions = [
                pos for pos in positions if not pos.endswith("=")
            ]
            dead_heat_positions_unique = [
                pos.rstrip("=") for pos in positions if pos.endswith("=")
            ]

            # Check for duplicates in non-dead heat positions
            has_non_deadheat_duplicates = len(non_dead_heat_positions) != len(
                set(non_dead_heat_positions)
            )
            ...
            if has_duplicates or positions:
                ...
                print(
                    f"      All positions: {[dog.get('finish_position') for dog in processed_dogs]}"
                )
                ...
                for i, dog in enumerate(processed_dogs):
                    print(
                        f"      Dog {i+1}: {dog.get('dog_clean_name', 'Unknown')} - Position: {dog.get('finish_position')}"
                    )

            # Enhanced field size analysis for box win rate correlation
            actual_field_size = len(
                [
                    dog
                    for dog in processed_dogs
                    if dog.get("finish_position") not in ["N/A", "", None]
            ...
            if scratched_count > 0:
                print(
                    f"   📊 Field analysis: {actual_field_size} runners, {scratched_count} scratched"
                )
            ...
            enhanced_race_info["scratch_rate"] = (
                scratched_count / total_dogs if total_dogs > 0 else 0
            )
            ...
            # Modified exclusion logic: Allow races with sufficient data even without scraping
            # Standard criteria (strict - requires all): scraped data + winner + URL + winner in data + no duplicates
            ...
            if not (meets_strict_criteria or meets_relaxed_criteria):
                print(
                    f"   🚫 Excluding race {enhanced_race_info['race_id']} - insufficient data quality"
                )
                print(
                    f"      Strict criteria: Scraped: {has_scraped_data}, Winner: {has_winner}, URL: {has_url}, Winner in data: {winner_in_data}, No duplicates: {not has_duplicates}"
                )
                print(
                    f"      Relaxed criteria: Data sufficient: {data_sufficient_without_scraping}, Winner: {has_winner}, Winner in data: {winner_in_data}, No duplicates: {not has_duplicates}"
                )
                print(
                    f"      Result: Strict={meets_strict_criteria}, Relaxed={meets_relaxed_criteria}"
                )
            ...
            print(
                f"   🌤️ Collecting weather data for {race_info['venue']} on {race_info['race_date']}..."
            )
            ...
            if weather_data:
                ...
                print(
                    f"   ✅ Weather data collected: {weather_data.get('weather_condition', 'Unknown')} conditions"
                )
            else:
            ...
            for dog in processed_dogs:
                print(
                    f"   🐶 Processed dog: {dog['dog_name']} with finish position {dog.get('finish_position', 'N/A')}"
                )
    ...
    def scrape_race_results(self, race_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if expert_form_data:
                ...
                print(
                    f"   ✅ Expert form data extracted: {len(expert_form_data)} dogs with ratings"
                )
    ...
    def find_race_url_from_date_page(self, race_info: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            for link in all_links:
                ...
                if (
                    "/racing/" in href
                    and date_str in href
                    and venue_url in href
                    and f"/{race_number}/" in href
                ):
            ...
            if unique_links:
                ...
                if unique_broader_links:
                    ...
                    print(
                        f"   ⚪ No matching race URLs found for {venue} Race {race_number}"
                    )
                    # Debug: show some sample links
                    sample_links = [
                        link.get("href")
                        for link in all_links[:10]
                        if link.get("href") and "/racing/" in link.get("href", "")
                    ]
                    if sample_links:
            ...
            for selector in winner_selectors:
                ...
                if winner_element:
                    ...
                    if name:
                        ...
                        clean_name = re.sub(
                            r"[\d\.]+[A-Z]*:?\s*", "", name
                        )  # Remove timestamps like "19.22T:"
                        clean_name = re.sub(
                            r"R/T:?\s*$", "", clean_name
                        )  # Remove "R/T:" at end
                        clean_name = re.sub(
                            r"T:?\s*[\w\s]*$", "", clean_name
                        )  # Remove trainer info
                        ...
                        if len(cells) >= 2:
                            ...
                            for i, cell in enumerate(cells):
                                ...
                                if (
                                    ...
                                    and not text in ["1st", "2nd", "3rd", "1", "2", "3"]
                                ):
                                    return {"name": text, "odds": None, "time": None}

            # Last resort: look for any text containing '1st' or position indicators
            position_texts = soup.find_all(
                text=re.compile(r"1st|winner|won", re.IGNORECASE)
            )
    ...
    def extract_track_conditions(self, soup) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            enhanced_result = extractor.extract_track_conditions_enhanced(
                soup, race_url
            )
            ...
            return self._extract_track_conditions_basic_filtered(
                soup, getattr(self, "_current_race_url", "")
            )
        except Exception as e:
    ...
    def _extract_track_conditions_basic_filtered(
        self, soup, race_url: str = ""
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for selector in condition_selectors:
                ...
                if elem:
                    ...
                    if (
                    ...
                    ):  # Not too long (likely navigation)
                        ...
                        if not any(
                            ...
                            if not (race_url and text.lower() in race_url.lower()):
                                ...
                                print(
                                    f"   🏁 Found track condition via selector '{selector}': {text}"
                                )
                                break
            ...
            if not conditions.get("condition"):
                ...
                for pattern in condition_patterns:
                    ...
                    for match in matches:
                        ...
                        if race_url and any(
                            sponsor in race_url.lower()
                            for sponsor in sponsorship_patterns
                        ):
                        ...
                        if condition_text in [
                            ...
                            print(
                                f"   🏁 Found track condition via filtered pattern: {condition_text.title()}"
                            )
                            break
        ...
        try:
            ...
            if results_container:
                ...
                if len(rows) > 1:  # Has header and data rows
                    for row in rows[1:]:  # Skip header
                        ...
                        if len(cells) >= 2:
                            try:
                                ...
                                parsed_info = self.parse_scraped_result_components(
                                    full_info
                                )

                                result = {
                                    "position": position,
                                    "name": parsed_info["clean_name"]
                                    or full_info.split("T:")[0].strip(),
                    ...
                    result_divs = results_container.find_all(
                        "div", class_=re.compile(r"result|runner|position")
                    )
                            ...
                            if text and any(char.isdigit() for char in text):
                                ...
                                if position_match:
                                    ...
                                    if name:
                                        ...
                                        print(
                                            f"   📊 Found div result: {position} - {name}"
                                        )
                        except:
                            continue
    ...
    def extract_expert_form_data(self, soup, race_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                self.driver.get(expert_form_url)
                time.sleep(5)  # Wait for dynamic content to load

                # Get the expert form page content
                expert_soup = BeautifulSoup(self.driver.page_source, "html.parser")

                # Enhanced selectors for expert form data
                expert_data = self._extract_expert_data_from_page(expert_soup)

                if expert_data:
                    print(
                        f"   ✅ Successfully extracted expert form data from dedicated page"
                    )
                    return expert_data
                else:
                    print(f"   ⚠️ No expert data found on dedicated expert form page")

            except Exception as e:
                print(f"   ⚠️ Could not access expert form page: {e}")

            # Fallback: Try to find expert form data on current page
            expert_selectors = [
                ".expert-form",
                ".form-expert",
                '[data-tab="expert"]',
            ...
            if not expert_section:
                print(
                    "   ⚠️ No specific expert section found, searching tables for speed ratings..."
                )
                ...
                for table in tables:
                    ...
                    if any(
                        "speed" in header or "rating" in header or "form" in header
                    ...
                    if len(cells) >= 2:
                        try:
                            ...
                            if dog_name and speed_rating is not None:
                                ...
                                print(
                                    f"   🎯 Extracted speed rating for {clean_name}: {speed_rating}"
                                )
            ...
            if not expert_data:
                print(
                    "   ⚠️ No expert data found in structured sections, trying text patterns..."
                )
                        ...
                        for match in matches:
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    if (
                                        len(clean_name) <= 30
                                        and clean_name.replace(" ", "").isalpha()
                                    ):
                                        expert_data[clean_name] = {
                                            "speed_rating": rating_val,
                                            "expert_analysis": True,
                                        ...
                                        print(
                                            f"   🎯 Extracted speed rating via pattern for {clean_name}: {rating_val}"
                                        )
                            except Exception as e:
                                print(
                                    f"   ⚠️ Error processing line for speed ratings: {e}"
                                )
                                continue
    ...
    def _extract_expert_data_from_page(self, soup) -> Dict[str, Any]:
        """Extract expert form data from the dedicated expert form page"""
        expert_data = {}

        try:
            # Look for runner/dog containers on the expert form page
            runner_selectors = [
                ".runner",
                ".participant",
                ".greyhound",
            ...
            for selector in runner_selectors:
                found_runners = soup.select(selector)
                if found_runners:
                    runners = found_runners
                    print(
                        f"   📊 Found {len(found_runners)} runners using selector: {selector}"
                    )
                    break

            if not runners:
                # Fallback: Look for table rows that might contain runner data
                tables = soup.find_all("table")
                for table in tables:
                    rows = table.find_all("tr")
                    if len(rows) > 2:  # Has header and multiple data rows
                        runners = rows[1:]  # Skip header
                        print(
                            f"   📊 Using table rows as runners: {len(runners)} found"
                        )
                        break

            # Extract data from each runner
            for i, runner in enumerate(runners):
                try:
                    runner_data = self._extract_runner_expert_data(runner, i + 1)
                    if runner_data and runner_data.get("dog_name"):
                        dog_name = runner_data["dog_name"]
                        expert_data[dog_name] = {
                            "speed_rating": runner_data.get("speed_rating"),
                            "expert_analysis": True,
                            "expert_comment": runner_data.get("expert_comment", ""),
                            "form_rating": runner_data.get("form_rating"),
                            "barrier_rating": runner_data.get("barrier_rating"),
                            "trainer_rating": runner_data.get("trainer_rating"),
                            "recent_form": runner_data.get("recent_form", ""),
                            "best_time": runner_data.get("best_time"),
                            "last_start": runner_data.get("last_start", ""),
                        }
                        # Clean None values
                        expert_data[dog_name] = {
                            k: v
                            for k, v in expert_data[dog_name].items()
                            if v is not None
                        }

                        print(
                            f"   🎯 Extracted expert data for {dog_name}: {list(expert_data[dog_name].keys())}"
                        )

                except Exception as e:
                    print(f"   ⚠️ Error processing runner {i+1}: {e}")
                    continue

            # Alternative: Look for JSON data in script tags
            if not expert_data:
                expert_data = self._extract_expert_data_from_scripts(soup)

            return expert_data

        except Exception as e:
            print(f"   ❌ Error extracting expert data from page: {e}")
            return {}

    def _extract_runner_expert_data(
        self, runner_element, position: int
    ) -> Dict[str, Any]:
        """Extract expert data from a single runner element"""
        runner_data = {}
        ...
        try:
            ...
            for selector in name_selectors:
                name_elem = runner_element.select_one(selector)
                if name_elem:
                    dog_name = name_elem.get_text(strip=True)
                    break

            # If no name found in selectors, try table cell approach
            if not dog_name:
                cells = runner_element.find_all(["td", "th"])
                for cell in cells:
                    text = cell.get_text(strip=True)
                    # Look for dog name pattern (not just numbers or short text)
                    if (
                        text
                        and len(text) > 3
                        and not text.isdigit()
                        and not text in ["1st", "2nd", "3rd", str(position)]
                    ):
                        # Additional validation - dog names typically have letters
            ...
            runner_data["dog_name"] = self.clean_dog_name(dog_name)

            # Extract speed rating
            speed_rating = self._extract_numeric_data(
                runner_element,
                [".speed-rating", ".rating", "[data-speed-rating]", ".sr", ".speed"],
                value_range=(0, 100),
            )

            if speed_rating:
                runner_data["speed_rating"] = speed_rating

            # Extract form rating
            form_rating = self._extract_numeric_data(
                runner_element,
                [".form-rating", ".form-score", "[data-form-rating]"],
                value_range=(0, 100),
            )

            if form_rating:
                runner_data["form_rating"] = form_rating

            # Extract barrier rating
            barrier_rating = self._extract_numeric_data(
                runner_element,
                [".barrier-rating", ".box-rating", "[data-barrier-rating]"],
                value_range=(0, 100),
            )

            if barrier_rating:
                runner_data["barrier_rating"] = barrier_rating

            # Extract trainer rating
            trainer_rating = self._extract_numeric_data(
                runner_element,
                [".trainer-rating", "[data-trainer-rating]"],
                value_range=(0, 100),
            )

            if trainer_rating:
                runner_data["trainer_rating"] = trainer_rating

            # Extract best time
            best_time = self._extract_numeric_data(
                runner_element,
                [".best-time", ".pb", "[data-best-time]", ".personal-best"],
                value_range=(20.0, 50.0),
            )  # Realistic race time range

            if best_time:
                runner_data["best_time"] = best_time

            # Extract expert comment
            comment_selectors = [
                ".expert-comment",
                ".comment",
                ".analysis",
            ...
            for selector in comment_selectors:
                comment_elem = runner_element.select_one(selector)
                if comment_elem:
                    comment_text = comment_elem.get_text(strip=True)
                    if comment_text and len(comment_text) > 5:  # Meaningful comments
                        runner_data["expert_comment"] = comment_text
                        break

            # Extract recent form (pattern like "1234")
            form_selectors = [".recent-form", ".form", "[data-form]", ".last-runs"]

            for selector in form_selectors:
                form_elem = runner_element.select_one(selector)
                if form_elem:
                    form_text = form_elem.get_text(strip=True)
                    # Look for form patterns (numbers only, typically 3-6 digits)
                    if form_text and form_text.isdigit() and 3 <= len(form_text) <= 6:
                        runner_data["recent_form"] = form_text
                        break

            # Extract last start info
            last_start_selectors = [".last-start", ".last-run", "[data-last-start]"]

            for selector in last_start_selectors:
                last_elem = runner_element.select_one(selector)
                if last_elem:
                    last_text = last_elem.get_text(strip=True)
                    if last_text and len(last_text) > 2:
                        runner_data["last_start"] = last_text
                        break

            return runner_data

        except Exception as e:
            print(f"   ⚠️ Error extracting runner data: {e}")
            return None

    def _extract_numeric_data(
        self, element, selectors: List[str], value_range: Tuple[float, float] = None
    ) -> Optional[float]:
        """Extract numeric data from element using multiple selectors"""
        for selector in selectors:
            found_elem = element.select_one(selector)
            if found_elem:
                text = found_elem.get_text(strip=True)
                if text:
    ...
    def _extract_expert_data_from_scripts(self, soup) -> Dict[str, Any]:
        """Extract expert data from JavaScript/JSON in script tags"""
        expert_data = {}
        ...
        try:
            ...
            for script in scripts:
                ...
                # Look for JSON data containing runner/dog information
                if (
                    "runner" in script_content.lower()
                    or "dog" in script_content.lower()
                    or "rating" in script_content.lower()
                    or "participant" in script_content.lower()
                ):
                    ...
                    for json_match in json_matches:
                        try:
                            ...
                            if isinstance(data, dict):
                                # Extract relevant data if it looks like runner info
                                dog_name = (
                                    data.get("name")
                                    or data.get("runner_name")
                                    or data.get("dog_name")
                                )
                                if dog_name:
                                    clean_name = self.clean_dog_name(dog_name)
                                    expert_data[clean_name] = {
                                    ...
                                    expert_data[clean_name] = {
                                        k: v
                                        for k, v in expert_data[clean_name].items()
                                        if v is not None
                                    }
                                    ...
                                    if expert_data[
                                        ...
                                        print(
                                            f"   🎯 Extracted JSON data for {clean_name}"
                                        )
            ...
            print(f"   ⚠️ Error extracting expert data from scripts: {e}")
            return {}

    def combine_race_data(
        self, race_info: Dict[str, Any], race_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if winner_name:
            ...
            combined["winner_race_classification"] = winner_components[
                "race_classification"
            ]
        ...
        combined["scraped_successfully"] = race_results.get(
            "scraped_successfully", False
        )
    ...
    def parse_scraped_result_components(self, result_string: str) -> Dict[str, str]:
        ...
        if match1:
            ...
            if (
                match2 and len(match2.group(1).split()) >= 2
            ):  # Dog name should have at least 2 words
    ...
    def enhance_race_metadata(self, race_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.weather_api_key:
            weather_data = self.get_weather_data(
                race_info["venue"], race_info["race_date"]
            )
            if weather_data:
                enhanced_info.update(weather_data)

        # Get track conditions if available
        track_conditions = self.get_track_conditions(
            race_info["venue"], race_info["race_date"]
        )
        if track_conditions:
    ...
    def get_track_conditions(self, venue: str, race_date) -> Optional[Dict[str, Any]]:
        ...
        return {"track_condition": "Good", "rail_position": "True", "track_rating": 5.0}
    ...
    def scrape_race_details(
        ...
        try:
            ...
            try:
                prize_element = self.driver.find_element(
                    By.CSS_SELECTOR, ".prize-money, .stake"
                )
                if prize_element:
                    prize_text = prize_element.text
                    prize_match = re.search(r"\$?([\d,]+)", prize_text)
                    if prize_match:
                        scraped_data["prize_money_total"] = float(
                            prize_match.group(1).replace(",", "")
                        )
            except:
                pass
            ...
            try:
                grade_element = self.driver.find_element(
                    By.CSS_SELECTOR, ".grade, .class"
                )
                if grade_element:
    ...
    def process_dog_data(
        self, row, race_info: Dict[str, Any], race_results: Dict[str, Any] = None
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if race_results:
                ...
                if winner_parsed["clean_name"] and self.dogs_match(
                    dog_data["dog_clean_name"], winner_parsed["clean_name"]
                ):
                ...
                for result in race_results.get("all_results", []):
                    if self.dogs_match(
                        dog_data["dog_clean_name"], result.get("name", "")
                    ):
                        dog_data["actual_finish_position"] = (
                            result.get("position") or dog_data["finish_position"]
                        )
                        ...
                        if scraped_name:
                            scraped_components = self.parse_scraped_result_components(
                                scraped_name
                            )
                            dog_data["scraped_trainer_name"] = scraped_components[
                                "trainer_name"
                            ]
                            dog_data["scraped_reaction_time"] = scraped_components[
                                "reaction_time"
                            ]
                            dog_data["scraped_nbtt"] = scraped_components["nbtt"]
                            dog_data["scraped_race_classification"] = (
                                scraped_components["race_classification"]
                            )
                            dog_data["scraped_raw_result"] = scraped_components[
                                "raw_result"
                            ]
                            # Use scraped trainer name if available
                            if scraped_components["trainer_name"]:
                                dog_data["trainer_name"] = scraped_components[
                                    "trainer_name"
                                ]
                        break
                ...
                if not any(
                    self.dogs_match(dog_data["dog_clean_name"], result.get("name", ""))
                    for result in race_results.get("all_results", [])
                ):
            ...
            if race_results and race_results.get("expert_form_data"):
                ...
                if clean_name in expert_data:
                    ...
                    print(
                        f"   🎯 Using scraped speed rating for {clean_name}: {scraped_speed_rating}"
                    )
    ...
    def validate_winner_consistency(
        ...
        try:
            ...
            form_guide_winners = [
                dog for dog in processed_dogs if dog.get("finish_position") == "1"
            ]
            ...
            if scraped_winner_in_form_guide:
                ...
                if scraped_winner_in_form_guide.get("finish_position") != "1":
                    ...
                    for dog in processed_dogs:
                        if (
                            dog != scraped_winner_in_form_guide
                            and dog.get("finish_position") == "1"
                        ):
                ...
                print(
                    f"   ⚠️ CSV/Scraping mismatch: Winner '{scraped_winner_name}' not found in form guide"
                )
                print(
                    f"   📋 Form guide dogs: {[dog.get('dog_clean_name', '') for dog in processed_dogs]}"
                )

                # If there are form guide winners, note the discrepancy
                if form_guide_winners:
                    form_guide_winner_names = [
                        dog.get("dog_clean_name", "") for dog in form_guide_winners
                    ]
                    ...
                    race_info["data_quality_note"] = (
                        f"Winner mismatch: Scraped='{scraped_winner_name}', Form guide={form_guide_winner_names}"
                    )

        except Exception as e:
            print(f"   ⚠️ Error validating winner consistency: {e}")

    def assign_finish_positions_from_scraped_results(
        self, race_results: Dict[str, Any], processed_dogs: List[Dict[str, Any]]
    ):
        ...
        try:
            ...
            for result in all_results:
                ...
                if result_name and position:
                    ...
                    scraped_positions[cleaned_name.lower()] = (
                        str(position)
                        .replace("st", "")
                        .replace("nd", "")
                        .replace("rd", "")
                ...
                if dog_name:
                    ...
                    if found_position:
                        ...
                        if old_position != found_position:
                            print(
                                f"   🔄 Updated {dog_name}: {old_position} -> {found_position}"
                            )
                    else:
            ...
            print(
                f"   ✅ Assigned {assigned_count}/{len(processed_dogs)} finish positions from scraped results"
            )
    ...
    def calculate_speed_rating(self, row) -> float:
        ...
        try:
            ...
            if (
                ...
                and win_time_str.replace(".", "").isdigit()
            ):
                time_val = float(time_str)
                ...
                if (
                    win_time_val > 0
                    and 20.0 <= time_val <= 50.0
                    and 20.0 <= win_time_val <= 50.0
                ):
    ...
    def calculate_probabilities(
        self, row, race_info: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    def generate_race_analytics(
        self, race_info: Dict[str, Any], dogs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if dogs:
            ...
            dogs_sorted = sorted(
                dogs, key=lambda x: x.get("win_probability", 0), reverse=True
            )

            # Predicted winner
            predicted_winner = dogs_sorted[0]
            analytics["predicted_winner"] = predicted_winner["dog_clean_name"]
            analytics["predicted_winner_probability"] = predicted_winner.get(
                "win_probability", 0
            )
            ...
            analytics["race_competitiveness"] = (
                np.std(probabilities) if probabilities else 0
            )

            # Average class rating
            class_ratings = [dog.get("class_rating", 50) for dog in dogs]
            analytics["average_class_rating"] = (
                np.mean(class_ratings) if class_ratings else 50
            )

        return analytics

    def determine_winner_from_form_guide(
        self, race_info: Dict[str, Any], dogs: List[Dict[str, Any]]
    ):
        ...
        try:
            ...
            for dog in valid_dogs:
                try:
                    ...
                    if isinstance(pos, str) and pos.isdigit():
                        ...
                        dog["_numeric_position"] = (
                            999  # High number for invalid positions
                        )
                except:
            ...
            valid_dogs.sort(
                key=lambda x: (x.get("_numeric_position", 999), x.get("dog_name", ""))
            )

            if valid_dogs:
                winner = valid_dogs[0]
                race_info["winner_name"] = winner.get(
                    "dog_clean_name", winner.get("dog_name", "")
                )
                race_info["data_quality_note"] = (
                    "Winner determined from form guide data (scraping failed)"
                )
                print(
                    f"   ✅ Winner determined from form guide: {race_info['winner_name']}"
                )
    ...
    def validate_race_data(
        ...
        try:
            ...
            if num_dogs > 12:
                print(
                    f"   ❌ Data validation failed: Too many dogs ({num_dogs}) - maximum is 12"
                )
                return False
            ...
            dog_names = [
                dog.get("dog_name", "").strip()
                for dog in dogs
                if dog.get("dog_name", "").strip()
            ]
            unique_dog_names = set(dog_names)
            if len(dog_names) != len(unique_dog_names):
                print(
                    f"   ❌ Data validation failed: Duplicate dog names detected (corruption indicator)"
                )
                return False
            ...
            for dog in dogs:
                ...
                if pos and str(pos).isdigit():
                    ...
                    if pos_num <= 0 or pos_num > 12:
                        print(
                            f"   ❌ Data validation failed: Invalid finish position {pos_num} for {dog.get('dog_name', 'unknown')}"
                        )
                        return False
            ...
            if len(unique_dog_names) > 12:
                print(
                    f"   ❌ Data validation failed: Too many unique dogs ({len(unique_dog_names)}) indicates data corruption"
                )
                return False
            ...
            print(
                f"   ✅ Data validation passed: {num_dogs} dogs, venue: {venue}, race_id: {race_id}"
            )
            return True
    ...
    def save_to_database(self, race_info: Dict[str, Any], dogs: List[Dict[str, Any]]):
        ...
        if not self.validate_race_data(race_info, dogs):
            print(
                f"   🚫 Skipping race {race_info.get('race_id', 'unknown')} - failed validation"
            )
            return False
        ...
        try:
            ...
            for dog in dogs:
                ...
                if not finish_position or finish_position in ["N/A", "n/a", "NA", ""]:
                    print(
                        f"   ⚠️ Skipping {dog.get('dog_clean_name', 'unknown')} - No valid finish position ({finish_position})"
                    )
                    continue
            ...
            conn.commit()
            print(
                f"✅ Saved {len(valid_dogs)} valid dogs to database for race {race_info['race_id']} (filtered out {len(dogs) - len(valid_dogs)} with N/A positions)"
            )
    ...
    def process_all_unprocessed(self) -> Dict[str, Any]:
        ...
        if self.driver and self.enable_web_scraping and results['processed_count'] > 0:
            ...
            print(f"\n💡 No new races processed - skipping backfill")
...
"""
            ...
            if len(recent_dogs) > 0:
                ...
                recent_dogs["win_probability_numeric"] = pd.to_numeric(
                    recent_dogs["win_probability"], errors="coerce"
                )

                # Filter out rows with NaN win_probability and get top dogs
                dogs_with_prob = recent_dogs[
                    recent_dogs["win_probability_numeric"].notna()
                ]
...
"""
            ...
            if len(recent_races) > 0:
                venue_stats = (
                    recent_races.groupby("venue").agg({"race_id": "count"}).round(0)
                )
    ...
    def collect_weather_data_for_race(
        self, race_info: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if weather_data:
                ...
                adjustment_factor = (
                    self.weather_service.calculate_weather_adjustment_factor(
                ...
                print(
                    f"     ✅ Weather collected: {weather_data.condition.value}, {weather_data.temperature:.1f}°C"
                )
                ...
                print(
                    f"     ⚠️ No weather data available for {venue_code} on {date_str}"
                )
                return None
    ...
    def backfill_winners_for_pending_races(self, max_races: int = 50, max_retries_per_race: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT race_id, venue, race_number, race_date, url, 
                       scraping_attempts, last_scraped_at
                FROM race_metadata 
                ...
                try:
                    ...
                    if race_results.get("scraped_successfully") and race_results.get("winner"):
                        ...
                        cursor.execute(
                            """
                            UPDATE race_metadata 
                            SET winner_name = ?, winner_source = 'scrape', 
                                winner_odds = ?, winner_time = ?, 
                                url = ?, results_status = 'complete',
                                track_condition = ?, weather = ?
                            WHERE race_id = ?
                            """,
                            (
                                clean_winner_name,
                                race_results.get("winner_odds"),
                                race_results.get("winner_time"),
