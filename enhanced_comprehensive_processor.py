    ...
    def process_csv_file(self, csv_file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not (
                ...
                print(
                    f"   🚫 Excluding race {enhanced_race_info['race_id']} - data quality issues"
                )
                print(
                    f"      Scraped: {has_scraped_data}, Winner: {has_winner}, URL: {has_url}"
                )
                print(
                    f"      Positioning: {positioning_ratio:.1%}, Winner in data: {winner_in_data}, Duplicates: {has_duplicates}"
                )
