...
def enhanced_data_processing_with_tgr(safe_log_func=None, progress_callback=None):
    ...
    try:
        log_message(
            "🚀 Starting Enhanced Data Processing with TGR Integration...", "INFO", 0
        )
        ...
        try:
            ...
            if spec and spec.loader:
                ...
                log_message(
                    f"✅ Traditional processing: {traditional_results.get('processed_count', 0)} files",
                    "INFO",
                    30,
                )
            else:
                log_message(
                    "⚠️ Enhanced processor not available, skipping traditional processing",
                    "WARNING",
                    30,
                )
                results["traditional_processing"] = {
                    "status": "skipped",
        ...
        try:
            ...
            if cache_dir.exists():
                ...
                log_message(
                    f"📁 Found {len(cached_files)} cached TGR files", "INFO", 50
                )

                if cached_files:
                    # Run comprehensive TGR data extraction
                    log_message(
                        "📊 Running comprehensive TGR data extraction...", "INFO", 55
                    )

                    # Import our comprehensive collection function
                    sys.path.insert(0, os.getcwd())
                    from run_full_enhanced_collection import (
                        run_full_enhanced_collection,
                    )
                    ...
                    if tgr_success:
                        log_message(
                            "✅ TGR comprehensive data collection completed!",
                            "INFO",
                            75,
                        )
                        ...
                        for pattern in [
                            f"enhanced_race_data_{timestamp_pattern}*.json",
                        ...
                        if tgr_files:
                            log_message(
                                f"📁 Generated {len(tgr_files)} TGR data files",
                                "INFO",
                                80,
                            )
                            results["files_generated"].extend(
                                [str(f) for f in tgr_files]
                            )

                            # Try to get statistics from analytics file
                            analytics_files = [
                                f for f in tgr_files if "analytics" in str(f)
                            ]
                            if analytics_files:
                                try:
                                    ...
                                    results["total_dogs_processed"] = summary.get(
                                        "total_unique_dogs", 0
                                    )
                                    results["total_races_processed"] = summary.get(
                                        "total_history_records", 0
                                    )

                                    log_message(
                                        f"📊 TGR Statistics: {results['total_dogs_processed']} dogs, {results['total_races_processed']} race records",
                                        "INFO",
                                        85,
                                    )
                                    ...
                                    log_message(
                                        f"⚠️ Could not read analytics: {e}", "WARNING"
                                    )
                        ...
                        log_message(
                            "❌ TGR comprehensive data collection failed", "ERROR", 60
                        )
                        results["tgr_processing"] = {
                            "status": "failed",
                            "error": "Collection process failed",
                        }
            ...
            results["tgr_processing"] = {
                "status": "failed",
                "error": f"TGR scraper import failed: {e}",
            }
        ...
        # Calculate totals
        traditional_count = results.get("traditional_processing", {}).get(
            "processed_count", 0
        )
        ...
        if results["errors"]:
            ...
            log_message(
                f"⚠️ Completed with {len(results['errors'])} errors", "WARNING", 95
            )
        else:
    ...
    def enhanced_process_data_background():
        ...
        try:
            ...
            results = enhanced_data_processing_with_tgr(
                safe_log_func=safe_log_wrapper, progress_callback=progress_wrapper
            )
