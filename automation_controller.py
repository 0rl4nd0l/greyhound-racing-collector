...
class AutomationController:
    ...
    def run_data_integrity_checks(self) -> Optional[Dict]:
        ...
        try:
            with DataIntegrityManager(self.db_path) as integrity_manager:
                ...
                report_path = self.reports_dir / f"integrity_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                ...
                if report.get("issues_found"):
                    self.logger.warning("Data integrity issues found. Aborting further steps.")
    ...
    def run_model_retraining(self):
        ...
        success, stdout, stderr = self.run_command(["python", "advanced_ensemble_ml_system.py", "--train"])
    ...
    def start(self):
        ...
        if self.check_for_new_data():
            ...
            self.logger.info("No significant new data found. Skipping model retraining.")
