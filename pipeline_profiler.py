...
class PipelineProfiler:
    ...
    def profile_function(self, func, *args, **kwargs) -> ProfileResult:
        ...
        cpu_time = (end_cpu_times.user + end_cpu_times.system) - (start_cpu_times.user + start_cpu_times.system)
        io_time = 0
        if start_io_counters and end_io_counters:
            io_time = (end_io_counters.read_time + end_io_counters.write_time) - (start_io_counters.read_time + start_io_counters.write_time)
    ...
    def profile_sql_queries(self, db_path: str) -> List[SQLQueryProfile]:
        ...
        for (table_name,) in tables:
            ...
            for query_template, op_type in operations:
                query = query_template.replace("*", f"* FROM {table_name}") if "FROM" not in query_template else query_template
                ...
                try:
                    ...
                    if is_slow:
                        print(f"   🐌 Slow query detected: {execution_time:.2f}ms - {query[:50]}...")
        ...
        print(f"✅ SQL profiling complete: {len(sql_profiles)} queries, {len(slow_queries)} slow")
        
        return sql_profiles
    
    def track_sequence_step(self, step_name: str, component: str, start_time: float, end_time: float, 
                           step_type: str = "processing", data_size: int = 0, **metadata):
        """
    ...
    def generate_sequence_diagram(self) -> str:
        ...
        for step in sorted_steps:
            ...
            diagram.append(f"note over {component}: {type_emoji} {step_name}\\n{duration:.3f}s")
    ...
    def generate_timing_table(self) -> str:
        ...
        for profile in self.profile_results:
            all_timings.append({
                ...
                "Type": "CPU" if profile.cpu_time > profile.total_time * 0.7 else "I/O",
                "Hotspot": "🔥" if profile.total_time > 1.0 else "⚡" if profile.total_time > 0.1 else "✅"
            })
        ...
        for step in self.sequence_data:
            hotspot = "🔥" if step['duration'] > 2.0 else "⚡" if step['duration'] > 0.5 else "✅"
            all_timings.append({
                "Component": step['component'],
                "Name": step['step_name'],
        ...
        # Create table
        table_lines = []
        table_lines.append("| Component | Name | Duration (s) | CPU Time (s) | I/O Time (ms) | Memory (MB) | Type | Hotspot |")
        table_lines.append("|-----------|------|--------------|--------------|---------------|-------------|------|---------|")
    ...
    def generate_comprehensive_report(self) -> str:
        ...
        for profile in self.profile_results:
            if profile.total_time > 1.0:
                critical_bottlenecks.append(f"🔥 **{profile.function_name}**: {profile.total_time:.3f}s (CPU: {profile.cpu_time:.3f}s)")
        
        for sql_profile in self.sql_profiles:
            if sql_profile.execution_time > 500:
                critical_bottlenecks.append(f"🐌 **SQL {sql_profile.operation_type}**: {sql_profile.execution_time:.2f}ms on {sql_profile.table_name}")
        ...
        # CPU recommendations
        cpu_heavy = [p for p in self.profile_results if p.cpu_time > p.total_time * 0.7 and p.total_time > 0.1]
        if cpu_heavy:
            report_lines.append("### CPU Optimization")
            for profile in cpu_heavy:
                report_lines.append(f"- Optimize **{profile.function_name}** - CPU bound ({profile.cpu_time:.3f}s)")
        ...
        if io_heavy:
            ...
            for profile in io_heavy:
                report_lines.append(f"- Optimize **{profile.function_name}** - I/O bound ({profile.io_time:.1f}ms)")
        ...
        memory_heavy = [p for p in self.profile_results if p.memory_peak > 100]  # >100MB
        if memory_heavy:
            report_lines.append("### Memory Optimization")
            for profile in memory_heavy:
                report_lines.append(f"- Optimize **{profile.function_name}** - High memory usage ({profile.memory_peak:.2f}MB)")
        ...
        for profile in sorted(self.profile_results, key=lambda x: x.total_time, reverse=True):
            ...
            if profile.bottlenecks:
                report_lines.append(f"  - Bottlenecks: {', '.join(profile.bottlenecks)}")
            report_lines.append("")
    ...
    def _analyze_bottlenecks(self, profiler_output: str, total_time: float) -> List[str]:
        """
...
if __name__ == "__main__":
    ...
    print("Usage:")
    print("  from pipeline_profiler import profile_function, track_sequence, pipeline_profiler")
