...
class GroundTruthExtractor:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db"):
        ...
        self.ground_truth_columns = {
            ...
            'win_time', 'bonus_time', 'race_result', 'final_time', 'race_winner',
            'actual_finish_position', 'actual_race_time', 'race_outcome'
        }
    ...
    def _setup_leakage_logging(self):
        ...
        if not any(isinstance(h, logging.FileHandler) and h.baseFilename == str(self.leakage_log_path) 
    ...
    def load_csv_with_ground_truth_separation(self, csv_path: str) -> Tuple[pd.DataFrame, pd.DataFrame, bool]:
        """
        ...
        try:
            ...
            if not validation_report.is_valid:
                logger.error(f"CSV validation failed for {csv_path}: {validation_report.errors}")
                return pd.DataFrame(), pd.DataFrame(), False
            
            # Convert to DataFrame
            full_dataframe = pd.DataFrame(parsed_race.records, columns=parsed_race.headers)
            ...
            prediction_columns = [col for col in full_dataframe.columns 
            ...
            if removed_columns:
                logger.info(f"🛡️ Extracted {len(removed_columns)} ground-truth columns: {removed_columns}")
            ...
            logger.info(f"✅ Successfully separated prediction data ({len(prediction_columns)} cols) "
    ...
    def validate_temporal_integrity_pre_prediction(self, prediction_data: pd.DataFrame, 
                                                 race_id: str) -> Tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            ground_truth_violations = prediction_columns.intersection(self.ground_truth_columns)
            
            if ground_truth_violations:
                violation_msg = f"Ground-truth columns found in prediction data: {ground_truth_violations}"
                violations.append(violation_msg)
                self._log_leakage_violation("ground_truth_in_prediction", race_id, violation_msg)
            
            # Check 2: No future timestamp columns
            timestamp_columns = [col for col in prediction_data.columns 
                               if any(keyword in col.lower() for keyword in 
                                    ['result', 'final', 'outcome', 'winner', 'finish'])]
            
            if timestamp_columns:
                violation_msg = f"Suspicious future-leakage columns found: {timestamp_columns}"
                violations.append(violation_msg)
                self._log_leakage_violation("suspicious_columns", race_id, violation_msg)
            ...
            try:
                ...
                if 'dog_clean_name' not in race_data.columns and 'Dog Name' in race_data.columns:
                    race_data['dog_clean_name'] = race_data['Dog Name']
                
                # Build features (this will validate temporal integrity internally)
                features_df = temporal_builder.build_features_for_race(race_data, race_id)
                
                # Run explicit temporal integrity validation
                temporal_builder.validate_temporal_integrity(features_df, race_data)
                
                logger.info("✅ Temporal integrity validation passed via TemporalFeatureBuilder")
                
            except AssertionError as ae:
                violation_msg = f"TemporalFeatureBuilder assertion failed: {str(ae)}"
                violations.append(violation_msg)
                self._log_leakage_violation("temporal_builder_assertion", race_id, violation_msg)
                
            except Exception as e:
                logger.warning(f"⚠️ TemporalFeatureBuilder validation failed with error: {e}")
            ...
            required_columns = ['Dog Name', 'BOX']  # Basic required columns for prediction
            missing_required = [col for col in required_columns if col not in prediction_data.columns]
            
            if missing_required:
                violation_msg = f"Missing required prediction columns: {missing_required}"
                violations.append(violation_msg)
                self._log_leakage_violation("missing_required_columns", race_id, violation_msg)
            
            is_valid = len(violations) == 0
            
            if is_valid:
                logger.info(f"✅ Temporal integrity validation passed for race {race_id}")
            else:
                logger.error(f"❌ Temporal integrity validation failed for race {race_id}: {violations}")
    ...
    def merge_ground_truth_for_evaluation(self, predictions: Dict[str, Any], 
                                        race_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            if race_id not in self.ground_truth_cache:
                ...
                if isinstance(predictions, dict) and 'metadata' in predictions:
                    predictions['metadata']['ground_truth_warning'] = "No cached ground-truth data available"
                return predictions
            ...
            evaluation_result['metadata']['ground_truth_merge_timestamp'] = datetime.now().isoformat()
            
            logger.info(f"✅ Successfully merged ground-truth data for {len(actual_results)} dogs")
    ...
    def mark_race_as_leakage_detected(self, race_id: str, violations: List[str]) -> Dict[str, Any]:
        """
        ...
        self._log_leakage_violation("race_marked_invalid", race_id, 
    ...
    def get_leakage_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'leakage_rate': len(self.leakage_records) / max(1, len(self.ground_truth_cache)),
...
class LeakageProtectedPredictor:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db"):
        ...
        logger.info("🛡️ LeakageProtectedPredictor initialized with ground-truth extraction")
    ...
    def predict_race_with_leakage_protection(self, csv_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            prediction_data, ground_truth_data, load_success = \
                self.ground_truth_extractor.load_csv_with_ground_truth_separation(csv_path)
            ...
            race_id = self.ground_truth_extractor._generate_race_id(csv_path, ground_truth_data)
            ...
            if not integrity_valid:
                ...
                leakage_record = self.ground_truth_extractor.mark_race_as_leakage_detected(
                    race_id, violations
                )
            ...
            preprocessed_data = self.ml_system.preprocess_upcoming_race_csv(prediction_data, race_id)
            ...
            evaluation_result = self.ground_truth_extractor.merge_ground_truth_for_evaluation(
                predictions, race_id
            )
            ...
            logger.info(f"✅ Leakage-protected prediction completed successfully for race {race_id}")
