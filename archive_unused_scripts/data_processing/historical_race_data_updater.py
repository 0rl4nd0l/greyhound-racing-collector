...
class HistoricalRaceDataUpdater:
    ...
    def identify_races_needing_update(
        self, limit: Optional[int] = None, date_range: Optional[Tuple[str, str]] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if date_range:
                ...
                base_query += (
                    f" AND rm.race_date BETWEEN '{start_date}' AND '{end_date}'"
                )
    ...
    def reconstruct_race_url(self, race_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if race_name:
                ...
                url_race_name = url_race_name.strip(
                    "-"
            ...
            print(
                f"❌ Error reconstructing URL for race {race_data.get('race_id', 'unknown')}: {e}"
            )
            return None

    def update_race_with_enhanced_data(
        ...
        try:
            ...
            print(
                f"🔄 Updating race: {race_data.get('venue')} Race {race_data.get('race_number')} on {race_data.get('race_date')}"
            )
            ...
            if extraction_success:
                ...
                processing_results = (
                    self.data_processor.process_comprehensive_json_files()
                )
                ...
                if processing_results["processed"] > 0:
                    ...
                    result["success"] = True
                    print(
                        f"✅ Database updated with {processing_results['processed']} records"
                    )
                else:
    ...
    def update_historical_races_batch(
        self,
        ...
        for i in range(0, len(race_list), batch_size):
            ...
            for j, race_data in enumerate(batch):
                ...
                if update_result["success"]:
                    ...
                    self.update_stats["new_records_created"] += update_result[
                        "records_created"
                    ]
        ...
        results["success_rate"] = (
            results["successful_updates"] / results["total_races"] * 100
        ...
        print(
            f"   • Successful: {results['successful_updates']} ({results['success_rate']:.1f}%)"
        )
    ...
    def update_races_by_date_range(
        self, start_date: str, end_date: str, max_races: int = 100
    ) -> Dict[str, Any]:
        ...
        race_list = self.identify_races_needing_update(
            limit=max_races, date_range=(start_date, end_date)
        )
    ...
    def update_recent_races(
        self, days_back: int = 30, max_races: int = 50
    ) -> Dict[str, Any]:
        ...
        return self.update_races_by_date_range(
            start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), max_races
        )
    ...
    def generate_update_report(self) -> Dict[str, Any]:
        ...
        # Save report
        report_filename = (
            f"historical_update_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        try:
            ...
            for table, description in tables.items():
                try:
                    ...
                    stats[table] = {
                        "count": 0,
                        "description": f"{description} (table not found)",
                    }
    ...
    def assess_update_health(self) -> Dict[str, Any]:
        ...
        total_attempts = (
            ...
            + self.update_stats["failed_updates"]
        )
        if total_attempts > 0:
            success_rate = (
                self.update_stats["successful_updates"] / total_attempts
            ) * 100
            ...
            if success_rate < 70:
                ...
                health["warnings"].append(
                    f"Moderate update success rate: {success_rate:.1f}%"
                )
                health["score"] -= 10
    ...
    def generate_update_recommendations(self) -> List[str]:
        ...
        if total_attempts > 0:
            ...
            if success_rate < 80:
                recommendations.append(
                    "Low success rate detected - review error logs and adjust URL reconstruction logic"
                )
        ...
        if error_count > 10:
            recommendations.append(
                "High error count - consider implementing more robust error handling and retry logic"
            )
    ...
    def run_comprehensive_historical_update(
        self, max_races: int = 100, days_back: int = 90
    ) -> Dict[str, Any]:
        ...
        comprehensive_results = {"start_time": datetime.now().isoformat(), "phases": {}}
...
def main():
    ...
    print("=" * 60)
    print(
        f"This tool enriches your existing race data with enhanced expert form metrics."
    )
