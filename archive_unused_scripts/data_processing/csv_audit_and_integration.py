...
class CSVAuditIntegrator:
    ...
    def check_database_integration(self, all_csvs):
        ...
        try:
            ...
            cursor.execute("SELECT DISTINCT filename FROM race_metadata WHERE filename IS NOT NULL")
            ...
            cursor.execute("SELECT DISTINCT race_file FROM enhanced_expert_data WHERE race_file IS NOT NULL")
    ...
    def check_enhanced_processing(self, all_csvs):
        ...
        if enhanced_csv_dir and os.path.exists(enhanced_csv_dir):
            ...
            for path, info in all_csvs.items():
                ...
                if info['filename'] in enhanced_filenames or enhanced_name in enhanced_filenames:
        ...
        if enhanced_json_dir and os.path.exists(enhanced_json_dir):
            json_files = [f.replace('.json', '.csv') for f in os.listdir(enhanced_json_dir) if f.endswith('.json')]
            ...
            if path not in serving_purpose:
                ...
                if self.is_malformed_file(path):
                    self.audit_results['malformed_files'].append({
                        'path': path,
                        'reason': 'Malformed or corrupted file',
                        **info
                    })
    ...
    def generate_integration_plan(self):
        ...
        print(f"\n📋 Integration Plan:")
        print(f"  • Integration candidates: {len(self.audit_results['integration_candidates'])}")
        print(f"  • Cleanup candidates: {len(self.audit_results['malformed_files']) + len(self.audit_results['duplicate_files'])}")
        ...
        for candidate in self.audit_results['integration_candidates']:
            ...
            if 'race' in filename and any(month in filename for month in ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december']):
    ...
    def execute_integration(self):
        ...
        for malformed_file in self.audit_results['malformed_files']:
            try:
                dest_path = f"./organized_csvs/cleanup_needed/{malformed_file['filename']}"
                if not os.path.exists(dest_path):
    ...
    def generate_report(self):
        ...
        report_path = f"csv_audit_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        
        # Calculate utilization rates
        total_csvs = self.audit_results['total_csvs']
        serving_purpose = self.audit_results['database_integrated'] + self.audit_results['enhanced_processed']
        ...
        report = {
            ...
            'summary': {
                ...
                'integration_candidates': len(self.audit_results['integration_candidates']),
                'cleanup_needed': len(self.audit_results['malformed_files']) + len(self.audit_results['duplicate_files'])
            },
...
def main():
    ...
    print(f"  Cleanup Needed: {len(results['malformed_files']) + len(results['duplicate_files']):,}")
    
    utilization = ((results['database_integrated'] + results['enhanced_processed']) / results['total_csvs'] * 100) if results['total_csvs'] > 0 else 0
