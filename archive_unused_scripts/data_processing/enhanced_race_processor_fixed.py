...
class EnhancedRaceProcessor:
    def __init__(self, db_path="greyhound_racing_data.db"):
        ...
        self.format_patterns = {
            "navigator_results": {
                ...
                "identifier": lambda df: all(
                    col in df.columns for col in ["first", "second", "third", "fourth"]
                ),
            },
            "form_guide": {
                "columns": ["PLC"],
                "position_method": "plc_column",
                "identifier": lambda df: "PLC" in df.columns
                and "Dog Name" in df.columns,
            },
            "standard_results": {
                "columns": ["position", "finish_position"],
                "position_method": "position_column",
                "identifier": lambda df: any(
                    col in df.columns for col in ["position", "finish_position"]
                ),
            },
        }
    ...
    def process_race_results(
        self, file_path: Path, race_info: Dict = None, move_processed: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # Read the CSV file
            df = pd.read_csv(
                file_path, sep="|" if "|" in open(file_path).read()[:500] else ","
            )
    ...
    def _process_navigator_results(
        self, df: pd.DataFrame, race_info: Dict
    ) -> List[Dict]:
    ...
    def _process_form_guide(self, df: pd.DataFrame, race_info: Dict) -> List[Dict]:
        ...
        print(
            "⚠️  Form guide detected - this contains historical data, not race results"
        )
        ...
        for _, row in df.iterrows():
            ...
            if (
                dog_name
                and dog_name != '""'
                and dog_name.lower() not in ["nan", "none"]
            ):
                results.append(
                    {
                        "dog_name": dog_name,
    ...
    def _process_standard_results(
        ...
        for _, row in df.iterrows():
            ...
            if dog_name and dog_name.lower() not in ["", "nan", "none"]:
                position_col = (
                    "position" if "position" in df.columns else "finish_position"
                )
    ...
    def _process_unknown_format(self, df: pd.DataFrame, race_info: Dict) -> List[Dict]:
        ...
        name_columns = [
            col for col in df.columns if "name" in col.lower() or "dog" in col.lower()
        ]
        position_columns = [
            col
            for col in df.columns
            if any(term in col.lower() for term in ["pos", "plc", "place", "finish"])
        ]
    ...
    def _create_position_map_from_placings(
        self, placings: Dict[int, str]
    ) -> Dict[str, str]:
        ...
        for position, dog_name in placings.items():
            if dog_name and dog_name.lower() not in ["", "nan", "none"]:
                ...
                if dog_name in position_map:
                    ...
                    print(
                        f"🏁 Dead heat detected: {dog_name} tied at positions {existing_pos} and {position}"
                    )
                else:
        ...
        if not is_form_guide:
            ...
            if not positions:
                validation["warnings"].append(
                    "No finish positions found - may be form guide data"
                )
            else:
    ...
    def _store_race_results(self, results: List[Dict], race_info: Dict):
        ...
        try:
            ...
            conn.commit()
            print(
                f"✅ Stored race results for {len(race_groups)} races: {sum(len(group) for group in race_groups.values())} dogs total"
            )
    ...
    def _store_with_quality_issues(
        self, results: List[Dict], race_info: Dict, validation: Dict
    ):
        """Store race results with quality issue markers"""
        # Mark race as needing review
        race_info["race_status"] = "needs_review"
        race_info["data_quality_note"] = "; ".join(
        ...
        self._store_race_results(results, race_info)
        print(
            f"⚠️  Stored race {race_info.get('race_id')} with quality issues: {race_info['data_quality_note']}"
        )
...
def process_race_file(
    file_path: str, db_path: str = "greyhound_racing_data.db"
) -> Dict:
