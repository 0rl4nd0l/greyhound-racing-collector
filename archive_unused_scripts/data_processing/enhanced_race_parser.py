...
class EnhancedRaceParser:
    ...
    def extract_race_info(self, filename: str) -> Dict[str, Any]:
        ...
        for pattern_config in self.filename_patterns:
            ...
            if match:
                try:
                    ...
                    self.logger.warning(f"Error processing pattern {pattern_config['format']}: {e}")
                    result['parse_warnings'].append(f"Pattern {pattern_config['format']} failed: {e}")
        ...
        self.logger.info(f"Parsed {filename} -> venue: {result['venue']}, race: {result['race_number']}, confidence: {result['parse_confidence']:.2f}")
    ...
    def _process_match(self, match, pattern_config):
        ...
        if len(groups) != len(group_names):
            raise ValueError(f"Group count mismatch: {len(groups)} vs {len(group_names)}")
    ...
    def _try_fallback_parsing(self, filename: str, current_result: Dict) -> Dict:
        ...
        if venue_matches:
            ...
            for venue in venue_matches:
                if venue in self.venue_mapping.values() or venue.lower() in self.venue_mapping:
                    result['venue'] = venue
    ...
    def _try_content_based_parsing(self, filename: str, current_result: Dict) -> Dict:
        ...
        try:
            ...
            for path in possible_paths:
                if path.exists():
                    try:
                        df = pd.read_csv(path, nrows=5, sep='|')  # Read first few rows only
                                ...
                                for value in values:
                                    ...
                                    for venue in venue_matches:
                                        if venue in self.venue_mapping.values():
                                            result['venue'] = venue
                                            result['parse_confidence'] = 0.3
                                            result['parse_method'] = 'content_based_scanning'
                                            break
                                    if result['venue']:
                                        break
                            if result['venue']:
    ...
    def _validate_and_clean(self, result: Dict) -> Dict:
        ...
        if not result['venue'] and not result['race_number']:
            result['parse_warnings'].append("No venue or race number could be extracted")
...
def patch_race_predictor():
    ...
    try:
        ...
        upcoming_race_predictor.UpcomingRacePredictor.extract_race_info = enhanced_extract_race_info
