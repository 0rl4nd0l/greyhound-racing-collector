...
def show_database_status():
    ...
    if 'coverage' in db_stats:
        ...
        print(f"   • Races needing update: {coverage['total_races'] - coverage['enhanced_races']:,}")
...
def update_recent_races(days_back: int = 30, max_races: int = 50):
    ...
    if results.get('success_rate', 0) > 0:
        print(f"✅ Update completed with {results.get('success_rate', 0):.1f}% success rate")
        print(f"📊 Results: {results.get('successful_updates', 0)} successful, {results.get('failed_updates', 0)} failed")
...
def update_specific_race_urls(race_urls: List[str]):
    ...
    results = system.process_race_urls_comprehensively(race_urls, use_both_methods=False)
...
def run_comprehensive_update(max_races: int = 100, days_back: int = 90):
    ...
    updater = HistoricalRaceDataUpdater()
    results = updater.run_comprehensive_historical_update(max_races=max_races, days_back=days_back)
    ...
    if results.get('success'):
        ...
        if 'updates' in phases:
            ...
            print(f"🔄 Phase 2 - Updates: {update_phase['successful_updates']}/{update_phase['total_races']} successful")
            print(f"💾 Records created: {update_phase['total_records_created']}")
        
        if 'reporting' in phases:
            print(f"📊 Phase 3 - Reporting: Complete")
    else:
        print(f"❌ Comprehensive update failed: {results.get('error', 'Unknown error')}")
...
def process_new_race_urls(race_urls: List[str]):
    ...
    if successful_extractions > 0:
        ...
        print(f"   • Database records created: {processing_results.get('processed', 0)}")
        ...
        return {'urls_processed': len(race_urls), 'successful_extractions': 0, 'records_created': 0}
...
def main():
    ...
    if 'coverage' in db_status:
        ...
        races_needing_update = db_status['coverage']['total_races'] - db_status['coverage']['enhanced_races']
