...
class DataQualityFixer:
    ...
    def _fix_unknown_races(self, conn, issue):
        ...
        for race in issue['races']:
            ...
            if race_id.startswith('UNK_0_UNKNOWN'):
                ...
                self._mark_race_for_review(conn, race_id, "Unknown race with mixed data sources")
            else:
                ...
                if parsed_info:
                    ...
                    print(f"   ✅ {race_id}: Updated venue to {parsed_info['venue']}, race {parsed_info['race_number']}")
    ...
    def _fix_duplicate_positions(self, conn, issue):
        ...
        for dup in issue['duplicates']:
            ...
            if len(dogs) == count:
                ...
                times = [float(dog[2]) if dog[2] and dog[2] != '' else None for dog in dogs]
                ...
                if len(unique_times) == 1 and len(dogs) <= 3:
                    ...
                    for dog in dogs:
                        cursor.execute("""
                            ...
                            WHERE id = ?
                        """, (f"{position}=", dog[0]))
                    print(f"   ✅ {race_id}: Marked {count} dogs as dead heat at position {position}")
                else:
                    ...
                    print(f"   ✅ {race_id}: Reassigned {count} dogs from position {position} to {position}-{int(position)+count-1}")
        ...
        for race in issue['races']:
            ...
            if positions:
                ...
                if missing_positions:
                    print(f"   ⚠️  {race_id}: Missing positions {sorted(missing_positions)} - marked for manual review")
                    self._mark_race_for_review(conn, race_id, f"Missing positions: {sorted(missing_positions)}")
                
                if 1 not in positions:
                    print(f"   ❌ {race_id}: Missing winner (position 1) - marked for urgent review")
                    self._mark_race_for_review(conn, race_id, "Missing winner - position 1 not found")
            ...
            if match:
                if len(match.groups()) == 3:
                    return {
                        ...
                        'race_number': int(match.group(2)) if match.group(2).isdigit() else 0,
    ...
    def validate_data_quality(self):
        ...
        validation_results['position_completeness'] = {
            'total_races': len(df),
            'races_with_all_positions': len(df[df['total_dogs'] == df['dogs_with_positions']]),
            'races_with_winner': len(df[df['min_position'] == 1]),
            'races_missing_positions': len(df[df['total_dogs'] != df['dogs_with_positions']])
        }
        ...
        print(f"   Identified venues: {rm['identified_races']} ({rm['identified_races']/rm['total_races']*100:.1f}%)")
        print(f"   Numbered races: {rm['numbered_races']} ({rm['numbered_races']/rm['total_races']*100:.1f}%)")
        print(f"   Needs review: {rm['needs_review']}")
        
        print(f"\n📊 POSITION DATA:")
        print(f"   Total races: {pc['total_races']}")
        print(f"   Complete positions: {pc['races_with_all_positions']} ({pc['races_with_all_positions']/pc['total_races']*100:.1f}%)")
        print(f"   Has winner: {pc['races_with_winner']} ({pc['races_with_winner']/pc['total_races']*100:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(description='Fix data quality issues in greyhound racing database')
    parser.add_argument('--analyze', action='store_true', help='Analyze data quality issues')
    ...
    parser.add_argument('--report', default='data_quality_report.json', help='Report output file')
