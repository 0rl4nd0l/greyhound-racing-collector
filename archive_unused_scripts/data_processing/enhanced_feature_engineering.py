...
class EnhancedFeatureEngineer:
    ...
    def create_advanced_features(self, dog_stats, race_context=None):
        """Create advanced features from basic dog statistics"""
        if not dog_stats:
        ...
        features.update(self._create_performance_features(dog_stats))
        
        # Time-based features
        features.update(self._create_temporal_features(dog_stats))
        
        # Consistency and reliability features
        features.update(self._create_consistency_features(dog_stats))
        
        # Speed and class features
        features.update(self._create_speed_class_features(dog_stats))
        
        # Environmental adaptation features
        features.update(self._create_environmental_features(dog_stats))
        
        # Race context features
        if race_context:
            features.update(self._create_contextual_features(dog_stats, race_context))
    ...
    def _create_performance_features(self, dog_stats):
        ...
        if recent_form:
            ...
            weighted_avg_position = np.average(recent_form, weights=weights)
        ...
        features['completion_rate'] = min(1.0, races_count / 10)  # Proxy for reliability
    ...
    def _create_speed_class_features(self, dog_stats):
        ...
        features['best_time_rating'] = min(1.0, (40 - dog_stats.get('best_time', 35)) / 10) if dog_stats.get('best_time', 0) > 0 else 0.5
        ...
        # Performance ratings
        features['performance_rating'] = dog_stats.get('avg_performance_rating', 50) / 100
        
        # Speed-class combination
        features['speed_class_composite'] = (features['speed_index'] + features['class_rating']) / 2
    ...
    def _create_environmental_features(self, dog_stats):
        ...
        # Track versatility
        features['track_versatility'] = min(1.0, dog_stats.get('track_versatility', 1) / 5)
        ...
        if track_performance:
            ...
            avg_positions = [data.get('avg_position', 4) for data in track_performance.values() if isinstance(data, dict)]
            if avg_positions:
                features['track_adaptability'] = max(0, (8 - np.mean(avg_positions)) / 8)
            else:
        ...
        features['distance_specialization'] = 0.5  # Default, could be enhanced with distance analysis
    ...
    def _create_contextual_features(self, dog_stats, race_context):
        ...
        features['field_size_normalized'] = field_size / 12  # Normalize to typical range
        ...
        if current_condition in track_performance:
            ...
            if isinstance(condition_data, dict) and condition_data.get('races', 0) >= 2:
                features['condition_match_score'] = max(0, (8 - condition_data.get('avg_position', 4)) / 8)
            else:
    ...
    def _create_composite_features(self, features, dog_stats):
        ...
        composite_features['form_momentum'] = np.mean([
            ...
            1 - features.get('weighted_recent_position', 4) / 8,  # Convert position to score
    ...
    def get_feature_importance_ranking(self):
        ...
        return {
            'high_importance': [
                'competitive_ability', 'form_momentum', 'win_strike_rate',
                'position_quality_score', 'weighted_recent_position'
            ],
            'medium_importance': [
                'fitness_score', 'contextual_advantage', 'place_strike_rate',
                'reliability_score', 'speed_class_composite'
            ],
            'low_importance': [
                'adaptability', 'field_size_normalized', 'race_distance',
                'temperature_suitability', 'venue_experience'
            ]
        }
