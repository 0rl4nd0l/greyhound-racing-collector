...
class IntegratedEnhancedFormSystem:
    ...
    def process_race_urls_comprehensively(
        self, race_urls: List[str], use_both_methods: bool = True
    ) -> Dict[str, Any]:
        ...
        for i, race_url in enumerate(race_urls):
            ...
            try:
                ...
                if use_both_methods and self.standard_scraper:
                    ...
                    try:
                        standard_success = (
                            self.standard_scraper.download_csv_from_race_page(race_url)
                        )
                        ...
                        race_result["errors"].append(
                            f"Standard scraper error: {str(e)}"
                        )
                ...
                race_result["overall_success"] = (
                    race_result["enhanced_success"] or race_result["standard_success"]
                )
        ...
        results["success_rate"] = (
            results["successful_races"] / results["total_races"] * 100
            if results["total_races"] > 0
            else 0
        )
        results["enhanced_rate"] = (
            results["enhanced_extractions"] / results["total_races"] * 100
            if results["total_races"] > 0
            else 0
        )
        results["standard_rate"] = (
            results["standard_downloads"] / results["total_races"] * 100
            if results["total_races"] > 0
            else 0
        )
        ...
        print(
            f"   • Enhanced extractions: {results['enhanced_extractions']} ({results['enhanced_rate']:.1f}%)"
        )
        print(
            f"   • Standard downloads: {results['standard_downloads']} ({results['standard_rate']:.1f}%)"
        )
    ...
    def process_extracted_data(self) -> Dict[str, Any]:
        ...
        if processing_results["processed"] > 0:
            self.processing_stats["database_records_created"] += processing_results[
                "processed"
            ]
            print(
                f"✅ Processed {processing_results['processed']} comprehensive data files"
            )
    ...
    def process_standard_csv_files(self) -> Dict[str, Any]:
        ...
        result = {"processed": 0, "integrated": 0, "errors": []}
    ...
    def find_race_urls_for_date_range(
        self, days_back: int = 7, max_races: int = 50
    ) -> List[str]:
    ...
    def assess_system_health(self) -> Dict[str, Any]:
        ...
        health = {"status": "healthy", "issues": [], "warnings": [], "score": 100}

        # Check processing success rates
        if self.processing_stats["races_processed"] > 0:
            success_rate = (
                self.processing_stats["races_processed"]
                / (
                    self.processing_stats["races_processed"]
                    + len(self.processing_stats["errors"])
                )
            ) * 100
            ...
            if success_rate < 80:
                ...
                health["warnings"].append(
                    f"Moderate processing success rate: {success_rate:.1f}%"
                )
                health["score"] -= 10
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if self.processing_stats["enhanced_data_extracted"] == 0:
            recommendations.append(
                "Consider running enhanced data extraction to get richer datasets"
            )

        if len(self.processing_stats["errors"]) > 0:
            recommendations.append(
                "Review and address processing errors to improve data quality"
            )
        ...
        try:
            ...
            if enhanced_count < standard_count * 0.1:
                recommendations.append(
                    "Enhanced data coverage is low - consider processing more expert form data"
                )
            ...
            recommendations.append(
                "Database health check failed - verify database integrity"
            )
    ...
    def print_report_summary(self, report: Dict[str, Any]):
        ...
        total_records = sum(
            ...
            if isinstance(v, int) and k != "recent_enhanced_records"
        )
        print(f"   • Total records: {total_records}")
        print(f"   • Enhanced records: {db_stats.get('enhanced_dog_performance', 0)}")
        print(
            f"   • Recent activity (24h): {db_stats.get('recent_enhanced_records', 0)}"
        )
        print(
            f"   • Average data quality: {db_stats.get('average_data_quality', 0):.2f}"
        )
    ...
    def run_full_pipeline(
        self, max_races: int = 20, days_back: int = 3
    ) -> Dict[str, Any]:
        ...
        pipeline_results = {"start_time": datetime.now().isoformat(), "stages": {}}

        try:
            # Stage 1: Find race URLs
            print(f"\n📍 STAGE 1: Finding Race URLs")
            race_urls = self.find_race_urls_for_date_range(
                days_back=days_back, max_races=max_races
            )
            ...
            processing_results = self.process_race_urls_comprehensively(
                race_urls, use_both_methods=True
            )
...
def main():
    ...
    print("=" * 60)
    print(
        f"This system combines standard form guide scraping with enhanced expert form data extraction"
    )
    print(
        f"to provide comprehensive greyhound racing data for advanced analysis and ML modeling."
    )
