...
class EnhancedTrackConditionExtractor:
    ...
    def extract_track_conditions_enhanced(
        self, soup: BeautifulSoup, race_url: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            conditions, confidence_score, extraction_method = (
                self._extract_from_official_elements(soup)
            )

            if not conditions.get("condition"):
                # Strategy 2: Look for meeting/race information sections
                conditions, confidence_score, extraction_method = (
                    self._extract_from_meeting_info(soup)
                )
            ...
            if not conditions.get("condition"):
                ...
                conditions, confidence_score, extraction_method = (
                    self._extract_with_context_validation(soup, race_url)
                )

            if not conditions.get("condition"):
                # Strategy 5: Look for venue-specific patterns
                conditions, confidence_score, extraction_method = (
                    self._extract_venue_specific(soup, race_url)
                )
            ...
            if conditions:
                ...
                if self._validate_extracted_condition(conditions, race_url):
                    print(
                        f"   ✅ Enhanced extraction successful: {conditions['condition']} (confidence: {confidence_score}, method: {extraction_method})"
                    )
                    return conditions
                else:
                    print(
                        f"   ❌ Extracted condition failed validation: {conditions['condition']}"
                    )
                    return None
    ...
    def _extract_from_official_elements(
        ...
        for selector in selectors:
            ...
            if elem:
                ...
                if condition:
                    return (
                        {"condition": condition, "raw_text": text},
                        95,
                        f"official_element_{selector}",
                    )
    ...
    def _extract_from_meeting_info(self, soup: BeautifulSoup) -> Tuple[Dict, int, str]:
        ...
        for selector in meeting_selectors:
            ...
            if section:
                ...
                if condition:
                    return (
                        {"condition": condition, "source_section": selector},
                        85,
                        f"meeting_info_{selector}",
                    )
        ...
        for script in json_scripts:
            try:
                ...
                if isinstance(data, dict):
                    ...
                    condition = self._find_in_json_data(
                        data, ["trackCondition", "condition", "surface"]
                    )
                    if condition:
                        return (
                            {
        ...
        for elem in microdata_elements:
            ...
            if track_condition:
                condition = self._normalize_condition(
                    track_condition.get_text(strip=True)
                )
                if condition:
                    return (
                        {"condition": condition, "source": "microdata"},
                        88,
    ...
    def _extract_with_context_validation(
        self, soup: BeautifulSoup, race_url: str
    ) -> Tuple[Dict, int, str]:
            ...
            for match in matches:
                ...
                if condition:
                    return (
                        ...
                        70,
                        "context_validated_pattern",
                    )

        return {}, 0, None

    def _extract_venue_specific(
        self, soup: BeautifulSoup, race_url: str
    ) -> Tuple[Dict, int, str]:
    ...
    def _is_in_sponsorship_context(
        self, match: re.Match, page_text: str, race_url: str
    ) -> bool:
    ...
    def _find_condition_in_text(
        self, text: str, exclude_sponsorship: bool = True
    ) -> Optional[str]:
    ...
    def _extract_nsw_track_pattern(self, soup: BeautifulSoup) -> Tuple[Dict, int, str]:
        ...
        nsw_selectors = [".grnsw-condition", ".nsw-track-info"]
    ...
    def _extract_sa_track_pattern(self, soup: BeautifulSoup) -> Tuple[Dict, int, str]:
        ...
        sa_selectors = [".grsa-condition", ".sa-track-details"]
