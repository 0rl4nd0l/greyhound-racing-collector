...
class ComprehensiveCSVEnhancedProcessor:
                ...
                if current_dog:
                    ...
                    if race_data:
                        ...
                        if enhanced_data['dogs']:
                            enhanced_data['dogs'][-1]['historical_races'].append(race_data)
            
            # Calculate enhanced metrics for each dog
            for dog_data in enhanced_data['dogs']:
                dog_data['enhanced_metrics'] = self.calculate_enhanced_metrics_for_dog(dog_data['historical_races'])
    ...
    def _safe_convert(self, value, data_type):
        ...
        try:
            ...
            # Numeric conversions
            numeric_fields = ['position', 'box', 'weight', 'distance', 'time', 'win_time', 
                             'bonus_time', 'first_sectional', 'margin', 'pir_rating', 'starting_price']
    ...
    def calculate_enhanced_metrics_for_dog(self, historical_races):
        ...
        try:
            ...
            positions = [r['position'] for r in historical_races if r.get('position') and r['position'] > 0]
            sectionals = [r['first_sectional'] for r in historical_races if r.get('first_sectional') and r['first_sectional'] > 0]
            weights = [r['weight'] for r in historical_races if r.get('weight') and r['weight'] > 0]
            margins = [r['margin'] for r in historical_races if r.get('margin') is not None]
            pir_ratings = [r['pir_rating'] for r in historical_races if r.get('pir_rating') and r['pir_rating'] > 0]
            times = [r['time'] for r in historical_races if r.get('time') and r['time'] > 0]
            ...
            if positions:
                ...
                metrics['win_rate'] = sum(1 for p in positions if p == 1) / len(positions)
                metrics['place_rate'] = sum(1 for p in positions if p <= 3) / len(positions)
            ...
            if weights:
                ...
                if len(weights) > 1:
                    metrics['weight_trend'] = np.polyfit(range(len(weights)), weights, 1)[0]
            ...
            if pir_ratings:
                ...
                if len(pir_ratings) > 1:
                    metrics['pir_trend'] = np.polyfit(range(len(pir_ratings)), pir_ratings, 1)[0]
            ...
            metrics['enhanced_data_available'] = len(sectionals) > 0 or len(pir_ratings) > 0
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Comprehensive CSV Enhanced Data Processor')
