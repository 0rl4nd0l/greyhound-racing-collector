...
class CorrectVenueAnalyzer:
    ...
    def calculate_per_dog_venue_performance(self, df):
        ...
        dog_overall_stats["overall_performance_score"] = (
            ...
            - dog_overall_stats["overall_avg_position"]
            + 1
        ) / dog_overall_stats["avg_field_size"]

        # Calculate each dog's performance at each venue
        dog_venue_stats = (
            df.groupby(["dog_name", "venue"])
            .agg({"finish_position": ["mean", "count"], "field_size": "mean"})
            .reset_index()
        )

        dog_venue_stats.columns = [
            "dog_name",
            "venue",
            "venue_avg_position",
            "venue_races",
        ...
        dog_venue_stats["venue_performance_score"] = (
            ...
            - dog_venue_stats["venue_avg_position"]
            + 1
        ) / dog_venue_stats["venue_avg_field_size"]
        ...
        comparison["position_difference"] = (
            comparison["overall_avg_position"] - comparison["venue_avg_position"]
        )
    ...
    def analyze_venue_characteristics(self, comparison_df):
        ...
        venue_analysis["confidence_score"] = np.minimum(
            venue_analysis["dogs_analyzed"] / 20, 1.0  # Normalize by 20 dogs
        )
        ...
        venue_analysis["venue_type"] = venue_analysis["avg_venue_effect"].apply(
            lambda x: (
                "Favors Strong Dogs"
                if x > 0.02
                else "Favors Weak Dogs" if x < -0.02 else "Neutral"
            )
        )
    ...
    def identify_venue_specialists(self, comparison_df):
        ...
        specialists = comparison_df[
            (comparison_df["venue_races"] >= 3)
            & (comparison_df["total_races"] >= 8)  # At least 3 races at venue
            & (  # At least 8 total races
                abs(comparison_df["venue_effect"]) > 0.1
            )  # Significant effect
        ].copy()
    ...
    def run_analysis(self):
        ...
        for _, venue in venue_analysis.head(15).iterrows():
            ...
            print(
                f"{venue['venue']:<8} {effect:<8} {venue_type:<18} {dogs:<5} {confidence:<10}"
            )
        ...
        if len(specialists) > 0:
            ...
            for _, spec in specialists.head(10).iterrows():
                ...
                print(
                    f"• {dog} at {venue}: {effect:+.3f} effect ({races} races) - {spec_type}"
                )
