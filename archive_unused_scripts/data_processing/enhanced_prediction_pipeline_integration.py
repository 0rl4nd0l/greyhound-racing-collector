...
class EnhancedPipelineIntegrator:
    ...
    def _sync_enhanced_data_to_database(self):
        ...
        try:
            ...
            if not integrator.enhanced_data_available:
                ...
                self.integration_report["errors"].append(
                    "No enhanced expert form data found"
                )
                return False

            print(
                f"📊 Found enhanced data: {len(list(integrator.enhanced_csv_dir.glob('*.csv')))} CSV files"
            )
            print(
                f"📄 Found enhanced data: {len(list(integrator.enhanced_json_dir.glob('*.json')))} JSON files"
            )
            ...
            if sync_success:
                ...
                self.integration_report["components_integrated"].append(
                    "enhanced_database_sync"
                )
                return True
            else:
    ...
    def _update_comprehensive_ml_system(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(DISTINCT dog_clean_name) FROM enhanced_expert_data WHERE first_sectional IS NOT NULL"
            )
            dogs_with_sectionals = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(DISTINCT dog_clean_name) FROM enhanced_expert_data WHERE pir_rating IS NOT NULL"
            )
    ...
    def _test_enhanced_feature_creation(self):
        ...
        try:
            ...
            if enhanced_data and enhanced_data.get("enhanced_features"):
                ...
                print(
                    f"   ✅ Enhanced features created: {len(enhanced_features)} features"
                )
                ...
                features_found = sum(
                    1 for feat in key_features if feat in enhanced_features
                )
                print(
                    f"   📊 Key enhanced features found: {features_found}/{len(key_features)}"
                )
    ...
    def _validate_enhanced_data_in_models(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"📊 Enhanced/Weather/Traditional features: {len(enhanced_features)}"
                )
            ...
            if WEATHER_PREDICTOR_AVAILABLE:
                try:
                    ...
                    if current_dog_name:
                        ...
                        enhanced_data = integrator.get_enhanced_dog_data(
                            current_dog_name, max_races=3
                        )

                        if enhanced_data and enhanced_data.get("enhanced_features"):
                            features = enhanced_data["enhanced_features"]
                            print(
                                f"   ✅ Enhanced data retrieved: {features.get('enhanced_data_quality', 0)} data points"
                            )

                            # Test specific enhanced features
                            sectional_data = len(
                                enhanced_data.get("sectional_times", [])
                            )
                            ...
                            self.integration_report["validation_results"][
                                "pipeline_test"
                            ] = {
    ...
    def _generate_integration_report(self):
        ...
        try:
            ...
            print(
                f"   Success rate: {((total_components/(total_components+total_errors))*100):.1f}%"
                if (total_components + total_errors) > 0
                else "N/A"
            )
            ...
            components = [
                (
                    ...
                    "enhanced_database_sync"
                    in self.integration_report["components_integrated"],
                ),
                (
                    "Comprehensive ML System",
                    "comprehensive_ml_system"
                    in self.integration_report["components_integrated"],
                ),
                (
            ...
            # Data statistics
            print(f"\n📊 DATA STATISTICS:")
            db_stats = self.integration_report["data_statistics"].get(
                "enhanced_database", {}
            )
            if db_stats:
                print(
                    f"   Enhanced records: {db_stats.get('total_enhanced_records', 0)}"
                )
                print(
                    f"   Dogs with enhanced data: {db_stats.get('dogs_with_enhanced_data', 0)}"
                )
                print(
                    f"   Races with enhanced data: {db_stats.get('races_with_enhanced_data', 0)}"
                )

            feature_stats = self.integration_report["data_statistics"].get(
                "enhanced_features", {}
            )
            if feature_stats:
                print(
                    f"   Dogs with sectional times: {feature_stats.get('dogs_with_sectionals', 0)}"
                )
                print(
                    f"   Dogs with PIR ratings: {feature_stats.get('dogs_with_pir', 0)}"
                )

            # Validation results
            print(f"\n🔍 VALIDATION RESULTS:")
            model_val = self.integration_report["validation_results"].get(
                "model_validation", {}
            )
            if model_val:
                print(
                    f"   Model enhanced features: {model_val.get('enhanced_features_count', 0)}"
                )
                print(
                    f"   Model accuracy: {model_val.get('model_metadata', {}).get('accuracy', 0):.3f}"
                )

            pipeline_test = self.integration_report["validation_results"].get(
                "pipeline_test", {}
            )
            if pipeline_test:
                print(
                    f"   Pipeline test: {'✅ PASSED' if pipeline_test.get('enhanced_data_retrieved') else '❌ FAILED'}"
                )
    ...
    def get_integration_status(self):
        ...
        try:
            ...
            if all(
                [
                    status["enhanced_data_available"],
                    status["database_synced"],
                    status["models_updated"],
                ]
            ):
...
def main():
    ...
    print(
        f"Enhanced Data Available: {'✅' if status['enhanced_data_available'] else '❌'}"
    )
    ...
    if status.get("enhanced_files"):
        print(
            f"Enhanced Files: {status['enhanced_files']['csv']} CSV, {status['enhanced_files']['json']} JSON"
        )
