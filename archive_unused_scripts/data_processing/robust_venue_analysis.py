...
class RobustVenueAnalyzer:
    ...
    def load_comprehensive_data(self):
        ...
        print(
            f"📊 Loaded {len(df)} race entries from {df['race_date'].nunique()} unique dates"
        )
    ...
    def analyze_data_coverage(self, df):
        ...
        coverage_stats = {
            ...
            "cross_venue_percentage": (
                (multi_venue_dogs / total_dogs * 100) if total_dogs > 0 else 0
            ),
            "total_venues": df["venue"].nunique(),
            "date_span_days": (
                pd.to_datetime(df["race_date"].max())
                - pd.to_datetime(df["race_date"].min())
            ).days
            + 1,
        }
        ...
        print(
            f"   Dogs racing at multiple venues: {coverage_stats['multi_venue_dogs']} ({coverage_stats['cross_venue_percentage']:.1f}%)"
        )
    ...
    def calculate_primary_venue_effects(self, df):
        ...
        dog_overall = (
            multi_venue_data.groupby("dog_name")
            .agg({"finish_position": "mean", "field_size": "mean"})
            .reset_index()
        )
        dog_overall.columns = [
            "dog_name",
            "overall_avg_position",
            "overall_avg_field_size",
        ]
        dog_overall["overall_performance_score"] = (
            dog_overall["overall_avg_field_size"]
            - dog_overall["overall_avg_position"]
            + 1
        ) / dog_overall["overall_avg_field_size"]

        # Calculate venue-specific performance
        dog_venue_performance = (
            multi_venue_data.groupby(["dog_name", "venue"])
            .agg({"finish_position": ["mean", "count"], "field_size": "mean"})
            .reset_index()
        )
        dog_venue_performance.columns = [
            "dog_name",
            "venue",
            "venue_avg_position",
            "venue_races",
            "venue_avg_field_size",
        ]
        dog_venue_performance["venue_performance_score"] = (
            dog_venue_performance["venue_avg_field_size"]
            - dog_venue_performance["venue_avg_position"]
            + 1
        ) / dog_venue_performance["venue_avg_field_size"]

        # Merge and calculate venue effects
        comparison = dog_venue_performance.merge(dog_overall, on="dog_name")
        comparison["venue_effect"] = (
            comparison["venue_performance_score"]
            - comparison["overall_performance_score"]
        )
        ...
        if len(reliable_comparisons) > 0:
            venue_effects = (
                reliable_comparisons.groupby("venue")
                .agg({"venue_effect": ["mean", "std", "count"], "dog_name": "nunique"})
                .round(4)
            )
            venue_effects.columns = [
                "avg_effect",
                "effect_std",
                "comparisons",
                "dogs_analyzed",
            ]
            venue_effects["confidence"] = np.minimum(
                venue_effects["dogs_analyzed"] / 10, 1.0
            )
    ...
    def calculate_secondary_venue_characteristics(self, df):
        ...
        venue_chars["dogs_per_race"] = (
            venue_chars["unique_dogs"] / venue_chars["total_races"]
        )
        venue_chars["competitiveness"] = (
            venue_chars["dogs_per_race"] / venue_chars["avg_field_size"]
        )
        ...
        if max_field > min_field:
            venue_chars["field_difficulty"] = (
                venue_chars["avg_field_size"] - min_field
            ) / (max_field - min_field)
        else:
            venue_chars["field_difficulty"] = 0.5

        # Calculate confidence based on sample size
        venue_chars["data_confidence"] = np.minimum(
            venue_chars["total_races"] / 20, 1.0
        )
    ...
    def create_venue_performance_system(self, df):
        ...
        for venue in df["venue"].unique():
            ...
            if venue in secondary_chars.index:
                ...
                venue_data["performance_effect"] = (
                    char_data["field_difficulty"] - 0.5
                ) * 0.1  # Scale to ±0.05 max
    ...
    def get_dog_venue_adjustment(
        self, dog_name: str, venue: str, venue_performance: Dict
    ) -> Tuple[float, float]:
    ...
    def display_venue_analysis(self, venue_performance: Dict, coverage_stats: Dict):
        ...
        print(
            f"{'Venue':<8} {'Effect':<8} {'Method':<10} {'Confidence':<10} {'Sample':<8} {'Field Size':<10}"
        )
        print("-" * 70)
        ...
        for venue, data in sorted_venues:
            ...
            field_size = (
                f"{data['characteristics'].get('avg_field_size', 0):.1f}"
                if data["characteristics"]
                else "N/A"
            )

            print(
                f"{venue:<8} {effect:<8} {method:<10} {confidence:<10} {sample:<8} {field_size:<10}"
            )

        print(f"\n📊 ANALYSIS SUMMARY:")
        print(f"   Data span: {coverage_stats['date_span_days']} days")
        print(
            f"   Cross-venue racing: {coverage_stats['cross_venue_percentage']:.1f}% of dogs"
        )

        primary_venues = sum(
            1 for v in venue_performance.values() if v["analysis_method"] == "primary"
        )
        secondary_venues = sum(
            1 for v in venue_performance.values() if v["analysis_method"] == "secondary"
        )
        tertiary_venues = sum(
            1 for v in venue_performance.values() if v["analysis_method"] == "tertiary"
        )
...
def main():
    ...
    effect, confidence = analyzer.get_dog_venue_adjustment(
        "Sample Dog", sample_venue, venue_performance
    )
    print(
        f"   Dog 'Sample Dog' at {sample_venue}: {effect:+.3f} effect (confidence: {confidence:.2f})"
    )
