...
class WeatherDataUpdater:
    ...
    def collect_weather_for_race(self, race: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if weather_data:
                ...
                print(
                    f"   ⚠️ No weather data available for {venue_code} on {race_date_str}"
                )
                return None

        except Exception as e:
            print(
                f"   ❌ Error collecting weather for race {race.get('race_id', 'unknown')}: {e}"
            )
            return None
    ...
    def update_all_races(self, batch_size: int = 10) -> None:
        ...
        if self.processed_count > 0:
            ...
            print(
                f"📊 Database now contains weather context for {self.processed_count} additional races"
            )
    ...
    def get_weather_coverage_stats(self) -> Dict:
        ...
        try:
            ...
            """
            with_weather = pd.read_sql_query(with_weather_query, conn)[
                "with_weather"
            ].iloc[0]
            ...
            coverage_percentage = (
                (with_weather / total_races * 100) if total_races > 0 else 0
            )
