...
class ExpertFormInvestigator:
    ...
    def analyze_page_structure(self, soup):
        ...
        expert_sections = soup.find_all(
            ["div", "section"], class_=re.compile(r"expert|form", re.I)
        )
        ...
        for i, section in enumerate(expert_sections[:10]):  # Limit to first 10
            ...
            data_attrs = {
                k: v for k, v in section.attrs.items() if k.startswith("data-")
            }
            if data_attrs:
            ...
            if headers:
                ...
                if any(
                    ...
                    rows = table.find_all("tr")[
                        1:3
                    ]  # Skip header, get first 2 data rows
    ...
    def attempt_csv_download(self, expert_form_url, soup):
        ...
        for form in soup.find_all("form"):
            if form.find("input", {"name": "export_csv"}) or form.find(
    ...
    def analyze_csv_structure(self, csv_data):
        ...
        try:
            ...
            if len(lines) == 1 and lines[0].startswith("https://"):
                ...
                try:
                    ...
                    if redirect_response.status_code == 200:
                        ...
                        print(
                            f"❌ Failed to follow redirect: {redirect_response.status_code}"
                        )
                        return
                except Exception as e:
            ...
            headers = rows[0]
            print(
                f"✅ Found CSV with {len(headers)} columns and {len(rows)-1} data rows"
            )
            ...
            for i, header in enumerate(headers):
                ...
                if non_empty_values:
                    # Check if numeric
                    numeric_count = sum(
                        1 for v in non_empty_values if self.is_numeric(v)
                    )
    ...
    def find_additional_data_fields(self, soup):
        ...
        for script in scripts:
            if script.string:
                ...
                for data_type, pattern in patterns.items():
                    ...
                    if matches:
                        print(
                            f"📊 Found {data_type} data: {matches[:5]}..."
                        )  # Show first 5
        ...
        if elements_with_data:
            ...
            for elem in elements_with_data[:10]:  # Show first 10
                data_attrs = {
                    k: v for k, v in elem.attrs.items() if k.startswith("data-")
                }
