...
class DataQualityFixer:
    ...
    def check_database_integrity(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM dog_race_data WHERE dog_name IS NULL OR dog_name = '' OR dog_name = 'nan'"
            )
            invalid_dogs = cursor.fetchone()[0]

            if invalid_dogs > 0:
                self.issues_found.append(
                    f"{invalid_dogs} records with invalid dog names"
                )
    ...
    def check_enhanced_analyzer(self):
        ...
        try:
            ...
            print(
                f"   📊 Performance scores: min={perf_stats['min']:.3f}, max={perf_stats['max']:.3f}, mean={perf_stats['mean']:.3f}"
            )
            ...
            if zero_venues > 0:
                self.issues_found.append(
                    f"{zero_venues} venues with zero performance scores"
                )
    ...
    def check_performance_calculations(self):
        ...
        try:
            ...
            if len(df) == 0:
                self.issues_found.append(
                    "No valid race data for performance calculation"
                )
                return

            # Test performance calculation
            df["calc_performance"] = (
                df["field_size"] - df["finish_position"] + 1
            ) / df["field_size"]

            # Validate ranges
            invalid_scores = (
                (df["calc_performance"] < 0) | (df["calc_performance"] > 1)
            ).sum()

            if invalid_scores > 0:
                self.issues_found.append(
                    f"{invalid_scores} records produce invalid performance scores"
                )
            ...
            print(
                f"   📊 Sample performance range: {df['calc_performance'].min():.3f} to {df['calc_performance'].max():.3f}"
            )
    ...
    def check_data_completeness(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM race_metadata WHERE venue IS NULL OR venue = ''"
            )
            missing_venues = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM race_metadata WHERE race_date IS NULL OR race_date = ''"
            )
            missing_dates = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM race_metadata WHERE field_size IS NULL OR field_size = 0"
            )
            ...
            if missing_field_sizes > 0:
                self.issues_found.append(
                    f"{missing_field_sizes} races missing field size"
                )
                print(f"   ⚠️ {missing_field_sizes} races missing field size")

            # Dog data completeness
            cursor.execute(
                "SELECT COUNT(*) FROM dog_race_data WHERE finish_position IS NULL"
            )
            missing_positions = cursor.fetchone()[0]

            if missing_positions > 0:
                self.issues_found.append(
                    f"{missing_positions} dogs missing finish position"
                )
    ...
    def check_for_duplicates(self):
        ...
        try:
            ...
            if duplicates:
                self.issues_found.append(
                    f"{len(duplicates)} duplicate dog entries found"
                )
    ...
    def fix_duplicates(self):
        ...
        try:
            ...
            if clean_count < original_count:
                ...
                cursor.execute(
                    "INSERT INTO dog_race_data SELECT * FROM dog_race_data_clean"
                )
    ...
    def fix_invalid_data(self):
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM dog_race_data WHERE dog_name IS NULL OR dog_name = '' OR dog_name = 'nan'"
            )
            deleted = cursor.rowcount

            if deleted > 0:
                self.fixes_applied.append(
                    f"Removed {deleted} records with invalid dog names"
                )
            ...
            if converted > 0:
                self.fixes_applied.append(
                    f"Converted {converted} finish positions to integers"
                )
...
def main():
    ...
    print(f"\n📋 SUMMARY:")
    print(
        f"   Issues diagnosed: {len(fixer.issues_found) if hasattr(fixer, 'issues_found') else 0}"
    )
