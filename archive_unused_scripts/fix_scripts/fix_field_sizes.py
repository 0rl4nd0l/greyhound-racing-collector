...
def analyze_field_size_issues():
    ...
    print(
        df.head(10)[
            ["race_id", "venue", "field_size", "actual_dog_count", "max_finish_pos"]
...
def fix_field_sizes(dry_run=True):
    ...
    for race_id, current_field_size, actual_dog_count, max_finish_pos in races:
        ...
        if (
            current_field_size is None
            or current_field_size > 12
            or abs(current_field_size - correct_field_size) > 1
        ):
            ...
            print(
                f"   {race_id}: {current_field_size} → {correct_field_size} (dogs: {actual_dog_count}, max_pos: {max_finish_pos})"
            )
