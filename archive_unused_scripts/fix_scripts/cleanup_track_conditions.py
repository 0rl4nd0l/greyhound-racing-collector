...
class TrackConditionCleaner:
    ...
    def analyze_current_data(self) -> Dict:
        ...
        for race_id, condition, url, race_name in races:
            # Count occurrences
            analysis["condition_counts"][condition] = (
                analysis["condition_counts"].get(condition, 0) + 1
            )
            ...
            # Rule 3: Known sponsorship patterns
                ...
                sponsor in url.lower()
                for sponsor in ["fast-withdrawals", "fast-form", "sportsbet-fast"]
            ):
            ...
            if is_false_positive:
                ...
                if is_suspicious:
                    ...
                    analysis["likely_valid"].append(
                        {"race_id": race_id, "condition": condition, "url": url}
                    )
    ...
    def clean_false_positives(self, dry_run: bool = True) -> Dict:
        ...
        for condition, count in sorted(
            analysis["condition_counts"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        if race_ids_to_clean:
            ...
            update_query = """
                ...
                WHERE race_id IN ({})
            """.format(
                ",".join(["?" for _ in race_ids_to_clean])
            )
    ...
    def restore_from_backup(self, backup_timestamp: str):
        ...
        print(
            f"✅ Restored {rows_affected} track conditions from backup {backup_timestamp}"
        )
