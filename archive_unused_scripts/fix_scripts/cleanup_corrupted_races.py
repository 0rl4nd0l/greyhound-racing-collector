...
def identify_corrupted_races():
    ...
    if len(df) > 0:
        ...
        print(
            df[
                ["race_id", "venue", "unique_dogs", "total_entries", "max_position"]
...
def analyze_corrupted_race(race_id):
    ...
    for pos in sorted(
        positions.keys(), key=lambda x: int(x) if x and str(x).isdigit() else 999
    ):
...
def clean_corrupted_race(race_id, dry_run=True):
    ...
    if race_id == "UNK_0_UNKNOWN":
        ...
        if not dry_run:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM dog_race_data WHERE race_id = ?", (race_id,)
            )
