...
class ComprehensiveMLPredictor:
                    ...
                    if dog_features is not None:
                        ...
                        if TRADITIONAL_ANALYSIS_AVAILABLE:
                            try:
                                ...
                                print(
                                    f"   ⚠️ Traditional score calculation failed for {dog_name}: {e}"
                                )
            ...
            prediction_summary = {
                ...
                "race_summary": {
                    ...
                    "average_confidence": safe_mean(
                        [p["confidence"] for p in predictions], 0.0
                    ),
                },
                ...
                if combined_data:
                    ...
                    original_dog_name = next(
                        (
                            ...
                            if dog["name"].upper() == dog_name.upper()
                        ),
                        dog_name,
                    )
    ...
    def _extract_embedded_historical_data(self, race_file_path):
        ...
        try:
            ...
            print(
                f"   📊 Extracted {total_embedded_races} embedded races for {len(embedded_data)} dogs"
            )
        ...
        try:
            ...
            if TRADITIONAL_ANALYSIS_AVAILABLE:
                try:
                    ...
                    print(
                        f"   🎯 Traditional analysis for {dog_info['name']}: {traditional_features.get('traditional_overall_score', 0):.3f}"
                    )

                except Exception as e:
                    print(
                        f"   ⚠️ Traditional analysis failed for {dog_info['name']}: {e}"
                    )
                    traditional_features = {}
    ...
    def _predict_dog_performance(self, dog_features):
        ...
        try:
            ...
            if hasattr(self.trained_model, "predict_proba"):
                ...
                raw_probability = (
                    prediction_proba[0][1] if prediction_proba.shape[1] > 1 else 0.5
                )
            else:
...
def main():
    ...
    if result["success"]:
        ...
        for i, prediction in enumerate(result["predictions"][:5], 1):
            print(
                f"{i}. {prediction['dog_name']} - Score: {prediction['prediction_score']:.3f} (Confidence: {prediction['confidence']:.2f})"
            )
    else:
