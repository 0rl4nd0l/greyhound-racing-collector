...
class ImprovedMLSystem:
    ...
    def load_and_prepare_data(self):
        ...
        try:
            ...
            enhanced_df = comp_system.create_comprehensive_features(
            ...
            # Prepare features
            prepared_df, feature_columns = comp_system.prepare_comprehensive_features(
                enhanced_df
            )
            ...
            print(
                f"📊 Loaded dataset: {len(prepared_df)} samples, {len(feature_columns)} features"
            )
    ...
    def run_improved_analysis(self):
        ...
        for sampling_name, (X_sample, y_sample) in sampling_results.items():
            ...
            for model_name, config in all_models.items():
                ...
                try:
                    ...
                    if (
                        "gradient_boosting" in model_name
                        and sampling_name == "original"
                    ):
                        # Calculate class weights for gradient boosting
                        classes = np.unique(y_sample)
                        class_weights = compute_class_weight(
                            "balanced", classes=classes, y=y_sample
                        )
                        # Apply sample weights during training
                        sample_weights = np.where(
                            y_sample == 1, class_weights[1], class_weights[0]
                        )
        ...
        results_file = (
            self.results_dir
            / f"improved_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
