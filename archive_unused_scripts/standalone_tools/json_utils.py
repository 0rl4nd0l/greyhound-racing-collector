...
class SafeJSONEncoder(json.JSONEncoder):
    ...
    def _sanitize_value(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            item_val = obj.item()
            if isinstance(item_val, (float, int)) and (math.isnan(item_val) or math.isinf(item_val)):
                return None
            return item_val
        else:
            return obj
    ...
    try:
        ...
        for x, y in zip(x_values, y_values):
            try:
                ...
                if (x_float is not None and y_float is not None and 
                    not math.isnan(x_float) and not math.isnan(y_float) and
                    not math.isinf(x_float) and not math.isinf(y_float)):
...
if __name__ == "__main__":
    ...
    print(f"safe_correlation([1,2],[NaN,4]): {safe_correlation([1, 2], [float('nan'), 4], 'default')}")
