...
class OddsMonitor:
    ...
    def update_odds_cycle(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Cycle complete: {len(races)} races, {odds_count} odds, {len(value_bets)} value bets ({cycle_duration:.1f}s)"
            )
            ...
            if value_bets:
                ...
                for bet in value_bets[:3]:  # Log top 3
                    logger.info(
                        f"  {bet['dog_clean_name']}: {bet['bet_recommendation']}"
                    )
    ...
    def print_statistics(self):
        ...
        if self.stats["start_time"]:
            ...
            if runtime_hours > 0:
                logger.info(
                    f"  Avg Races/Hour: {self.stats['total_races_processed']/runtime_hours:.1f}"
                )
                logger.info(
                    f"  Avg Odds/Hour: {self.stats['total_odds_collected']/runtime_hours:.1f}"
                )
