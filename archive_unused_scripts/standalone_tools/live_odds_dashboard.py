...
def run_streamlit_dashboard():
    ...
    if not race_summary.empty:
        ...
        race_summary["minutes_ago"] = (
            datetime.now() - race_summary["last_updated"]
        ).dt.total_seconds() / 60
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "race_time": "Time",
                "dog_count": "Dogs",
                "favorite_odds": st.column_config.NumberColumn(
                    "Favorite", format="$%.2f"
                ),
                "longest_odds": st.column_config.NumberColumn(
                    "Longest", format="$%.2f"
                ),
                "minutes_ago": st.column_config.NumberColumn(
                    "Updated (min ago)", format="%.1f"
                ),
            },
        ...
        st.info(
            "No current race data available. Start the odds collector to begin gathering data."
        )
    ...
    with col1:
        ...
        if not value_bets.empty:
            for _, bet in value_bets.head(10).iterrows():
                with st.expander(
                    f"🎯 {bet['dog_clean_name']} - {bet['value_percentage']:.1f}% Value"
                ):
                    st.write(f"**Recommendation:** {bet['bet_recommendation']}")
                    st.write(f"**Market Odds:** ${bet['market_odds']:.2f}")
                    st.write(
                        f"**Predicted Probability:** {bet['predicted_probability']:.1%}"
                    )
    ...
    if not current_odds.empty:
        ...
        for race_id in current_odds["race_id"].unique()[:5]:  # Show first 5 races
            ...
            with st.expander(f"🏁 {venue} Race {race_num} ({race_time})"):
                ...
                # Odds table
                display_data = race_data[
                    ["box_number", "dog_name", "odds_decimal", "odds_fractional"]
                ].copy()
