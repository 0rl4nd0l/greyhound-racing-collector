...
def investigate_ui_statistics():
    ...
    try:
        ...
        # Races with winners
        cursor.execute(
            "SELECT COUNT(*) FROM race_metadata WHERE winner_name IS NOT NULL AND winner_name != '' AND winner_name != 'nan'"
        )
        races_with_winners = cursor.fetchone()[0]

        # Races without winners
        cursor.execute(
            "SELECT COUNT(*) FROM race_metadata WHERE winner_name IS NULL OR winner_name = '' OR winner_name = 'nan'"
        )
        ...
        cursor.execute(
            "SELECT processing_status, COUNT(*) FROM race_metadata GROUP BY processing_status"
        )
        ...
        if os.path.exists(path):
            try:
                if name == "processed":
                    ...
                    files = [
                        f for f in os.listdir(path) if f.endswith((".csv", ".json"))
                    ]
                    count = len(files)
                ...
                if (
                    name in ["enhanced_expert_data/csv", "enhanced_expert_data/json"]
                    and count > 0
                ):
                    if name == "processed":
                        sample_files = []
                        for root, dirs, files in os.walk(path):
                            sample_files.extend(
                                [f for f in files if f.endswith(".csv")][:3]
                            )
                    else:
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM race_metadata WHERE filename IS NOT NULL AND filename != ''"
        )
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM race_metadata WHERE processing_status = 'enhanced'"
        )
    ...
    print("\n📋 CACHED DATA & REPORTS:")
    report_files = [
        f for f in os.listdir(".") if "report" in f.lower() and f.endswith(".json")
    ]
        ...
        try:
            with open(report_file, "r") as f:
                ...
                if "cleanup_summary" in data:
                    ...
                    print(
                        f"    Files integrated: {summary.get('files_integrated', 'N/A')}"
                    )
                    print(
                        f"    DB records added: {summary.get('database_records_added', 'N/A')}"
                    )
                elif "summary" in data:
                    summary = data["summary"]
                    print(
                        f"    Total CSV files: {summary.get('total_csv_files', 'N/A')}"
                    )
                    print(
                        f"    Enhanced files: {summary.get('enhanced_csv_files', 'N/A')}"
                    )
        except Exception as e:
    ...
    try:
        ...
        if os.path.exists("./enhanced_expert_data/csv"):
            enhanced_csv_count = len(
                [
                    f
                    for f in os.listdir("./enhanced_expert_data/csv")
                    if f.endswith(".csv")
                ]
            )
