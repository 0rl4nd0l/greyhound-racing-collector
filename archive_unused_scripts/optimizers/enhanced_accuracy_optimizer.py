#!/usr/bin/env python3
"""
Enhanced Accuracy Optimizer V4 (Archived)
========================================

This file has been archived and is no longer part of the active code path.
Original purpose: Provide an ensemble-based optimizer and monkey-patch
MLSystemV4.predict_race. The system now runs native MLSystemV4 without this
layer.

If you need to inspect the previous implementation for reference, it remains
here unchanged.
"""
    ...
    def predict_with_ensemble(
        ...
        for i, (_, row) in enumerate(base_features.iterrows()):
            try:
                ...
                # Build optimizer confidence reason (agreement/extremeness blend)
                try:
                    dog_predictions = []
                    for _mid, _probs in model_predictions.items():
                        if i < len(_probs):
                            dog_predictions.append(float(_probs[i]))
                    pred_std = float(np.std(dog_predictions)) if dog_predictions else float("nan")
                    agreement = 1.0 / (1.0 + pred_std) if dog_predictions else 0.5
                    extremeness = 2 * abs(float(calibrated_win_prob) - 0.5)
                    confidence_reason = (
                        f"opt_conf: 0.7*agreement + 0.3*extremeness; agreement={agreement:.3f}, "
                        f"extremeness={extremeness:.3f}, models={len(model_predictions)}"
                    )
                except Exception:
                    confidence_reason = "opt_conf: model agreement and extremeness blend"
            ...
            try:
                ...
                if ensemble_count > 1:
                    ...
                    try:
                        ...
                        confidence_reason = (
                            f"opt_conf: 0.7*agreement + 0.3*extremeness; agreement={agreement:.3f}, "
                            f"extremeness={extremeness:.3f}, models={ensemble_count}"
                        )
                    except Exception:
                        confidence_reason = "opt_conf: model agreement and extremeness blend"
                else:
                    ...
                    try:
                        confidence_reason = (
                            f"opt_conf(single): 0.7*agreement + 0.3*extremeness; agreement={model_agreement:.3f}, "
                            f"extremeness={extremeness:.3f}, models={ensemble_count}"
                        )
                    except Exception:
                        confidence_reason = "opt_conf(single): extremeness-driven confidence"
                predictions.append(
                    {
                        "dog_clean_name": (
                            names[i]
