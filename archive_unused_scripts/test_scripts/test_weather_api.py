...
class MockWeatherService:
    ...
    def get_mock_weather_data(
        self, venue_code: str, weather_type: str = "current"
...
def test_weather_service():
    ...
    for code, venue in venues.items():
        print(
            f"     - {venue.venue_name} ({venue.venue_code}) in {venue.city}, {venue.state}"
        )
    ...
    if summary:
        ...
        for venue_code, stats in summary.items():
            print(
                f"     - {venue_code}: {stats['record_count']} records, avg temp {stats['avg_temperature']}°C"
            )
    else:
    ...
    for i, scenario in enumerate(extreme_scenarios):
        ...
        print(
            f"     - Temp: {scenario.temperature}°C, Humidity: {scenario.humidity}%, Wind: {scenario.wind_speed} km/h"
        )
        print(
            f"     - Condition: {scenario.condition.value}, Rain: {scenario.precipitation}mm"
        )
