...
class WeatherServiceTester:
    ...
    def test_venue_coverage(self):
        ...
        for venue in venues:
            ...
            if not missing_fields:
                ...
                self.print_test(
                    f"Venue {venue_code}", "FAIL", f"Missing fields: {missing_fields}"
                )
                self.test_results["failed"] += 1
        ...
        for venue_code in venues:
            try:
                ...
                if weather_data:
                    ...
                    data_valid = (
                        -10 <= weather_data.temperature <= 50
                        and 0 <= weather_data.humidity <= 100  # Reasonable temp range
                        and 0 <= weather_data.wind_speed <= 150  # Valid humidity
                        and weather_data.pressure  # Reasonable wind
                        > 950  # Reasonable pressure
                    )

                    if data_valid:
                        self.print_test(
                            f"Current Weather {venue_code}",
                            "PASS",
                    ...
                    self.print_test(
                        f"Current Weather {venue_code}", "FAIL", "No data returned"
                    )
                    self.test_results["failed"] += 1

            except Exception as e:
                self.print_test(
                    f"Current Weather {venue_code}", "FAIL", f"Exception: {str(e)}"
                )
                self.test_results["failed"] += 1
    ...
    def test_historical_weather(self):
        ...
        for date_str, description in historical_dates:
            try:
                start_time = time.time()
                weather_data = self.weather_service.get_forecast_weather(
                    test_venue, date_str
                )
                response_time = time.time() - start_time
                ...
                self.print_test(
                    f"Historical {description}", "FAIL", f"Exception: {str(e)}"
                )
                self.test_results["failed"] += 1
                self.test_results["errors"].append(
                    {
    ...
    def test_weather_adjustment_calculations(self):
        ...
        for scenario in test_scenarios:
            try:
                ...
                from weather_service_open_meteo import WeatherCondition, WeatherData
                ...
                adjustment = self.weather_service.calculate_weather_adjustment_factor(
                    weather_data, "SAN"
                )
                ...
                self.print_test(
                    f"Adjustment {scenario['name']}", "FAIL", f"Exception: {str(e)}"
                )
                self.test_results["failed"] += 1
                self.test_results["errors"].append(
                    {
    ...
    def test_database_operations(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'weather%'"
            )
            tables = cursor.fetchall()

            if len(tables) >= 1:
                self.print_test(
                    "Database Tables", "PASS", f"Found {len(tables)} weather tables"
                )
                self.test_results["passed"] += 1
            else:
            ...
            if record_count > 0:
                self.print_test(
                    "Database Records", "PASS", f"Found {record_count} weather records"
                )
                self.test_results["passed"] += 1
                ...
                if valid_records == len(recent_records):
                    self.print_test(
                        "Data Quality",
                        "PASS",
                        f"All {len(recent_records)} recent records valid",
                    )
                    self.test_results["passed"] += 1
                else:
                    self.print_test(
    ...
    def test_performance_metrics(self):
        ...
        if self.test_results["performance"]:
            response_times = [
                perf["response_time"] for perf in self.test_results["performance"]
            ]
            ...
            if avg_response < 5.0:  # Average under 5 seconds
                ...
                self.print_test(
                    "Average Response Time", "FAIL", f"{avg_response:.2f}s (>5s)"
                )
                self.test_results["failed"] += 1
            ...
            if max_response < 10.0:  # Max under 10 seconds
                ...
                self.print_test(
                    "Maximum Response Time", "FAIL", f"{max_response:.2f}s (>10s)"
                )
                self.test_results["failed"] += 1
            ...
            self.print_test(
                "Performance Metrics", "FAIL", "No performance data collected"
            )
            self.test_results["failed"] += 1
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            if invalid_weather is None:
                self.print_test(
                    "Invalid Venue Handling", "PASS", "Correctly returned None"
                )
                self.test_results["passed"] += 1
            else:
                self.print_test(
                    "Invalid Venue Handling",
                    "FAIL",
                    "Should return None for invalid venue",
                )
                self.test_results["failed"] += 1
        except Exception as e:
        ...
        # Test invalid dates
        try:
            invalid_forecast = self.weather_service.get_forecast_weather(
                "SANDOWN", "2025-13-45"
            )
            if invalid_forecast is None:
                self.print_test(
                    "Invalid Date Handling", "PASS", "Correctly handled invalid date"
                )
                self.test_results["passed"] += 1
            else:
                self.print_test(
            ...
            self.print_test(
                "Invalid Date Handling",
                "PASS",
                "Exception raised for invalid date (acceptable)",
            )
            self.test_results["passed"] += 1
    ...
    def generate_final_report(self):
        ...
        if self.test_results["performance"]:
            avg_time = sum(
                p["response_time"] for p in self.test_results["performance"]
            ) / len(self.test_results["performance"])
        ...
        for venue, results in self.test_results["venue_results"].items():
            ...
            print(
                f"   {status} {venue}: {results.get('temperature', 'N/A')}°C, {results.get('condition', 'N/A')}"
            )
        ...
        if success_rate >= 90:
            print(
                f"   🌟 EXCELLENT: Weather service is working at {success_rate:.1f}% success rate"
            )
            print(f"   ✅ Ready for production use in greyhound racing predictions")
        elif success_rate >= 75:
            print(
                f"   👍 GOOD: Weather service is working at {success_rate:.1f}% success rate"
            )
            print(f"   ⚠️  Minor issues to address before full deployment")
        elif success_rate >= 50:
            print(
                f"   ⚠️  PARTIAL: Weather service working at {success_rate:.1f}% success rate"
            )
            print(f"   🔧 Significant issues need to be resolved")
        else:
            print(
                f"   ❌ POOR: Weather service only working at {success_rate:.1f}% success rate"
            )
