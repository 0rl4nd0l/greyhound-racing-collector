...
def test_enhanced_integration():
        ...
        if file_path.exists():
            ...
            try:
                ...
                if result.get('races_processed', 0) > 0:
                    print(f"   ✅ Enhanced processing: {result.get('races_processed', 0)} races processed")
                    ...
                    if result.get('data_quality'):
                        ...
                        print(f"   📊 Data Quality - Warnings: {len(quality.get('warnings', []))}, Errors: {len(quality.get('errors', []))}")
                    ...
                    print(f"   ✅ Enhanced processing successful: {result.get('summary', 'Success')}")
                else:
                    print(f"   ❌ Enhanced processing failed: {result.get('error', 'Unknown error')}")
    ...
    if enhanced_races:
        ...
        for race in enhanced_races:
            print(f"   📍 {race[0]} ({race[1]} R{race[2]}) - Status: {race[4]} - Winner: {race[5] or 'N/A'}")
    else:
    ...
    if dead_heat_entries:
        ...
        for entry in dead_heat_entries:
            print(f"   📍 {entry[0]} - {entry[1]} - Position: {entry[2]} - Note: {entry[3] or 'N/A'}")
    else:
