...
def demonstrate_weather_predictions():
        ...
        try:
            ...
            if result['success']:
                ...
                if weather_info and 'note' not in weather_info:
                    ...
                    weather_adj = result['summary']['race_summary']['weather_adjustment_applied']
                    print(f"⚡ Weather adjustment: {weather_adj:.3f} ({((weather_adj - 1) * 100):+.1f}%)")
                else:
                    print("🌤️ Weather: No data available")
                
                print(f"\n🏆 TOP 3 PREDICTIONS:")
                for j, pred in enumerate(predictions[:3], 1):
                    base_score = pred.get('base_prediction_score', pred['prediction_score'])
                ...
                results.append({
                    ...
                    'weather_available': weather_info and 'note' not in weather_info,
                    'weather_condition': weather_info.get('weather_condition', 'Unknown') if weather_info else 'Unknown',
                    'top_pick': predictions[0]['dog_name'] if predictions else 'None',
                    'weather_adjustment': result['summary']['race_summary']['weather_adjustment_applied']
                })
    ...
    if results:
        ...
        print(f"📈 Weather data coverage: {(weather_available_count/len(results)*100):.1f}%")
        
        print(f"\n🎯 RACE RESULTS:")
        for r in results:
            weather_status = "✅" if r['weather_available'] else "❌"
            adj_str = f"{r['weather_adjustment']:.3f}" if r['weather_adjustment'] != 1.0 else "No adj"
            print(f"  {r['venue']}: Top pick: {r['top_pick']}")
            print(f"    Weather: {weather_status} {r['weather_condition']} (Adj: {adj_str})")
        
        # Weather impact analysis
        weather_adjustments = [r['weather_adjustment'] for r in results if r['weather_available']]
        if weather_adjustments:
            ...
            print(f"  Impact range: {min(weather_adjustments):.3f} to {max(weather_adjustments):.3f}")
