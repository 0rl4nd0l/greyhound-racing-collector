    ...
    try:
        ...
        if result["success"]:
            ...
            for i, pred in enumerate(predictions[:3], 1):
                ...
                print(
                    f"      {i}. {pred['dog_name']:15}: {pred['prediction_score']:.3f} ({confidence})"
                )
...
def main():
    ...
    if result:
        ...
        print(
            f"   Performance: {result['accuracy']:.3f} accuracy, {result['auc']:.3f} AUC"
        )
        ...
        print(
            f"   You can now run predictions and they will use the freshly trained model!"
        )
