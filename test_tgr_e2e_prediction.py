        ...
        try:
            ...
            if result and result.get("success"):
                ...
                if predictions:
                    ...
                    # Show top prediction
                    print(
                        f"   🏆 Top prediction: {sample_pred.get('dog_name', 'Unknown')} "
            ...
            if race_data:
                ...
                if result and result.get("success"):
                    ...
                    if predictions:
                        ...
                        print(
                            f"   🥇 Winner: {top_pred.get('dog_name', 'Unknown')} "
                            f"(Win: {top_pred.get('win_probability', 0):.3f})"
                        )

                        # Check total probability (should sum to ~1.0)
                        total_prob = sum(
                            p.get("win_probability", 0) for p in predictions
                        )
                        print(
                            f"   🎯 Total win probability: {total_prob:.3f} (should be ~1.0)"
                        )
