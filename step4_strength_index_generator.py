...
class DogStrengthIndexGenerator:
    ...
    def __init__(
        self, features_file: str = None, model_type: str = "gradient_boosting"
    ):
        """
        ...
        """
        self.features_file = (
            features_file or "step2_comprehensive_features_20250804_133843.csv"
        )
    ...
    def load_features(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loaded features for {len(features_df)} dogs with {len(features_df.columns)} features"
            )
            return features_df
    ...
    def create_synthetic_target(self, features_df: pd.DataFrame) -> np.ndarray:
        ...
        target = (
            ...
            * (
                1
                - (features_df["best_race_time"] - features_df["best_race_time"].min())
                / (
                    features_df["best_race_time"].max()
                    - features_df["best_race_time"].min()
                )
            )
            +
            ...
            0.05
            * np.maximum(
                0, features_df["recent_position_trend"]
            ...
            if feature in features_df.columns:
                ...
                if feature_values.max() != feature_values.min():
                    if weight > 0:  # Higher values are better
                        ...
                        normalized_values = (feature_values.max() - feature_values) / (
                            feature_values.max() - feature_values.min()
                        )
    ...
    def generate_strength_scores(self, normalize: bool = True) -> pd.DataFrame:
        ...
        if self.model_type == "gradient_boosting":
            ...
            logger.info(
                "Calculating strength scores using weighted linear combination..."
            )
        ...
        if normalize:
            ...
            if max_score != min_score:
                normalized_scores = (
                    100 * (raw_scores - min_score) / (max_score - min_score)
                )
            else:
        ...
        results_df = results_df.sort_values(
            "normalized_strength_score", ascending=False
        )
    ...
    def predict_strength_for_new_dogs(
        self, new_features_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        if self.model is None:
            raise ValueError(
                "No model has been trained. Call generate_strength_scores() first."
            )
        ...
        if self.model_type == "gradient_boosting":
            ...
            available_features = [
                col for col in feature_columns if col in new_features_df.columns
            ]
        ...
        normalized_scores = (
            self.normalizer.fit_transform(raw_scores.reshape(-1, 1)).flatten() * 100
        )
