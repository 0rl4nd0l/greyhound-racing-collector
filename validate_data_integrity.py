...
class DataIntegrityValidator:
        ...
        for table_name, key_columns in tables_to_check.items():
            try:
                ...
                duplicate_checks[table_name] = {
                    ...
                    'max_duplicate_count': int(duplicates_df['duplicate_count'].max()) if len(duplicates_df) > 0 else 0,
                    'sample_duplicates': duplicates_df.head(10).to_dict('records') if len(duplicates_df) > 0 else []
                }
                
                if len(duplicates_df) > 0:
                    logger.warning(f"Found {len(duplicates_df)} duplicate key combinations in {table_name}")
                else:
        ...
        for table_name, fields in critical_fields.items():
            try:
                ...
                for field in fields:
                    ...
                    """
                    missing_count = pd.read_sql_query(missing_query, conn).iloc[0]['missing_count']
                    ...
                    if missing_count > 0:
                        logger.warning(f"{table_name}.{field}: {missing_count}/{total_count} ({field_stats[field]['missing_percentage']}%) missing")
    ...
    def check_referential_integrity(self, conn):
        ...
        try:
            ...
            race_metadata_ids = set(pd.read_sql_query("SELECT DISTINCT race_id FROM race_metadata WHERE race_id IS NOT NULL", conn)['race_id'])
            dog_race_ids = set(pd.read_sql_query("SELECT DISTINCT race_id FROM dog_race_data WHERE race_id IS NOT NULL", conn)['race_id'])
            expert_ids = set(pd.read_sql_query("SELECT DISTINCT race_id FROM enhanced_expert_data WHERE race_id IS NOT NULL", conn)['race_id'])
            ...
            if orphaned_dog_races:
                logger.warning(f"Found {len(orphaned_dog_races)} orphaned dog race records")
            if orphaned_expert_data:
                logger.warning(f"Found {len(orphaned_expert_data)} orphaned expert data records")
    ...
    def check_data_quality(self, conn):
        ...
        try:
            ...
            if len(invalid_box_numbers) > 0:
                logger.warning(f"Found {len(invalid_box_numbers)} records with invalid box numbers")
            if len(invalid_finish_positions) > 0:
                logger.warning(f"Found {len(invalid_finish_positions)} records with invalid finish positions")
    ...
    def generate_summary(self):
        ...
        if 'duplicates' in self.results['checks']:
            ...
            if total_duplicates > 0:
                ...
                summary['recommendations'].append(f"Resolve {total_duplicates} duplicate records")
        ...
        if 'referential_integrity' in self.results['checks']:
            ...
            orphaned_total = ref_check.get('orphaned_dog_races', 0) + ref_check.get('orphaned_expert_data', 0)
            if orphaned_total > 0:
                summary['overall_status'] = 'WARNING'
                summary['recommendations'].append(f"Fix {orphaned_total} orphaned records")
        ...
        if 'data_quality' in self.results['checks']:
            ...
            if quality_issues > 0:
                summary['overall_status'] = 'WARNING'
                summary['recommendations'].append(f"Fix {quality_issues} data quality issues")
    ...
    def run_all_checks(self):
        ...
        try:
            ...
            logger.info(f"Data integrity validation completed. Status: {self.results['summary']['overall_status']}")
            ...
            self.results['errors'].append(error_msg)
            self.results['summary'] = {'overall_status': 'FAIL', 'critical_error': str(e)}
