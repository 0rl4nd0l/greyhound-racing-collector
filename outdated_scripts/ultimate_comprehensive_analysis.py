...
class UltimateGreyhoundAnalysis:
    ...
    def load_all_data(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                self.sectionals_data = pd.read_sql_query('SELECT * FROM race_sectionals', conn)
                self.positions_data = pd.read_sql_query('SELECT * FROM in_running_positions', conn)
            ...
            print(f"   Ultimate dataset: {len(self.ultimate_data) if self.ultimate_data is not None else 0}")
    ...
    def create_ultimate_dataset(self):
        ...
        try:
            ...
            if not self.odds_data.empty:
                odds_agg = self.odds_data.groupby(['race_id', 'dog_clean_name'])['odds_decimal'].agg(['mean', 'std', 'count']).reset_index()
                odds_agg.columns = ['race_id', 'dog_clean_name', 'avg_odds', 'odds_volatility', 'odds_updates']
                self.ultimate_data = pd.merge(self.ultimate_data, odds_agg, on=['race_id', 'dog_clean_name'], how='left')
    ...
    def add_track_bias_features(self):
        ...
        try:
            ...
            venue_box_stats = self.ultimate_data.groupby(['venue', 'box_number']).agg({
                'finish_position': lambda x: (x == '1st').mean(),
                'dog_name': 'count'
            }).reset_index()
            venue_box_stats.columns = ['venue', 'box_number', 'venue_box_win_rate', 'venue_box_runs']
            ...
            if 'track_condition' in self.ultimate_data.columns:
                track_stats = self.ultimate_data.groupby(['venue', 'track_condition']).agg({
                    'finish_position': lambda x: (x == '1st').mean(),
                    'dog_name': 'count'
                }).reset_index()
                track_stats.columns = ['venue', 'track_condition', 'track_condition_win_rate', 'track_condition_runs']
                track_stats = track_stats[track_stats['track_condition_runs'] >= 5]
                
                self.ultimate_data = pd.merge(
                    self.ultimate_data,
                    track_stats[['venue', 'track_condition', 'track_condition_win_rate']],
                    on=['venue', 'track_condition'],
                    how='left'
                )
            
            # Distance-specific venue performance
            distance_venue_stats = self.ultimate_data.groupby(['venue', 'distance']).agg({
                'finish_position': lambda x: (x == '1st').mean(),
                'dog_name': 'count'
            }).reset_index()
            distance_venue_stats.columns = ['venue', 'distance', 'venue_distance_win_rate', 'venue_distance_runs']
            distance_venue_stats = distance_venue_stats[distance_venue_stats['venue_distance_runs'] >= 5]
    ...
    def add_form_analysis_features(self):
        ...
        try:
            ...
            self.ultimate_data['race_date'] = pd.to_datetime(self.ultimate_data['race_date'])
            
            # Sort by dog and date for form analysis
            form_data = self.ultimate_data.sort_values(['dog_clean_name', 'race_date']).copy()
            
            # Calculate days since last run for each dog
            form_data['days_since_last_run'] = form_data.groupby('dog_clean_name')['race_date'].diff().dt.days
            ...
            for window in [3, 5, 10]:
                ...
                form_data[f'weighted_form_{window}'] = (
                    form_data.groupby('dog_clean_name')['finish_position']
                    .rolling(window=window, min_periods=1)
                    .apply(lambda x: np.average([(pos in ['1st', '2nd', '3rd']) for pos in x], 
                                              weights=weights[:len(x)]))
                    .reset_index(level=0, drop=True)
                )
            
            # Career statistics
            career_stats = form_data.groupby('dog_clean_name').agg({
                'finish_position': [
                    lambda x: (x == '1st').sum(),  # Total wins
                    lambda x: x.isin(['1st', '2nd', '3rd']).sum(),  # Total places
                    'count'  # Total runs
                ]
            }).reset_index()
            
            career_stats.columns = ['dog_clean_name', 'career_wins', 'career_places', 'career_runs']
            career_stats['career_win_rate'] = career_stats['career_wins'] / career_stats['career_runs']
            career_stats['career_place_rate'] = career_stats['career_places'] / career_stats['career_runs']
            
            # Merge back to main dataset
            self.ultimate_data = pd.merge(
                self.ultimate_data,
                form_data[['id', 'days_since_last_run'] + 
                         [col for col in form_data.columns if col.startswith(('recent_', 'weighted_'))]],
                on='id',
                how='left'
            )
    ...
    def add_market_microstructure_features(self):
        ...
        try:
            ...
            odds_movement = self.odds_data.sort_values(['race_id', 'dog_clean_name', 'snapshot_timestamp'])
            
            # Calculate odds changes
            odds_movement['odds_change'] = odds_movement.groupby(['race_id', 'dog_clean_name'])['odds_decimal'].pct_change()
            odds_movement['odds_trend'] = odds_movement.groupby(['race_id', 'dog_clean_name'])['odds_change'].cumsum()
            
            # Steam/drift detection
            odds_summary = odds_movement.groupby(['race_id', 'dog_clean_name']).agg({
                'odds_decimal': ['first', 'last', 'min', 'max', 'std'],
                'odds_change': ['sum', 'count'],
                'odds_trend': 'last'
            }).reset_index()
            
            odds_summary.columns = ['race_id', 'dog_clean_name', 'opening_odds', 'closing_odds', 
                                  'min_odds', 'max_odds', 'odds_volatility', 'total_movement', 
                                  'num_updates', 'final_trend']
            
            # Calculate movement metrics
            odds_summary['odds_movement_pct'] = (
                (odds_summary['closing_odds'] - odds_summary['opening_odds']) / odds_summary['opening_odds'] * 100
            )
            
            # Classify movement patterns
            odds_summary['steam_move'] = odds_summary['odds_movement_pct'] < -15  # Significant shortening
            odds_summary['drift_move'] = odds_summary['odds_movement_pct'] > 20   # Significant lengthening
            odds_summary['stable_odds'] = np.abs(odds_summary['odds_movement_pct']) < 5  # Minimal movement
            
            # Market rank (favorite = 1, second fav = 2, etc.)
            odds_summary['market_rank'] = odds_summary.groupby('race_id')['opening_odds'].rank()
    ...
    def add_environmental_features(self):
        ...
        try:
            ...
            self.ultimate_data['day_of_week'] = self.ultimate_data['race_date'].dt.dayofweek
            self.ultimate_data['quarter'] = self.ultimate_data['race_date'].dt.quarter
            self.ultimate_data['is_weekend'] = self.ultimate_data['day_of_week'].isin([5, 6]).astype(int)
            ...
            if 'weather' in self.ultimate_data.columns:
                weather_performance = self.ultimate_data.groupby('weather').agg({
                    'finish_position': lambda x: (x == '1st').mean(),
                    'dog_name': 'count'
                }).reset_index()
                weather_performance.columns = ['weather', 'weather_win_rate', 'weather_runs']
                weather_performance = weather_performance[weather_performance['weather_runs'] >= 10]
        ...
        try:
            ...
            for _, group in positions.groupby(['race_id', 'box_number']):
                features = {
                    ...
                    'early_position': group['section_1_position'].iloc[0] if 'section_1_position' in group.columns else None,
                    'mid_position': group['section_2_position'].iloc[0] if 'section_2_position' in group.columns else None,
                    'late_position': group['section_3_position'].iloc[0] if 'section_3_position' in group.columns else None,
                    'final_position': group['section_4_position'].iloc[0] if 'section_4_position' in group.columns else None
                }
                
                # Calculate position changes
                if features['early_position'] and features['mid_position']:
                    features['early_to_mid_change'] = features['mid_position'] - features['early_position']
                if features['mid_position'] and features['late_position']:
                    features['mid_to_late_change'] = features['late_position'] - features['mid_position']
                if features['early_position'] and features['final_position']:
                    features['overall_position_change'] = features['final_position'] - features['early_position']
                
                # Running pattern classification
                if all(x is not None for x in [features['early_position'], features['final_position']]):
                    if features['early_position'] <= 2 and features['final_position'] <= 2:
                        features['running_pattern'] = 'Front_Runner'
                    elif features['early_position'] > 4 and features['final_position'] <= 2:
                        features['running_pattern'] = 'Come_From_Behind'
                    elif features['early_position'] <= 2 and features['final_position'] > 4:
    ...
    def add_advanced_performance_metrics(self):
        ...
        try:
            ...
            # Distance analysis
            self.ultimate_data['distance_numeric'] = self.ultimate_data['distance'].str.extract(r'(\d+)').astype(float)
            
            # Speed calculations
            self.ultimate_data['avg_speed'] = (
                self.ultimate_data['distance_numeric'] / self.ultimate_data['individual_time_numeric']
            )
            ...
            for metric in ['individual_time_numeric', 'weight_numeric', 'avg_speed']:
                if metric in self.ultimate_data.columns:
                    self.ultimate_data[f'{metric}_rank'] = (
                        self.ultimate_data.groupby('race_id')[metric].rank()
                    )
                    self.ultimate_data[f'{metric}_relative'] = (
                        self.ultimate_data[metric] - 
            ...
            running_style_map = {
                'FM': 'Fast_Beginner', 'FR': 'Fast_Beginner',
                'GM': 'Good_Beginner', 'SM': 'Slow_Beginner'
            }
            self.ultimate_data['running_style_category'] = (
                self.ultimate_data['running_style'].map(running_style_map).fillna('Unknown')
            )
    ...
    def add_risk_management_features(self):
        ...
        try:
            ...
            if 'avg_odds' in self.ultimate_data.columns:
                self.ultimate_data['implied_probability'] = 1 / self.ultimate_data['avg_odds']
                
                # Value analysis
                self.ultimate_data['is_winner'] = (self.ultimate_data['finish_position'] == '1st').astype(int)
                
                # Historical performance by odds range
                odds_brackets = pd.cut(
                    self.ultimate_data['avg_odds'],
                    bins=[0, 2, 3, 5, 8, 15, float('inf')],
                    labels=['Heavy_Fav', 'Favorite', 'Second_Fav', 'Mid_Range', 'Longshot', 'Very_Long']
                )
                self.ultimate_data['odds_bracket'] = odds_brackets
                
                # Calculate expected value components
                bracket_performance = self.ultimate_data.groupby('odds_bracket', observed=True).agg({
                    'is_winner': 'mean',
                    'avg_odds': 'mean'
                }).reset_index()
                bracket_performance.columns = ['odds_bracket', 'actual_win_rate', 'avg_bracket_odds']
                bracket_performance['bracket_implied_prob'] = 1 / bracket_performance['avg_bracket_odds']
                bracket_performance['value_ratio'] = (
                    bracket_performance['actual_win_rate'] / bracket_performance['bracket_implied_prob']
                )
                ...
                self.ultimate_data['kelly_fraction'] = np.where(
                    ...
                    (self.ultimate_data['value_ratio'] - 1) / (self.ultimate_data['avg_odds'] - 1),
                    0
                )
                
            # Confidence metrics
            if 'odds_volatility' in self.ultimate_data.columns:
                self.ultimate_data['market_confidence'] = 1 / (1 + self.ultimate_data['odds_volatility'])
    ...
    def build_ensemble_models(self):
        ...
        try:
            ...
            numeric_features = [
                'box_number', 'weight_numeric', 'individual_time_numeric',
                'avg_speed', 'days_since_last_run', 'career_win_rate',
                'recent_wins_3', 'recent_places_5', 'weighted_form_10',
                'odds_movement_pct', 'market_rank', 'implied_probability',
                'individual_time_numeric_relative', 'weight_numeric_relative',
                'early_to_mid_change', 'overall_position_change',
                'value_ratio', 'kelly_fraction', 'days_since_epoch'
            ]
            
            # Select categorical features
            categorical_features = [
                'venue', 'weather', 'season', 'running_style_category',
                'trainer_name', 'running_pattern', 'class_level', 'odds_bracket'
            ]
            ...
            # Prepare data
            feature_data = model_data[existing_numeric + existing_categorical + ['is_winner']].copy()
            ...
            models = {
                ...
                'extra_trees': ExtraTreesClassifier(n_estimators=200, max_depth=10, random_state=42),
            ...
            for name, model in models.items():
                try:
                    ...
                    results[name] = {
                        ...
                        'feature_importance': self.get_feature_importance(model, final_features)
                    }
    ...
    def get_feature_importance(self, model, feature_names):
        ...
        try:
            ...
            feature_imp = [{'feature': name, 'importance': float(imp)} 
    ...
    def analyze_track_bias(self):
        ...
        try:
            ...
            bias_data = self.ultimate_data[self.ultimate_data['venue_box_win_rate'].notna()].copy()
            ...
            # Venue summary
            venue_bias_summary = bias_data.groupby('venue').agg({
                'venue_box_win_rate': ['mean', 'std', 'count'],
                'box_number': lambda x: x.mode().iloc[0] if not x.mode().empty else None
            }).round(3)
            
            venue_bias_summary.columns = ['avg_bias', 'bias_std', 'positions_analyzed', 'best_box']
    ...
    def analyze_form_patterns(self):
        ...
        try:
            form_data = self.ultimate_data[self.ultimate_data['career_win_rate'].notna()].copy()
            ...
            # Recent form impact
            recent_form_analysis = form_data.groupby('form_category', observed=True).agg({
                'recent_wins_3': 'mean',
            ...
            recent_form_analysis.columns = ['avg_recent_wins_3', 'avg_recent_places_5', 
                                          'avg_weighted_form_10', 'current_win_rate', 'count']
            recent_form_analysis = recent_form_analysis.reset_index()
            
            # Days since last run impact
            if 'days_since_last_run' in form_data.columns:
                freshness_analysis = form_data.groupby(
                    pd.cut(form_data['days_since_last_run'], 
                          bins=[0, 7, 14, 28, 56, float('inf')],
                          labels=['Week', '2_Weeks', 'Month', '2_Months', 'Long_Break'])
                , observed=True).agg({
            ...
            return {
                ...
                'freshness_analysis': freshness_analysis.to_dict('records') if isinstance(freshness_analysis, pd.DataFrame) else [],
    ...
    def analyze_market_dynamics(self):
        ...
        try:
            market_data = self.ultimate_data[self.ultimate_data['odds_movement_pct'].notna()].copy()
            
            if market_data.empty:
                return {'note': 'No market data available'}
            
            # Movement pattern analysis
            movement_patterns = market_data.groupby(['steam_move', 'drift_move', 'stable_odds']).agg({
                'is_winner': ['mean', 'count'],
                'odds_movement_pct': 'mean'
            }).round(3)
            ...
            if 'market_rank' in market_data.columns:
                rank_performance = market_data.groupby('market_rank').agg({
                    'is_winner': ['mean', 'count'],
                    'avg_odds': 'mean'
                }).round(3)
                ...
                rank_performance = rank_performance[rank_performance['count'] >= 10]  # Minimum sample
            else:
                rank_performance = []
            ...
            if 'odds_volatility' in market_data.columns:
                ...
                volatility_analysis = market_data.groupby(volatility_brackets, observed=True).agg({
                    'is_winner': ['mean', 'count'],
                    'odds_volatility': 'mean'
                }).round(3)
            ...
            return {
                ...
                'market_rank_performance': rank_performance.to_dict('records') if isinstance(rank_performance, pd.DataFrame) else [],
                'volatility_analysis': volatility_analysis.to_dict('records') if isinstance(volatility_analysis, pd.DataFrame) else [],
    ...
    def generate_betting_recommendations(self):
        ...
        try:
            ...
            top_value_bets = value_data.nlargest(10, 'kelly_fraction')[
                ['dog_name', 'venue', 'race_date', 'avg_odds', 'value_ratio', 'kelly_fraction']
            ].to_dict('records')
            
            # Risk-adjusted recommendations
            safe_bets = value_data[
                (value_data['market_confidence'] > 0.7) & 
                (value_data['kelly_fraction'] > 0.05)
            ].nlargest(5, 'value_ratio')[
                ['dog_name', 'venue', 'avg_odds', 'value_ratio', 'market_confidence']
            ].to_dict('records')
    ...
    def generate_ultimate_insights(self):
        ...
        insights = {
            'data_summary': {
                ...
                'analysis_methods': [
                    'Track Bias Analysis', 'Advanced Form Analysis', 'Market Microstructure',
                    'Environmental Modeling', 'Position Dynamics', 'Risk Management',
                    'Ensemble Machine Learning', 'Value Betting Identification'
                ]
            },
    ...
    def print_ultimate_summary(self, insights):
        ...
        if 'top_biased_positions' in track_bias:
            ...
            if track_bias['top_biased_positions']:
                ...
                print(f"   • Top bias: Box {top_bias['box_number']} at {top_bias['venue']} ({top_bias['venue_box_win_rate']:.1%})")
        ...
        if 'form_category_performance' in form:
            ...
            for category in form['form_category_performance']:
                print(f"   • {category['form_category']}: {category['current_win_rate']:.1%} current win rate")
        ...
        if 'movement_patterns' in market:
            ...
            steam_moves = [p for p in market['movement_patterns'] if p.get('steam_move')]
            if steam_moves:
                print(f"   • Steam moves detected: {steam_moves[0]['win_rate']:.1%} success rate")
        ...
        if 'model_results' in models:
            ...
            best_accuracy = max([r.get('accuracy', 0) for r in models['model_results'].values()])
        ...
        if 'top_value_opportunities' in betting:
            print(f"\n💰 Betting Recommendations:")
            print(f"   • {betting['total_opportunities']} value opportunities identified")
