...
class EnhancedMLModelSystem:
    def __init__(self, db_path="greyhound_racing_data.db"):
        ...
        self.base_models = {
            ...
            'gradient_boosting': {
                ...
                'params': {'n_estimators': 200, 'learning_rate': 0.15, 'max_depth': 6, 'random_state': 42}
            },
            'extra_trees': {
        ...
        self.high_impact_features = [
            'market_confidence', 'current_odds_log', 'recent_form_avg', 'avg_position',
            'venue_experience', 'win_rate', 'place_rate', 'current_weight'
        ]
        
        self.stable_features = [
            'avg_position', 'time_consistency', 'grade_encoded', 'venue_experience',
            'place_rate', 'current_weight', 'track_condition_encoded'
        ]
        
        self.unstable_features = [
            'preferred_boxes_count', 'distance_numeric', 'avg_performance_rating',
            'races_count', 'avg_time', 'recent_races_last_30d', 'win_rate', 'best_time'
        ]
            ...
            for _, row in df.iterrows():
                ...
                if len(historical_data) >= 1:  # Minimum 1 race for basic features
                    ...
                    for _, hist_row in historical_data.iterrows():
                        ...
                        # Time data
                        if pd.notna(hist_row['individual_time']) and hist_row['individual_time'] > 0:
                        ...
                        # Odds data
                        if pd.notna(hist_row['starting_price']) and hist_row['starting_price'] > 0:
                    ...
                    recent_form_avg = np.mean(positions[:5]) if len(positions) >= 5 else np.mean(positions)
                    ...
                    current_odds_log = np.log(row['starting_price'] + 1) if pd.notna(row['starting_price']) else np.log(10)
                    
                    # Venue experience (stable, high impact)
                    venue_experience = len([r for _, r in historical_data.iterrows() if r['venue'] == row['venue']])
                    ...
                    weight_stability = 1 / (np.std(weights) + 0.1) if len(weights) > 1 else 0.5
                    
                    # NEW ADVANCED FEATURES
                    
                    # Form trend (momentum indicator) - adapted for limited data
                    if len(positions) >= 2:
                        recent_half = np.mean(positions[:len(positions)//2]) if len(positions) > 2 else positions[0]
                        older_half = np.mean(positions[len(positions)//2:]) if len(positions) > 2 else np.mean(positions[1:])
                        form_trend = (older_half - recent_half) / (older_half + 1) if older_half > 0 else 0
                    else:
                        form_trend = 0
                    ...
                    same_distance_races = [r for _, r in historical_data.iterrows() 
                    ...
                    distance_performance = np.mean(distance_positions) if distance_positions else avg_position
                    
                    # Box draw performance
                    current_box = row['box_number'] if pd.notna(row['box_number']) else 4
                    same_box_races = [r for _, r in historical_data.iterrows() 
                                     if r['box_number'] == current_box]
                    box_experience = len(same_box_races)
                    
                    # Class/Grade performance
                    same_grade_races = [r for _, r in historical_data.iterrows() 
                                       if r['grade'] == row['grade']]
                    grade_experience = len(same_grade_races)
                    
                    # Recent activity (days since last race)
                    try:
                        last_race_date = datetime.fromisoformat(historical_data.iloc[0]['race_date'])
    ...
    def prepare_enhanced_features(self, df):
        ...
        try:
            ...
            priority_2_features = [
                'win_rate', 'avg_weight', 'weight_stability', 'form_trend',
                'position_consistency', 'distance_experience', 'distance_performance',
                'box_experience', 'grade_experience', 'fitness_score'
            ]
            
            priority_3_features = [
                'days_since_last', 'competition_strength', 'current_box', 'field_size'
            ]
    ...
    def create_ensemble_models(self, X_train, y_train):
        ...
        try:
            ...
            voting_hard = VotingClassifier(
                estimators=base_estimators[:4],  # Use first 4 models
                voting='hard'
            )
    ...
    def run_enhanced_analysis(self):
        ...
        try:
            ...
            print(f"📊 Enhanced dataset: {len(prepared_df)} samples, {len(feature_columns)} features")
            ...
            for model_name, config in self.base_models.items():
                ...
                if hasattr(optimized_model, 'feature_importances_'):
                    feature_importance = list(zip(feature_columns, optimized_model.feature_importances_))
                    feature_importance.sort(key=lambda x: x[1], reverse=True)
                
                results['model_results'][model_name] = {
                    'accuracy': accuracy,
                    'auc': auc,
                    'feature_importance': feature_importance[:15] if feature_importance else None
                }
            ...
            if best_model_name in self.base_models:
                ...
                best_model = self.optimize_model_parameters(best_model, X_train_scaled, y_train, best_model_name)
            ...
            # Save the best model
            model_file = self.models_dir / f"enhanced_best_model_{datetime.now().strftime('%Y%m%d')}.joblib"
            joblib.dump({
                'model': best_model,
                'scaler': scaler,
            ...
            # Save results
            results_file = self.results_dir / f"enhanced_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        try:
            ...
            for model_name in current_results['model_results']:
                if model_name in previous_results['model_results']:
                    current_acc = current_results['model_results'][model_name]['accuracy']
                    previous_acc = previous_results['model_results'][model_name]['accuracy']
                    improvement = current_acc - previous_acc
                    improvements[model_name] = improvement
                    
                    if improvement > 0.02:
                        print(f"✅ {model_name}: +{improvement:.3f} accuracy improvement")
            ...
            improvement_file = self.results_dir / f"improvement_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def generate_improvement_recommendations(self, improvements):
        ...
        if avg_improvement > 0.01:
            recommendations.append("✅ Models showing positive trend - continue current approach")
        elif avg_improvement < -0.01:
            recommendations.append("⚠️ Models declining - consider feature engineering or data quality issues")
        
        # Model-specific recommendations
        best_improving = max(improvements.keys(), key=lambda k: improvements[k]) if improvements else None
        if best_improving:
            recommendations.append(f"🏆 Focus on {best_improving} - showing best improvement")
...
def main():
    ...
    parser.add_argument('--command', choices=['analyze', 'improve', 'status'], 
