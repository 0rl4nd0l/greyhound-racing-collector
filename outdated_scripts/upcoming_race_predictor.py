...
class UpcomingRacePredictor:
                ...
                if found_dog:
                    ...
                    if pd.notna(row.get("TIME")):
                        try:
                            ...
                            if (
                                position is None
                                and pd.notna(row["PIR"])
                                and str(row["PIR"]).strip()
                            ):
                                try:
    ...
    def analyze_form_data(self, form_data, dog_name):
        ...
        def assess_competition_quality(entry):
            return (
                len(set(entry["winner_runner_up"].split(", ")))
                if entry["winner_runner_up"]
                else None
            )
        ...
        def analyze_weight_impact(weights, positions):
            ...
            try:
                correlation = (
                    np.corrcoef(weights, positions)[0, 1] if len(weights) > 1 else 0
                )
        ...
        def analyze_margin_patterns(margins, positions):
            ...
            winning_margins = [
                ...
                if i < len(positions) and positions[i] == 1
            ]
            losing_margins = [
                abs(m)
                for i, m in enumerate(margins)
            ...
            return {
                ...
                "avg_winning_margin": (
                    np.mean(winning_margins) if winning_margins else 0
                ),
        ...
        def analyze_sectional_speed_patterns(sectionals, times, positions):
            ...
            try:
                ...
                early_speed_positions = [
                    (s, p)
                    for s, p in zip(early_speeds, positions[: len(early_speeds)])
                    if s and p
                ]
                if len(early_speed_positions) > 2:
                    speeds, pos = zip(*early_speed_positions)
                    early_speed_correlation = (
                        -np.corrcoef(speeds, pos)[0, 1] if len(speeds) > 1 else 0
                    )  # Negative because faster = lower time, better position
                else:
                ...
                closing_positions = [
                    (c, p)
                    for c, p in zip(
                        closing_sectionals, positions[: len(closing_sectionals)]
                    )
                    if c and p
                ]
                if len(closing_positions) > 2:
                    ...
                    closing_correlation = (
                        -np.corrcoef(closings, pos)[0, 1] if len(closings) > 1 else 0
                    )
                else:
                ...
                return {
                    ...
                    "avg_closing": (
                        np.mean(closing_sectionals) if closing_sectionals else 0
                    ),
                }
            except:
                return {"early_speed_advantage": 0, "closing_ability": 0}
        ...
        def analyze_distance_performance_curve(distances, positions):
            ...
            if distance_stats:
                best_distance = min(
                    distance_stats.keys(),
                    key=lambda d: distance_stats[d]["avg_position"],
                )
                versatility = len(distance_stats) / len(
                    set(distances)
                )  # How many distances performed well at
            else:
                best_distance = (
        ...
        try:
            ...
            box_numbers = [
                entry["box"] for entry in form_data if entry["box"] is not None
            ]
            ...
            sectional_analysis = analyze_sectional_speed_patterns(
                sectionals, times, positions
            )
            ...
            stats = {
                ...
                "preferred_box_numbers": (
                    [
                        box
                        for box, stats in box_analysis.items()
                ...
                "early_speed_advantage": sectional_analysis.get(
                    "early_speed_advantage", 0
                ),
                "closing_ability": sectional_analysis.get("closing_ability", 0),
                "speed_profile": (
                    "Early"
                    if sectional_analysis.get("early_speed_advantage", 0)
                    > sectional_analysis.get("closing_ability", 0)
                    else "Closing"
                ),
                # Distance Specialization
                "distance_versatility": distance_analysis.get(
                    "distance_versatility", 0
                ),
                "optimal_distance": distance_analysis.get("optimal_distance"),
                "distance_specialization": distance_analysis.get(
                    "distance_performance", {}
                ),
                # Pressure/Pace Analysis
                "handles_pressure": margin_analysis.get("close_finish_rate", 0) > 0.3
                and any(pos <= 3 for pos in positions),
                "front_runner": sectional_analysis.get("early_speed_advantage", 0)
                > 0.6,
    ...
    def calculate_market_confidence(self, starting_prices, positions):
        ...
        try:
            ...
            return max(
                0,
                min(
                    1, 0.5 - volatility + 0.3 * favored_positions / len(starting_prices)
                ),
            )
        except:
            return 0
    ...
    def analyze_sectional_positions(self, pir_data):
        ...
        try:
            avg_positions = [
                np.mean([int(p[i]) for p in pir_data if len(p) > i]) for i in range(4)
            ]
    ...
    def count_quality_opponents(self, winners_beaten, positions):
        ...
        return sum(
            1 for w in winners_beaten if w and positions[winners_beaten.index(w)] < 3
        )
    ...
    def calculate_recent_activity(self, results):
        ...
        try:
            ...
            return {
                "days_since_last_race": days_since_last,
                "recent_frequency": frequency,
                "activity_score": max(0, min(1, (30 - days_since_last) / 30))
                * min(1, frequency / 2),
            }
        except:
            return {
    ...
    def _calculate_traditional_score(self, dog_stats, race_context):
        ...
        form_trend_normalized = max(
            0, min(1, (dog_stats["form_trend"] + 2) / 4)
        )  # Normalize to 0-1
        ...
        if all(
            ...
            combined_rating = (
                dog_stats["avg_performance_rating"]
                + dog_stats["avg_speed_rating"]
                + dog_stats["avg_class_rating"]
            ) / 3
    ...
    def apply_contextual_adjustments(self, base_score, dog_stats, race_context):
        ...
        try:
            ...
            if (
                ...
                and str(preferred_distance) == str(current_distance)
            ):
        ...
        if not df.empty:
            ...
            for col in possible_condition_cols:
                if col in df.columns:
                    ...
                    if (
                        pd.notna(cond_value)
                        and cond_value
                        and str(cond_value).lower() != "nan"
                    ):
                        track_condition = str(cond_value).title()
                        print(
                            f"   🌤️ Extracted track condition from CSV: {track_condition}"
                        )
                        break

            # Method 2: Try to get from database for this venue and date
            if not track_condition:
                track_condition = self._get_track_condition_from_db(
                    context["venue"], context["race_date"]
                )
                if track_condition:
            ...
            if not track_condition and self.use_weather_data:
                ...
                if weather_data and weather_data.get("track_condition"):
                    ...
                    print(
                        f"   🌤️ Derived track condition from weather: {track_condition}"
                    )
            ...
            if track_condition:
                ...
                context["track_condition"] = self._get_venue_default_condition(
                    context["venue"]
                )
                print(
                    f"   ⚠️ No track condition found, using venue default: {context['track_condition']}"
                )
    ...
    def identify_key_factors(self, dog_stats, race_context):
        ...
        if race_context:
            ...
            current_condition = race_context.get("track_condition", "Good")
            if (
                current_condition in track_perf
                and track_perf[current_condition]["races"] >= 2
            ):
    ...
    def train_ml_models(self):
        ...
        cursor.execute(
            "SELECT COUNT(DISTINCT dog_clean_name) FROM dog_race_data WHERE finish_position IS NOT NULL"
        )
        ...
        if total_records < 500 or unique_dogs < 50:
            print(
                f"⚠️ Insufficient database records for ML training ({total_records} records, {unique_dogs} dogs)"
            )
            print("   Using traditional scoring methods with CSV form data instead")
            return

        print(
            f"🤖 Training enhanced ML models with {total_records} records from {unique_dogs} dogs..."
        )
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if (
                    len(hist_results) >= 1
                ):  # Minimum historical data (lowered to use available data)
                    # Calculate enhanced features
                    hist_positions = [
                        int(r[0]) for r in hist_results if r[0] and str(r[0]).isdigit()
                    ]
                    hist_times = [float(r[1]) for r in hist_results if r[1]]
                    hist_margins = [float(r[2]) for r in hist_results if r[2]]
                    hist_boxes = [
                        int(r[3]) for r in hist_results if r[3] and str(r[3]).isdigit()
                    ]
                    hist_odds = [float(r[4]) for r in hist_results if r[4]]

                    # Enhanced feature calculations
                    recent_form = (
                        np.mean(hist_positions[:3]) if len(hist_positions) >= 3 else 5
                    )
                    form_trend = (
                        -np.polyfit(range(len(hist_positions)), hist_positions, 1)[0]
                        if len(hist_positions) >= 3
                        else 0
                    )
                    consistency = (
                        1 / (np.std(hist_positions) + 1)
                        if len(hist_positions) > 1
                        else 0.5
                    )
                    avg_margin = (
                        np.mean([abs(m) for m in hist_margins]) if hist_margins else 2.0
                    )
                    time_consistency = (
                        1 / (np.std(hist_times) + 0.1) if len(hist_times) > 1 else 0.5
                    )
                    box_versatility = len(set(hist_boxes)) if hist_boxes else 1
                    market_confidence = (
                        1 / (np.mean(hist_odds) + 1) if hist_odds else 0.1
                    )
            ...
            if len(enhanced_features) < 100:
                print("⚠️ Insufficient enhanced data for ML training")
                return
            ...
            if enhanced_df is None or len(enhanced_df) < 100:
                print("⚠️ Could not prepare enhanced ML features")
                return
            ...
            complete_df = enhanced_df[
                feature_columns + ["is_winner", "is_placer"]
            ].dropna()

            if len(complete_df) < 100:
                print("⚠️ Insufficient complete enhanced data for ML training")
                return
            ...
            win_ensemble = VotingClassifier(
                [("rf", rf_win), ("gb", gb_win), ("lr", lr_win)], voting="soft"
            )
            ...
            place_ensemble = VotingClassifier(
                [("rf", rf_place), ("gb", gb_place)], voting="soft"
            )
            ...
            try:
                ...
                self.feature_importance = dict(
                    zip(feature_columns, fitted_rf.feature_importances_)
                )
            except:
                # Fallback if feature importance extraction fails
                self.feature_importance = {
                    col: 1.0 / len(feature_columns) for col in feature_columns
                }

            print(f"✅ Enhanced ML models trained")
            print(f"   🏆 Win prediction accuracy: {win_accuracy:.3f}")
            print(f"   🥉 Place prediction accuracy: {place_accuracy:.3f}")
            print(f"   📊 Training samples: {len(complete_df)}")
            print(f"   🔧 Features used: {len(feature_columns)}")
            print(
                f"   📈 Top features: {sorted(self.feature_importance.items(), key=lambda x: x[1], reverse=True)[:3]}"
            )
    ...
    def prepare_enhanced_ml_dataframe(self, df):
        ...
        try:
            ...
            if not hasattr(self, "venue_encoder"):
                ...
                df["track_condition_encoded"] = self.condition_encoder.fit_transform(
                    conditions
                )
            else:
                df["venue_encoded"] = self.encode_with_unknown(
                    venues, self.venue_encoder
                )
                df["track_condition_encoded"] = self.encode_with_unknown(
            ...
            print(f"❌ Enhanced ML dataframe preparation error: {e}")
            return None
    ...
    def prepare_ml_features(self, dog_stats, race_context):
        ...
        try:
            ...
            features = [
                race_context.get(
                    "field_size", 8
                ),  # box_number - use field_size as proxy
            ...
            if dog_name in market_odds:
                ...
                if our_probability > market_probability * 1.1:  # 10% edge threshold
                    ...
                    value_bet = {
                        ...
                        "recommendation": self.get_value_bet_recommendation(
                            value_percentage, our_probability
                        ),
                    }
    ...
    def analyze_market_sentiment(self, predictions, market_odds):
        ...
        if comparison_count > 0:
            ...
            sentiment_analysis["market_efficiency"] = max(
                0, min(1, 1 - (avg_variance * 2))
            )
            ...
            try:
                ...
                if self.has_race_completed(csv_file):
                    if self.update_database_with_results(csv_file):
                        updated_count += 1
                        print(
                            f"   ✅ Updated database with results from {csv_file.name}"
                        )
    ...
    def update_database_with_results(self, csv_file):
        ...
        try:
            ...
            if self.use_enhanced_processor and self.race_processor:
                ...
                result = self.race_processor.process_race_results(
                    csv_file, move_processed=True
                )

                if (
                    result.get("success")
                    or result.get("status") == "success_with_issues"
                ):
                    print(
                        f"   ✅ Enhanced processing successful: {result.get('summary', '')}"
                    )
                    ...
                    print(
                        f"   ⚠️ Enhanced processing failed, falling back to legacy method"
                    )
        ...
        try:
            ...
            """,
                (
                    ...
                    (
                        df.get("DIST", {}).iloc[0]
                        if "DIST" in df.columns and not df.empty
                        else "500"
                    ),
                    len(df),
            ...
            flattened_positions = [
                item for sublist in finish_positions for item in sublist
            ]
    ...
    def predict_race(self, race_file_path):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if (
                    raw_dog_name
                    and raw_dog_name.lower() != "nan"
                    and raw_dog_name != '""'
                ):
                    if any(
                        char.isdigit() for char in raw_dog_name[:3]
                    ):  # Has number prefix
                        dog_count += 1

            print(f"   🎯 Analyzing {dog_count} dogs with enhanced metrics...")

            # Train ML models once if we have dogs with data
            ml_trained = False
            if self.use_ml_predictions:
                print("🤖 Training enhanced ML models with more data...")
                self.train_ml_models()
                ml_trained = True
            ...
            for _, row in df.iterrows():
                ...
                if ml_trained and dog_stats and "race_winner" in self.ml_models:
                    ml_prediction_score = self.get_ml_prediction(
                        dog_stats, race_context
                    )

                # Combine traditional and ML scores
                final_score = self.combine_prediction_scores(
            ...
            race_summary = {
                ...
                "ml_predictions_used": any(
                    p.get("ml_score") is not None for p in predictions
                ),
    ...
    def predict_all_upcoming_races(self):
            ...
            if prediction:
                ...
                if prediction["predictions"]:
                    ...
                    for i, pred in enumerate(prediction["predictions"][:3]):
                        ...
                        factors_str = (
                            f" ({', '.join(key_factors[:2])})" if key_factors else ""
                        )
                        print(
                            f"      {i+1}. {pred['dog_name']} (Box {pred['box_number']}) - {pred['prediction_score']:.1%} - {pred['recommended_bet']} [Confidence: {confidence:.1%}]{factors_str}"
                        )
                    ...
                    if value_bets:
                        ...
                        for vb in value_bets[:2]:  # Show top 2 value bets
                            print(
                                f"      🎯 {vb['dog_name']}: {vb['value_percentage']:.1f}% value - {vb['recommendation']}"
                            )

                    # Show race summary
                    summary = prediction.get("race_summary", {})
                    dogs_with_data = summary.get("dogs_with_data", 0)
                    total_dogs = summary.get(
                        "total_dogs", len(prediction["predictions"])
                    )
                    print(
                        f"   📊 Analysis: {dogs_with_data}/{total_dogs} dogs with historical data"
                    )
        ...
        summary_path = (
            ...
            / f"prediction_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def predict_upcoming_race(csv_file_path):
    ...
    if result and "predictions" in result:
        ...
        return [
            f"{p['dog_name']} - {p['prediction_score']:.0%} chance to win ({p['recommended_bet']})"
