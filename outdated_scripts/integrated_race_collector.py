...
class IntegratedRaceCollector:
    ...
    async def process_form_guide_file(self, file_path, file_index):
        ...
        try:
            ...
            race_number, location, date = self.navigator.extract_race_details(file_path.name)
            ...
            results = await self.navigator.fetch_race_results(race_number, location, date)
            ...
            if results:
                ...
                if race_url:
                    ...
                    if race_data and race_data.get('race_metadata'):
                        print(f"   ✅ Successfully processed: {len(race_data['dogs_data'])} dogs")
                        return True
                    else:
    ...
    async def collect_additional_races(self, start_from_processed_count):
        ...
        try:
            ...
            existing_df = pd.read_csv(self.results_csv) if os.path.exists(self.results_csv) else pd.DataFrame()
            ...
            if new_results and len(new_results) > existing_count:
                ...
                print(f"✅ Updated CSV with {len(new_df)} total races (+{len(new_df) - existing_count} new)")
    ...
    async def run_integrated_collection(self):
        ...
        while start_index < total_files:
            ...
            if success_count > 0 and not self.verify_save_integrity(expected_count):
                ...
                print("   Please investigate the database save issue before continuing.")
