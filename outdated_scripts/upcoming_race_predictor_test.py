...
class UpcomingRacePredictor:
                ...
                if found_dog:
                    ...
                    if pd.notna(row.get('PLC')) and pd.notna(row.get('TIME')):
                        try:
                            ...
                            print(f"    📊 Extracted form entry: PLC={form_entry['position']}, SP={form_entry['starting_price']}, PIR={form_entry['position_in_race']}, Sex={form_entry['sex']}")
    ...
    def analyze_form_data(self, form_data, dog_name):
        ...
        try:
            ...
            print(f"    📊 Data extracted - Positions: {len(positions)}, SPs: {len(starting_prices)}, PIR: {len(pir_data)}, Sex: {len(sexes)}")
            ...
            print(f"    ✅ Analysis complete - Win rate: {stats['win_rate']:.1%}, Avg SP: {stats['avg_starting_price']:.1f}, Running style: {stats['running_style']}")
            return stats
    ...
    def calculate_market_confidence(self, starting_prices, positions):
        ...
        try:
            ...
            favorite_success_rate = favored_positions / len(starting_prices) if starting_prices else 0
            
            # Volatility penalty (normalize by average price to make it scale-independent)
            avg_price = np.mean(starting_prices)
            normalized_volatility = volatility / (avg_price + 1)  # +1 to avoid division by zero
            volatility_penalty = min(0.5, normalized_volatility)  # Cap penalty at 0.5
            
            confidence = max(0, min(1, favorite_success_rate + 0.3 - volatility_penalty))
    ...
    def determine_running_style(self, pir_data):
        ...
        try:
            ...
            print(f"    🏃 PIR Analysis: Early avg={avg_early:.1f}, Late avg={avg_late:.1f}")
    ...
    def calculate_gender_modifier(self, gender, distances, positions):
        ...
        try:
            if gender.lower() == 'b':  # Bitch
                favored_distance = max(set(distances), key=distances.count) if distances else None
                if favored_distance:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                prediction = {
                    ...
                    'recommended_bet': self.get_enhanced_bet_recommendation(prediction_score, dog_stats),
                ...
                print(f"     📊 {dog_name}: {prediction_score:.1%} confidence - {prediction['recommended_bet']}")
...
def test_analysis_pipeline(csv_file_path):
    ...
    if result and 'predictions' in result:
        ...
        print(f"   - Dogs with historical data: {result['race_summary']['dogs_with_data']}")
        print(f"   - Average confidence: {result['race_summary']['average_confidence']:.1%}")
        ...
        for i, pred in enumerate(result['predictions'][:3]):
            ...
            print(f"      Score: {pred['prediction_score']:.1%} | {pred['recommended_bet']} | Confidence: {pred['confidence_level']:.1%}")
        ...
        for pred in result['predictions'][:3]:
            ...
            if stats:
                print(f"\n   {pred['dog_name']}:")
                print(f"   - Market confidence: {stats.get('market_confidence', 0):.3f}")
                print(f"   - Competition quality: {stats.get('competition_quality', 0)}")
                print(f"   - Sectional analysis: {stats.get('sectional_analysis', [])}")
                print(f"   - Running style: {stats.get('running_style', 'Unknown')}")
                print(f"   - Gender modifier: {stats.get('gender_performance_modifier', 1.0):.3f}")
