...
class ComprehensiveFormGuideProcessor:
    ...
    def extract_race_results_from_csv(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if dog_name_str and not dog_name_str.startswith('"') and dog_name_str != 'nan':
                ...
                elif current_dog and (pd.isna(dog_name) or dog_name_str.startswith('"') or dog_name_str == 'nan'):
    ...
    def find_race_url(self, race_number: int, venue: str, race_date: datetime) -> Optional[str]:
    ...
    def get_database_race_count(self) -> int:
        ...
        try:
            with sqlite3.connect(self.collector.db_path) as conn:
                result = pd.read_sql_query("SELECT COUNT(*) as count FROM race_metadata", conn)
    ...
    def generate_race_id(self, race_number: int, venue: str, race_date: datetime) -> str:
    ...
    async def process_single_form_guide(self, file_path: Path) -> bool:
        ...
        try:
            ...
            print(f"   Race {race_number} at {venue} on {race_date.strftime('%Y-%m-%d')}")
            ...
            print(f"   📊 Extracted data for {len(csv_results.get('dogs', []))} dogs from CSV")
            ...
            if web_data and web_data.get('race_metadata'):
                ...
                print(f"   ✅ Successfully processed: {len(combined_data.get('dogs_data', []))} dogs")
                ...
                self.save_csv_data_only(csv_results, race_id, race_number, venue, race_date)
                return True
    ...
    def combine_csv_and_web_data(self, csv_data: Dict, web_data: Dict) -> Dict:
        ...
        web_dog_names = {dog.get('name', '') for dog in combined_data.get('dogs_data', [])}
    ...
    def save_csv_data_only(self, csv_data: Dict, race_id: str, race_number: int, venue: str, race_date: datetime):
        ...
        try:
            with sqlite3.connect(self.collector.db_path) as conn:
                ...
                metadata_df.to_sql('race_metadata', conn, if_exists='append', index=False)
                ...
                if dogs_data:
                    ...
                    dogs_df.to_sql('dog_race_data', conn, if_exists='append', index=False)
            ...
            with sqlite3.connect(self.collector.db_path) as conn:
                ...
                for dog in combined_data.get('dogs_data', []):
                    ...
                    if 'form_guide_data' in dog_data:
                        dog_data['form_guide_json'] = json.dumps(dog_data['form_guide_data'])
    ...
    async def process_all_form_guides(self, max_files: Optional[int] = None):
        ...
        for i, file_path in enumerate(unprocessed_files, 1):
            ...
            if i % 5 == 0:
                ...
                print(f"💾 Progress saved: {successful_count} successful, {failed_count} failed")
        ...
        print(f"📊 Database races: {initial_count} → {final_count} (+{final_count - initial_count})")
