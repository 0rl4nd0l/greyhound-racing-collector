...
def get_venue_bias(insights_data, venue):
    ...
    if (
        "track_bias_analysis" in insights_data
        and "venue_bias_summary" in insights_data["track_bias_analysis"]
    ):
...
def run_race_prediction(race_file_path):
    ...
    for p in predictions:
        p["confidence"] = (
            p["score"] / total_score if total_score > 0 else 1 / len(predictions)
        )
