...
from standalone_prediction import run_comprehensive_prediction, print_detailed_analysis
...
def generate_ai_insights(predictions, ultra_insights, race_data):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a world-class greyhound racing analyst with deep expertise in form analysis, betting strategies, and race dynamics. Provide professional, actionable insights."},
...
# Run the base comprehensive prediction
    ...
    predictions, ultimate_insights = run_comprehensive_prediction(race_file, ultimate_insights)
    ...
    if client is not None:
        try:
            strategy_response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a professional betting strategist. Provide specific, actionable betting advice with risk management."},
...
if __name__ == "__main__":
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        print("⚠️  OpenAI API key not found. Set OPENAI_API_KEY environment variable for AI insights.")
