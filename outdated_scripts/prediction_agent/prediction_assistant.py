...
def parse_pir(pir):
    ...
    """
    return [int(p) for p in pir] if isinstance(pir, str) and pir.isdigit() else [np.nan] * 3
...
def calculate_metrics(form_data):
    ...
    relative_time = avg_time / recent_races['DIST'].mean() if recent_races['DIST'].mean() > 0 else 0
    
    # Calculate early speed from sectional times
    early_speed = (recent_races['DIST'] / recent_races['1 SEC']).mean() if recent_races['1 SEC'].mean() > 0 else 0
    ...
    metrics = {
        ...
        'recent_form_score': np.average(recent_races['PLC'].fillna(8), weights=[0.5, 0.3, 0.2, 0.1, 0.05][:len(recent_races)]),
...
def run_prediction_assistant(form_guide_path):
    ...
    for dog in sorted(form_data['dog_clean'].unique()):  # Sort for deterministic order
        ...
        # Add prediction
        top_feature = max(ultra_insights['ultra_predictive_model']['feature_importance'], key=lambda x: x['importance'])['feature']
        predictions.append({
...
if __name__ == "__main__":
    ...
    if not os.path.exists("../ultra_insights.json"):
        ...
        print("Please ensure ultra_insights.json is in the greyhound_racing_collector directory.")
        exit(1)
