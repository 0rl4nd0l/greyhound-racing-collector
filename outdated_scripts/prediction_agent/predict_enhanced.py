...
from dynamic_prediction import run_dynamic_prediction, print_dynamic_results
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        if os.path.exists(downloads_dir):
            race_files = [f for f in os.listdir(downloads_dir) if f.endswith('.csv') and 'Race' in f]
            if race_files:
        ...
        print(f"   python3 predict_enhanced.py \"~/Downloads/Race 3 - HEA - 11 July 2025.csv\"")
        print(f"   python3 predict_enhanced.py \"form_guides/Race 2 - HEA - 11 July 2025.csv\"")
        return
    ...
    try:
        ...
        if predictions:
            ...
            print("=" * 60)
            print(f"✅ Used insights: {metadata['file_name']} ({metadata['data_type']})")
            ...
            for i, pred in enumerate(predictions[:3], 1):
                ...
                print(f"   {i}. {pred['greyhound_name']} (Box {pred['box']}) - {pred['confidence']*100:.1f}% @ ${pred['starting_price']:.1f}")
