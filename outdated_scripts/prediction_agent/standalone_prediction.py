...
def run_comprehensive_prediction(race_file_path):
    ...
    print(f"Model Accuracy: {ultra_insights['ultra_predictive_model']['model_accuracy']:.2%}")
    print(f"Training Data: {ultra_insights['data_summary']['total_races']} races, {ultra_insights['data_summary']['total_entries']} entries")
    print()
    ...
    for _, dog_row in race_data.iterrows():
        ...
        base_score, feature_contributions = calculate_prediction_score(metrics, ultra_insights)
        ...
        winners_avg_early_speed = ultra_insights['speed_analysis']['early_speed_analysis']['winners_avg_early_speed']
...
def print_detailed_analysis(predictions, ultra_insights):
    ...
    for i, pred in enumerate(predictions, 1):
        ...
        print(f"   🔥 Top Feature: {pred['top_feature']} (contribution: {pred['top_feature_value']:.4f})")
    ...
    print(f"• TOP PICK: {top_pick['greyhound_name']} with {top_pick['confidence']*100:.1f}% confidence")
    ...
    print(f"  - Fastest early speed: {fastest_early['greyhound_name']} ({fastest_early['early_speed']:.1f} m/s)")
    print(f"  - Fastest average speed: {fastest_avg['greyhound_name']} ({fastest_avg['avg_speed']:.1f} m/s)")
    ...
    print(f"• MODEL INSIGHTS (from {ultra_insights['ultra_predictive_model']['model_accuracy']:.1%} accuracy model):")
