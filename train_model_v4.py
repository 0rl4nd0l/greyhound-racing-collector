...
class ConditionalRetrainingManager:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db"):
        ...
        if candidate and str(candidate).strip():
            ...
            resolved_db = str(
                (Path(__file__).resolve().parent / "greyhound_racing_data.db").resolve()
            )
        ...
        logger.info(
            f"   Data volume threshold: {self.thresholds['data_volume_increase_pct']}%"
        )
        logger.info(
            f"   Performance drop threshold: {self.thresholds['performance_drop_threshold']}"
        )
        ...
        try:
            ...
            if self.current_state["last_data_volume"] > 0:
                volume_increase_pct = (
                    ...
                    / self.current_state["last_data_volume"]
                    * 100
                )
                ...
                if volume_increase_pct >= self.thresholds["data_volume_increase_pct"]:
                    ...
                    logger.info(
                        f"📈 Data volume increase: {volume_increase_pct:.1f}% (threshold: {self.thresholds['data_volume_increase_pct']}%)"
                    )
                else:
                    logger.info(
                        f"📊 Data volume increase: {volume_increase_pct:.1f}% (below threshold)"
                    )
            else:
    ...
    def check_model_performance_flags(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if performance_flag_raised:
                logger.info(
                    f"🚨 {len(recent_performance_alerts)} recent performance alerts found"
                )
            else:
    ...
    def check_data_drift(self) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            recent_data = pd.read_sql_query(
                query, conn, params=[recent_cutoff.strftime("%Y-%m-%d")]
            )
            conn.close()
            ...
            high_drift_features = drift_results.get("summary", {}).get(
                "high_drift_features", []
            )
            drift_detected = len(high_drift_features) > 0

            logger.info(
                f"🌊 Drift analysis complete: {len(high_drift_features)} high-drift features"
            )
    ...
    def check_time_threshold(self) -> Tuple[bool, Dict]:
        ...
        if self.current_state["last_training_timestamp"]:
            last_training = datetime.fromisoformat(
                self.current_state["last_training_timestamp"]
            )
            ...
            if days_since_training >= self.thresholds["days_since_last_training"]:
                ...
                logger.info(
                    f"⏰ Time threshold crossed: {days_since_training} days since last training"
                )
            else:
                logger.info(
                    f"⏰ {days_since_training} days since last training (threshold: {self.thresholds['days_since_last_training']})"
                )
        else:
    ...
    def calculate_dataset_checksum(self) -> str:
        ...
        try:
            ...
            # Race metadata
            df_races = pd.read_sql_query(
                "SELECT * FROM race_metadata ORDER BY race_id", conn
            )
            tables_data.append(df_races.to_string())

            # Dog race data
            df_dogs = pd.read_sql_query(
                "SELECT * FROM dog_race_data ORDER BY race_id, box_number", conn
            )
            tables_data.append(df_dogs.to_string())

            # Enhanced expert data
            df_expert = pd.read_sql_query(
                "SELECT * FROM enhanced_expert_data ORDER BY race_id, dog_clean_name",
                conn,
            )
    ...
    def create_training_manifest(
        self, retraining_triggers: List[str], model_metrics: Dict, dataset_checksum: str
    ) -> Dict:
    ...
    def save_training_artifacts(self, manifest: Dict, training_data: pd.DataFrame):
        ...
        try:
            ...
            with open(summary_file, "w") as f:
                ...
                f.write("=" * 50 + "\n\n")
                f.write(
                    f"Training Date: {manifest['training_metadata']['timestamp']}\n"
                )
                f.write(
                    f"Git Commit: {manifest['training_metadata']['git_commit_hash']}\n"
                )
                f.write(
                    f"Dataset Checksum: {manifest['dataset_info']['dataset_checksum']}\n"
                )
                f.write(
                    f"Triggers: {', '.join(manifest['training_metadata']['training_triggers'])}\n"
                )
                f.write(
                    f"Test Accuracy: {manifest['performance_metrics'].get('test_accuracy', 'N/A')}\n"
                )
                f.write(
                    f"Test AUC: {manifest['performance_metrics'].get('test_auc', 'N/A')}\n"
                )
                f.write("\nArtifacts:\n")
    ...
    try:
        ...
        if should_retrain:
            ...
            for condition, info in details.items():
                logger.info(
                    f"   {condition}: {'✅' if info['condition_met'] else '❌'}"
                )
