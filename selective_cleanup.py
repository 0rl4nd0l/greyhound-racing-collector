...
def selective_cleanup(execute=False):
    ...
    import json
    summary_file = f"{archive_dir}/cleanup_summary_{datetime.now().strftime('%Y%m%d')}.json"
    ...
    print(f"   • Space saved: {(len(archive_races)/len(df))*100:.1f}% of single-dog races moved to archive")
...
def main():
    parser = argparse.ArgumentParser(description='Selective cleanup of single dog races')
