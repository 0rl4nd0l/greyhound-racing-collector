...
class DriftMonitor:
    ...
    def __init__(
        self, reference_data: pd.DataFrame, correlation_threshold: float = 0.3
    ):
        ...
        logger.info(
            f"DriftMonitor initialized with {len(self.reference_data)} reference samples (disabled={self.disabled})"
        )
    ...
    def _calculate_baseline_correlations(self):
        ...
        try:
            ...
            self.baseline_pearson = self.reference_data[numeric_cols].corr(
                method="pearson"
            )
            self.baseline_spearman = self.reference_data[numeric_cols].corr(
                method="spearman"
            )

            logger.info(
                f"Calculated baseline correlations for {len(numeric_cols)} numeric features"
            )
            ...
            if not pd.api.types.is_numeric_dtype(expected):
                ...
                try:
                    expected_counts, bin_edges = np.histogram(
                        expected.dropna(), bins=buckets, density=True
                    )
                    actual_counts, _ = np.histogram(
                        actual.dropna(), bins=bin_edges, density=True
                    )
    ...
    def check_for_drift(self, current_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for feature in manual_drift_results["feature_drift"]:
                ...
                evident_drift = evidently_drift_results.get("feature_drift", {}).get(
                    feature, {}
                )
                ...
                std_change = (
                    "std_change" in evident_drift and evident_drift["std_change"] > 0.2
                )
                mean_change = (
                    "mean_change" in evident_drift
                    and evident_drift["mean_change"] > 0.2
                )
        ...
        try:
            ...
            audit_ts_path = (
                f"audit_results/{datetime.now().strftime('%Y%m%dT%H%M%SZ')}/drift"
            )
            ...
            with open("audit.log", "a") as log_file:
                log_file.write(
                    f"{datetime.now().isoformat()} - Drift detection completed. High drift features: {', '.join(drift_results['summary']['high_drift_features'])}\n"
                )
    ...
    def _trigger_model_retrain(self, reason: str):
        ...
        logger.info(f"🔄 Retraining requested due to {reason}")
        # Record a retrain request for orchestrators
        try:
            Path("logs").mkdir(exist_ok=True)
            with open("logs/retrain_requested.jsonl", "a") as f:
                f.write(json.dumps({"timestamp": datetime.now().isoformat(), "reason": reason}) + "\n")
        except Exception as e:
            logger.warning(f"Failed to record retrain request: {e}")
    ...
    def _check_correlations(self, current_data: pd.DataFrame) -> list:
        ...
        try:
            ...
            pearson_diff = (
                current_pearson - self.baseline_pearson.loc[common_cols, common_cols]
            )
            ...
            for col1 in common_cols:
                for col2 in common_cols:
                    if col1 != col2:
                        ...
                        if (
                            pearson_change > self.correlation_threshold
                            and not np.isnan(pearson_change)
                        ):
                            alert = {
                                ...
                                "baseline_corr": float(
                                    self.baseline_pearson.loc[col1, col2]
                                ),
            ...
            spearman_diff = (
                current_spearman - self.baseline_spearman.loc[common_cols, common_cols]
            )
            ...
            for col1 in common_cols:
                for col2 in common_cols:
                    if col1 != col2:
                        ...
                        if (
                            spearman_change > self.correlation_threshold
                            and not np.isnan(spearman_change)
                        ):
                            alert = {
                                ...
                                "baseline_corr": float(
                                    self.baseline_spearman.loc[col1, col2]
                                ),
