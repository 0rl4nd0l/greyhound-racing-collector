...
class EnhancedTGRCollector:
    ...
    def __init__(self, db_path: str = "greyhound_racing_data.db"):
        ...
        if HAS_TGR_COMPONENTS:
            try:
                ...
                use_cache = os.getenv("TGR_DISABLE_CACHE", "0").lower() not in (
                    "1",
                    "true",
                    "yes",
                )
                ...
                logger.info(
                    f"   TGR settings: rate_limit={rate_limit}s, use_cache={use_cache}, cache_dir={cache_dir}"
                )
    ...
    def ensure_database_setup(self):
        ...
        try:
            ...
            if not cursor.fetchone():
                logger.warning(
                    "Enhanced TGR tables not found. Please run migrate_tgr_enhancement.py first"
                )
    ...
    def collect_comprehensive_dog_data(self, dog_names: List[str]) -> Dict[str, Any]:
        ...
        for dog_name in dog_names:
            try:
                ...
                if not enhanced_data or not enhanced_data.get("form_entries"):
                    try:
                        profile_entries = self.scraper.fetch_dog_profile_history(
                            dog_name
                        )
                        if profile_entries:
                            ...
                            logger.info(
                                f"🐕 Profile fallback collected {len(profile_entries)} entries for {dog_name}"
                            )
                ...
                if enhanced_data and enhanced_data.get("form_entries"):
                    ...
                    results["total_entries"] += len(
                        enhanced_data.get("form_entries", [])
                    )
                    results["total_insights"] += len(
                        enhanced_data.get("recent_comments", [])
                    )

                    logger.info(
                        f"✅ {dog_name}: {len(enhanced_data.get('form_entries', []))} entries, "
        ...
        logger.info(
            f"🏁 Collection complete: {results['dogs_processed']} dogs, "
    ...
    def collect_data_for_recent_races(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        logger.info(
            f"📅 Collecting TGR data for dogs from races in last {days_back} days"
        )
    ...
    def run_maintenance(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Maintenance complete: {deleted_cache} cache entries cleaned, "
