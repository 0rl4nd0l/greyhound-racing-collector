...
class SimpleBulkProcessor:
        ...
        try:
            ...
            for row in rows:
                if len(row) > 0:
                    ...
                    if not row[0] or row[0].lower().startswith(('dog', 'name', 'pos', '#')):
                        continue
    ...
    def save_race_to_db(self, cursor, race_id, venue, race_date, race_number, 
                       winner_name, dogs, filename):
        ...
        """, (
            race_id, venue, race_date, race_number, winner_name,
            results_status, winner_source, datetime.now(),
    ...
    def process_single_file(self, filepath):
        ...
        with self.connect_db() as conn:
            ...
            # Save to database
            try:
                self.save_race_to_db(cursor, race_id, venue, race_date, race_number,
                                   winner_name, dogs, filename)
                conn.commit()
                
                status = "complete" if winner_name else "pending"
                return "success", f"Saved {race_id} ({len(dogs)} dogs, status: {status})"
