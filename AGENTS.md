...
## Bounded Offline Filesystem Work

Use `scripts/run_bounded_offline.py` for broad searches, bulk hashing, test
discovery, report archaeology, and similar read-heavy agent/offline work. Name
one exact absolute worktree or narrower subtree per invocation; never expand a
shell glob across Greyhound sibling worktrees. The wrapper defaults to one
worker, excludes archives and other irrelevant/heavy trees, applies a finite
timeout and low CPU/I/O priority, and verifies cgroup-v2 hard read limits before
the workload starts. It fails closed when those limits cannot be proved.

Normal narrow interactive searches in a known source directory may still run
directly. Do not place the entire Codex/app server or the active collector in a
constrained cgroup. See `docs/development/bounded_offline_workloads.md` for
command-specific examples and the explicit inclusion flags.
