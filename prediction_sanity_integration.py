...
def apply_sanity_checks_to_response(response: Dict, auto_fix: bool = True) -> Dict:
    ...
    if validation_results["flags"] and auto_fix:
        logger.info(
            f"Applying automatic fixes for {len(validation_results['flags'])} issues"
        )
        ...
        logger.info(
            f"✅ Fixes applied. Issues remaining: {len(fixed_validation['flags'])}"
        )
    else:
...
class SanityCheckMiddleware:
    ...
    def __init__(self, auto_fix: bool = True, log_issues: bool = True):
        ...
        self.stats = {"predictions_processed": 0, "issues_found": 0, "fixes_applied": 0}
    ...
    def process(self, predictions: List[Dict]) -> List[Dict]:
        ...
        if validation_results["flags"]:
            ...
            if self.log_issues:
                logger.warning(
                    f"Found {len(validation_results['flags'])} sanity check issues"
                )
...
if __name__ == "__main__":
    ...
    print(
        f"Direct usage - Issues found: {info['original_issues']}, Fixed: {info['fixes_applied']}"
    )

    # Example 2: Response processing
    test_response = {"success": True, "predictions": test_predictions, "method": "test"}

    processed_response = apply_sanity_checks_to_response(test_response)
    print(
        f"Response processing - Issues: {processed_response['sanity_check']['issues_found']}"
    )
