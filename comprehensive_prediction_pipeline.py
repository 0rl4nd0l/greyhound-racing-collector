...
class ComprehensivePredictionPipeline:
    ...
    def _initialize_systems(self):
        ...
        if WEATHER_PREDICTOR_AVAILABLE:
            ...
            try:
                ...
                print("✅ Open-Meteo Weather Service initialized")
            except Exception as e:
                print(f"⚠️ Open-Meteo Weather Service initialization failed: {e}")
        ...
        if GPT_ENHANCER_AVAILABLE:
            try:
                self.gpt_enhancer = GPTPredictionEnhancer()
                print("✅ GPT Prediction Enhancer initialized")
            except Exception as e:
                print(f"⚠️ GPT Prediction Enhancer initialization failed: {e}")
    ...
    def _print_system_status(self):
        ...
        print(f"  Weather Service: {'✅' if self.weather_service else '❌'}")
        print(f"  Form Scraper: {'✅' if self.form_scraper else '❌'}")
        print(f"  Processor: {'✅' if self.processor else '❌'}")
        print(f"  GPT Enhancer: {'✅' if self.gpt_enhancer else '❌'}")
        ...
        print(f"  Overall: {available_systems}/7 systems available")
    ...
    def validate_race_file(self, race_file_path, min_file_size=100):
        ...
        try:
            ...
            try:
                ...
                return (
                    False,
                    f"CSV parsing error: {str(e)}. File may be corrupted or not a valid CSV.",
                )
            ...
            return (
                True,
                f"Valid race file with {len(df)} dogs, {completeness:.2%} completeness",
            )
    ...
    def collect_comprehensive_dog_data(self, dog_name, venue=None, race_file_data=None):
        ...
        try:
            ...
            if self.enhanced_integrator:
                try:
                    enhanced_data = self.enhanced_integrator.get_enhanced_dog_data(
                        dog_name, venue, max_races=20
                    )
            ...
            # Get weather data for venue if available
            if self.weather_service and venue:
                try:
                    weather_data = self._get_venue_weather_data(venue)
                    dog_data["weather_data"] = weather_data
                    print(f"✅ Weather data collected for {venue}")
                except Exception as e:
                    print(f"⚠️ Weather data collection failed for {venue}: {e}")

            # Get form guide data from race file first (embedded historical data)
            if race_file_data is not None:
                try:
                    race_file_form_data = self._extract_race_file_form_data(
                        dog_name, race_file_data
                    )
                    if race_file_form_data:
                        dog_data["form_guide_data"] = race_file_form_data
                        print(
                            f"✅ Race file form data extracted for {dog_name}: {len(race_file_form_data)} races"
                        )
                    else:
                        print(
                            f"🔍 No embedded form data found for {dog_name} in race file"
                        )
                except Exception as e:
                    print(
                        f"⚠️ Race file form data extraction failed for {dog_name}: {e}"
                    )
            ...
            if self.ml_system:
                try:
                    ...
                    if form_guide_matches:
                        ...
                        duplicates_removed = len(form_guide_matches) - len(
                            deduplicated_matches
                        )

                        print(
                            f"✅ Combined form data for {dog_name}: {existing_races} (race file) + {len(deduplicated_matches)} (external, {duplicates_removed} duplicates removed) = {len(dog_data['form_guide_data'])} total"
                        )
                    elif not dog_data["form_guide_data"]:
                        print(
                            f"🔍 No external form guide data found for {dog_name} (tried: {test_names})"
                        )

                except Exception as e:
                    print(
                        f"⚠️ External form guide data collection failed for {dog_name}: {e}"
                    )

            # Try live scraping if still insufficient data and form scraper is available
            if (
                len(dog_data["form_guide_data"]) < 3
                and len(dog_data["database_data"]) == 0
                and self.form_scraper
                and venue
            ):
                try:
                    ...
                    if scraped_data:
                        ...
                        print(
                            f"✅ Live scraped data for {dog_name}: {existing_races} (existing) + {len(scraped_data)} (scraped) = {len(dog_data['form_guide_data'])} total"
                        )
                    else:
                        print(
                            f"⚠️ No additional data found via live scraping for {dog_name}"
                        )
                except Exception as e:
    ...
    def _get_database_dog_data(self, dog_name, venue=None):
        ...
        try:
            ...
            SELECT 
                dp.*, 
                d.dog_name as normalized_dog_name,
                rm.venue,
                rm.race_date,
                rm.distance,
                rm.grade,
            ...
            JOIN dogs d ON dp.dog_id = d.dog_id
            LEFT JOIN race_metadata rm ON dp.race_id = rm.race_id
            WHERE UPPER(TRIM(d.dog_name)) = UPPER(TRIM(?))
            ...
            if result.empty:
                ...
                print(
                    f"✅ Found exact match for '{clean_name}': {len(result)} records from venues: {set(result['venue'].unique())}"
                )
    ...
    def _get_venue_weather_data(self, venue):
        """Get current and recent weather data for a venue"""
        try:
            if not self.weather_service:
                return {}

            # Map venue names to weather service locations
            venue_mapping = {
                "ANGLE_PARK": "ANGLE_PARK",
                "SANDOWN": "SANDOWN",
                "WENTWORTH_PARK": "WENTWORTH_PARK",
                "THE_MEADOWS": "THE_MEADOWS",
                "DAPTO": "DAPTO",
                "HOBART": "HOBART",
                "GOSFORD": "GOSFORD",
            ...
            venue_key = venue_mapping.get(venue.upper(), venue.upper())

            # Get current weather data
            current_weather = self.weather_service.get_current_weather(venue_key)

            # Get race weather data
            race_date = datetime.now().strftime("%Y-%m-%d")
            recent_weather = self.weather_service.get_weather_for_race(
                venue_key, race_date
            )

            return {
                "current": current_weather,
                "recent_history": recent_weather,
                "venue": venue,
                "data_timestamp": datetime.now().isoformat(),
            }

        except Exception as e:
            print(f"⚠️ Error getting weather data for {venue}: {e}")
            return {}
    ...
    def _calculate_data_quality(self, dog_data):
        ...
        has_enhanced = dog_data["enhanced_data"] and dog_data["enhanced_data"].get(
            "sectional_times"
        )
        if has_enhanced:
            score += 1.0
        ...
        form_count = (
            len(dog_data["form_guide_data"]) if dog_data["form_guide_data"] else 0
        )
        ...
        try:
            if dog_data["database_data"]:
                latest_race = max(
                    dog_data["database_data"], key=lambda x: x.get("race_date", "")
                )
        ...
        print(
            f"   Enhanced data: {has_enhanced} (sectional_times: {bool(dog_data['enhanced_data'].get('sectional_times') if dog_data['enhanced_data'] else False)})"
        )
        print(
            f"   DB data: {db_count} races (min: {self.min_historical_races}) -> {has_db_data}"
        )
        print(
            f"   Form data: {form_count} races (min: {self.min_historical_races}) -> {has_form_data}"
        )
    ...
    def predict_race_file(self, race_file_path, force_rerun=False):
        ...
        if not is_valid:
            return {"success": False, "error": validation_message, "predictions": []}
        ...
        print(
            f"📊 Race info: {race_info['venue']} - {race_info['date']} - {len(race_df)} dogs"
        )
        ...
        for idx, row in race_df.iterrows():
            ...
            if dog_name and dog_name != "nan":
                ...
                dogs_data[dog_name] = self.collect_comprehensive_dog_data(
                    dog_name, race_info["venue"], race_df
                )

        # Generate predictions using all available methods
        predictions = self._generate_comprehensive_predictions(
            race_df, dogs_data, race_info
        )

        # Validate and rank predictions
        final_predictions = self._validate_and_rank_predictions(predictions, dogs_data)

        # Apply GPT enhancement if available
        if self.gpt_enhancer and len(final_predictions) > 0:
            try:
                enhanced_predictions = self._apply_gpt_enhancement(
                    final_predictions, race_info, dogs_data
                )
                final_predictions = enhanced_predictions
                print("✅ GPT enhancement applied to predictions")
            except Exception as e:
        ...
        prediction_results = {
            ...
            "dogs_with_quality_data": sum(
                1 for d in dogs_data.values() if d["data_quality_score"] > 0.4
            ),
        }
        ...
        print(
            f"🎯 Top pick: {final_predictions[0]['dog_name'] if final_predictions else 'None'}"
        )

        return prediction_results

    def predict_race_file_with_all_enhancements(
        self, race_file_path, enhancement_level="full"
    ):
        """Enhanced prediction method with all available enhancements"""
        print(
            f"🚀 Starting comprehensive prediction with {enhancement_level} enhancements..."
        )
        return self.predict_race_file(race_file_path)
    ...
    def _extract_race_info(self, race_file_path, race_df):
        ...
        try:
            ...
            # Try multiple filename parsing patterns
            race_number = None
            venue = None
            date = None

            # Pattern 1: "Race 1 - VENUE - 25 July 2025"
            match = re.match(r"Race (\d+) - ([A-Z_]+) - (\d{1,2} \w+ \d{4})", filename)
            if match:
                race_number, venue, date = match.groups()

            # Pattern 2: "20250730162247_Race_01_VENUE_2025-07-25"
            if not match:
                match = re.match(r"\w+_Race_(\d+)_([A-Z_]+)_([\d-]+)", filename)
                if match:
                    race_number, venue, date = match.groups()
                # Convert date format using flexible parsing
                try:
                    date_formatted = parse_date_flexible(date)
                    date_obj = datetime.strptime(date_formatted, "%Y-%m-%d")
                    date = date_obj.strftime("%d %B %Y")
                except ValueError:
                    pass

            # Pattern 3: "Race_11_-_CASO_-_22_July_2025"
            if not match:
                match = re.match(r"Race_(\d+)_-_([A-Z_]+)_-_([\d_A-Za-z]+)", filename)
                if match:
                    race_number, venue, date_part = match.groups()
                    # Convert underscore date format back to normal
            ...
            if not race_df.empty:
                ...
                if "TRACK" in race_df.columns and race_info["venue"] == "UNKNOWN":
                    ...
                    if pd.notna(track_code):
                        ...
                        race_info["venue"] = track_mapping.get(
                            str(track_code).upper(), str(track_code)
                        )
    ...
    def _generate_comprehensive_predictions(self, race_df, dogs_data, race_info):
        ...
        for idx, row in race_df.iterrows():
            ...
            if self.ml_system and dog_data["data_quality_score"] > 0.3:
                try:
                    ml_score = self._get_ml_prediction_score(
                        dog_name, dog_data, race_info
                    )
            ...
            if dog_data["enhanced_data"]:
                enhanced_score = self._get_enhanced_data_score(
                    dog_data["enhanced_data"]
                )
            ...
            prediction["final_score"] = self._calculate_weighted_final_score(
                prediction["prediction_scores"], dog_data["data_quality_score"]
            )
    ...
    def _get_weather_prediction_score(self, dog_name, dog_data, race_info):
        ...
        try:
            ...
            if dog_data.get("weather_performance"):
                ...
                weather_adjustment = dog_data["weather_performance"].get(
    ...
    def _get_traditional_analysis_score(self, dog_name, dog_data, race_info):
        ...
        try:
            ...
            if dog_data["database_data"]:
                ...
                if recent_races:
                    recent_positions = [
                        float(r.get("finish_position", 4))
                        for r in recent_races
                        if r.get("finish_position")
                        and str(r["finish_position"]).replace(".", "").isdigit()
                    ]
                    if recent_positions:
            ...
            venue_races = [
                r
                for r in dog_data["database_data"]
                if r.get("venue") == race_info["venue"]
            ]
            if venue_races:
                venue_positions = [
                    float(r.get("finish_position", 4))
    ...
    def _get_enhanced_data_score(self, enhanced_data):
        ...
        try:
            ...
            if enhanced_data.get("pir_ratings"):
                pir_values = [
                    p["pir"]
                    for p in enhanced_data["pir_ratings"]
                    if p["pir"] is not None
                ]
                if pir_values:
            ...
            if enhanced_data.get("sectional_times"):
                ...
                sectionals = [
                    s["first_section"]
                    for s in enhanced_data["sectional_times"]
                    if s["first_section"] is not None
                ]
                if sectionals:
                    ...
                    sectional_score = max(
                        0.1, 1.0 - (avg_sectional - 5.0) / 10.0
                    )  # Rough approximation
    ...
    def _calculate_historical_stats(self, dog_data, race_info):
        ...
        try:
            ...
            all_races = dog_data.get("database_data", []) + dog_data.get(
                "form_guide_data", []
            )
            ...
            for race in all_races:
                ...
                if (
                    time_val
                    and str(time_val).replace(".", "").replace(":", "").isdigit()
                ):
                    try:
                        ...
                        if ":" in str(time_val):
                            ...
                            if len(time_parts) == 2:
                                time_seconds = float(time_parts[0]) * 60 + float(
                                    time_parts[1]
                                )
                            else:
            ...
            venue_races = [
                r for r in all_races if r.get("venue") == race_info.get("venue")
            ]
            venue_record = (
                f"{len(venue_races)} starts at {race_info.get('venue', 'venue')}"
                if venue_races
                else "No previous starts at venue"
            )
            ...
            distance_record = (
                f"{len(distance_races)} starts at {target_distance}m"
                if distance_races and target_distance
                else "No distance data"
            )
            ...
            grade_record = (
                f"{len(grade_races)} starts in {target_grade}"
                if grade_races and target_grade
                else "No grade data"
            )
            ...
            if len(venue_races) >= 3:
                venue_positions = [
                    int(float(r.get("finish_position", r.get("place", 5))))
                    for r in venue_races
                    if str(r.get("finish_position", r.get("place", "")))
                    .replace(".", "")
                    .isdigit()
                ]
            ...
            if all_races:
                ...
                sorted_races = sorted(
                    all_races,
                    key=lambda x: x.get("race_date", x.get("date", "1900-01-01")),
                    reverse=True,
                )
                if sorted_races:
                    most_recent_race = sorted_races[0]
                    race_date_str = most_recent_race.get(
                        "race_date", most_recent_race.get("date", "")
                    )
                    ...
                    if race_date_str:
                        try:
                            ...
                            recent_activity["days_since_last_race"] = (
                                days_since_last_race
                            )
                            recent_activity["last_race_date"] = race_date_str
                            recent_activity["last_race_position"] = (
                                most_recent_race.get(
                                    "finish_position",
                                    most_recent_race.get("place", "N/A"),
                                )
                            )
                        except (ValueError, TypeError) as e:
                            print(f"⚠️ Error parsing race date '{race_date_str}': {e}")
                            recent_activity["days_since_last_race"] = (
                                999  # Default for unparseable dates
                            )
                    else:
                        recent_activity["days_since_last_race"] = (
                            999  # Default for missing dates
                        )
                else:
                    recent_activity["days_since_last_race"] = (
                        999  # Default for no races
                    )
            else:
            ...
            return {
                ...
                "average_position": (
                    round(avg_position, 2) if avg_position > 0 else "N/A"
                ),
    ...
    def _apply_gpt_enhancement(self, predictions, race_info, dogs_data):
        """Apply GPT enhancement to existing predictions"""
        try:
            # Create a temporary race file for GPT analysis
            temp_race_data = {
                "race_id": f"{race_info['venue']}_{race_info['race_number']}_{race_info['date']}",
                "venue": race_info["venue"],
                "race_number": race_info["race_number"],
                "date": race_info["date"],
                "distance": race_info.get("distance", "500m"),
                "grade": race_info.get("grade", "Grade 5"),
                "dogs": [],
            }

            # Convert prediction data to GPT format
            for pred in predictions:
                ...
                if dog_name in dogs_data:
                    dog_info = {
                        "name": dog_name,
                        "box": pred["box_number"],
                        "historical_data": dogs_data[dog_name].get("database_data", [])[
                            :10
                        ],
                        "form_data": dogs_data[dog_name].get("form_guide_data", [])[
                            :10
                        ],
                        "enhanced_data": dogs_data[dog_name].get("enhanced_data", {}),
                        "weather_data": dogs_data[dog_name].get("weather_data", {}),
                        "ml_prediction_score": pred["final_score"],
                        "data_quality": pred["data_quality"],
                    }
                    temp_race_data["dogs"].append(dog_info)

            # Get GPT insights (simplified approach)
            gpt_insights = self._get_gpt_race_insights(temp_race_data)

            # Apply GPT adjustments to predictions
            enhanced_predictions = []
            for pred in predictions:
                enhanced_pred = pred.copy()

                # Apply GPT adjustments if available
                if gpt_insights and pred["dog_name"] in gpt_insights:
                    gpt_data = gpt_insights[pred["dog_name"]]

                    # Adjust confidence based on GPT analysis
                    gpt_confidence_boost = gpt_data.get("confidence_adjustment", 0.0)

                    # Apply small adjustment to final score based on GPT insights
                    gpt_score_adjustment = gpt_data.get("score_adjustment", 0.0)
                    enhanced_pred["final_score"] = min(
                        max(pred["final_score"] + gpt_score_adjustment, 0.1), 0.9
                    )

                    # Add GPT reasoning
                    if gpt_data.get("reasoning"):
                        enhanced_pred["reasoning"].append(
                            f"GPT: {gpt_data['reasoning']}"
                        )

                    # Update confidence if GPT suggests higher confidence
                    if gpt_confidence_boost > 0.1 and pred["confidence_level"] == "LOW":
                        enhanced_pred["confidence_level"] = "MEDIUM"
                    elif (
                        gpt_confidence_boost > 0.2
                        and pred["confidence_level"] == "MEDIUM"
                    ):
                        enhanced_pred["confidence_level"] = "HIGH"

                enhanced_predictions.append(enhanced_pred)

            # Re-sort by updated final scores
            enhanced_predictions.sort(key=lambda x: x["final_score"], reverse=True)
            ...
            print(f"⚠️ Error in GPT enhancement: {e}")
            return predictions  # Return original predictions if GPT fails

    def _get_gpt_race_insights(self, race_data):
        """Get GPT insights for race analysis (simplified version)"""
        try:
            # This is a simplified implementation
            # In a full implementation, you would use the GPT enhancer to analyze the race

            insights = {}

            # For each dog, create basic GPT-style insights
            for dog in race_data["dogs"]:
                dog_name = dog["name"]

                # Analyze historical performance
                recent_positions = []
                if dog["historical_data"]:
                    recent_positions = [
                        r.get("finish_position", 5)
                        for r in dog["historical_data"][:5]
                        if r.get("finish_position")
                    ]

                # Simple GPT-style analysis
                ...
                if recent_positions:
                    ...
                    if avg_recent <= 2.0:  # Strong recent form
                        confidence_adjustment = 0.15
                        score_adjustment = 0.03
                        reasoning = "Strong recent form trend"
                    elif avg_recent >= 5.0:  # Poor recent form
                        confidence_adjustment = -0.1
                        score_adjustment = -0.02
                        reasoning = "Concerning recent form"

                # Check for venue experience
                venue_experience = len(
                    [
                        r
                        for r in dog["historical_data"]
                        if r.get("venue") == race_data["venue"]
                    ]
                )
                if venue_experience >= 3:
                    confidence_adjustment += 0.05
                    score_adjustment += 0.01
                    reasoning += " + Good venue experience"
            ...
            print(f"⚠️ Error getting GPT insights: {e}")
            return {}
    ...
    def _get_prediction_methods_used(self):
        ...
        if self.gpt_enhancer:
            methods.append("gpt_enhanced")

        return methods

    def _save_prediction_results(
        self, race_info, prediction_results, force_rerun=False
    ):
        """Save prediction results to file with duplicate prevention"""
        try:
            # Extract race_id from the original race filename
            race_filename = race_info.get("filename", "")
            if race_filename:
                # Use the utility function to extract race_id from CSV filename
                race_id = extract_race_id_from_csv_filename(race_filename)
            else:
                # Fallback to constructed race ID
                race_id = f"Race_{race_info['race_number']}_{race_info['venue']}_{race_info['date']}"
            ...
            if filepath.exists() and not force_rerun:
                ...
                if file_age < 3600:  # Less than 1 hour old
                    print(
                        f"⚠️ Recent prediction already exists: {filename} (age: {file_age/60:.1f} min)"
                    )
                ...
                filepath.rename(backup_path)
                print(
                    f"🔄 Force rerun: Backed up existing prediction to: {backup_filename}"
                )
                ...
                try:
                    with open(filepath, "r") as existing_file:
                        ...
                        if "unified_predictor_version" in existing_data:
                            unified_metadata = {
                                "unified_predictor_version": existing_data[
                                    "unified_predictor_version"
                                ],
                                "analysis_version": existing_data.get(
                                    "analysis_version", "Unified Predictor v1.0"
                                ),
                            }
                except Exception as e:
                    print(
                        f"⚠️ Error reading existing prediction for metadata preservation: {e}"
                    )
            ...
            with open(filepath, "w") as f:
                json.dump(
                    prediction_results, f, indent=2, default=self._json_safe_serializer
                )
    ...
    def predict_all_upcoming_races(
        self, upcoming_dir="upcoming_races", force_rerun=False
    ):
        """
        Predicts all races in the upcoming_races directory.

        Args:
            upcoming_dir (str): The directory containing upcoming race CSV files.
            force_rerun (bool): Whether to force regeneration of predictions.

        Returns:
            dict: A summary of the prediction results.
        """
        print(
            f"🚀 Starting batch prediction for all upcoming races in '{upcoming_dir}'"
        )
        ...
        if not race_files:
            return {
                "success": True,
                "message": "No upcoming races found to predict.",
                "total_races": 0,
                "predictions": [],
            }
        ...
        for race_file in race_files:
            ...
            results = self.predict_race_file(race_file_path, force_rerun=force_rerun)
        ...
        # Save a summary of the batch prediction
        summary_filename = (
            f"batch_prediction_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        summary_filepath = self.predictions_dir / summary_filename
        with open(summary_filepath, "w") as f:
            json.dump(summary, f, indent=2, default=self._json_safe_serializer)

        print(f"✅ Batch prediction complete. Summary saved to {summary_filepath}")
        return summary
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python comprehensive_prediction_pipeline.py <race_file_path_or_all> [--force-rerun]"
        )
        print(
            "  <race_file_path_or_all>: Path to a single race file or 'all' to run all upcoming races."
        )
    ...
    if target.lower() == "all":
        # Run predictions for all upcoming races
        batch_results = pipeline.predict_all_upcoming_races(force_rerun=force_rerun)
        if batch_results["success"]:
            print("\n🏆 BATCH PREDICTION SUMMARY:")
            print("=" * 50)
            print(f"  Total races processed: {batch_results['total_races']}")
            print(
                f"  Successful predictions: {batch_results['successful_predictions']}"
            )
            sys.exit(0)
        else:
            print(
                f"❌ Batch prediction failed: {batch_results.get('message', 'Unknown error')}"
            )
            sys.exit(1)

    else:
        # Generate prediction for a single file
        results = pipeline.predict_race_file(target, force_rerun=force_rerun)
        ...
        if results["success"]:
            ...
            for i, pred in enumerate(results["predictions"][:5], 1):
                ...
                print(
                    f"   Score: {pred['final_score']:.3f} | Confidence: {pred['confidence_level']}"
                )
            ...
            print(
                f"   Dogs with Good Data: {quality['dogs_with_good_data']}/{quality['total_dogs']}"
            )
