...
class WorkflowLogger:
    ...
    def __init__(self, log_dir: str = "./logs", archive_dir: str = "./backups/archives"):
    ...
    def archive_old_logs(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        print(f"🗂️  Archiving logs older than {self.archive_days} days (before {cutoff_date.date()})")
        ...
        try:
            ...
            for log_file in self.log_dir.glob("*.log*"):
                ...
                if file_mtime < cutoff_date:
                    ...
                    if not dry_run:
                        ...
                        archive_name = f"{log_file.stem}_{file_mtime.strftime('%Y%m%d')}.gz"
            ...
            if not dry_run:
                print(f"✅ Archived {len(archived_files)} log files ({archive_summary['total_size_mb']} MB)")
            else:
                print(f"🔍 Would archive {len(archived_files)} log files ({archive_summary['total_size_mb']} MB)")
