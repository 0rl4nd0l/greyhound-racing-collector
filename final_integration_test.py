...
def test_system_health():
    ...
    try:
        ...
        if model_health.get("ready"):
            log_test("ML Models", "✅ PASS", f"Ready with {model_health.get('feature_count', 0)} features")
        else:
...
def test_model_registry_workflow():
    ...
    try:
        ...
        response = requests.get(f"{BASE_URL}/api/model_registry/status", timeout=TIMEOUT)
        ...
        if registry_data.get("success"):
            ...
            log_test("Model Registry", "✅ PASS", f"{model_count} models, {best_models} best models")
        else:
            log_test("Model Registry", "❌ FAIL", "Registry not accessible")
            return False
            
        # Test model listing
        response = requests.get(f"{BASE_URL}/api/model_registry/models", timeout=TIMEOUT)
        if response.status_code == 200:
...
def test_enhanced_prediction_service():
    ...
    try:
        ...
        if model_health.get("source") == "model_registry" and model_health.get("ready"):
            log_test("Enhanced Service", "✅ PASS", "Service active with model registry integration")
        else:
            log_test("Enhanced Service", "⚠️ WARN", "Service may not be fully integrated")
            
        # Test prediction endpoint that uses enhanced service
        response = requests.get(f"{BASE_URL}/predict_page", timeout=TIMEOUT)
        if response.status_code == 200:
            log_test("Enhanced Predictions", "✅ PASS", "Enhanced prediction endpoint accessible")
        else:
            log_test("Enhanced Predictions", "❌ FAIL", f"Status {response.status_code}")
            return False
