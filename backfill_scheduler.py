...
class BackfillScheduler:
    ...
    def get_pending_tasks(self, limit: int = 1000) -> List[BackfillTask]:
        ...
        try:
            ...
            for row in rows:
                race_id, venue, race_date, race_number, attempts, note, last_scraped = (
                    row
                )
    ...
    def _calculate_priority(
        self, race_date: str, attempts: int, venue: str, note: str
    ) -> Priority:
    ...
    def create_backfill_plan(
        self, max_tasks: int = 100, time_budget_minutes: int = 60
    ) -> Dict[str, Any]:
        """Create an optimized backfill execution plan"""
        tasks = self.get_pending_tasks(
            max_tasks * 2
        )  # Get more than needed for selection
        ...
        viable_tasks = [
            t for t in tasks if t.scraping_attempts < self.max_attempts_per_race
        ]
        ...
        return {
            ...
            "priority_breakdown": {
                p: len(tasks) for p, tasks in priority_groups.items()
            },
            "plan": plan,
    ...
    def execute_backfill_plan(
        self, plan: List[BackfillTask], dry_run: bool = False
    ) -> Dict[str, Any]:
        ...
        print(
            f"📊 Estimated time: {sum(t.estimated_effort * 5 for t in plan) / 60:.1f} minutes"
        )
        ...
        results["summary"] = {
            ...
            "success_rate": self.session_stats["succeeded"]
            / max(1, self.session_stats["processed"]),
            "tasks_per_minute": self.session_stats["processed"]
            / max(1, total_time / 60),
        }
    ...
    def _process_single_task(self, task: BackfillTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_success:
                ...
                # Simulate failure
                error_msg = (
                    f"Simulated scraping failure (attempt {task.scraping_attempts + 1})"
                )
    ...
    def _simulate_backfill_execution(self, plan: List[BackfillTask]) -> Dict[str, Any]:
        ...
        return {
            ...
            "priority_breakdown": {
                p.name: len([t for t in plan if t.priority == p]) for p in Priority
            },
        }
    ...
    def generate_progress_report(self) -> str:
        ...
        if plan["status"] == "no_tasks":
            ...
            if critical_count > 0:
                report.append(
                    f"   🔴 URGENT: {critical_count} critical priority races (last 7 days)"
                )
                report.append("      → Process these immediately")

            if high_count > 0:
                report.append(
                    f"   🟡 HIGH: {high_count} high priority races (last 30 days)"
                )
                report.append("      → Process these next")

            if plan["estimated_time_minutes"] > 120:
                report.append(
                    "   ⏰ Large processing time - consider running in batches"
                )
...
def main():
    ...
    plan_parser.add_argument(
        "--time-budget", type=int, default=60, help="Time budget in minutes"
    )
    ...
    if args.command == "report":
        ...
        for i, task in enumerate(plan["plan"][:10], 1):
            print(
                f"   {i:2d}. {task.race_id} ({task.priority.name}, {task.scraping_attempts} attempts)"
            )
        ...
        for i, task in enumerate(tasks, 1):
            ...
            print(
                f"     Priority: {task.priority.name}, Attempts: {task.scraping_attempts}"
            )
