...
def main():
    ...
    try:
        from utils.file_naming import build_prediction_filename, extract_race_id_from_csv_filename, parse_prediction_filename
    ...
    for race_id, method in test_cases:
        ...
        if all(part in filename for part in expected_parts) and filename.endswith('.json'):
    ...
    filenames = [
        ...
        build_prediction_filename("race1", timestamp.replace(second=timestamp.second+1), "method1"),
    ...
    test_filename = build_prediction_filename("SANDOWN_R3_20250730", datetime.now(), "comprehensive")
    parsed = parse_prediction_filename(test_filename)
    
    if parsed['is_valid'] and parsed['race_id'] == "SANDOWN_R3_20250730" and parsed['method'] == "comprehensive":
