...
def test_core_function():
    ...
    try:
        ...
        if is_list and len(races) > 0:
            ...
            print(
                f"   ✅ Sample race structure: date={first_race.get('race_date')}, venue={first_race.get('venue')}"
            )
        ...
        with app.test_client() as client:
            ...
            if response.status_code == 200:
                ...
                if is_races_list:
                    ...
                    print(f"\n   🎯 TASK REQUIREMENT VALIDATION:")
                    print(
                        f"      assert isinstance(response.json()['races'], list) = {is_races_list}"
                    )
                    print(
                        f"      Response format: {{ 'success': {success_value}, 'races': [{len(races)} items] }}"
                    )
