        ...
        if prediction_response.status_code == 200:
            ...
            try:
                ...
                if response_data.get("success"):
                    ...
                    print(
                        f"❌ Prediction failed: {response_data.get('message', 'Unknown error')}"
                    )
                    return False
        ...
        if logs_response.status_code == 200:
            ...
            for log in logs:
                ...
                if any(
                    ...
                    for keyword in ["prediction", "completed", "success"]
                ):
            ...
            if prediction_logs:
                ...
                for log in prediction_logs[-3:]:  # Show last 3
                    print(
                        f"   {log.get('timestamp', 'N/A')}: {log.get('message', 'N/A')}"
                    )
            else:
        ...
        if log_dir.exists():
            ...
            if process_log.exists():
                with open(process_log, "r") as f:
                    ...
                    prediction_lines = [
                        line for line in lines if "prediction" in line.lower()
                    ]
                    if prediction_lines:
                        print("✅ Found prediction entries in process.log:")
                    ...
                    for line in lines:
                        try:
                            ...
                            if "prediction" in log_entry.get("message", "").lower():
                                print(
                                    f"✅ Workflow log entry: {log_entry.get('message', 'N/A')}"
                                )
                        except json.JSONDecodeError:
                            continue
