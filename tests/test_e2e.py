...
def test_e2e_smoke_test(setup_test_environment):
    ...
    try:
        result = subprocess.run([sys.executable, 'run.py', 'collect'], 
                              ...
                              cwd=PROJECT_ROOT, timeout=30)  # Reduced timeout
        ...
        collection_success = (result.returncode == 0 or 
                            "Data collection completed" in result.stdout or 
                            "Data collection completed" in result.stderr)
    ...
    try:
        # Run the analysis
        analysis_result = subprocess.run([sys.executable, 'run.py', 'analyze'], 
                              capture_output=True, text=True, 
        ...
        analysis_success = (analysis_result.returncode == 0 or 
                          "Data analysis completed" in analysis_result.stdout or 
                          "Data analysis completed" in analysis_result.stderr)
    ...
    if len(prediction_files) > 0:
        ...
        test_result = {
            ...
            'analysis_success': analysis_success and final_processed_count >= initial_processed_count,
