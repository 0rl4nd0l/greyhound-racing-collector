...
class ModelComparisonHarness:
    ...
    def load_and_preprocess_csvs(self, csv_dir: Path) -> List[Tuple[str, pd.DataFrame]]:
        ...
        for csv_file in csv_dir.glob("*.csv"):
            try:
                ...
                processed_df = self.apply_form_guide_preprocessing(
                    raw_df, csv_file.stem
                )
                ...
                if processed_df is not None and not processed_df.empty:
                    ...
                    logger.warning(
                        f"   Skipped {csv_file.name} - no valid data after preprocessing"
                    )
    ...
    def apply_form_guide_preprocessing(
        ...
        try:
            ...
            columns_to_drop = [
                col for col in post_outcome_columns if col in processed_df.columns
            ]
            if columns_to_drop:
            ...
            if "dog_name" in processed_df.columns:
                ...
                processed_df["dog_name"] = processed_df["dog_name"].str.replace(
                    r"^\d+\.\s*", "", regex=True
                )
    ...
    def inject_race_into_models(
        self, race_name: str, race_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        for model_name, model in self.models.items():
            try:
                logger.info(
                    f"🔮 Running predictions with model {model_name} for race {race_name}"
                )

                model_result = self.get_model_predictions(
                    model, model_name, race_data, race_name
                )
                race_results["models"][model_name] = model_result

                logger.info(
                    f"   ✅ {model_name} predictions: {len(model_result.get('predictions', []))} dogs"
                )
    ...
    def get_model_predictions(
        self, model, model_name: str, race_data: pd.DataFrame, race_name: str
    ) -> Dict[str, Any]:
    ...
    def get_v3_predictions(
        ...
        if not has_trained_model:
            logger.warning(
                f"Model {model_name} has no trained pipeline, using mock predictions"
            )
            ...
            for idx, row in race_data.iterrows():
                ...
                mock_win_prob = max(
                    0.05, min(0.80, (float(pir_rating) / 100.0) if pir_rating else 0.5)
                )
                if starting_price:
            ...
            for idx, row in race_data.iterrows():
                ...
                pred_result = {
                    ...
                    "raw_win_probability": prediction.get(
                        "raw_win_probability", prediction.get("win_probability", 0.5)
                    ),
                    "raw_place_probability": prediction.get(
                        "raw_place_probability",
                        prediction.get("place_probability", 0.65),
                    ),
                    "normalized_win_probability": prediction.get(
                        "win_probability", 0.5
                    ),
                    "normalized_place_probability": prediction.get(
                        "place_probability", 0.65
                    ),
                    "confidence": prediction.get("confidence", 0.0),
                    "model_metadata": {
                        "model_info": prediction.get("model_info", "unknown"),
                        "calibration_applied": prediction.get(
                            "calibration_applied", False
                        ),
    ...
    def get_v4_predictions(
        self, model: MLSystemV4, race_data: pd.DataFrame, race_name: str
    ) -> Dict[str, Any]:
        ...
        for pred in result.get("predictions", []):
            standardized_pred = {
                ...
                "raw_place_probability": pred.get("win_prob_raw", 0.5)
                * 2.8,  # Estimate from raw
    ...
    def run_comparison(self, model_names: List[str], csv_dir: Path) -> bool:
        ...
        logger.info(
            f"🚀 Starting model comparison: models={model_names}, csv_dir={csv_dir}"
        )
        ...
        for model_name, model_result in race_results["models"].items():
            if model_result.get("success", False):
                ...
                if predictions:
                    ...
                    print(
                        f"  {model_name.upper()}: Top pick = {top_pick['dog_name']} "
                ...
                print(
                    f"  {model_name.upper()}: FAILED - {model_result.get('error', 'Unknown error')}"
                )
    ...
    def generate_comparison_report(self):
        ...
        output_file = (
            f"model_comparison_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def print_comparison_statistics(self):
            ...
            for model_name, model_result in result["models"].items():
                ...
                if model_result.get("success", False):
                    ...
                    for pred in predictions:
                        ...
                        actual_win = (
                            pred_rank == 1
                        )  # Assuming position 1 means the dog won
                        ...
                        predicted_ev = (
                            pred.get("normalized_win_probability") - 0.5
                        )  # Estimated EV
                        ...
                        var_spread = abs(
                            pred.get("raw_win_probability") - normalized_win_prob
                        )
        ...
        for model_name, stats in model_stats.items():
            ...
            top1_acc = (
                sum(stats["top1_accuracy"]) / len(stats["top1_accuracy"])
                if stats["top1_accuracy"]
                else 0
            )
            avg_brier = (
            ...
            ev_corr = (
                (
                    sum(x * y for x, y in stats["ev_correlations"])
                    / len(stats["ev_correlations"])
                )
                if stats["ev_correlations"]
                else 0
            )

            print(f"\n{model_name.upper()} Model:")
            print(
                f"  Success Rate: {success_rate:.1f}% ({stats['successful_races']}/{stats['total_races']})"
            )
            ...
            avg_variance_spread = (
                sum(stats["variance_spread"]) / len(stats["variance_spread"])
                if stats["variance_spread"]
                else 0
            )
            avg_conf_vs_disp = (
                sum(stats["confidence_vs_dispersion"])
                / len(stats["confidence_vs_dispersion"])
                if stats["confidence_vs_dispersion"]
                else 0
            )
