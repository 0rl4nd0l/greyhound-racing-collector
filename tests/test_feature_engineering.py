...
def test_feature_store():
    ...
    new_features = pd.DataFrame(
        {
            "v3_distance_avg_time": np.random.normal(
                29.0, 1.5, 50
...
def main():
    ...
    try:
        ...
        print(
            f"  • Drift detection: {sum(1 for r in drift_results.values() if r['drift_detected'])}/{len(drift_results)} features with drift"
        )
        print(
            f"  • Integration test: {integration_features.shape[0]} dogs, {integration_features.shape[1]} features"
        )
