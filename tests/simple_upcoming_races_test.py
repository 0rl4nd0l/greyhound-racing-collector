...
def check_javascript_issues():
    ...
    if "localeCompare" in content:
        ...
        if (
            "venueA.localeCompare(venueB)" in content
            or "a.venue.localeCompare(b.venue)" in content
        ):
...
def check_venue_handling():
    ...
    print(
        "   ⚠️  Explicit 'Unknown Venue' handling not found, but may be handled dynamically"
    )
