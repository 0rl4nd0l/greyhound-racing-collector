...
def test_test_race_csv_exists():
    ...
    assert os.path.exists(TEST_RACE_PATH), f"Test race CSV not found at {TEST_RACE_PATH}"
...
def test_csv_structure_and_key_consistency():
    ...
    assert 'Dog Name' in df.columns, f"Expected 'Dog Name' column not found. Available columns: {df.columns.tolist()}"
...
@pytest.mark.parametrize("prediction_layer", ["ml_system", "weather_enhanced", "unified"])
def test_prediction_layers_key_consistency(prediction_layer, test_db_path, setup_test_environment):
    ...
    try:
        if prediction_layer == "ml_system":
            ...
            assert hasattr(predictor, 'predict_race_file'), "WeatherEnhancedPredictor missing predict_race_file method"
            
        elif prediction_layer == "unified" and UNIFIED_AVAILABLE:
            predictor = UnifiedPredictor()
            assert hasattr(predictor, 'predict_race_file'), "UnifiedPredictor missing predict_race_file method"
        else:
...
def test_prediction_pipeline_no_key_errors(test_db_path, setup_test_environment):
    ...
    try:
        ...
        if result.get('success', False):
            ...
            assert 'prediction_tier' in result, "Successful prediction missing 'prediction_tier'"
            assert result['prediction_tier'] != "dummy_fallback", f"Unexpected dummy fallback: {result.get('fallback_reasons', [])}"
            logger.info(f"✅ Successful prediction with tier: {result['prediction_tier']}")
        else:
            # If failed, check for meaningful error information
            assert 'error' in result or 'fallback_reasons' in result, "Failed prediction missing error information"
            logger.warning(f"⚠️ Prediction failed: {result.get('error', 'Unknown error')}")
            
            # Even failed predictions should not crash with KeyError
            if 'fallback_reasons' in result:
                for reason in result['fallback_reasons']:
                    assert 'KeyError' not in str(reason.get('reason', '')), f"KeyError detected in fallback: {reason}"
...
def test_weather_enhanced_predictor_key_handling(test_db_path, setup_test_environment):
    ...
    try:
        ...
        assert isinstance(result, dict), "WeatherEnhancedPredictor result should be a dictionary"
        
        if result.get('success', False):
            assert 'prediction_tier' in result, "Successful weather prediction missing 'prediction_tier'"
            assert result['prediction_tier'] != "dummy_fallback", "Weather predictor should not use dummy fallback"
            logger.info(f"✅ Weather-enhanced prediction successful with tier: {result['prediction_tier']}")
        else:
            logger.warning(f"⚠️ Weather-enhanced prediction failed: {result.get('error', 'Unknown error')}")
...
def test_loaders_accept_constant_keys(csv_file):
    ...
    for csv_column, constant_key in expected_mappings.items():
        assert csv_column in available_columns, f"Expected CSV column '{csv_column}' (maps to {constant_key}) not found"
...
def test_error_handling_and_fallback_logic(test_db_path, setup_test_environment):
    ...
    try:
        ...
        if 'fallback_reasons' in result:
            for reason in result['fallback_reasons']:
                assert isinstance(reason, dict), "Fallback reason should be a dictionary"
                ...
                assert 'KeyError' not in reason_text, f"KeyError found in fallback reason: {reason_text}"
...
def test_integration_all_layers_consistent_keys(test_db_path, setup_test_environment):
    ...
    logger.info("Running integration test for consistent key handling across all layers")
    ...
    logger.info(f"Successfully initialized {len(layers_tested)} layers: {layers_tested}")
