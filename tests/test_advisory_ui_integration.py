...
class AdvisoryUITestSuite:
    ...
    def create_test_prediction_data(self):
        ...
        high_quality_data = {
            ...
            'venue': 'TEST_VENUE',
            'predictions': [
                {'dog_name': 'High Quality Dog 1', 'box_number': 1, 'win_prob': 0.4, 'confidence': 0.9},
                {'dog_name': 'High Quality Dog 2', 'box_number': 2, 'win_prob': 0.3, 'confidence': 0.85},
                {'dog_name': 'High Quality Dog 3', 'box_number': 3, 'win_prob': 0.3, 'confidence': 0.8}
            ]
        }
        ...
        low_quality_data = {
            ...
            'venue': 'TEST_VENUE',
            'predictions': [
                {'dog_name': 'Low Conf Dog 1', 'box_number': 1, 'win_prob': 0.1, 'confidence': 0.2},  # Low confidence warning
                {'dog_name': 'Zero Prob Dog', 'box_number': 2, 'win_prob': 0.0, 'confidence': 0.0},    # Critical issue
                {'dog_name': 'Normal Dog', 'box_number': 3, 'win_prob': 0.9, 'confidence': 0.1}       # High prob, low confidence
            ]
        }
    ...
    async def test_advisory_collapse_toggle(self, page: Page):
        ...
        # Assertions
        assert result == 'Advisory rendered successfully', "Advisory should render successfully"
    ...
    async def test_advisory_color_coding(self, page: Page):
        ...
        unique_border_colors = set(styles['borderLeftColor'] for styles in color_results.values())
        unique_backgrounds = set(styles['backgroundColor'] for styles in color_results.values())
        ...
        expected_classes = ['alert-info', 'alert-success', 'alert-warning', 'alert-danger']
        found_classes = []
        ...
        # Assertions
        assert len(color_results) == len(message_types), "Should render all message types"
        assert len(unique_border_colors) >= 3, "Should have at least 3 different border colors"
        assert len(found_classes) >= len(expected_classes) - 1, "Should use Bootstrap alert classes"
    ...
    async def test_advisory_responsiveness(self, page: Page):
        ...
        for viewport in viewport_sizes:
            ...
            if advisory_id:
                element_metrics = await page.evaluate(f"""
                () => {{
        ...
        print(f"     Mobile horizontal scroll: {'Yes' if has_horizontal_scroll else 'No'}")
        ...
        # Assertions
        assert len(responsiveness_results) == len(viewport_sizes), "Should test all viewport sizes"
    ...
    async def test_advisory_workflow_integration(self, page: Page):
        ...
        if workflow_result.get('success'):
            print(f"   ✅ Advisory API accessible: {workflow_result['advisoryGenerated']}")
        ...
        print(f"   UI blocking test:")
        print(f"     Render time for 5 advisories: {ui_test_result['renderTime']:.2f}ms")
        print(f"     Button still clickable: {ui_test_result['buttonClicked']}")
        print(f"     Advisories rendered: {ui_test_result['advisoryCount']}")
        
        # Assertions
        if workflow_result.get('success'):
            assert workflow_result['advisoryGenerated'], "Advisory should be generated successfully"
            assert workflow_result['messageCount'] > 0, "Should generate advisory messages"
            assert workflow_result['processingTime'] < 5000, "Should process within 5 seconds"
