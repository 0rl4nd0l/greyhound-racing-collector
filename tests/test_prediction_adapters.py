...
def validate_result_structure(result, adapter_name):
    ...
    for i, pred in enumerate(predictions):
        ...
        for key in required_pred_keys:
            if key not in pred:
                print(
                    f"❌ {adapter_name}: Missing required prediction key '{key}' in prediction {i}"
                )
                return False
        ...
        if not (0 <= pred["win_prob_norm"] <= 1):
            print(
                f"❌ {adapter_name}: win_prob_norm ({pred['win_prob_norm']}) out of range [0,1] for {pred['dog']}"
            )
            return False

        if not (0 <= pred["raw_prob"] <= 1):
            print(
                f"❌ {adapter_name}: raw_prob ({pred['raw_prob']}) out of range [0,1] for {pred['dog']}"
            )
            return False
    ...
    if predictions:
        ...
        if not (0.95 <= norm_prob_sum <= 1.05):  # Allow small numerical errors
            print(
                f"❌ {adapter_name}: Normalized probabilities sum to {norm_prob_sum:.3f}, expected ~1.0"
            )
            return False
...
def print_result_summary(result, adapter_name):
    ...
    if "temporal_leakage_protected" in metadata:
        print(
            f"   Temporal Leakage Protected: {metadata['temporal_leakage_protected']}"
        )
...
def find_test_race_file():
    ...
    possible_locations = ["upcoming_races", "test_data", "sample_data", "."]

    for location in possible_locations:
        if os.path.exists(location):
            for file in os.listdir(location):
                if file.endswith(".csv") and (
                    "race" in file.lower() or "dog" in file.lower()
                ):
...
def main():
    ...
    if failed_adapters:
        ...
        for name in failed_adapters:
            error = (
                results[name]["metadata"].get("error", "Unknown error")
                if results[name]
