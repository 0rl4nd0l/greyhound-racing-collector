...
def test_class_imbalance():
    ...
    print(f"  - Extreme imbalance test:")
    print(f"    * Normalized entropy: {result1['normalized_entropy']:.3f}")
    print(f"    * Dominant probability: {result1['dominant_runner']['probability']:.3f}")
    print(f"    * Extreme imbalance flag: {result1['imbalance_flags']['extreme_imbalance']}")
    ...
    print(f"  - Balanced distribution test:")
...
def test_leakage_detection():
    ...
    future_race = {
        ...
        'predictions': [
            {'dog_name': 'Test Dog', 'box_number': 1, 'win_prob': 0.5}
        ]
    }
    
    result1 = qa_analyzer.detect_leakage_and_date_drift(future_race)
    print(f"  - Future race test:")
    ...
    print(f"  - Temporal inconsistency test:")
    print(f"    * Temporal inconsistency flag: {result2['flags']['temporal_inconsistency_flag']}")
    ...
    leakage_data = {
        ...
        'extraction_time': '2025-08-04T12:00:00',
        'predictions': [
            {'dog_name': 'Winner Dog', 'box_number': 1, 'win_prob': 0.5, 'finish_position': 1},  # Leakage!
            {'dog_name': 'Loser Dog', 'box_number': 2, 'win_prob': 0.5, 'actual_time': 31.2}    # Leakage!
        ]
    }
    
    result3 = qa_analyzer.detect_leakage_and_date_drift(leakage_data)
    print(f"  - Potential leakage test:")
...
def test_calibration_with_outcomes():
    ...
    print(f"  - Well calibrated: {result.get('calibration_quality', {}).get('well_calibrated', 'N/A')}")
    print(f"  - Issues detected: {result['issues_detected']}")
    print()
...
def test_comprehensive_analysis():
    ...
    mixed_issues = {
        ...
        'extraction_time': '2025-08-04T12:00:00',
        'predictions': [
            {'dog_name': 'Dominant Dog', 'box_number': 1, 'win_prob': 0.82},  # High probability
            {'dog_name': 'Weak Dog 1', 'box_number': 2, 'win_prob': 0.08},   # Low confidence
            {'dog_name': 'Weak Dog 2', 'box_number': 3, 'win_prob': 0.06},   # Low confidence
            {'dog_name': 'Weak Dog 3', 'box_number': 4, 'win_prob': 0.04}    # Low confidence
        ]
    }
...
def main():
    ...
    print(f"📝 Detailed logs available in: logs/qa/qa.jsonl")
    ...
    try:
        with open('logs/qa/qa.jsonl', 'r') as f:
            ...
            for line in lines[-3:]:
                ...
                print(f"  [{entry['level']}] {entry['analysis_type']}: {entry['outcome']} "
