...
def test_live_scrape_maps_times_by_each_canonical_race_url(monkeypatch):
    ...
    original_extract = browser.extract_race_info_from_link
    observed_card_hashes = []

    def record_card_hash(link, href, date, *, meeting_card_sha256=None):
        observed_card_hashes.append(meeting_card_sha256)
    ...
    monkeypatch.setattr(browser, "extract_race_info_from_link", record_card_hash)
    links = _links(
        """
    ...
    assert set(observed_card_hashes) == {hashlib.sha256(FakeResponse.content).hexdigest()}
...
def test_meeting_card_grade_hint_rejects_unknown_or_mismatched_identity():
    ...
    invalid_hints = [
        ...
        {**base, "target_grade_source_sha256": "bad"},
