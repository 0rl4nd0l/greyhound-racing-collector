...
def test_temporal_feature_builder_basic():
    ...
    try:
        ...
        assert 'finish_position' in builder.post_race_features, "finish_position should be post-race"
        assert 'box_number' in builder.pre_race_features, "box_number should be pre-race"
...
def test_feature_classification():
    ...
    try:
        ...
        expected_pre_race = [
            'box_number', 'weight', 'starting_price', 'trainer_name',
            'venue', 'grade', 'distance', 'track_condition', 'weather',
            'temperature', 'humidity', 'wind_speed', 'field_size',
            'race_date', 'race_time'
        ]
        
        expected_post_race = [
            'finish_position', 'individual_time', 'sectional_1st', 
            'sectional_2nd', 'margin'
        ]
