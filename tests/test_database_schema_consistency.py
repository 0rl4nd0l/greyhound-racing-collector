...
class DatabaseSchemaConsistencyTester:
    ...
    def test_alembic_schema_consistency(self) -> Dict[str, Any]:
        ...
        Skips gracefully when Alembic CLI/env is not available.
        """
        result = {
        ...
        # Pre-checks: require alembic CLI and versions dir
        if shutil.which("alembic") is None:
            result["skipped"] = True
            result["message"] = "Alembic CLI not found; skipping autogenerate check"
            return result
        if not Path("alembic.ini").exists() or not Path("alembic/versions").exists():
            result["skipped"] = True
            result["message"] = (
                "Alembic config or versions directory missing; skipping autogenerate check"
            )
            return result
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if process.returncode != 0:
                    # Treat environment issues as SKIP rather than FAIL for local runs
                    result["skipped"] = True
                    result["message"] = (
                        f"Alembic command failed; skipping: {process.stderr.strip()}"
                    )
                    return result
                ...
                if not migration_files:
                    result["skipped"] = True
                    result["message"] = "No test migration file generated; skipping"
                    return result
                ...
                if has_operations:
                    result["message"] = (
                        "Schema drift detected! Alembic found differences between models and database."
                    )
                    result["details"]["migration_content"] = migration_content
                else:
                    result["passed"] = True
                    result["message"] = (
                        "Schema is consistent - no changes detected by Alembic"
                    )

        except Exception as e:
            # Treat unexpected environment errors as SKIP rather than FAIL
            result["skipped"] = True
            result["message"] = (
                f"Alembic check skipped due to environment error: {str(e)}"
            )
    ...
    def test_foreign_key_indexes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["passed"]:
                result["message"] = (
                    f"All foreign keys have appropriate indexes ({len(result['details']['foreign_keys_checked'])} checked)"
                )
            else:
                missing_count = len(result["details"]["missing_indexes"])
                result["message"] = (
                    f"Found {missing_count} foreign keys without indexes"
                )
    ...
    def test_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Summary message
            total_issues = (
                len(result["details"]["orphan_records"])
        ...
        try:
            ...
            for table_name in tables:
                ...
                schema_info[table_name] = {
                    ...
                    "primary_key": (
                        sorted(primary_keys["constrained_columns"])
                        if primary_keys
                        else []
                    ),
    ...
    def compare_schema_snapshots(
        self, snapshot1_file: str, snapshot2_file: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            differences = {
                "schema_hash_changed": snapshot1["schema_hash"]
                != snapshot2["schema_hash"],
            ...
            if removed_tables:
                differences["summary"].append(
                    f"Removed tables: {', '.join(removed_tables)}"
                )
            ...
            for table in common_tables:
                ...
                if table1 != table2:
                    differences["table_differences"][table] = {
                        ...
                        "foreign_keys_changed": table1["foreign_keys"]
                        != table2["foreign_keys"],
                    }
...
def test_alembic_schema_consistency(schema_tester):
    ...
    if result.get("skipped"):
        pytest.skip(result.get("message") or "Alembic check skipped")
...
def test_foreign_key_indexes(schema_tester):
    ...
    if not result["passed"]:
        ...
        for missing in missing_indexes:
            suggestions.append(
                f"CREATE INDEX {missing['suggested_index_name']} ON {missing['table']}({', '.join(missing['foreign_key_columns'])});"
            )
    ...
    if not result["passed"]:
        ...
        if details["orphan_records"]:
            ...
            for orphan in details["orphan_records"]:
                error_summary.append(
                    f"  - {orphan['description']}: {orphan['count']} records"
                )
        ...
        if details["null_violations"]:
            ...
            for null_viol in details["null_violations"]:
                error_summary.append(
                    f"  - {null_viol['description']}: {null_viol['count']} records"
                )
        ...
        if details["constraint_violations"]:
            ...
            for constraint in details["constraint_violations"]:
                error_summary.append(
                    f"  - {constraint['description']}: {constraint['count']} records"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--compare-snapshots", nargs=2, help="Compare two schema snapshots"
    )
    parser.add_argument(
        "--generate-hash", action="store_true", help="Generate schema hash"
    )
    ...
    if args.save_snapshot:
        ...
        differences = tester.compare_schema_snapshots(
            args.compare_snapshots[0], args.compare_snapshots[1]
        )
        ...
        if result1.get("skipped"):
            print(f"   Result: SKIP - {result1['message']}")
        else:
            print(
                f"   Result: {'PASS' if result1['passed'] else 'FAIL'} - {result1['message']}"
            )

        print("\n2. Testing foreign key indexes...")
        result2 = tester.test_foreign_key_indexes()
        print(
            f"   Result: {'PASS' if result2['passed'] else 'FAIL'} - {result2['message']}"
        )

        print("\n3. Testing data integrity...")
        result3 = tester.test_data_integrity()
        print(
            f"   Result: {'PASS' if result3['passed'] else 'FAIL'} - {result3['message']}"
        )
        ...
        all_passed = all(
            [
                result1.get("passed", False) or result1.get("skipped", False),
                result2["passed"],
                result3["passed"],
            ]
        )
