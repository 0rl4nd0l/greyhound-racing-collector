...
class SimplePerformanceTest:
    ...
    def enable_explain_analyze(self):
        ...
        try:
            response = requests.get("http://localhost:5002/api/enable-explain-analyze", timeout=5)
    ...
    def run_locust_test(self):
        ...
        cmd = [
            ...
            "--headless",
            "-u", "500",
            "-r", "50",  # spawn 50 users per second
            "-t", "300s",  # 5 minutes
            ...
            while True:
                ...
                if output:
                    ...
                    if "P95 latency threshold exceeded" in line:
                        ...
                        if "P95 latency threshold exceeded" not in str(self.failure_reasons):
                            self.failure_reasons.append("P95 latency > 2s threshold exceeded")
                    ...
                    if len(lines) > 1:  # Skip header
                        for line in lines[1:]:
                            ...
                            if len(parts) > 3:
                                ...
                                results['failures'] += int(parts[3]) if parts[3].isdigit() else 0
        except Exception as e:
    ...
    def generate_report(self):
        ...
        print(f"\n🚨 TEST CRITERIA:")
        print(f"   P95 Latency < 2s: {'❌ FAILED' if 'P95 latency' in str(self.failure_reasons) else '✅ PASSED'}")
        print(f"   Memory Leak < 5MB/min: {'❌ FAILED' if memory_leak else '✅ PASSED'}")
