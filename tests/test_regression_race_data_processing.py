...
class RaceDataProcessor:
    ...
    def process_csv_file(self, csv_file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(df) >= 10:
                ...
                race_data = {
                    'venue': df.get('TRACK', df.get('Venue', ['Unknown'])).iloc[0] if 'TRACK' in df.columns or 'Venue' in df.columns else 'Unknown',
                    'date': df.get('DATE', df.get('Date', [datetime.now().strftime('%Y-%m-%d')])).iloc[0] if 'DATE' in df.columns or 'Date' in df.columns else datetime.now().strftime('%Y-%m-%d'),
                    'race_number': df.get('RACE', df.get('Race', [1])).iloc[0] if 'RACE' in df.columns or 'Race' in df.columns else 1,
                    'source': 'csv',
                    'data_rows': len(df),
                    'dogs': []
                }
                ...
                for _, row in df.iterrows():
                    dog_data = {
                        'dog_name': row.get('Dog Name', row.get('DOG NAME', f'Dog_{len(race_data["dogs"]) + 1}')),
                        'box': row.get('BOX', row.get('Box', len(race_data["dogs"]) + 1)),
    ...
    def combine_data_sources(self, csv_races: List[Dict], json_races: List[Dict]) -> List[Dict]:
        """
...
Duplicate Dog Ten,2,2,Sandown,2024-08-01,5,515"""
        ...
        json_data = {
            ...
            "dogs": [
                {"dog_name": "JSON Duplicate Dog", "box": 1, "place": 1}
            ]
        }
    ...
    def test_corrupt_csv_skipped(self, processor, corrupt_csv_file, empty_csv_file):
        ...
        assert processor.error_count >= 2, "Error count should be incremented for corrupt files"
    ...
    def test_combined_list_length_matches(self, processor, sample_csv_10_rows, sample_json_file):
        """
        ...
        assert actual_length == expected_length, f"Expected {expected_length} races, got {actual_length}"
    ...
    def test_duplicate_venue_date_race_not_repeated(self, processor, duplicate_race_data):
        """
        ...
        assert len(combined_races) == 1, f"Expected 1 race after deduplication, got {len(combined_races)}"
    ...
    def test_comprehensive_workflow(self, processor, sample_csv_10_rows, sample_json_file, corrupt_csv_file):
        """
        ...
        assert processor.error_count >= 1, "Should have recorded errors from corrupt CSV"
        
        print("✅ Comprehensive workflow test PASSED")

    @pytest.mark.skipif(not CSV_INGESTION_AVAILABLE, reason="CSV ingestion module not available")
...
Test Dog Three,3,3,Sandown,2024-08-01,515,30.8,30.00"""
        ...
        try:
            ...
            if validation_result.is_valid:
                ...
                print(f"Processed data length: {len(processed_data) if processed_data else 0}")
                ...
                if processed_data:
                    ...
                    for record in processed_data:
                        ...
                        assert record['dog_name'] in ['Test Dog One', 'Test Dog Two', 'Test Dog Three']
                else:
