...
def driver():
    ...
    if not chromedriver_path:
        pytest.skip(
            "System chromedriver not found; skipping UI badge test (install via: brew install chromedriver)"
        )
...
def test_gpt_rerank_badge_summary(driver, flask_server):
    ...
    assert (
        ok is True
    ), "displayPredictionResults not available (test export hook failed)"
