...
class ResolveRootTests(unittest.TestCase):
    ...
    def test_resolves_a_symlink_before_mount_construction(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(bounded.resolve_root(str(link), repo_root=repo), target.resolve())
    ...
    def test_rejects_a_symlink_whose_resolved_target_contains_a_mount_delimiter(self) -> None:
        with (
            ...
            with self.assertRaisesRegex(bounded.ConfigurationError, "resolved --root.*comma"):
...
class ArgumentAndExclusionTests(unittest.TestCase):
    def test_timeout_and_hard_limits_are_bounded(self) -> None:
        ...
        with self.assertRaises(SystemExit):
            parser.parse_args(["--root", "/tmp/example", "--timeout-seconds", "0", "tests"])
        with self.assertRaises(SystemExit):
            parser.parse_args(
                ["--root", "/tmp/example", "--read-mib-per-sec", "64", "tests"]
            )
        with self.assertRaises(SystemExit):
            parser.parse_args(["--root", "/tmp/example", "--read-iops", "1000", "tests"])

    def test_defaults_exclude_vcs_worktrees_caches_envs_generated_and_large_data(self) -> None:
    ...
    def test_explicit_category_inclusion_removes_only_that_exclusion_group(self) -> None:
    ...
    def test_image_id_must_be_an_immutable_sha256_digest(self) -> None:
        ...
        for invalid in ("alpine:3.20", "sha256:abc", "sha256:" + "G" * 64):
            with self.subTest(invalid=invalid):
                with self.assertRaisesRegex(bounded.ConfigurationError, "immutable image ID"):
...
class CommandConstructionTests(unittest.TestCase):
    ...
    def test_docker_command_is_read_only_low_priority_and_hard_limited(self) -> None:
        ...
        self.assertIn("type=bind,src=/tmp/exact,dst=/scan,readonly", command)
        self.assertIn("type=bind,src=/opt/host/rg,dst=/opt/bounded/rg,readonly", command)
        ...
        self.assertIn("/sys/fs/cgroup/io.max", bootstrap)
    ...
    def test_docker_command_rejects_a_mount_source_delimiter(self) -> None:
        ...
        with self.assertRaisesRegex(bounded.ConfigurationError, "resolved rg path.*comma"):
...
class ExecutionTests(unittest.TestCase):
    class FakeProcess:
        def __init__(self, first_error: BaseException | None = None, returncode: int = 0) -> None:
    ...
    def test_timeout_cleans_container_and_returns_124(self) -> None:
        process = self.FakeProcess(subprocess.TimeoutExpired(["docker"], 1), returncode=-15)
        cleanup = mock.Mock()
