...
def test_single_race(race_file):
    ...
    race_name = (
        race_file.split("/")[-1].replace(".csv", "").replace("20250730162231_", "")
    )
    print(f"\n📄 Testing: {race_name}")
    ...
    try:
        ...
        venue = (
            race_data["TRACK"].iloc[0] if "TRACK" in race_data.columns else "Unknown"
        )
        date = race_data["DATE"].iloc[0] if "DATE" in race_data.columns else "Unknown"
        distance = (
            race_data["DIST"].iloc[0] if "DIST" in race_data.columns else "Unknown"
        )
