...
def create_test_race_file_with_missing_keys():
    ...
    test_csv_data = {
        "Dog Name": ["1. Test Dog 1", "2. Test Dog 2", "3. Test Dog 3", "", "5. Test Dog 5"],
        "Box": [1, 2, 3, 4, 5],
...
def create_test_prediction_json_with_missing_keys():
    ...
    prediction_data = {
        ...
        "predictions": [
            ...
            {"wrong_key": "Test Dog 2", "box_number": 2, "final_score": 0.7},  # Missing DOG_NAME_KEY
...
def test_key_error_handling_in_weather_enhanced_predictor():
    ...
    try:
        ...
        with patch.object(key_mismatch_logger, 'log_key_error') as mock_logger:
            ...
            predictor.feature_columns = ['avg_position', 'recent_form_avg', 'market_confidence']
            ...
            with patch('weather_enhanced_predictor.COMPREHENSIVE_ML_AVAILABLE', True):
                ...
                with patch('weather_enhanced_predictor.ComprehensiveEnhancedMLSystem') as mock_ml_system:
                    ...
                    with patch.object(predictor, '_extract_participating_dogs', return_value=mock_dogs_with_missing_keys):
                        ...
                        assert mock_logger.call_count >= 1, f"Expected at least 1 KeyError log call, got {mock_logger.call_count}"
                        ...
                        print("✅ Weather-enhanced predictor KeyError handling test passed")
            ...
            with patch('weather_enhanced_predictor.COMPREHENSIVE_ML_AVAILABLE', False):
                ...
                with patch.object(predictor, '_extract_participating_dogs', return_value=mock_dogs_with_missing_keys):
                    ...
                    for call in mock_logger.call_args_list:
                        ...
                        if 'error_context' in kwargs:
                            error_context = kwargs['error_context']
                            assert error_context['operation'] == 'dog_name_extraction_in_fallback_prediction'
                            assert error_context['race_file_path'] == race_file_path
                            assert error_context['missing_key'] == DOG_NAME_KEY
                            assert error_context['step'] == 'fallback_prediction_processing'
...
def test_key_error_handling_in_flask_app():
    ...
    try:
        ...
        with patch.object(key_mismatch_logger, 'log_key_error') as mock_logger:
            ...
            first_pred = prediction_data['predictions'][1]  # This one has missing DOG_NAME_KEY
            ...
            # Verify that key_mismatch_logger.log_key_error was called
            assert mock_logger.call_count == 1, f"Expected 1 KeyError log call, got {mock_logger.call_count}"
            ...
            assert error_context['operation'] == 'top_pick_creation_from_prediction_results'
            assert error_context['missing_key'] == DOG_NAME_KEY
...
def test_logger_captures_required_context():
    ...
    with patch.object(key_mismatch_logger, 'log_key_error') as mock_logger:
        ...
        error_context = kwargs['error_context']
        for required_field in ['operation', 'race_file_path', 'dog_record', 'available_keys', 'missing_key', 'step']:
            assert required_field in error_context, f"Missing required field: {required_field}"
