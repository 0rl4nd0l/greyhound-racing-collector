...
def sample_gpt_analysis():
    ...
    return {
        ...
        'enhanced_ml_predictions': {
            'enhanced_insights': {
                'key_factors': ['Track conditions favor early speed']
            }
        },
        'betting_strategy': {
            'betting_strategy': 'Focus on win bets for top pick'
        },
        'pattern_analysis': {
            'venue_patterns': 'Inside boxes perform well'
        },
        'merged_predictions': [
            {
                'dog_name': 'Dog A',
...
def test_merged_prediction_snapshot(mock_get_enhancer, sample_gpt_analysis, client):
    ...
    expected_structure_keys = {
        'race_info', 'gpt_race_analysis', 'enhanced_ml_predictions',
        'betting_strategy', 'pattern_analysis', 'merged_predictions',
        'analysis_summary', 'tokens_used'
    }
    assert set(stable_data.keys()) == expected_structure_keys

# Test prediction merging preserves ML scores
@patch('app.get_gpt_enhancer')
def test_prediction_merging_preserves_ml_scores(mock_get_enhancer, sample_gpt_analysis, client):
