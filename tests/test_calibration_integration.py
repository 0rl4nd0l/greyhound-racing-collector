...
def test_calibrator_standalone():
    ...
    for raw_win, raw_place in test_cases:
        ...
        print(
            f"  Calibrated: win={result['calibrated_win_prob']:.3f}, place={result['calibrated_place_prob']:.3f}"
        )
        print(
            f"  Applied: win={result['win_calibration_applied']}, place={result['place_calibration_applied']}"
        )
        print()
...
def test_ml_integration():
    ...
    try:
        ...
        print(
            f"    Win Probability: {win_prob:.4f if isinstance(win_prob, (int, float)) else win_prob}"
        )
        print(
            f"    Place Probability: {place_prob:.4f if isinstance(place_prob, (int, float)) else place_prob}"
        )
        print(
            f"    Raw Win Probability: {raw_win_prob:.4f if isinstance(raw_win_prob, (int, float)) else raw_win_prob}"
        )
        print(
            f"    Raw Place Probability: {raw_place_prob:.4f if isinstance(raw_place_prob, (int, float)) else raw_place_prob}"
        )
        print(
            f"    Calibration Applied: {prediction.get('calibration_applied', False)}"
        )
        print(
            f"    Win Calibration Applied: {prediction.get('win_calibration_applied', False)}"
        )
        print(
            f"    Place Calibration Applied: {prediction.get('place_calibration_applied', False)}"
        )
        print(
            f"    Confidence: {confidence:.4f if isinstance(confidence, (int, float)) else confidence}"
        )
        ...
        if prediction.get("calibration_applied"):
            ...
            print(
                "  ⚠️ Calibration not applied (may be expected if no calibrator trained)"
            )
