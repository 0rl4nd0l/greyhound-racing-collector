...
def validate_predictions_and_betting():
    ...
    for file_path in prediction_files:
        ...
        with open(file_path, "r") as f:
            ...
            if "predictions" in data and isinstance(data["predictions"], list):
                ...
                "summary" in data
                and "predictions" in data["summary"]
                and isinstance(data["summary"]["predictions"], list)
            ):
                predictions = data["summary"]["predictions"]

            if predictions is None:
                issues.append(
                    f"- **{file_path.name}**: Missing or invalid 'predictions' list."
                )
                continue
            ...
            for i, prediction in enumerate(predictions):
                ...
                for key in [
                    ...
                    if key in prediction:
                        ...
                        if prob is not None and (
                            not isinstance(prob, (int, float)) or not (0 <= prob <= 1)
                        ):
                            issues.append(
                                f"- **{file_path.name}**: Invalid `{key}` for **{dog_name}**. Value: `{prob}` (not in [0, 1])."
                            )
                        if pd.isna(prob):
                            issues.append(
                                f"- **{file_path.name}**: NaN value for `{key}` for **{dog_name}**"
                            )
                ...
                if "predicted_rank" in prediction:
                    ...
                    if rank in ranks:
                        issues.append(
                            f"- **{file_path.name}**: Duplicate rank `{rank}` for **{dog_name}**."
                        )
                    ranks.add(rank)
            ...
            if "bet_suggestions" in data and isinstance(data["bet_suggestions"], list):
                for suggestion in data["bet_suggestions"]:
                    ...
                    if bet_type == "Win" and win_prob is not None and win_prob <= 0.25:
                        issues.append(
                            f"- **{file_path.name}**: Low win probability ({win_prob}) for Win bet suggestion."
                        )

                    if value is not None and odds is not None and value <= odds:
                        issues.append(
                            f"- **{file_path.name}**: Value ({value}) is not greater than odds ({odds}) for bet suggestion."
                        )
    ...
    if issues:
        ...
        print(
            f"Found {len(unique_issues)} unique issues. Report generated at {report_path}"
        )
    else:
