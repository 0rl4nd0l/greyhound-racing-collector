...
def test_csv_with_empty_rows_quarantine():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
        ...
        try:
            ...
            assert not validation_report.is_valid
            assert any("Empty record found" in error for error in validation_report.errors)
        finally:
            os.unlink(f.name)
...
def test_invisible_characters_detection():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as f:
        ...
        try:
            ...
            assert "Invisible characters found and removed" in validation_report.warnings
        finally:
            os.unlink(f.name)
