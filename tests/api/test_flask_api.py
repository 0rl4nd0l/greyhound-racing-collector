...
from app import UPCOMING_DIR, app
...
def test_single_race_prediction_by_id(client, example_id_race_file):
    response = client.post(
        "/api/predict_single_race_enhanced", json={"race_id": "example_id"}
    )
...
def test_single_race_prediction_by_filename(client, test_race_file):
    response = client.post(
        "/api/predict_single_race_enhanced", json={"race_filename": test_race_file}
    )
    assert response.status_code == 200
    data = response.get_json()
    assert data["success"] is True
