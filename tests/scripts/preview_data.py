...
def analyze_cleanup_options():
    ...
    for venue, data in venue_analysis.head(10).iterrows():
        print(f"{venue:8s}: {int(data['single_dog_count']):4d} races ({data['percentage']:5.1f}%) across {int(data['unique_dates'])} dates")
    ...
    print("-" * 40)
    sample_races = df.sample(min(10, len(df)))[['race_id', 'venue', 'race_date', 'single_dog_name']]
    for _, race in sample_races.iterrows():
        print(f"{race['race_id'][:50]:<50} | {race['venue']} | {race['single_dog_name'][:20]}")
    ...
    print(f"   Date range: {df['race_date'].min().strftime('%Y-%m-%d')} to {df['race_date'].max().strftime('%Y-%m-%d')}")
    
    print(f"\n🎯 RECOMMENDED ACTIONS:")
    
    # Option 1: Conservative cleanup
    print(f"\n1. 🟢 CONSERVATIVE APPROACH (Recommended):")
    print(f"   - Keep all single-dog races (they contain valid individual dog data)")
    print(f"   - Fix race_name column (currently 100% null)")
    print(f"   - Use for individual dog performance analysis")
    print(f"   - Mark clearly as 'incomplete race data' for race predictions")
    ...
    print(f"   - Focus data collection improvements on recent data")
    
    # Option 3: Aggressive cleanup
    print(f"\n3. 🔴 AGGRESSIVE CLEANUP (Not recommended):")
    print(f"   - Delete all {total_races:,} single-dog races")
    print(f"   - Risk: Lose valuable individual dog performance data")
    print(f"   - Risk: May indicate broader data collection issues")
