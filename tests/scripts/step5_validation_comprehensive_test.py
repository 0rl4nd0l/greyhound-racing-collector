...
def test_valid_predictions():
    ...
    try:
        ...
        for i in range(min(3, len(predictions))):
            ...
            logger.info(f"Row {i+1}: {row['dog_clean_name']} -> {row['win_probability']:.4f}")
...
def test_unnormalized_predictions():
    ...
    try:
        ...
        print(f"✓ Correctly detected non-normalized probabilities (sum = {prob_sum:.6f})")
        return True
    except Exception as e:
...
def test_missing_columns():
    ...
    try:
        # Validation assertions from task
        prob_sum = predictions['win_probability'].sum()  # This will fail - column doesn't exist
        assert abs(prob_sum - 1) < 1e-3, "Probabilities not normalized"
        assert all(col in predictions.columns for col in ["dog_clean_name","win_probability"])
...
def test_edge_case_normalization():
    ...
    predictions = pd.DataFrame({
        'dog_clean_name': ['DOG_A', 'DOG_B', 'DOG_C'],
        'win_probability': [0.502, 0.302, 0.202]  # Sum = 1.006 (error = 0.006 > 0.001)
    })
    ...
    try:
        ...
        print(f"✓ EXPECTED FAILURE: {e}")
        print(f"✓ Correctly detected normalization error beyond tolerance (sum = {prob_sum:.6f})")
        return True
    except Exception as e:
        print(f"✗ UNEXPECTED ERROR: {e}")
        return False
...
def test_within_tolerance():
    ...
    try:
        ...
        print(f"✓ PASSED: Within tolerance (sum = {prob_sum:.6f}, error = {abs(prob_sum-1):.6f})")
        print("✓ PASSED: Required columns present")
