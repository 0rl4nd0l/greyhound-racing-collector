...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'debug_call_chain_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class CallChainDebugger:
    def __init__(self, base_url='http://localhost:5002'):
        ...
        self.session.headers.update({
            'User-Agent': 'CallChainDebugger/1.0',
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if races:
                            first_race = races[0]
                            logger.info(f"   First Race Keys: {list(first_race.keys())}")
                            logger.info(f"   First Race Sample: {json.dumps(first_race, indent=2)[:200]}...")
                            ...
                            if len(race_ids) != len(unique_race_ids):
                                logger.warning(f"   ⚠️  DUPLICATE DETECTED: {len(race_ids)} total vs {len(unique_race_ids)} unique race_ids")
                                
                                # Find duplicates
                                from collections import Counter
                                id_counts = Counter(race_ids)
                                duplicates = {race_id: count for race_id, count in id_counts.items() if count > 1}
    ...
    def analyze_csv_file(self, file_path):
        ...
        try:
            ...
            key_columns = ['Race Name', 'race_name', 'Venue', 'venue', 'Dog Name', 'dog_name']
            ...
            if len(df) > 0:
                ...
                logger.info(f"      Sample rows: {json.dumps(sample_data, indent=2, default=str)[:300]}...")
                ...
                if 'Dog Name' in df.columns or 'dog_name' in df.columns:
                    ...
                    logger.info(f"      Unique dogs: {unique_dogs} (suggests this is a race form guide)")
    ...
    def analyze_json_file(self, file_path):
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                logger.info(f"      Sample: {json.dumps(data, indent=2, default=str)[:300]}...")
    ...
    def check_data_integrity(self):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"   ❌ Cannot get data for integrity check: {response.status_code}")
                return
            ...
            if len(race_ids) != len(unique_race_ids):
                logger.warning(f"   ⚠️  DUPLICATE RACE_IDS: {len(race_ids)} total vs {len(unique_race_ids)} unique")
            ...
            if len(filenames) != len(unique_filenames):
                logger.warning(f"   ⚠️  DUPLICATE FILENAMES: {len(filenames)} total vs {len(unique_filenames)} unique")
            ...
            for field in required_fields:
                ...
                if missing_count > 0:
                    logger.warning(f"   ⚠️  {missing_count} races missing '{field}' field")
                else:
    ...
    def capture_reference_files(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"   ❌ Could not capture good response: {response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/upcoming_races_csv?refresh=true",
                timeout=30
            )
...
def main():
    ...
    try:
        ...
        logger.error("❌ Cannot connect to server. Please ensure Flask app is running on localhost:5002")
