...
class ConcurrencyTester:
        ...
        for i in range(num_requests):
            try:
                ...
                if response.status_code != 200:
                    errors += 1
                    print(
                        f"❌ Request {i+1}: {response.status_code} - {response.text[:100]}"
                    )
                else:
    ...
    def test_endpoint_concurrent(
        self, endpoint: str, num_requests: int = 20, max_workers: int = 10
    ) -> Dict[str, Any]:
    ...
    def run_comprehensive_test(self):
        ...
        for endpoint, num_requests, max_workers in test_cases:
            try:
                ...
                # Test concurrent
                concurrent_result = self.test_endpoint_concurrent(
                    endpoint, num_requests, max_workers
                )
    ...
    def generate_report(self, results: List[Dict[str, Any]]):
        ...
        for result in results:
            if "successful_requests" in result and result["successful_requests"] > 0:
                ...
                print(
                    f"   Error Rate: {(result['errors']/result['total_requests']*100):.1f}%"
                )
                ...
                if "avg_response_time" in result:
                    ...
                    print(
                        f"   Median Response Time: {result['median_response_time']:.2f}ms"
                    )
        ...
        if successful_tests:
            ...
            print(
                f"   Overall Success Rate: {(total_successful/total_requests*100):.1f}%"
            )
            ...
            concurrent_results = [
                ...
                if r.get("test_type") == "concurrent" and "avg_response_time" in r
            ]
            if concurrent_results:
                avg_concurrent_time = statistics.mean(
                    [r["avg_response_time"] for r in concurrent_results]
                )
