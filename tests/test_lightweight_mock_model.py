...
def test_lightweight_mock_model_creation():
    ...
    try:
        ...
        if system.feature_columns:
            print(
                f"✅ feature_columns populated: {len(system.feature_columns)} features"
            )
    ...
    try:
        ...
        if system.calibrated_pipeline is not None:
            ...
            if "Mock" in model_type:
                print(
                    "ℹ️ Loaded model is a mock model (this is expected if no real training has occurred)"
                )
            else:
...
if __name__ == "__main__":
    ...
    print(
        "This tests the implementation of calling existing logic when no calibrated_pipeline is detected on disk"
    )
    print()
    ...
    if overall_success:
        ...
        print(
            "   • This avoids re-training while exercising preprocessing, ColumnTransformer, calibration layers, and EV logic"
        )
    else:
        print("\n❌ SOME TESTS FAILED")
