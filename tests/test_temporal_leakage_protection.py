...
class TestTemporalLeakageProtection:
    ...
    def temp_db(self):
        ...
        # Insert test data
        test_races = [
            ('race_001', 'TRACK_A', 'G5', '500m', 'Good', 'Fine', 20.0, 60.0, 10.0, 8, '2024-01-01', '14:30'),
            ('race_002', 'TRACK_A', 'G5', '500m', 'Good', 'Fine', 21.0, 65.0, 12.0, 8, '2024-01-02', '15:00'),
            ('race_003', 'TRACK_B', 'G4', '520m', 'Slow', 'Cloudy', 18.0, 70.0, 15.0, 8, '2024-01-03', '16:30')
        ]
        ...
        test_dogs = [
            ...
            (7, 'race_003', 'DOG_A', 1, 30.4, 2.60, 'TRAINER_1', None, None, None, None),
            (8, 'race_003', 'DOG_E', 2, 29.9, 3.80, 'TRAINER_5', None, None, None, None),
        ]
    ...
    def test_no_post_race_features_in_target_race(self, temp_db):
        ...
        assert len(leakage_features) == 0, f"Found post-race features in target: {leakage_features}"
    ...
    def test_historical_features_present(self, temp_db):
        ...
        historical_features = [col for col in features.columns if col.startswith('historical_')]
        ...
        assert dog_a_features['historical_win_rate'] > 0, "DOG_A should have historical win rate"
    ...
    def test_temporal_order_validation(self, temp_db):
        ...
        for _, row in historical_data.iterrows():
            ...
            assert hist_timestamp < target_timestamp, f"Historical race {row['race_id']} timestamp {hist_timestamp} not before target {target_timestamp}"
...
class TestMLSystemV4:
    ...
    def temp_db_with_more_data(self):
        ...
        for i in range(10):  # 10 races
            ...
            races.append((
                race_id, 'TRACK_A', 'G5', '500m', 'Good', 'Fine',
                20.0 + i * 0.5, 60.0 + i, 10.0 + i * 0.2, 8,
                race_date.strftime('%Y-%m-%d'), '14:30'
            ))
            ...
            for j in range(8):
                ...
                dogs.append((
                    dog_id, race_id, dog_name, box_number, weight, starting_price,
                    trainer, finish_pos, individual_time, sectional, margin
                ))
    ...
    def test_time_ordered_split_no_overlap(self, temp_db_with_more_data):
        ...
        assert train_max_date <= test_min_date, "Test races should occur after training races"
    ...
    def test_group_normalization_sums_to_one(self, temp_db_with_more_data):
        ...
        assert 0.95 <= prob_sum <= 1.05, f"Win probabilities sum to {prob_sum}, not ≈1.0"
    ...
    def test_calibration_metadata(self, temp_db_with_more_data):
        ...
        assert result['calibration_meta']['method'] == 'isotonic', "Should use isotonic calibration"
        
        # Check prediction has calibration info
        prediction = result['predictions'][0]
        assert prediction['calibration_applied'], "Calibration should be applied to prediction"
