...
def test_workflow_integration():
    ...
    try:
        ...
        prediction_data = {
            ...
            'venue': 'TEST_VENUE',
            'predictions': [
                {'dog_name': 'Fast Dog', 'box_number': 1, 'win_prob': 0.45, 'confidence': 0.85},
                {'dog_name': 'Medium Dog', 'box_number': 2, 'win_prob': 0.35, 'confidence': 0.75},
                {'dog_name': 'Slow Dog', 'box_number': 3, 'win_prob': 0.20, 'confidence': 0.65},
                {'dog_name': 'Risky Dog', 'box_number': 4, 'win_prob': 0.10, 'confidence': 0.25}  # Low confidence
            ]
        }
        ...
        if advisory_result['success']:
            ...
            warning_msgs = [m for m in messages if m['type'] == MessageType.WARNING.value]
        ...
        if error_result['success']:
            ...
            print(f"   ⚠️ Failed on minimal data: {error_result.get('error', 'Unknown')}")
        
        # Test with file not found
        file_error_result = advisory_generator.generate_advisory(file_path='/nonexistent/file.json')
...
def test_ui_data_format():
    ...
    try:
        ...
        ui_test_data = {
            ...
            'predictions': [
                {'dog_name': 'UI Dog 1', 'box_number': 1, 'win_prob': 0.6, 'confidence': 0.9},
                {'dog_name': 'UI Dog 2', 'box_number': 2, 'win_prob': 0.4, 'confidence': 0.1}  # Low confidence
            ]
        }
        ...
        if result['success']:
            ...
            if 'feature_flags' in ml_json:
                ...
                expected_flags = ['has_quality_issues', 'low_quality_score', 'has_validation_errors']
            ...
            for i, msg in enumerate(messages):
                required_msg_fields = ['type', 'category', 'title', 'message', 'timestamp']
