...
from utils.file_content_validator import FileContentValidator, validate_file_content, validate_directory_files
...
<p>Please try again later</p>
    ...
    with open(large_csv, "w") as f:
        ...
        for i in range(50):
            f.write(f"Test Dog {i+1},{(i%8)+1},SAN,2025-01-15,{(i%8)+1},32.{i%10},29.{50+i%50},500\n")
...
def test_individual_validation():
    ...
    for file_type, file_path in test_files:
        ...
        if not is_valid and file_info.get('html_indicators'):
            print(f"   HTML indicators found: {', '.join(file_info['html_indicators'][:3])}")
        elif not is_valid and file_info.get('non_csv_patterns'):
            print(f"   Non-CSV patterns found: {', '.join(file_info['non_csv_patterns'][:3])}")
    
    # Test standalone function
    print(f"\n🔧 Testing standalone function...")
    is_valid, message = validate_file_content(test_files[0][1], min_file_size=50)
    print(f"   Standalone result: {'✅ VALID' if is_valid else '❌ INVALID'} - {message}")
