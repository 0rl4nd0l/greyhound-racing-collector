...
assert train_rids.isdisjoint(
    test_rids
), f"Race ID overlap found: {len(train_rids.intersection(test_rids))}"
...
assert pairs_tr.isdisjoint(
    pairs_te
), f"Race-dog overlap found: {len(pairs_tr.intersection(pairs_te))}"
...
assert (
    tr_hist["race_datetime"].max() < te_hist["race_datetime"].min()
