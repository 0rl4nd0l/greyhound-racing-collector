...
def main():
    ...
    for i, (_, dog) in enumerate(race_df.iterrows()):
        print(f"{i+1}. {dog['dog_name']:<15} Score: {dog['normalized_strength_score']:6.2f}")
    ...
    for temp in temperatures:
        ...
        print(f"τ = {temp}: Sum = {prob_sum:.6f} {'✓' if abs(prob_sum - 1.0) < 1e-10 else '✗'}")
    ...
    print("=" * 50)
    print(f"{'Rank':<4} {'Dog Name':<15} {'Probability':<12} {'Percentage':<10} {'Odds':<10}")
    print("-" * 50)
    ...
    for i, (_, row) in enumerate(probabilities_df.iterrows()):
        ...
        print(f"{i+1:<4} {row['dog_name']:<15} {prob:.4f}       {prob*100:5.2f}%     {odds_str:<10}")
    ...
    print(f"✓ Sum of probabilities: {total_prob:.6f} {'(exactly 1.0)' if abs(total_prob - 1.0) < 1e-10 else '(ERROR!)'}")
