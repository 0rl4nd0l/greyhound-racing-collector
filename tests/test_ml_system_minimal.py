...
def test_all_in_one():
    ...
    try:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if race_data.empty:
                ...
                features_df = builder.build_features_for_race(race_data, historical_race_id)
                
                # Check for numpy warnings
                numpy_warnings = [warning for warning in w if 'numpy' in str(warning.message).lower()]
        ...
        if race_data.empty:
            ...
            if result and result.get('success', False):
                ...
                if len(predictions) > 0:
                    ...
                    if prob_sum_check:
                        print(f"✅ Softmax probability sum ≈ 1.0 (actual: {prob_sum:.3f})")
                    ...
                    for i, pred in enumerate(predictions[:3]):
                        print(f"   {i+1}. {pred['dog_clean_name']}: {pred['win_prob_norm']:.3f}")
    ...
    try:
        ...
        if future_race_data.empty:
            print(f"⚠️ No race data found for future race {future_race_id} - using mock data")
            ...
            future_race_data = pd.DataFrame({
                ...
                'race_date': ['22 August 2025', '22 August 2025', '22 August 2025'],  # Use proper format
                'race_time': ['14:30', '14:30', '14:30']
            })
        
        print(f"Attempting prediction on race with date: {future_race_data.iloc[0]['race_date']}")
    ...
    core_tests_passed = (
        test_results['mock_model_creation'] and 
        test_results['feature_construction'] and 
    ...
    if passed_tests == total_tests:
        ...
        print(f"⚠️ {total_tests - passed_tests} critical test(s) failed. Review above for details.")
        return False
