...
class FrontendTester:
    ...
    def test_health_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result(
                    "Health Endpoint", False, f"HTTP {response.status_code}"
                )
        ...
        for url, name in pages:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_result(
                        f"Page: {name}", False, f"HTTP {response.status_code}"
                    )
            except Exception as e:
    ...
    def test_single_prediction(self):
        ...
        try:
            data = {"race_files": "Race 1 - AP_K - 2025-08-04.csv", "action": "single"}
                ...
                if "prediction-json" in html_content:
                    ...
                    if json_match:
                        try:
                            ...
                            winner = (
                                prediction_data["predictions"][0]["dog_name"]
                            ...
                            self.log_result(
                                "Single Prediction", False, f"JSON decode error: {e}"
                            )
                    else:
                        self.log_result(
                            "Single Prediction",
                            False,
                            "Could not extract JSON from HTML",
                        )
                else:
                    self.log_result(
                        "Single Prediction",
                        False,
                        "No prediction JSON found in response",
                    )
            else:
                self.log_result(
                    "Single Prediction", False, f"HTTP {response.status_code}"
                )
        except Exception as e:
    ...
    def test_multiple_predictions(self):
        ...
        races = ["Race 1 - AP_K - 2025-08-04.csv", "Race 2 - AP_K - 2025-08-04.csv"]

        successful_predictions = 0

        for race in races:
            try:
                data = {"race_files": race, "action": "single"}
                response = requests.post(
                    f"{self.base_url}/predict", data=data, timeout=30
                )
    ...
    def test_batch_prediction_api(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/predict_batch", json=payload, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                self.log_result(
                    "Batch Prediction API", False, f"HTTP {response.status_code}"
                )
        except Exception as e:
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        self.log_result(
                            f"API: {name}", True, "Valid JSON response", data
                        )
                    except json.JSONDecodeError:
                        self.log_result(f"API: {name}", False, "Invalid JSON response")
                else:
                    self.log_result(
                        f"API: {name}", False, f"HTTP {response.status_code}"
                    )
            except Exception as e:
