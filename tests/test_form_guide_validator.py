...
class TestFormGuideValidator:
    ...
    def test_line_ending_normalization(self, tmp_path):
        ...
        line_ending_issues = [
            issue for issue in issues if "line endings" in issue.message
        ]
    ...
    def test_mixed_delimiters_detection(self, tmp_path):
        ...
        mixed_delimiter_issues = [
            issue for issue in issues if "Mixed delimiters" in issue.message
        ]
        assert len(mixed_delimiter_issues) > 0
        assert all(
            issue.severity == ValidationSeverity.WARNING
            for issue in mixed_delimiter_issues
        )
    ...
    def test_invisible_unicode_characters(self, tmp_path):
        ...
        invisible_char_issues = [
            issue for issue in issues if "invisible Unicode" in issue.message
        ]
        assert len(invisible_char_issues) > 0
        assert all(
            issue.severity == ValidationSeverity.WARNING
            for issue in invisible_char_issues
        )
    ...
    def test_embedded_newlines_detection(self, tmp_path):
        ...
        embedded_newline_issues = [
            issue for issue in issues if "Embedded newline" in issue.message
        ]
        assert len(embedded_newline_issues) > 0
        assert all(
            issue.severity == ValidationSeverity.WARNING
            for issue in embedded_newline_issues
        )
...
"""
        ...
        forward_fill_issues = [
            issue for issue in issues if "Forward-filled" in issue.message
        ]
...
"""
        ...
        dog_name_errors = [
            issue
            for issue in issues
            if "dog name" in issue.message.lower()
...
"""
        ...
        box_errors = [
            issue
            for issue in issues
            if "Box number" in issue.message and "outside valid range" in issue.message
        ]
...
"""
        ...
        weight_warnings = [
            issue
            for issue in issues
            if "Weight" in issue.message and "outside typical range" in issue.message
        ]
    ...
    def test_comprehensive_edge_case(self, tmp_path):
        ...
        assert (
            ValidationSeverity.WARNING in issue_types
        )  # Mixed delimiters, embedded newlines, weight range
