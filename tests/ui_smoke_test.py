...
def run_ui_smoke_test():
    ...
    try:
        ...
        try:
            ...
            page_source_path = f'audit_results/{AUDIT_TS}/ui_test/page_source_error.html'
        ...
        try:
            predict_button = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, "//button[contains(text(), 'Predict Single')]"))
            )
        ...
        try:
            ...
            if prediction_text:
                ...
                page_source_path = f'audit_results/{AUDIT_TS}/ui_test/page_source_no_results.html'
                with open(page_source_path, 'w') as f:
                    f.write(driver.page_source)
        ...
        if test_results['prediction_results']:
            ...
            if isinstance(prediction_data, dict):
                ...
                if validation_passed:
                    ...
                    test_results['steps_completed'].append('structure_validation_passed')
                else:
        ...
        if len(test_results['errors']) == 0:
            ...
            print("⚠️  UI smoke test partially successful - UI works but no prediction results")
        else:
    ...
    try:
        ...
        with open(audit_log_path, 'a') as f:
            ...
            f.write(f"Prediction Results: {'Found' if test_results['prediction_results'] else 'Not Found'}\n")
            if test_results['errors']:
                f.write(f"Error Details: {', '.join(test_results['errors'])}\n")
            f.write("Artifacts: smoke_test_results.json, screenshots\n")
            f.write(f"Overall Status: {'COMPLETED' if test_results['test_status'] != 'FAIL' else 'FAILED'}\n")
            f.write("\n")
