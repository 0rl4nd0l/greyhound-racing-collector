...
class TestArchiveRedundantSources(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.legacy_ingestion_dir_patch = patch('archive_redundant_sources.LEGACY_INGESTION_DIR', self.archive_dir)
        self.legacy_scripts_patch = patch('archive_redundant_sources.LEGACY_SCRIPTS', [os.path.join(self.test_dir, p) for p in LEGACY_SCRIPTS])
    ...
    def test_execute_moves_files_and_creates_manifest(self):
        ...
        for script_path in original_script_paths:
            ...
            archived_path = os.path.join(self.archive_dir, os.path.basename(script_path))
    ...
    def test_conflict_resolution(self):
        ...
        archive_conflict_path = os.path.join(self.archive_dir, os.path.basename(conflicting_script_path))
        ...
        for f in archived_files:
            if f.startswith(base) and f.endswith(ext) and f != os.path.basename(conflicting_script_path):
