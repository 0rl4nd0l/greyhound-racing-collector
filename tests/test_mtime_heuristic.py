...
try:
    from utils.mtime_heuristic import create_mtime_heuristic, FileEntry
...
def test_basic_functionality():
    ...
    with tempfile.TemporaryDirectory(prefix="mtime_test_") as temp_dir:
        ...
        try:
            ...
            files_found = list(heuristic.scan_directory_optimized(
                str(test_dir), 
                strict_scan=True
            ))
            print(f"  Files found: {len(files_found)}")
            assert len(files_found) == len(created_files), f"Should find all {len(created_files)} files"
            ...
            print(f"  Set mtime: {datetime.fromtimestamp(middle_file_mtime).strftime('%Y-%m-%d %H:%M:%S')}")
            
            # Test 5: Optimized scan (should only find newer files)
            print("\n📋 Test 5: Optimized scan (should find only newer files)")
            newer_files = list(heuristic.scan_directory_optimized(
                str(test_dir), 
                strict_scan=False
            ))
            print(f"  Files found: {len(newer_files)}")
            expected_newer = sum(1 for _, mtime in created_files if mtime > middle_file_mtime)
            assert len(newer_files) == expected_newer, f"Should find {expected_newer} newer files, found {len(newer_files)}"
            ...
            assert updated_mtime == expected_max_mtime, f"Mtime should be updated to {expected_max_mtime}, got {updated_mtime}"
            ...
            assert stats['last_processed_mtime'] is not None, "Should have last processed mtime"
...
def test_with_user_directory(test_dir: str):
    ...
    try:
        ...
        full_scan_files = list(heuristic.scan_directory_optimized(
            test_dir, 
            strict_scan=True
        ))
        ...
        if full_scan_files:
            ...
            for i, file_entry in enumerate(full_scan_files[:3]):
                mtime_str = datetime.fromtimestamp(file_entry.mtime).strftime("%Y-%m-%d %H:%M:%S")
        ...
        optimized_files = list(heuristic.scan_directory_optimized(
            test_dir, 
            strict_scan=False
        ))
        ...
        if len(optimized_files) != len(full_scan_files):
            print(f"  🚀 Optimization filtered out {len(full_scan_files) - len(optimized_files)} files")
        else:
            print("  ℹ️ No optimization applied (no previous mtime or all files are newer)")
...
def main():
    ...
    if success:
        ...
        print("\nNext steps:")
        print("  1. Run migration to add db_meta table: python migrations/add_db_meta_table.py")
        print("  2. Use optimized scanning: python run.py analyze")
        print("  3. Force full re-scan when needed: python run.py analyze --strict-scan")
    else:
