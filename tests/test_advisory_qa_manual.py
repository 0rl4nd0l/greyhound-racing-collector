...
class TestAdvisoryManualQA:
    ...
    def test_single_prediction_advisory_high_quality(self):
        ...
        quality_msgs = [
            m for m in result["messages"] if m["category"] == "quality_assessment"
        ]
        assert len(quality_msgs) >= 1, "Should have quality assessment message"

        quality_msg = quality_msgs[0]
        assert (
            quality_msg["type"] == MessageType.INFO.value
        ), "High quality should be INFO type"
    ...
    def test_single_prediction_advisory_low_quality(self):
        ...
        assert (
            result["success"] == True
        ), "Advisory generation should succeed even with bad data"
        ...
        warning_critical_msgs = [
            m
            for m in result["messages"]
            if m["type"] in [MessageType.WARNING.value, MessageType.CRITICAL.value]
        ]
        assert (
            len(warning_critical_msgs) >= 1
        ), "Should have warning or critical messages for low quality data"

        print(
            f"✅ Low quality advisory generated with {len(warning_critical_msgs)} warning/critical messages"
        )
    ...
    def test_batch_prediction_advisory(self):
        ...
        for test_file in test_files:
            ...
            batch_results.append({"file": test_file.name, "result": result})
        ...
        ), "All batch files should be processed successfully"

        print("✅ Batch processing completed:")
        print(f"   Files processed: {len(batch_results)}")
        print(f"   Total processing time: {total_processing_time:.1f}ms")
        print(
            f"   Average time per file: {total_processing_time/len(batch_results):.1f}ms"
        )
        ...
        assert (
            quality_distribution["INFO"] >= 1
        ), "Should have at least one high quality case"
        assert (
            quality_distribution["WARNING"] >= 1
        ), "Should have at least one warning case"
    ...
    def test_advisory_auto_vs_manual_mode(self):
        ...
        auto_critical = [
            m
            for m in auto_result["messages"]
            if m["type"] == MessageType.CRITICAL.value
        ]
        manual_critical = [
            m
        ...
        assert len(auto_critical) == len(
            manual_critical
        ), "Both modes should identify same critical issues"
        ...
        if auto_result.get("openai_used", False):
            ...
            assert (
                auto_summary != manual_summary
            ), "Auto and manual summaries should differ when OpenAI is available"
    ...
    def test_error_simulation_backend_disconnect(self):
        ...
        invalid_file_result = self.advisory_generator.generate_advisory(
        ...
        assert (
            error_msg["type"] == MessageType.CRITICAL.value
        ...
        invalid_json_result = self.advisory_generator.generate_advisory(
        ...
        print(
            f"    ✅ Empty data handled with {len(empty_result['messages'])} messages"
        )
        ...
        with patch.object(self.advisory_generator, "openai_client") as mock_client:
            ...
            api_failure_result = self.advisory_generator.generate_advisory(
                data=test_data
            )

            # Should succeed with fallback to template mode
            assert (
                api_failure_result["success"] == True
            ), "Should fallback to template mode"
            assert (
                api_failure_result.get("openai_used", True) == False
            ), "Should indicate OpenAI not used"

            print("    ✅ OpenAI failure handled with fallback to template mode")
    ...
    def test_api_advisory_integration(self):
        ...
        if app_running:
            ...
            try:
                ...
                ), f"API should return 200, got {api_response.status_code}"

                api_result = api_response.json()
                assert (
                    api_result.get("success", False) == True
                ), "API advisory should succeed"
                assert (
                    len(api_result.get("messages", [])) >= 1
                ), "API should return messages"

                print(
                    f"    ✅ API advisory endpoint working: {len(api_result['messages'])} messages"
                )
    ...
    def test_ui_integration_simulation(self):
        ...
        for flag in expected_flags:
            ...
            ), f"Feature flag '{flag}' should be present for UI"

        print("✅ UI simulation completed:")
        ...
        assert (
            message_types.get(MessageType.WARNING.value, 0) >= 1
        ), "Should have warning messages for UI"
    ...
    def test_performance_responsiveness(self):
        ...
        print("  ✅ Single advisory performance:")
        ...
        assert (
            processing_time < 5.0
        ), "Single advisory should complete in under 5 seconds"
        ...
        for i in range(5):
            ...
            concurrent_result = self.advisory_generator.generate_advisory(
                data=test_data_concurrent
            )
            concurrent_results.append(concurrent_result)

        concurrent_time = time.time() - concurrent_start

        print("  ✅ Concurrent advisory performance:")
        ...
        assert (
            len(successful_concurrent) == 5
        ), "All concurrent advisories should succeed"

        # Total time should be reasonable
        assert (
            concurrent_time < 15.0
        ), "5 concurrent advisories should complete in under 15 seconds"
    ...
    def test_workflow_integration_no_blocking(self):
        ...
        prediction_data = {
            ...
            "predictions": [
                {
                    "dog_name": "Workflow Dog 1",
                    "box_number": 1,
                    "win_prob": 0.45,
                    "confidence": 0.85,
                },
                {
                    "dog_name": "Workflow Dog 2",
                    "box_number": 2,
                    "win_prob": 0.35,
                    "confidence": 0.75,
                },
                {
                    "dog_name": "Workflow Dog 3",
                    "box_number": 3,
                    "win_prob": 0.20,
                    "confidence": 0.65,
                },
        ...
        advisory_result = self.advisory_generator.generate_advisory(
        ...
        assert (
            advisory_time < 2.0
        ), "Advisory should not add significant delay to workflow"

        # Advisory should provide useful information without blocking
        assert (
            len(advisory_result["messages"]) >= 1
        ), "Advisory should provide information"
        assert (
            advisory_result.get("ml_json", {})
            .get("summary", {})
            .get("total_messages", 0)
            >= 1
        ), "Should have summary for downstream processing"
