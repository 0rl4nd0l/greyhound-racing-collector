...
class TestSanityChecks(unittest.TestCase):
    ...
    def test_invalid_probability_range(self):
        ...
        self.assertTrue(
            any(
                "Win probability out of range [0, 1] for Dog A" in flag
    ...
    def test_rank_misalignment(self):
        ...
        self.assertTrue(
            any(
                "Rank by probability does not align with numeric rank output" in flag
                for flag in result["flags"]
            )
        )
    ...
    def test_duplicate_rank(self):
        ...
        result = self.sanity_checker.validate_predictions(predictions)
        self.assertTrue(
            any("Duplicate numeric ranks found" in flag for flag in result["flags"])
        )
