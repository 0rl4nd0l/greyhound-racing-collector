...
def test_temporal_feature_builder_contextual_adjustments():
    ...
    try:
        ...
        historical_data = pd.DataFrame({
            ...
            'race_timestamp': pd.to_datetime(['2025-01-01', '2025-01-02', '2025-01-03', '2025-01-04', '2025-01-05'])
        })
        ...
        logger.info(f"✅ Ballarat targeting - Average time: {features_ballarat['historical_avg_time']:.2f}s")
        ...
        logger.info(f"✅ SANDOWN targeting - Average time: {features_other['historical_avg_time']:.2f}s")
        ...
        logger.info(f"✅ Distance adjustment - Average time: {features_distance['historical_avg_time']:.2f}s")
        
        logger.info("✅ TemporalFeatureBuilder contextual adjustments working correctly!")
        return True
...
def test_dog_performance_contextual_adjustments():
    ...
    try:
        ...
        mock_data = pd.DataFrame({
            ...
            'race_date': pd.to_datetime(['2025-01-01', '2025-01-02', '2025-01-03', '2025-01-04', '2025-01-05'])
        })
        
        # Test venue-specific features with Ballarat targeting
        logger.info("Test 1: Venue-specific features with Ballarat targeting")
        venue_features = engineer._calculate_venue_specific_features(mock_data, target_venue='BALLARAT')
        
        # Should find Ballarat races and calculate stats
        assert venue_features['ballarat_experience'] > 0
        logger.info(f"✅ Ballarat experience: {venue_features['ballarat_experience']} races")
        ...
        venue_features_other = engineer._calculate_venue_specific_features(mock_data, target_venue='SANDOWN')
        
        # Should find SANDOWN races
        logger.info(f"✅ Venue experience for SANDOWN: {venue_features_other.get('ballarat_experience', 0)} races")
        
        logger.info("✅ DogPerformanceFeatureEngineer contextual adjustments working correctly!")
        return True
...
2. Test Dog 2,2,Test Trainer,31.0,4.00,G5,400"""
            ...
            # Make API request
            response = client.post('/api/predict_single_race_enhanced', 
            ...
            if response.status_code == 404:
                ...
                logger.info(f"✅ API endpoint responded with status: {response.status_code}")
