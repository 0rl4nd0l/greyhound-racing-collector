...
class TestPrefilterCaching:
    ...
    def temp_environment(self):
        ...
        assert ensure_processed_files_table(db_path), "Failed to create test database table"
...
"""
    ...
    def _mark_files_as_processed(self, db_path: str, csv_files: List[str], 
                                count_to_mark: int = 990) -> Set[str]:
        ...
        try:
            for i in range(count_to_mark):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (file_hash, race_date, venue, race_no, filepath, file_size, 'processed'))
    ...
    def test_create_1000_mock_csv_files(self, temp_environment):
        ...
        assert existing_files == 1000, f"Expected 1000 files on disk, found {existing_files}"
    ...
    def test_mark_990_files_as_processed(self, temp_environment):
        ...
        marked_filenames = self._mark_files_as_processed(db_path, csv_files, count_to_mark=990)
        
        # Verify correct number marked
        assert len(marked_filenames) == 990, f"Expected 990 marked files, got {len(marked_filenames)}"
        
        # Verify database entries
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        cursor.execute("SELECT COUNT(*) FROM processed_race_files WHERE status = 'processed'")
        processed_count = cursor.fetchone()[0]
        assert processed_count == 990, f"Expected 990 processed entries, got {processed_count}"
    ...
    def test_prefilter_returns_only_10_unprocessed(self, temp_environment):
        ...
        assert len(processed_filenames_set) == 990, f"Expected 990 cached files, got {len(processed_filenames_set)}"
        
        # Verify all marked files are in the cached set
        missing_from_cache = marked_filenames - processed_filenames_set
        assert len(missing_from_cache) == 0, f"Missing {len(missing_from_cache)} files from cache"
        ...
        assert len(unprocessed_filenames) == 10, f"Expected 10 unprocessed files, got {len(unprocessed_filenames)}"
        
        print(f"✅ Pre-filter correctly identified {len(unprocessed_filenames)} unprocessed files")
    ...
    def test_early_exit_optimizer_with_cached_directory(self, temp_environment):
        ...
        # Verify scan results
        assert scan_result.total_files == 1000, f"Expected 1000 total files, got {scan_result.total_files}"
        assert scan_result.processed_files == 990, f"Expected 990 processed files, got {scan_result.processed_files}"
        assert scan_result.unprocessed_files == 10, f"Expected 10 unprocessed files, got {scan_result.unprocessed_files}"
        
        # Verify cache ratio calculation
        expected_ratio = 990 / 1000  # 0.99
        assert abs(scan_result.cache_ratio - expected_ratio) < 0.001, \
            f"Expected cache ratio ~{expected_ratio}, got {scan_result.cache_ratio}"
        
        # Verify early exit decision
        assert scan_result.should_early_exit, "Early exit should be triggered with 99% cache ratio"
        
        print(f"✅ Early exit optimizer correctly analyzed directory:")
    ...
    def test_early_exit_optimizer_get_unprocessed_files(self, temp_environment):
        ...
        assert len(unprocessed_file_paths) == 10, \
        ...
        for file_path in unprocessed_file_paths:
            assert os.path.exists(file_path), f"Unprocessed file doesn't exist: {file_path}"
            filename = os.path.basename(file_path)
            assert filename not in processed_filenames_set, \
        ...
        actual_unprocessed_files = [os.path.basename(fp) for fp in unprocessed_file_paths]
        
        # Sort both lists for comparison
        expected_unprocessed_files.sort()
        actual_unprocessed_files.sort()
        
        assert set(expected_unprocessed_files) == set(actual_unprocessed_files), \
            "Mismatch between expected and actual unprocessed files"
        
        print(f"✅ Early exit optimizer quickly found {len(unprocessed_file_paths)} unprocessed files")
        print(f"   Sample unprocessed files:")
    ...
    def test_performance_o1_lookup(self, temp_environment):
        ...
        print(f"✅ O(1) performance test passed:")
    ...
    def test_edge_cases(self, temp_environment):
        ...
        assert len(processed_set) == 0, "Empty directory should return empty processed set"
        ...
        assert len(processed_set) == 0, "Non-existent directory should return empty processed set"
        ...
        assert len(processed_set) == 0, "Mixed directory with no processed files should return empty set"
    ...
    def test_large_scale_verification(self, temp_environment):
        ...
        marked_filenames = self._mark_files_as_processed(db_path, csv_files, count_to_mark=4750)
        ...
        # Verify accuracy
        assert len(processed_filenames_set) == 4750, \
        ...
        # Verify early exit decision
        assert should_exit, "Early exit should be triggered with 95% cache ratio and 250 unprocessed files"
        assert scan_result.total_files == 5000, f"Expected 5000 total files, got {scan_result.total_files}"
        assert scan_result.processed_files == 4750, f"Expected 4750 processed files, got {scan_result.processed_files}"
        assert scan_result.unprocessed_files == 250, f"Expected 250 unprocessed files, got {scan_result.unprocessed_files}"
