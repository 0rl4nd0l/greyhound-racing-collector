        ...
        if hasattr(logger, "debug_mode") and logger.debug_mode:
            ...
            if dog_count != expected_dogs:
                logger.debug_logger.debug(
                    f"⚠️ Race {race_id}: Dog count deviation detected! Expected {expected_dogs}, found {dog_count}"
                )
            ...
            if empty_dog_names > 0:
                logger.debug_logger.debug(
                    f"🔄 Race {race_id}: Forward-fill used for {empty_dog_names} rows (greyhound form guide format)"
                )
