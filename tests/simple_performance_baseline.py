...
'''
    ...
    def analyze_page_resources(self):
        ...
        try:
            ...
            resources = {
                ...
                'estimated_dom_nodes': html_content.count('<') + html_content.count('</'),
            }
    ...
    def estimate_performance_metrics(self, curl_results, api_results, resource_analysis):
        ...
        for key in curl_results[0].keys():
            if isinstance(curl_results[0][key], (int, float)):
                values = [r[key] for r in curl_results if isinstance(r.get(key), (int, float))]
                if values:
        ...
        metrics = {
            ...
            'tcp_connection_time': (avg_metrics.get('time_connect', 0) - avg_metrics.get('time_namelookup', 0)) * 1000,
            'ssl_handshake_time': (avg_metrics.get('time_appconnect', 0) - avg_metrics.get('time_connect', 0)) * 1000,
            ...
            'estimated_dom_content_loaded': avg_metrics.get('time_starttransfer', 0) * 1000 + 500,  # TTFB + 500ms
            'estimated_time_to_interactive': avg_metrics.get('time_total', 0) * 1000 + 1000,  # Total + 1s
    ...
    def generate_lighthouse_style_report(self, metrics):
        ...
        def score_metric(value, good_threshold, poor_threshold):
            ...
            if value <= good_threshold:
                ...
                return int(100 * (poor_threshold - value) / (poor_threshold - good_threshold))
        ...
        scored_metrics = {
            ...
            'time-to-interactive': {
                ...
                'score': score_metric(metrics.get('estimated_time_to_interactive', 0), 3000, 6000),
                'unit': 'ms',
        ...
        return {
            ...
            'network': {
                ...
                'estimated_requests': metrics.get('resource_analysis', {}).get('css_files', 0) + 
                                   metrics.get('resource_analysis', {}).get('js_files', 0) + 
                                   metrics.get('resource_analysis', {}).get('img_tags', 0) + 10  # Base requests
            },
            'raw_metrics': metrics
        }
    ...
    def save_reports(self, lighthouse_report, raw_metrics):
        ...
        try:
            ...
            lighthouse_file = os.path.join(REPORTS_DIR, f'baseline-lighthouse-{self.timestamp}.json')
            with open(lighthouse_file, 'w') as f:
                json.dump(lighthouse_report, f, indent=2)
            
            # Save raw metrics
            metrics_file = os.path.join(REPORTS_DIR, f'baseline-metrics-{self.timestamp}.json')
            with open(metrics_file, 'w') as f:
                json.dump(raw_metrics, f, indent=2)
            
            # Generate markdown summary
            summary = self.generate_summary_report(lighthouse_report)
            summary_file = os.path.join(REPORTS_DIR, f'baseline-summary-{self.timestamp}.md')
...
"""
        ...
        if api_perf:
            ...
            for endpoint, stats in api_perf.items():
                status = '✅ Good' if stats['avg_response_time'] < 200 else '⚠️ Slow' if stats['avg_response_time'] < 500 else '❌ Very Slow'
                summary += f"| {endpoint} | {stats['avg_response_time']:.2f}ms | {status} |\n"
...
"""
        ...
        if metrics['first-byte']['score'] < 75:
            recommendations.append("- **Improve Server Response Time:** Consider caching, database optimization, or CDN")
        if metrics['time-to-interactive']['score'] < 75:
            recommendations.append("- **Reduce JavaScript Execution Time:** Optimize or defer non-critical scripts")
        if network['total_size'] > 1024 * 1024:  # > 1MB
            recommendations.append("- **Reduce Page Size:** Compress images, minify CSS/JS, remove unused resources")
        if network['estimated_requests'] > 50:
            recommendations.append("- **Reduce HTTP Requests:** Combine files, use sprites, implement lazy loading")
    ...
    def display_results(self, lighthouse_report):
        ...
        print(f"🎯 Time to Interactive: {metrics['time-to-interactive']['value']:.2f}ms") 
    ...
    def run_profiling(self):
        ...
        # Calculate metrics
        metrics = self.estimate_performance_metrics(curl_results, api_results, resource_analysis)
