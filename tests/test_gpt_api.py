...
def test_gpt_enhance_multiple_with_real_api(client):
    ...
    response = client.post(
        "/api/gpt/enhance_multiple", json={"race_files": race_files, "max_races": 1}
    )
...
def test_api_error_handling_without_api_key(client):
    ...
    try:
        ...
        response = client.post(
            "/api/gpt/enhance_multiple",
            json={"race_files": ["sample_data/test_race_1.csv"], "max_races": 1},
        )
        ...
        response = client.post(
            "/api/gpt/comprehensive_report", json={"race_ids": ["test_race_1.csv"]}
        )
