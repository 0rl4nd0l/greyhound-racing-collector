...
class AutomatedBacktestingSystem:
    def __init__(self, db_path="greyhound_racing_data.db"):
        ...
        self.model_configs = {
            ...
            'gradient_boosting': {
                ...
                'params': {'n_estimators': 100, 'learning_rate': 0.2, 'max_depth': 5, 'random_state': 42}
            },
            'logistic_regression': {
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if len(historical_data) >= 1:
                    ...
                    # Market data
                    odds = [float(o) for o in historical_data['starting_price'] 
            ...
            # Feature columns
            feature_columns = [
                'recent_form_avg', 'win_rate', 'avg_position', 'market_confidence',
                'current_box', 'current_weight', 'field_size'
            ]
            ...
            # Fill missing values
            complete_df = enhanced_df[feature_columns + ['is_winner', 'is_placer', 'race_date', 'race_id']].copy()
    ...
    def run_automated_backtest(self):
        ...
        print(f"🚀 Running automated backtest - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        try:
            ...
            print(f"📊 Testing with {len(prepared_df)} samples and {len(feature_columns)} features")
            ...
            for model_name, config in self.model_configs.items():
                ...
                if model_name == 'random_forest':  # Our best model from previous analysis
                    model_file = self.models_dir / f"best_model_{datetime.now().strftime('%Y%m%d')}.joblib"
                    joblib.dump({
                        'model': model,
                        'scaler': scaler,
        ...
        try:
            ...
            for model_name in current_results['model_results']:
                if model_name in last_results['model_results']:
                    ...
                    accuracy_change_pct = (accuracy_change / last_acc) * 100 if last_acc > 0 else 0
                    ...
                    # Check for alerts
                    if accuracy_change < -self.performance_thresholds['accuracy_drop_critical']:
                        alert = {
                            'level': 'CRITICAL',
                            'model': model_name,
                        ...
                        print(f"🚨 CRITICAL: {model_name} accuracy dropped {accuracy_change_pct:.1f}%")
                        
                    elif accuracy_change < -self.performance_thresholds['accuracy_drop_warning']:
                        alert = {
                            'level': 'WARNING',
                            'model': model_name,
                        ...
                        drift_analysis['alerts'].append(alert)
                        print(f"⚠️ WARNING: {model_name} accuracy dropped {accuracy_change_pct:.1f}%")
                    
                    elif accuracy_change > 0.02:  # 2% improvement
                        print(f"✅ GOOD: {model_name} accuracy improved {accuracy_change_pct:.1f}%")
            
            # Save drift analysis
            drift_file = self.results_dir / f"drift_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def send_performance_alerts(self, alerts):
        ...
        # Save alerts to file
        alert_file = self.results_dir / f"alerts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        try:
            ...
            for model_name in df['model'].unique():
                ...
                summary[model_name] = {
                    ...
                    'accuracy_trend': self.calculate_trend(model_data['accuracy'].values),
            ...
            for model_name, stats in summary.items():
                trend_symbol = "📈" if stats['accuracy_trend'] > 0 else "📉" if stats['accuracy_trend'] < 0 else "➡️"
                print(f"{model_name}:")
...
def main():
    ...
    parser.add_argument('--command', choices=['run', 'schedule', 'stop', 'report', 'status'], 
    ...
    parser.add_argument('--time', default='02:00', help='Time for scheduled runs (HH:MM)')
