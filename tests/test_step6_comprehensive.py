        ...
        csv_files = [
            {
                ...
                'headers': ['Race Name', 'Venue', 'Race Date', 'Distance', 'Grade', 'Race Number'],
...
Dummy Race One,WPK,2025-02-01,500m,Grade 5,1'''
            },
            {
                ...
                'headers': ['Venue', 'Race_Date', 'Race_Name', 'Distance', 'Grade', 'Race_Number'],
...
MEA,2025-02-02,Dummy Race Two,520m,Grade 4,2'''
            },
            {
                ...
                'headers': ['Dog Name', 'Box Number', 'Weight', 'Trainer Name'],  # Minimal headers
...
Dummy Dog Gamma,3,30.8,Dummy Trainer C'''
    ...
    def test_requirement_1_temp_dir_three_csvs_correct_count(self, client, create_three_dummy_csvs):
        """
        ...
        assert len(data['races']) == 3, f"API should return exactly 3 races, got {len(data['races'])}"
        assert data['pagination']['total_count'] == 3, f"Total count should be 3, got {data['pagination']['total_count']}"
        
        print("✅ REQUIREMENT 1 PASSED: API returns correct count (3) for three dummy CSV files")
    
    def test_requirement_2_api_structure_validation(self, client, create_three_dummy_csvs):
        """
        ...
        required_top_level_fields = ['success', 'races', 'pagination', 'sort_by', 'order', 'search']
        ...
        assert race['winner_name'] == 'Unknown', "Upcoming races should have winner_name='Unknown'"
        assert race['winner_odds'] == 'N/A', "Upcoming races should have winner_odds='N/A'"
        assert race['winner_margin'] == 'N/A', "Upcoming races should have winner_margin='N/A'"
        assert race['url'] == '', "Upcoming races should have empty URL"
        assert race['track_condition'] == 'Unknown', "Upcoming races should have track_condition='Unknown'"
        
        print("✅ REQUIREMENT 2 PASSED: API returns correct structure for all races")
    
    def test_requirement_3_different_csv_headers_parsing(self, client, create_three_dummy_csvs):
        """
        ...
        assert csv1_race['race_name'] == 'Dummy Race One', "CSV1: Race name should be parsed from headers"
        assert csv1_race['venue'] == 'WPK', "CSV1: Venue should be parsed from headers"
        assert csv1_race['race_date'] == '2025-02-01', "CSV1: Race date should be parsed from headers"
        assert csv1_race['distance'] == '500m', "CSV1: Distance should be parsed from headers"
        assert csv1_race['grade'] == 'Grade 5', "CSV1: Grade should be parsed from headers"
        assert csv1_race['race_number'] == 1, "CSV1: Race number should be parsed from headers"
        ...
        assert csv2_race['race_name'] == 'Dummy Race Two', "CSV2: Race name should be parsed from underscore headers"
        assert csv2_race['venue'] == 'MEA', "CSV2: Venue should be parsed from underscore headers"
        assert csv2_race['race_date'] == '2025-02-02', "CSV2: Race date should be parsed from underscore headers"
        assert csv2_race['distance'] == '520m', "CSV2: Distance should be parsed from underscore headers"
        assert csv2_race['grade'] == 'Grade 4', "CSV2: Grade should be parsed from underscore headers"
        assert csv2_race['race_number'] == 2, "CSV2: Race number should be parsed from underscore headers"
        
        # Test CSV 3: Minimal headers (should extract from filename)
        csv3_race = race_by_filename['dummy_race_3_GOSF_2025-02-03.csv']
        assert csv3_race['venue'] == 'GOSF', "CSV3: Venue should be extracted from filename"
        assert csv3_race['race_date'] == '2025-02-03', "CSV3: Race date should be extracted from filename"
        assert csv3_race['race_number'] == 3, "CSV3: Race number should be extracted from filename"
        # Race name should fallback to filename-based name
        assert 'dummy_race_3' in csv3_race['race_name'].lower() or csv3_race['race_name'] == 'dummy_race_3_GOSF_2025-02-03', "CSV3: Race name should be derived from filename"
        
        print("✅ REQUIREMENT 3 PASSED: Three different CSV header formats parsed correctly")
    
    def test_requirement_4_pagination_functionality(self, client, create_three_dummy_csvs):
        """
    ...
    def test_requirement_5_search_functionality(self, client, create_three_dummy_csvs):
        ...
        data = response.get_json()
        assert len(data['races']) == 1, "Search for 'Dummy Race Two' should return 1 race"
        assert data['races'][0]['race_name'] == 'Dummy Race Two', "Found race should have correct race name"
        ...
        assert 'GOSF' in data['races'][0]['filename'], "Found race filename should contain 'GOSF'"
        
        # Search with no results
        response = client.get('/api/upcoming_races_csv?search=NONEXISTENT_VENUE')
        data = response.get_json()
        assert len(data['races']) == 0, "Search for non-existent term should return 0 races"
        assert data['pagination']['total_count'] == 0, "Total count should be 0 for no results"
    ...
    def test_requirement_6_sorting_functionality(self, client, create_three_dummy_csvs):
        ...
        assert races[1]['race_date'] == '2025-02-02', "Second race should be middle date"
        ...
        assert venues == sorted(venues), f"Venues should be sorted alphabetically: {venues}"
        
        print("✅ REQUIREMENT 6 PASSED: Sorting functionality works correctly")
    
    def test_requirement_7_race_id_generation_consistency(self, client, create_three_dummy_csvs):
        """
        ...
        for race in races:
            ...
            assert race['race_id'] == expected_race_id, f"Race ID should be MD5 hash of filename for {race['filename']}"
...
Melbourne Cup Heat 1,MEA,2025-03-15,520m,Group 1,1,Rapid Fire,6,30.6,Diana Evans'''
        ...
        realistic_file = os.path.join(upcoming_dir, 'MEA_Melbourne_Cup_Heat_1_2025-03-15.csv')
...
Test Race {i+1},{venue},2025-02-{i+1:02d},500m,Grade {(i%5)+1},{i+1}'''
        ...
        try:
            ...
            response = client.get('/api/upcoming_races_csv?search=WPK&page=1&per_page=5')
            data = response.get_json()
            ...
            assert data['pagination']['total_pages'] == 2  # 6 races / 3 per page = 2 pages
