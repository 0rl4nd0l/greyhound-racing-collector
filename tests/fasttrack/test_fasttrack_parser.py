...
def load_html_sample(filename):
    ...
    base_dir = os.path.dirname(os.path.dirname(__file__))  # This should be the 'tests' directory
...
def test_parse_race_result_structure(scraper, race_result_html):
    ...
    expected_keys = ['race_id', 'race_name', 'venue', 'race_date', 'distance', 'grade', 'track_condition', 'results']
    for key in expected_keys:
        assert key in parsed_data

@pytest.mark.parametrize("dog_result_index, expected_fields", [
    (0, {'dog_name': str, 'box_number': int, 'finish_position': (int, type(None)), 'race_time': (float, type(None))}),
    (1, {'dog_name': str, 'box_number': int, 'finish_position': (int, type(None)), 'race_time': (float, type(None))}),
    # Add more dogs to test if needed
])
def test_parse_race_dog_performance_fields(scraper, race_result_html, dog_result_index, expected_fields):
    ...
    for field, expected_type in expected_fields.items():
        ...
        assert isinstance(dog_data[field], expected_type), f"Field {field} has type {type(dog_data[field])} but expected {expected_type}"
