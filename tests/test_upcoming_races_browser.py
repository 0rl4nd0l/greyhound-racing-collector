    ...
    with test_server() as base_url:
        with sync_playwright() as p:
            ...
            try:
                ...
                localecompare_errors = [
                    err for err in console_errors if "localeCompare" in err
                ]
                ...
                try:
                    api_response = requests.get(
                        f"{base_url}/api/upcoming_races", timeout=10
                    )
                    if api_response.status_code == 200:
                        ...
                        if "races" in data and isinstance(data["races"], list):
                            ...
                            print(
                                f"   ❌ API races field is not an array: {type(data.get('races', None))}"
                            )
                            return False
                    else:
                ...
                try:
                    page.wait_for_selector(
                        "[data-testid='race-card'], .race-card, .no-races-message",
                        timeout=10000,
                    )

                    # Check for "Unknown Venue" handling
                    unknown_venue_elements = page.query_selector_all(
                        "text=Unknown Venue"
                    )
                    if unknown_venue_elements:
                        print(
                            f"   ✅ Found {len(unknown_venue_elements)} races with 'Unknown Venue' (null venues handled correctly)"
                        )
                    else:
                        print(
                            "   ✅ No null venues found (or all venues have proper names)"
                        )
                ...
                # Test stats display
                stats_elements = page.query_selector_all(
                    "#totalRaces, #totalVenues, #downloadedRaces"
                )
                if stats_elements:
                    ...
                    for error in console_errors:
                        error_type = (
                            ...
                            if "forEach" in error
                                ...
                                "localeCompare" if "localeCompare" in error else "other"
                            )
                        )
                        error_types[error_type] = error_types.get(error_type, 0) + 1

                    if (
                        error_types.get("forEach", 0) > 0
                        or error_types.get("localeCompare", 0) > 0
                    ):
