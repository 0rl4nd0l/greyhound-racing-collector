    ...
    try:
        ...
        for scenario in test_scenarios:
            ...
            result = optimizer.calculate_betting_value(
                scenario["win_prob"], scenario["market_odds"], scenario["confidence"]
            )
...
def test_market_odds_lookup():
    ...
    try:
        ...
        result = optimizer.calculate_betting_value_with_odds_lookup(
            win_prob=0.3, confidence=0.75, race_id=test_race_id, dog_name=test_dog_name
        )

        print(
            f"   📊 Market Odds Found: {'YES' if result.get('market_odds_found') else 'NO'}"
        )
        print(f"   💰 Expected Value: {result['expected_value']:.4f}")
        print(f"   🎯 Has Value: {'YES' if result['has_value'] else 'NO'}")
    ...
    try:
        ...
        if result["success"]:
            ...
            if "betting_recommendation" in prediction:
                ...
                print(
                    f"      - Has Value: {'YES' if betting_rec['has_value'] else 'NO'}"
                )
