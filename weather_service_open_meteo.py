...
class OpenMeteoWeatherService:
            ...
            if 'current_weather' in data:
                ...
                if hourly and 'time' in hourly:
                    ...
                    try:
                        ...
                        if 'relativehumidity_2m' in hourly and len(hourly['relativehumidity_2m']) > hour_index:
                            humidity = float(hourly['relativehumidity_2m'][hour_index] or 50.0)
                        if 'precipitation' in hourly and len(hourly['precipitation']) > hour_index:
                            precipitation = float(hourly['precipitation'][hour_index] or 0.0)
                        if 'pressure_msl' in hourly and len(hourly['pressure_msl']) > hour_index:
                            pressure = float(hourly['pressure_msl'][hour_index] or 1013.25)
                        if 'visibility' in hourly and len(hourly['visibility']) > hour_index:
                            visibility = float(hourly['visibility'][hour_index] or 10000.0) / 1000.0  # Convert m to km
                ...
                weather_data = WeatherData(
                    ...
                    pressure=pressure,
                    condition=self._weather_code_to_condition(int(current['weathercode'])),
    ...
    def get_forecast_weather(self, venue_code: str, target_date: str) -> Optional[WeatherData]:
        ...
        try:
            ...
            if 'daily' in data and data['daily']['time']:
                ...
                if 'hourly' in data and data['hourly']['time']:
                    ...
                    if 'relativehumidity_2m' in hourly and len(hourly['relativehumidity_2m']) > midday_index:
                        humidity = float(hourly['relativehumidity_2m'][midday_index] or 60.0)
                    if 'pressure_msl' in hourly and len(hourly['pressure_msl']) > midday_index:
                        pressure = float(hourly['pressure_msl'][midday_index] or 1013.25)
                ...
                weather_data = WeatherData(
                    ...
                    pressure=pressure,
                    condition=self._weather_code_to_condition(int(daily['weathercode'][0] or 0)),
            ...
            logger.error(f"Error getting forecast weather for {venue_code} on {target_date}: {e}")
            return None
    ...
    def get_weather_for_race(self, venue_code: str, race_date: str, race_time: str = None) -> Optional[WeatherData]:
...
def main():
    ...
    if weather_data:
        ...
        adjustment = weather_service.calculate_weather_adjustment_factor(weather_data, 'SAN')
        print(f"\n   🎯 Racing Impact:")
