...
def generate_mock_data(num_races=10, dogs_per_race=8):
    ...
    for race_id in range(1, num_races + 1):
        ...
        for model in models:
            ...
            brier_score = sum(
                [
                    ((1 if i == actual_winner_idx else 0) - prob) ** 2
...
"""
    ...
    for result in results:
        predictions = [
            result["models"][m]["predicted_winner"] for m in ["v3", "v3s", "v4"]
        ]
...
def print_console_summary(model_metrics):
    ...
    for model_name, metrics in model_metrics.items():
        ...
        print(
            f"  Brier Score: {brier_color}{metrics['avg_brier']:.3f}{Colors.END} {brier_status} (≤ 0.18)"
        )
        ...
        print(
            f"  Accuracy@1:  {acc_color}{metrics['accuracy']:.1%}{Colors.END} {acc_status} (≥ 25%)"
        )
