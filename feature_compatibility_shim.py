...
class FeatureCompatibilityShim:
    ...
    def __init__(self, expected_features: List[str]):
        ...
        self.enabled = os.getenv('ENABLE_FEATURE_COMPAT_SHIM', '0').lower() in ('1', 'true', 'yes')
    ...
    def apply_compatibility_fixes(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_features or extra_features:
            logger.log(self.logger_level,
                f"🔧 FEATURE COMPATIBILITY SHIM ACTIVE:")
            logger.log(self.logger_level,
        ...
        feature_cols = [col for col in self.expected_features if col in fixed_df.columns]
        metadata_cols_present = [col for col in metadata_cols if col in fixed_df.columns]
    ...
    def _fix_data_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_features = [col for col in df.columns 
                          ...
                          or col in ['box_number', 'weight', 'distance', 'temperature', 
                                   'humidity', 'wind_speed', 'field_size']]
...
def apply_feature_compatibility_shim(features_df: pd.DataFrame, 
                                   expected_features: List[str]) -> pd.DataFrame:
...
def test_compatibility_shim():
    ...
    expected_features = [
        'box_number', 'venue', 'grade', 'distance', 'historical_win_rate',
        'tgr_win_rate', 'tgr_avg_finish_position', 'track_condition', 'weather',
        'trainer_name', 'weight'
    ]
