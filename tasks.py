...
@task
def process_race_file(file_path: str, metadata: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not ingestion_result.get('success', False):
            raise Exception(f"CSV ingestion failed: {ingestion_result.get('error', 'Unknown error')}")
        ...
        logger.info(f"✅ Successfully processed race file {file_path} in {processing_time:.2f}s")
        return result
...
def download_race_data(race_url: str, save_path: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if downloaded_file and os.path.exists(downloaded_file):
            logger.info(f"🔄 Triggering background processing for downloaded file: {downloaded_file}")
        ...
        logger.info(f"✅ Successfully downloaded race data from {race_url} in {download_time:.2f}s")
        return result
...
@task
def generate_predictions(race_ids: List[str], prediction_types: List[str] = None) -> Dict[str, Any]:
    """
    ...
    logger.info(f"🧠 Starting background prediction generation for {len(race_ids)} races")
    ...
    try:
        ...
        logger.info(f"✅ Generated predictions for {successful_predictions}/{len(race_ids)} races in {processing_time:.2f}s")
        return result
...
def update_race_notes(race_id: str, notes: str, user_id: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        with sqlite3.connect(database_path) as conn:
            ...
            # Verify the update
            cursor.execute("SELECT notes, updated_at FROM race_notes WHERE race_id = ?", (race_id,))
