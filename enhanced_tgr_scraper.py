    ...
    class EnhancedTGRScraper(TheGreyhoundRecorderScraper):
        ...
        def fetch_enhanced_dog_data(self, dog_name: str, limit_days: int = 365) -> Dict[str, Any]:
            ...
            try:
                ...
                logger.info(f"✅ Enhanced data collected for {dog_name}: {len(enhanced_data['form_entries'])} total race entries")
        ...
        def _extract_dog_histories_from_meeting(self, meeting_url: str, target_dog_name: str) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                for dog_section in dog_sections:
                    ...
                    race_entries = self._extract_race_entries_from_dog_section(dog_section, target_dog_name)
                    dog_histories.extend(race_entries)
                
                # Method 2: Look for table-based racing histories
                if not dog_histories:
                    dog_histories = self._extract_from_racing_tables(soup, target_name_clean)
                
                # Method 3: Look for list-based racing histories  
                if not dog_histories:
                    dog_histories = self._extract_from_racing_lists(soup, target_name_clean)
        ...
        def _find_dog_sections(self, soup, target_dog_name: str) -> List:
            ...
            potential_patterns = [
                ...
                soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5'], string=lambda text: text and target_dog_name.lower() in text.lower()),
                
                # Dog name in strong/bold text
                soup.find_all('strong', string=lambda text: text and target_dog_name.lower() in text.lower()),
                
                # Dog name in spans or divs
                soup.find_all(['span', 'div'], string=lambda text: text and target_dog_name.lower() in text.lower()),
                
                # Dog name in table cells (might be section headers)
                soup.find_all(['th', 'td'], string=lambda text: text and target_dog_name.lower() in text.lower()),
            ]
        ...
        def _contains_racing_data(self, element) -> bool:
            ...
            racing_indicators = [
                'ballarat', 'bendigo', 'sandown', 'melbourne', 'traralgon',  # venues
                'jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec',  # months
                'm ', 'metres', 'meter', 'distance',  # distances
                'time:', 'box', 'weight', 'grade',  # racing terms
                '1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th',  # positions
            ]
            ...
            has_racing_content = any(indicator in text for indicator in racing_indicators)
        ...
        def _looks_like_dog_name_header(self, element) -> bool:
            ...
            is_header_like = element.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'strong', 'b'] or \
                           'header' in str(element.get('class', [])).lower() or \
                           'title' in str(element.get('class', [])).lower()
            
            return is_header_like and text.isalpha()
        
        def _extract_race_entries_from_dog_section(self, section, dog_name: str) -> List[Dict[str, Any]]:
        ...
        def _extract_from_racing_tables(self, soup, target_dog_name: str) -> List[Dict[str, Any]]:
            ...
            for table in tables:
                ...
                if headers:
                    ...
                    racing_headers = ['date', 'venue', 'distance', 'time', 'position', 'box', 'grade']
                    if any(rh in ' '.join(header_text) for rh in racing_headers):
                        ...
                        for row in rows[1:]:
                            ...
                            if target_dog_name in row_text:
                                race_entry = self._parse_race_history_row(row, target_dog_name)
                                if race_entry:
                                    race_entries.append(race_entry)
            
            return race_entries
        
        def _extract_from_racing_lists(self, soup, target_dog_name: str) -> List[Dict[str, Any]]:
            ...
            for list_elem in lists:
                ...
                for item in items:
                    ...
                    if target_dog_name in item_text and self._contains_racing_data(item):
                        race_entry = self._parse_race_history_item(item, target_dog_name)
                        if race_entry:
                            race_entries.append(race_entry)
            
            return race_entries
        
        def _parse_race_history_row(self, row, dog_name: str) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                for i, cell in enumerate(cells):
                    ...
                    elif self._is_race_time(cell_text) and not race_entry['individual_time']:
                        race_entry['individual_time'] = cell_text
                    
                    # Winning time (second time field, often in "Win T" column)
                    elif self._is_race_time(cell_text) and race_entry['individual_time'] and not race_entry['winning_time']:
                ...
                if (race_entry['race_date'] or race_entry['venue'] or 
                    race_entry['finish_position'] or race_entry['individual_time']):
                    return race_entry
        ...
        def _parse_race_history_item(self, item, dog_name: str) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                if not date_match:
                    date_match = re.search(r'(\d{1,2}(?:st|nd|rd|th)?\s+(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec))', text, re.IGNORECASE)
                if date_match:
                    race_entry['race_date'] = date_match.group(1)
                
                # Extract venue patterns
                venues = ['ballarat', 'bendigo', 'sandown', 'melbourne', 'traralgon', 'cranbourne', 'sale', 'warragul', 
                         'richmond', 'horsham', 'geelong', 'shepparton', 'mildura', 'ascot park']
                for venue in venues:
        ...
        def _parse_race_history_div(self, div, dog_name: str) -> Optional[Dict[str, Any]]:
        ...
        def _is_venue(self, text: str) -> bool:
            ...
            venues = ['ballarat', 'bendigo', 'sandown', 'melbourne', 'traralgon', 'cranbourne', 'sale', 'warragul', 
                     'richmond', 'horsham', 'geelong', 'shepparton', 'mildura']
        ...
        def _calculate_performance_metrics(self, form_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            if metrics['total_starts'] > 0:
                ...
                metrics['place_percentage'] = ((metrics['wins'] + metrics['places']) / metrics['total_starts']) * 100
        ...
        if enhanced_data.get('form_entries'):
            ...
            if venue_analysis:
                ...
                for venue, stats in list(venue_analysis.items())[:3]:  # Show top 3
                    logger.info(f"    {venue}: {stats.get('starts', 0)} starts, {stats.get('win_percentage', 0):.1f}% win rate")
