...
class StatisticalAnalyzer:
    ...
    def load_data(self):
        ...
        print(f"Loaded {len(self.race_data)} race records and {len(self.dog_data)} dog performance records")
    ...
    def analyze_completeness(self, df, name):
        ...
        sorted_completeness = sorted(completeness_stats.items(), 
        ...
        for field, stats in sorted_completeness:
            print(f"{field:<35} {stats['null_count']:<10} {stats['null_percentage']:<12.2f} {stats['completeness']:<12.2f}")
        
        # Flag critical missing data
        critical_missing = [(field, stats) for field, stats in completeness_stats.items() 
    ...
    def identify_numeric_fields(self, df):
        ...
        target_numeric_fields = [
            'weight', 'odds_decimal', 'starting_price', 'beaten_margin', 
            'performance_rating', 'speed_rating', 'class_rating',
            'win_probability', 'place_probability', 'best_time',
            'temperature', 'humidity', 'wind_speed', 'pressure',
            'visibility', 'precipitation', 'winner_odds', 'winner_margin',
            'prize_money_total', 'field_size', 'actual_field_size',
            'scratch_rate', 'finish_position', 'placing'
        ]
    ...
    def analyze_numeric_distribution(self, df, field):
        ...
        cv = stats_dict['std'] / abs(stats_dict['mean']) if stats_dict['mean'] != 0 else float('inf')
    ...
    def create_numeric_plots(self, df, field, stats_dict, output_dir='analysis_plots'):
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/{field}_distribution_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def identify_categorical_fields(self, df):
        ...
        target_categorical_fields = [
            'venue', 'grade', 'track_condition', 'weather', 'wind_direction',
            'running_style', 'trainer_name', 'dog_name', 'race_name',
            'data_source', 'weather_condition', 'venue_slug', 'form',
        ...
        for col in df.select_dtypes(include=['object']).columns:
            if col not in categorical_fields and col not in ['race_id', 'extraction_timestamp']:
                # Check if it's not a numeric field stored as string
                try:
                    numeric_count = pd.to_numeric(df[col], errors='coerce').notna().sum()
                    total_count = df[col].notna().sum()
                    if total_count > 0 and numeric_count / total_count < 0.8:  # Less than 80% numeric
    ...
    def analyze_categorical_distribution(self, df, field):
        ...
        stats_dict['max_entropy'] = np.log2(len(value_counts)) if len(value_counts) > 0 else 0
        stats_dict['normalized_entropy'] = (stats_dict['entropy'] / stats_dict['max_entropy']) if stats_dict['max_entropy'] > 0 else 0
        
        # Sparsity analysis
        singleton_count = (value_counts == 1).sum()  # Values appearing only once
        sparse_threshold = 0.01 * total_count  # Values appearing in less than 1% of records
        sparse_count = (value_counts < sparse_threshold).sum()
        
        stats_dict['singleton_count'] = singleton_count
        stats_dict['singleton_percentage'] = (singleton_count / len(value_counts) * 100) if len(value_counts) > 0 else 0
        stats_dict['sparse_count'] = sparse_count
        stats_dict['sparse_percentage'] = (sparse_count / len(value_counts) * 100) if len(value_counts) > 0 else 0
        ...
        stats_dict['top_categories'] = list(zip(top_10.index, top_10.values, (top_10.values / total_count * 100)))
    ...
    def create_categorical_plots(self, df, field, stats_dict, output_dir='analysis_plots'):
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/{field}_categorical_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def flag_model_training_issues(self, numeric_results, categorical_results):
        ...
        for field, stats in numeric_results.items():
            ...
            if abs(stats['skewness']) > 2:
                issues.append(f"🔴 {field}: Highly skewed (skewness: {stats['skewness']:.2f})")
            elif abs(stats['skewness']) > 1:
                warnings.append(f"🟡 {field}: Moderately skewed (skewness: {stats['skewness']:.2f})")
            
            # High outlier percentage
            if stats['outlier_percentage'] > 10:
                issues.append(f"🔴 {field}: High outlier rate ({stats['outlier_percentage']:.1f}%)")
            elif stats['outlier_percentage'] > 5:
                warnings.append(f"🟡 {field}: Moderate outlier rate ({stats['outlier_percentage']:.1f}%)")
            
            # High coefficient of variation
            if stats['coefficient_of_variation'] > 2:
                warnings.append(f"🟡 {field}: High variability (CV: {stats['coefficient_of_variation']:.2f})")
        ...
        for field, stats in categorical_results.items():
            ...
            if stats['sparse_percentage'] > 50:
                issues.append(f"🔴 {field}: Very sparse ({stats['sparse_percentage']:.1f}% categories are rare)")
            elif stats['sparse_percentage'] > 25:
                warnings.append(f"🟡 {field}: Moderately sparse ({stats['sparse_percentage']:.1f}% categories are rare)")
            
            # High cardinality
            if stats['unique_values'] > 1000:
                issues.append(f"🔴 {field}: Very high cardinality ({stats['unique_values']:,} unique values)")
            elif stats['unique_values'] > 100:
                warnings.append(f"🟡 {field}: High cardinality ({stats['unique_values']:,} unique values)")
            
            # Highly imbalanced
            if stats['most_common_percentage'] > 90:
                issues.append(f"🔴 {field}: Highly imbalanced ({stats['most_common_percentage']:.1f}% in one category)")
            elif stats['most_common_percentage'] > 80:
                warnings.append(f"🟡 {field}: Moderately imbalanced ({stats['most_common_percentage']:.1f}% in one category)")
            
            # Low entropy (low information content)
            if stats['normalized_entropy'] < 0.1:
                warnings.append(f"🟡 {field}: Low information content (entropy: {stats['normalized_entropy']:.3f})")
    ...
    def generate_summary_report(self, completeness_results, numeric_results, categorical_results, issues, warnings):
        ...
        report.append(f"\nCOMPLETENESS SUMMARY:")
        complete_fields = len([f for f, s in completeness_results['merged'].items() if s['completeness'] >= 90])
        total_fields = len(completeness_results['merged'])
        report.append(f"• Fields with ≥90% completeness: {complete_fields}/{total_fields} ({(complete_fields/total_fields)*100:.1f}%)")
        
        critical_missing = len([f for f, s in completeness_results['merged'].items() if s['null_percentage'] > 50])
        ...
        skewed_fields = len([f for f, s in numeric_results.items() if s and abs(s['skewness']) > 1])
        report.append(f"• Fields with significant skewness: {skewed_fields}")
        
        outlier_fields = len([f for f, s in numeric_results.items() if s and s['outlier_percentage'] > 5])
        report.append(f"• Fields with notable outliers (>5%): {outlier_fields}")
        
        # Categorical analysis summary
        report.append(f"\nCATEGORICAL FIELDS ANALYSIS:")
        valid_categorical = len([f for f, s in categorical_results.items() if s is not None])
        report.append(f"• Analyzed categorical fields: {valid_categorical}")
        
        high_cardinality = len([f for f, s in categorical_results.items() if s and s['unique_values'] > 100])
        report.append(f"• High cardinality fields (>100 categories): {high_cardinality}")
        
        sparse_fields = len([f for f, s in categorical_results.items() if s and s['sparse_percentage'] > 25])
        report.append(f"• Sparse fields (>25% rare categories): {sparse_fields}")
        
        imbalanced_fields = len([f for f, s in categorical_results.items() if s and s['most_common_percentage'] > 80])
        report.append(f"• Highly imbalanced fields (>80% in one category): {imbalanced_fields}")
        ...
        if len(issues) == 0:
            ...
            report.append("• Overall assessment: NEEDS SIGNIFICANT PREPROCESSING before training")
    ...
    def run_complete_analysis(self):
        ...
        completeness_results['race_data'] = self.analyze_completeness(self.race_data, "RACE METADATA")
        completeness_results['dog_data'] = self.analyze_completeness(self.dog_data, "DOG PERFORMANCE DATA")
        completeness_results['merged'] = self.analyze_completeness(self.merged_data, "MERGED DATASET")
        ...
        print(f"Identified {len(numeric_fields)} numeric fields: {', '.join(numeric_fields)}")
        ...
        print(f"Identified {len(categorical_fields)} categorical fields: {', '.join(categorical_fields)}")
        ...
        for field in categorical_fields:
            ...
            if stats:
                ...
                print(f"  • Entropy: {stats['entropy']:.4f} (normalized: {stats['normalized_entropy']:.4f})")
                print(f"  • Most common: {stats['most_common_value']} ({stats['most_common_percentage']:.1f}%)")
                print(f"  • Sparse categories: {stats['sparse_count']} ({stats['sparse_percentage']:.1f}%)")
                
                # Create plots
                self.create_categorical_plots(self.merged_data, field, stats)
        
        # Flag model training issues
        issues, warnings = self.flag_model_training_issues(numeric_results, categorical_results)
