...
def find_latest_predictions_file() -> Optional[str]:
    ...
    step5_files = [
        f for f in os.listdir(".") if f.startswith("step5_win_probabilities_")
    ]
    if step5_files:
    ...
    if os.path.exists(predictions_dir):
        ...
        if json_files:
            latest_file = os.path.join(
                ...
                max(
                    json_files,
                    key=lambda f: os.path.getctime(os.path.join(predictions_dir, f)),
                ),
            )
...
def load_predictions_data(filepath: str) -> pd.DataFrame:
    ...
    if filepath.endswith(".csv"):
        ...
        for pred in data["predictions"]:
            ...
            elif "prediction_scores" in pred and isinstance(
                pred["prediction_scores"], dict
            ):
        ...
        if len(predictions) > 0:
            ...
            if abs(prob_sum - 1.0) > 1e-3:
                ...
                predictions["win_probability"] = (
                    predictions["win_probability"] / prob_sum
                )
...
def validate_probability_normalization(predictions: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        if validation_results["zero_probabilities"] > 0:
            validation_results["issues"].append(
                f"{validation_results['zero_probabilities']} dogs have 0% probability"
            )

        if validation_results["min_probability"] < 0:
            validation_results["issues"].append(
                f"Negative probabilities found: min = {validation_results['min_probability']:.6f}"
            )
...
def run_validation_assertions(predictions: pd.DataFrame):
    ...
    try:
        ...
        required_cols = ["dog_clean_name", "win_probability"]
        assert all(
            col in predictions.columns for col in required_cols
        ), f"Missing required columns. Available: {list(predictions.columns)}"
...
def main():
    ...
    if predictions_file is None:
        logger.warning(
            "No predictions file found. Creating demo data for validation test."
        )
    ...
    logger.info(
        f"Required Columns Present: {validation_results['required_columns_present']}"
    )
    ...
    if validation_results["passed"]:
        print(
            "✓ VALIDATION PASSED: Probabilities are properly normalized and formatted"
        )
    else:
        print(
            "✗ VALIDATION FAILED: Issues found with probability normalization or formatting"
        )
        sys.exit(1)

    print(f"✓ Validated {validation_results['total_dogs']} dog predictions")
    print(
        f"✓ Probability sum: {validation_results['prob_sum']:.6f} (tolerance: ±0.001)"
    )
