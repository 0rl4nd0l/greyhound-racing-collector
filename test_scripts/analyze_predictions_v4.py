...
def top1_accuracy(df: pd.DataFrame) -> float:
    ...
    pred_top = (
        df.sort_values(
            ["race_id", "win_prob_norm", "predicted_rank"],
...
def reliability_bins(df: pd.DataFrame, bins: int = 10):
    ...
    df = df.copy()
    df["bin"] = pd.cut(
        df["win_prob_norm"], bins=bins, labels=False, include_lowest=True
    )
    out = []
    for b, g in df.groupby("bin"):
        ...
        out.append(
            {"bin": int(b), "avg_pred": mean_p, "emp_rate": emp_rate, "n": int(len(g))}
        )
    return out
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze MLSystemV4 prediction accuracy and calibration"
    )
    parser.add_argument(
        "--glob",
        dest="pattern",
        default="predictions/*.psv",
        help="Glob for prediction files",
    )
    ...
    if df.empty:
        print(
            "No valid data loaded. Ensure files contain required columns:",
            REQUIRED_COLUMNS,
        )
        return
