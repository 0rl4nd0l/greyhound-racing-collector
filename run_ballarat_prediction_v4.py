        ...
        if result.get("success", False):
            ...
            if predictions:
                ...
                sorted_predictions = sorted(
                    predictions, key=lambda x: x.get("win_prob_norm", 0), reverse=True
                )
                ...
                for i, pred in enumerate(sorted_predictions, 1):
                    ...
                    win_prob = (
                        pred.get("win_prob_norm", 0.0) * 100
                    )  # Use the correct key
                    confidence = pred.get("confidence", "Unknown")

                    print(
                        f"{i:4} | {box_num:3} | {dog_name} | {win_prob:7.1f}% | {confidence:.3f}"
                    )
                ...
                if result.get("fallback_used"):
                    print(
                        f"⚠️  Fallback system used: {result.get('fallback_reason', 'Unknown')}"
                    )
