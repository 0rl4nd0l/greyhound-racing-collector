...
def upgrade():
    ...
    with op.batch_alter_table('race_metadata', schema=None) as batch_op:
        try:
            batch_op.add_column(sa.Column('new_additional_column', sa.String))  # Example addition
        except Exception:
    ...
    with op.batch_alter_table('dog_race_data', schema=None) as batch_op:
        try:
            batch_op.add_column(sa.Column('new_analysis_column', sa.Float))  # Example addition
        except Exception:
            pass  # Column likely exists, handle safely
