...
def main():
    ...
    try:
        ...
        results = processor.backfill_winners_for_pending_races(
            max_races=args.max_races, max_retries_per_race=args.max_retries
        )
        ...
        if results.get("status") == "success":
            print(
                f"✅ Successfully backfilled: {results.get('backfilled_count', 0)} races"
            )
            ...
            if individual_results:
                ...
                for result in individual_results[:10]:  # Show first 10 results
                    ...
                    if status == "success":
                        ...
                        print(
                            f"   ✅ {race_id}: Winner = {winner} (attempt {attempts})"
                        )
                    elif status == "failed":
