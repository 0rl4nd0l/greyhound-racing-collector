...
class TGRServiceScheduler:
    ...
    def __init__(
        self, db_path: str = "greyhound_racing_data.db", config: SchedulerConfig = None
    ):
    ...
    def start_scheduler(self):
        ...
        self.scheduler_thread = threading.Thread(
            target=self._scheduler_loop, name="TGRScheduler", daemon=True
        )
    ...
    def _run_health_check(self):
        ...
        try:
            ...
            self._record_performance_metric(
                "overall_health_score", health_report.get("health_score", 0)
            )

            logger.info(
                f"✅ Health check completed in {execution_time:.1f}s, took {len(actions_taken)} actions"
            )
    ...
    def _analyze_health_and_act(self, health_report: Dict[str, Any]) -> List[str]:
        ...
        if (
            health_report.get("data_quality", {}).get("avg_data_age_hours", 0)
            > self.config.data_freshness_hours
        ):
            ...
            if stale_dogs:
                jobs_added = self._schedule_priority_enrichment(
                    stale_dogs, "data_freshness"
                )
                actions_taken.append(f"scheduled_{jobs_added}_freshness_jobs")

        # Check collection performance
        collection_success = health_report.get("performance", {}).get(
    ...
    def _run_daily_enrichment(self):
        ...
        try:
            ...
            if priority_dogs:
                ...
                logger.info(
                    f"✅ Daily enrichment scheduled: {jobs_added} jobs in {execution_time:.1f}s"
                )
            else:
    ...
    def _optimize_performance(self):
        ...
        try:
            ...
            if (
                perf_metrics.get("job_success_rate", 1.0)
            ...
            logger.info(
                f"✅ Performance optimization completed in {execution_time:.1f}s"
            )

        except Exception as e:
            logger.error(f"Performance optimization failed: {e}")
            self._log_action(
                "performance_optimization", f"Failed: {str(e)}", success=False
            )
        ...
        try:
            ...
            if queue_size < 5 and service_status.get("service_running"):
                ...
                if recent_success_rate > 0.8:  # 80% success rate
                    ...
                    if candidates:
                        jobs_added = self._schedule_priority_enrichment(
                            candidates, "opportunistic"
                        )

                        if jobs_added > 0:
                            logger.info(
                                f"🎯 Opportunistically scheduled {jobs_added} enrichment jobs"
                            )
    ...
    def _record_performance_metric(
        self, metric_name: str, value: float, context: str = None
    ):
    ...
    def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "last_health_check": (
                self.last_health_check.get("timestamp")
...
def main():
    ...
    try:
        ...
        if "enrichment_status" in status:
            ...
            print(
                f"   Success Rate: {enrich_stats['jobs_succeeded']}/{enrich_stats['jobs_processed']}"
            )
