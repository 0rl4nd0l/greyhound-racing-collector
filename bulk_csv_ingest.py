...
from csv_ingestion import FormGuideCsvIngestionError, FormGuideCsvIngestor
...
def process_batch(
    ...
    validator: FileContentValidator = None,
) -> Dict[str, Any]:
    """
    ...
    for csv_file in batch:
        ...
        if not file_metadata.get("exists", False):
            batch_results["failed_files"].append(
                {"file": csv_file, "error": "File does not exist"}
            )
            continue
        ...
        if not is_valid:
            batch_results["skipped_files"].append(
                {"file": csv_file, "reason": validation_message, "file_info": file_info}
            )
            continue
        ...
        try:
            ...
            if processed_data:
                ...
                batch_results["failed_files"].append(
                    {"file": csv_file, "error": "No data extracted from file"}
                )

        except FormGuideCsvIngestionError as e:
            batch_results["failed_files"].append(
                {"file": csv_file, "error": f"Ingestion error: {str(e)[:100]}..."}
            )
        except Exception as e:
            batch_results["failed_files"].append(
                {"file": csv_file, "error": f"Unexpected error: {str(e)[:100]}..."}
            )
...
def batch_save_to_database(
    processed_data_list: List[Dict[str, Any]], db_path: str = "greyhound_racing_data.db"
) -> Tuple[int, int]:
    """
            ...
            races[race_key]["dogs"].append(
                {
                    ...
                    "dog_clean_name": (
                        record.get("dog_name", "").strip()
                        if record.get("dog_name")
                        else ""
                    ),
...
def bulk_ingest_with_database_save():
    ...
    print(
        f"📊 Initial database state: {initial_races:,} races, {initial_dogs:,} dog records"
    )
    ...
    with file_lock:
        ...
        for batch in chunked(csv_files, BATCH_SIZE):
            batch_count += 1
            print(
                f"🔄 Processing batch {batch_count}/{total_batches} ({len(batch)} files)..."
            )
            ...
            if results["batch_data"]:
                races_saved, dogs_saved = batch_save_to_database(
                    results["batch_data"], "greyhound_racing_data.db"
                )
            ...
            if results.get("skipped_files"):
                for skipped_file in results["skipped_files"]:
                    print(
                        f"   ⚠️  Skipped: {os.path.basename(skipped_file['file'])} - {skipped_file['reason']}"
                    )

            # Show progress
            if (
                batch_count % 5 == 0 or batch_count == total_batches
            ):  # Every 5 batches or final batch
                elapsed = time.time() - start_time
                rate = stats["processed_files"] / (elapsed / 60) if elapsed > 0 else 0
                eta = (
                    (len(csv_files) - stats["processed_files"]) / rate
                    if rate > 0
                    else 0
                )
                print(
                    f'   Progress: {stats["processed_files"]:,}/{len(csv_files):,} '
    ...
    if (
        abs(stats["total_races_added"] - final_races_added) > 10
    ):  # Allow small discrepancy for concurrency
        print(
            f"⚠️ Race count discrepancy: tracked {stats['total_races_added']}, actual {final_races_added}"
        )
        stats["total_races_added"] = final_races_added

    if abs(stats["total_dogs_added"] - final_dogs_added) > 10:
        print(
            f"⚠️ Dog count discrepancy: tracked {stats['total_dogs_added']}, actual {final_dogs_added}"
        )
    ...
    print(
        f'📈 Processing rate: {stats["processed_files"]/(elapsed_time/60):.1f} files/min'
    )
...
def bulk_ingest_with_early_exit_optimization(
    ...
    should_early_exit, unprocessed_files = check_directory_for_early_exit(
        directory, processed_files_set, cache_ratio_threshold, unprocessed_threshold
    )
...
def _process_files_fast(
    files_to_process: List[str], db_path: str, early_exit: bool = False
) -> Dict[str, Any]:
    """
    ...
    with file_lock:
        if early_exit:
            print(
                f"⚡ Processing {len(files_to_process)} files with early-exit optimization"
            )
        else:
            print(
                f"🔄 Processing {len(files_to_process):,} unprocessed files with batch optimization"
            )
            print(f"📦 Using batch processing with BATCH_SIZE = {BATCH_SIZE}")
        ...
        for batch in chunked(files_to_process, BATCH_SIZE):
            ...
            if not early_exit and (
                batch_count % 5 == 0 or batch_count == total_batches
            ):
                elapsed = time.time() - start_time
                rate = stats["processed_files"] / (elapsed / 60) if elapsed > 0 else 0
                eta = (
                    (len(files_to_process) - stats["processed_files"]) / rate
                    if rate > 0
                    else 0
                )
                print(
                    f'   Progress: {stats["processed_files"]:,}/{len(files_to_process):,} '
    ...
    if early_exit:
        ...
        print(
            f'📁 Files: {stats["processed_files"]:,} processed, {stats["failed_files"]:,} failed'
        )
        print(
            f'🏁 Added: {stats["total_races_added"]:,} races, {stats["total_dogs_added"]:,} dog records'
        )
    else:
        print(f"\n" + "=" * 60)
        ...
        print(f"⏱️  Total time: {elapsed_time/60:.1f} minutes")
        print(
            f'📁 Files processed: {stats["processed_files"]:,}/{len(files_to_process):,}'
        )
        print(f'❌ Files failed: {stats["failed_files"]:,}')
        print(f'📊 Total records processed: {stats["total_records"]:,}')
        print(f'🏁 New races added: {stats["total_races_added"]:,}')
