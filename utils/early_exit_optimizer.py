...
class EarlyExitOptimizer:
    ...
    def analyze_directory_cache_status(
        ...
        should_early_exit = (
            self.config.enable_early_exit and
            cache_ratio >= self.config.cache_ratio_threshold and
            unprocessed_files <= self.config.unprocessed_threshold and
            total_files > 0  # Don't early exit on empty directories
        )
    ...
    def print_early_exit_summary(self, scan_result: DirectoryScanResult):
        ...
        print(f"   ✅ Cached/processed: {scan_result.processed_files:,} ({cache_percentage:.1f}%)")
        ...
        if scan_result.unprocessed_files > 0:
            print(f"   🔄 Remaining {scan_result.unprocessed_files} files will be processed quickly")
