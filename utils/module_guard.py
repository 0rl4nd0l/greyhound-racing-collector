...
DEFAULT_ALLOWED_PREFIXES: Tuple[str, ...] = (
    ...
    'builtins', 'collections', 'datetime', 'functools', 'hashlib', 'heapq', 'itertools', 'json',
    'logging', 'math', 'os', 'pathlib', 'random', 're', 'sys', 'threading', 'time', 'typing', 'uuid',
    'sqlite3', 'subprocess', 'argparse', 'glob', 'warnings', 'shutil',
    # Third-party commonly used in this project
    'numpy', 'pandas', 'sklearn', 'flask', 'flask_cors', 'flask_compress', 'werkzeug', 'joblib', 'dotenv', 'yaml',
    # Project-local
    'features', 'utils', 'config', 'advisory', 'endpoint_cache', 'database_manager', 'assets',
    'prediction_pipeline_v4', 'ml_system_v4', 'temporal_feature_builder', 'model_registry',
)
...
def _classify_loaded_modules(allowed_prefixes: List[str], disallowed_prefixes: List[str]):
    ...
    unknown_loaded = [m for m in loaded if m not in disallowed_loaded and m not in allowed_loaded]
...
def _build_resolution_guidance(disallowed_loaded: List[str], context: str) -> List[str]:
    ...
    if any(m.startswith(('selenium', 'playwright', 'pyppeteer')) for m in disallowed_loaded):
    ...
    if any(m.startswith('openai') for m in disallowed_loaded):
        steps.append("Ensure advisory or LLM components are decoupled from core prediction path.")
    return steps
...
def _raise_or_log_violation(disallowed_loaded: List[str], unknown_loaded: List[str], strict: bool, context: str):
    ...
    if strict:
        raise ModuleGuardViolation(message=message, details=details, resolution=resolution)
    else:
...
def startup_module_sanity_check() -> None:
    ...
    if disallowed_loaded:
        _raise_or_log_violation(disallowed_loaded, unknown_loaded, strict, context='startup')


def pre_prediction_sanity_check(context: str = 'prediction', extra_info: Dict[str, str] | None = None) -> None:
        ...
        if context == 'manual_prediction':
            def is_results_scraper(mod: str) -> bool:
                if 'the_greyhound_recorder_scraper' in mod:
                    ...
                    return False
                return (
                    mod.startswith('src.collectors') or
                    'scraper' in mod or
