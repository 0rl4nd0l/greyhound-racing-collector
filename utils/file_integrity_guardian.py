...
class FileIntegrityGuardian:
    ...
    def __init__(self):
        ...
        print(f"📏 CSV size limits: {self.min_csv_size} bytes to {self.max_csv_size/1024/1024:.1f}MB")
    ...
    def _cleanup_hash_cache(self):
        ...
        for file_path, hash_state in self.hash_cache.items():
            ...
            try:
                ...
                if (current_mtime != hash_state.last_modified or 
                    current_size != hash_state.file_size):
                    stale_keys.append(file_path)
            except OSError:
                stale_keys.append(file_path)
    ...
    def _compute_incremental_hash(self, file_path: str) -> Optional[FileHashState]:
        ...
        try:
            ...
            if file_path in self.hash_cache:
                ...
                if (cached_state.last_modified == last_modified and 
                    cached_state.file_size == file_size):
                    return cached_state
    ...
    def _detect_hash_anomalies(self, hash_state: FileHashState) -> List[str]:
        ...
        try:
            ...
            if max_consecutive > 5:
                issues.append(f"Suspicious pattern: {max_consecutive + 1} consecutive identical chunks detected")
            
            # Check for predominantly empty chunks (null bytes)
            null_chunk_hash = hashlib.sha256(b'\x00' * self.chunk_size).hexdigest()
            null_chunks = sum(1 for h in hash_state.chunk_hashes if h == null_chunk_hash)
            
            if null_chunks > len(hash_state.chunk_hashes) * 0.5:
                issues.append(f"File appears to be mostly null bytes: {null_chunks}/{len(hash_state.chunk_hashes)} chunks")
            
            # Check for unusual hash distribution (too many repeated hashes)
            unique_hashes = len(set(hash_state.chunk_hashes))
            if unique_hashes < len(hash_state.chunk_hashes) * 0.3 and len(hash_state.chunk_hashes) > 10:
                issues.append(f"Low hash diversity: only {unique_hashes} unique hashes in {len(hash_state.chunk_hashes)} chunks")
    ...
    def check_file_corruption(self, file_path: str) -> Tuple[List[str], Optional[str]]:
        ...
        try:
            ...
            if file_size > 0 and file_size % (1024 * 1024) == 0 and file_size >= 10 * 1024 * 1024:
            ...
            if file_size > self.max_csv_size:
                issues.append(f"File too large: {file_size} bytes (max: {self.max_csv_size})")
            
            if file_size < self.min_csv_size:
                issues.append(f"File too small: {file_size} bytes (min: {self.min_csv_size})")
            ...
            with open(file_path, 'rb') as f:
                ...
                if null_count > len(first_chunk) * 0.8:  # More than 80% null bytes
                    issues.append("File appears to start with mostly null bytes (corrupted)")
                
                # Check for HTML content in CSV files
                if file_path.endswith('.csv'):
                    first_text = first_chunk.decode('utf-8', errors='ignore').lower()
                    if any(tag in first_text for tag in ['<html>', '<body>', '<!doctype', '<div>', '<script>']):
    ...
    def scan_directory(self, directory: str, extensions: List[str] = ['.csv'], max_age_hours: int = None) -> List[FileValidationResult]:
            ...
            for file in files:
                ...
                if any(file.lower().endswith(ext) for ext in extensions):
                    ...
                    if result.should_quarantine:
                        ...
                        self.quarantine_file(file_path, f"Validation failed: {issues_summary}")
...
def main():
    ...
    parser.add_argument('--scan', type=str, help='Directory to scan for problematic files')
    parser.add_argument('--cleanup-tests', action='store_true', help='Remove test files from main directories')
    parser.add_argument('--validate-file', type=str, help='Validate a specific file')
    parser.add_argument('--report', type=str, help='Generate integrity report and save to file')
