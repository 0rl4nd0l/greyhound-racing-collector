...
class FileContentValidator:
        ...
        try:
            ...
            if file_size < self.min_file_size:
                ...
                if self.log_skipped_files:
                    self.logger.warning(
                        f"⚠️  Skipping file '{filename}': Too small ({file_size} bytes < {self.min_file_size} bytes minimum)"
                    )
            ...
            if html_found or non_csv_found:
                ...
                if self.log_skipped_files:
                    ...
                    self.logger.warning(
                        f"⚠️  Skipping file '{filename}': Contains {content_type} content, not CSV data"
                    )
                    self.skipped_files.append(
                        {
                            "filename": filename,
                            "reason": (
                                "html_content" if html_found else "non_csv_content"
                            ),
                            "file_size": file_size,
                            "path": file_path,
                            "indicators": (
                                found_indicators if html_found else found_patterns
                            ),
                        }
                    )
            ...
            if has_csv_pattern:
                ...
                message = (
                    f"File format uncertain but no HTML detected ({file_size} bytes)"
                )
                return True, message, file_info
        ...
        for file_path in file_paths:
            ...
            if is_valid:
                results["valid_files"].append(
                    {"path": file_path, "message": message, "file_info": file_info}
                )
    ...
    def print_validation_summary(self, results: Dict[str, Any]):
        ...
        print(
            f"Valid files: {stats['valid_count']} ({stats['valid_count']/results['total_files']*100:.1f}%)"
        )
        print(
            f"Invalid files: {stats['invalid_count']} ({stats['invalid_count']/results['total_files']*100:.1f}%)"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python file_content_validator.py <file_or_directory_path> [min_size_bytes]"
        )
