        ...
        try:
            ...
            if self.session_data is not None:
                ...
                with self._data_lock:
                    ...
                    audit_ts = datetime.utcnow().isoformat().replace(':', '').replace('.', '')
                    audit_dir = f"audit_results/{audit_ts}"
                    os.makedirs(audit_dir, exist_ok=True)
                    
                    # Separate stage recordings from session data
                    stage_recordings = [rec for rec in self.recordings if 'stage_name' in rec]
                    session_recordings = [rec for rec in self.recordings if 'race_id' in rec]
                    
                    # Prepare collected data structure as specified
                    collected_data = {
                        'timestamps': [rec.get('end_time', datetime.now().isoformat()) for rec in stage_recordings],
                        'stage_durations': {rec['stage_name']: rec['duration'] for rec in stage_recordings},
                        'meta': {
                    ...
                    with open(log_path, 'w') as f:  # Use 'w' to create new file per session
                        f.write(f"=== Profiling Session Report ===\n")
                        f.write(f"Session ID: {self.session_data.get('race_id', 'unknown')}\n")
                        f.write(f"Model Version: {self.session_data.get('model_version', 'unknown')}\n")
                        f.write(f"Method: {self.session_data.get('method', 'unknown')}\n")
                        f.write(f"Dogs Count: {self.session_data.get('n_dogs', 'unknown')}\n")
                        f.write(f"Total Duration: {duration:.4f}s\n")
                        f.write(f"Session Start: {self.session_data.get('start_time')}\n")
                        ...
                        for rec in stage_recordings:
                            ...
                            f.write(f"Stage: {stage_name:<30} Duration: {stage_duration:.4f}s\n")
            ...
            print(f"Warning: ProfilingRecorder end_session error (gracefully handled): {e}")
    ...
    def flush_to_file(self):
        with self._data_lock:
            ...
            with open("audit.log", "a") as f:
                f.write(f"[{datetime.now().isoformat()}] Profiling data written to {file_path}\n")
