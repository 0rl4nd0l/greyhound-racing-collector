...
class RaceFileManager:
    ...
    def load_collected_races(self, directories: Optional[List[str]] = None):
        ...
        print(
            f"📊 Loaded {len(self.collected_races)} unique races from {total_files} files"
        )

    def parse_filename_to_race_id(
        self, filename: str
    ...
    def mark_file_processed(
        self,
        file_path: str,
        race_info: Optional[Tuple[str, str, str]] = None,
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT AVG(file_size), MIN(file_size), MAX(file_size) FROM processed_race_files WHERE file_size > 0"
            )
...
def batch_filter_unprocessed_files(
    file_paths: List[str], database_path: str = "./databases/greyhound_racing.db"
) -> List[str]:
    """
...
if __name__ == "__main__":
    ...
    if stats:
        ...
        print(
            f"   Memory cache: {memory_stats.get('processed_hashes', 0)} hashes, "
