...
class MtimeHeuristic:
    ...
    def scan_directory_optimized(
        ...
        if not strict_scan:
            ...
            if last_processed_mtime:
                formatted_time = datetime.fromtimestamp(last_processed_mtime).strftime("%Y-%m-%d %H:%M:%S")
                logger.info(f"Using mtime heuristic - filtering files older than {formatted_time}")
        ...
        try:
            ...
            if max_mtime > 0:
                ...
                if current_mtime is None or max_mtime > current_mtime:
                    ...
                    formatted_time = datetime.fromtimestamp(max_mtime).strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(f"Updated last processed mtime to {formatted_time}")
                else:
                    logger.debug("No mtime update needed - processed files are not newer")
            else:
    ...
    def reset_mtime_heuristic(self):
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM db_meta WHERE meta_key = ?",
                (self.meta_key,)
            )
...
if __name__ == "__main__":
    ...
    for file_entry in heuristic.scan_directory_optimized(
        ...
        print(f"  {file_entry.name} (mtime: {datetime.fromtimestamp(file_entry.mtime)})")
