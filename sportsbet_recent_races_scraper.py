...
class SportsbetRecentRacesScraper:
    ...
    def setup_driver(self):
        ...
        options.add_argument(
            "--user-agent=Mozilla/5.0 (Macintosh; Intel Mac OS X 11_0_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36"
        )
    ...
    def scrape_recent_races(self, days_back=3) -> List[Dict]:
        ...
        try:
            print(
                f"🏁 Scraping recent race results from Sportsbet ({days_back} days back)..."
            )
            ...
            if len(all_races) < 20:
                ...
                if main_page_races:
                    ...
                    for race in all_races:
                        race_key = (
                            f"{race['venue']}_{race['race_number']}_{race['date']}"
                        )
            ...
            for i, element in enumerate(result_elements[:50]):  # Limit to first 50
                try:
                    ...
                    if race_result:
                        ...
                        print(
                            f"  ✅ Added: {race_result['venue']} Race {race_result['race_number']} - Winner: {race_result.get('winner_name', 'Unknown')}"
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if any(
                        ...
                        if race_result:
                            races.append(race_result)
                            print(
                                f"  ✅ Added completed race: {race_result['venue']} Race {race_result['race_number']}"
                            )
    ...
    def _extract_race_result_from_element(self, element, index: int) -> Optional[Dict]:
        ...
        try:
            ...
            if not any(
                ...
                for indicator in ["final", "result", "winner", "won"]
            ):
    ...
    def _extract_distance_from_text(self, text: str) -> Optional[str]:
        ...
        distance_patterns = [r"(\d{3,4})m", r"(\d{3,4})\s*metre", r"(\d{3,4})\s*meter"]
        ...
        try:
            ...
            for race in races:
                try:
                    ...
                    cursor.execute(
                        "SELECT race_id FROM race_metadata WHERE race_id = ?",
                        (race_id,),
                    )
                    ...
                    print(
                        f"  ❌ Error saving race {race.get('venue', 'Unknown')} Race {race.get('race_number', '?')}: {e}"
                    )
                    continue
            ...
            print(
                f"💾 Database updated: {saved_count} new races, {updated_count} updated races"
            )
    ...
    try:
        ...
        if recent_races:
            ...
            for i, race in enumerate(recent_races[:10], 1):  # Show first 10
                winner_info = (
                    f" - Winner: {race['winner_name']}"
                    if race.get("winner_name")
                    else ""
                )
                print(
                    f"{i:2d}. {race['venue']} Race {race['race_number']} ({race['date']}){winner_info}"
                )
                if race.get("grade"):
