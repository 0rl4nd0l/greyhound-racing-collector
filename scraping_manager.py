...
class ScrapingManager:
    ...
    def scrape_upcoming_races(self, days_ahead: int = 1) -> Dict:
        ...
        if not browser:
            return {'success': False, 'error': 'Could not initialize upcoming race browser'}
    ...
    def scrape_fasttrack_data(self, max_dogs: int = 100, sample_meetings: int = 5) -> Dict:
        """Scrape FastTrack data for dogs and meetings."""
        logger.info(f"🏁 Scraping FastTrack data (max {max_dogs} dogs, {sample_meetings} meetings)...")
        ...
        try:
            ...
            logger.info(f"✅ FastTrack scraping complete: {len(results['dogs'])} dogs, {len(results['meetings'])} meetings")
    ...
    def scrape_greyhound_recorder(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"✅ TGR scraping complete: {len(form_guides.get('meetings', []))} meetings")
    ...
    def run_comprehensive_collection(self, max_dogs: int = 50) -> Dict:
        ...
        if not collector:
            return {'success': False, 'error': 'Could not initialize comprehensive collector'}
    ...
    def get_status(self) -> Dict:
        ...
        status = {
            'scraping_enabled': {
                ...
                'comprehensive_collector': os.getenv('COMPREHENSIVE_COLLECTOR_ALLOWED') == '1'
            },
            'runtime_stats': {
            ...
            'available_scrapers': {
                ...
                'comprehensive_collector': self.get_comprehensive_collector() is not None
            }
        }
    ...
    def print_status_report(self):
        ...
        print(f"   Results Scrapers: {'✅ ENABLED' if status['scraping_enabled']['results_scrapers'] else '❌ DISABLED'}")
        print(f"   Live Scraping: {'✅ ENABLED' if status['scraping_enabled']['live_scraping'] else '❌ DISABLED'}")
        print(f"   Comprehensive Collector: {'✅ ENABLED' if status['scraping_enabled']['comprehensive_collector'] else '❌ DISABLED'}")
        
        print("\n🌐 Available Scrapers:")
        print(f"   FastTrack (GRV): {'✅ Ready' if status['available_scrapers']['fasttrack'] else '❌ Failed'}")
        print(f"   The Greyhound Recorder: {'✅ Ready' if status['available_scrapers']['greyhound_recorder'] else '❌ Failed'}")
        print(f"   Upcoming Races Browser: {'✅ Ready' if status['available_scrapers']['upcoming_browser'] else '❌ Failed'}")
        print(f"   Comprehensive Collector: {'✅ Ready' if status['available_scrapers']['comprehensive_collector'] else '❌ Failed'}")
        
        print("\n📊 Runtime Statistics:")
        print(f"   Uptime: {status['runtime_stats']['uptime_seconds']:.1f} seconds")
        print(f"   Scrapers Used: {', '.join(status['runtime_stats']['scrapers_loaded']) if status['runtime_stats']['scrapers_loaded'] else 'None'}")
...
def main():
    ...
    parser.add_argument('--mode', choices=['upcoming', 'fasttrack', 'tgr', 'comprehensive', 'status'], 
                       required=True, help='Scraping mode to run')
    parser.add_argument('--days', type=int, default=1, help='Days ahead for upcoming races')
    parser.add_argument('--dogs', type=int, default=20, help='Max dogs to scrape from FastTrack')
    parser.add_argument('--meetings', type=int, default=5, help='Sample meetings to scrape')
    parser.add_argument('--full', action='store_true', help='Run full comprehensive collection')
