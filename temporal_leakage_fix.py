...
class TemporalLeakageValidator:
    ...
    def create_historical_features(self, df: pd.DataFrame, target_race_id: str, 
                                 target_dog: str, lookback_days: int = 365) -> Dict[str, float]:
        """
        ...
        if len(valid_positions) > 0:
            ...
            if len(recent_positions) >= 3:
                ...
                features['historical_form_trend'] = float(-trend)  # Negative slope = improving
            else:
        ...
        if len(same_venue_races) > 0:
            venue_positions = pd.to_numeric(same_venue_races['finish_position'], errors='coerce').dropna()
        ...
        if len(same_grade_races) > 0:
            grade_positions = pd.to_numeric(same_grade_races['finish_position'], errors='coerce').dropna()
    ...
    def validate_temporal_splits(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(f"Train period: {train_data['race_date'].min()} to {train_data['race_date'].max()}")
        logger.info(f"Test period: {test_data['race_date'].min()} to {test_data['race_date'].max()}")
        ...
        validation_results = {
            ...
            'feature_count': len([col for col in train_features.columns if col not in ['target', 'race_id', 'dog_clean_name']]),
