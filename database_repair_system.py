...
class DatabaseRepairSystem:
    ...
    def create_backup(self):
        ...
        try:
            ...
            backup_path = self.backup_dir / f"greyhound_racing_data_backup_{timestamp}.db"
    ...
    def analyze_database_issues(self):
        ...
        try:
            ...
            print(f"   • Races with duplicate finish positions: {issues['duplicate_positions']}")
        ...
        try:
            ...
            for race_id in missing_records['race_id'].unique():
                ...
                for idx, row in race_dogs.iterrows():
                    ...
                    if pd.notna(row['finish_position']) and str(row['finish_position']).isdigit():
                        ...
                        used_boxes = set(existing_boxes['box_number'].tolist()) if len(existing_boxes) > 0 else set()
        ...
        try:
            ...
            for _, row in missing_metadata.iterrows():
                ...
                if race_id:
                    ...
                    match = re.search(r'Race[_\s]*(\d+)[_\s]*([A-Z_]+)[_\s]*([\d-]+)', race_id, re.IGNORECASE)
                    if match:
